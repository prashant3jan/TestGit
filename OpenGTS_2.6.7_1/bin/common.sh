...
#   GTS_CHARSET [optional] The character set used when starting up the Java process
...
# --- commands
if [ "$JAVA_HOME" != "" ]; then
    CMD_JAVA="${JAVA_HOME}/bin/java ${JAVA_CHARSET} ${JAVAMEM}"
else
    CMD_JAVA="java ${JAVA_CHARSET} ${JAVAMEM}"
fi
CMD_CAT="cat"
CMD_KILL="kill"
CMD_LS="ls"
