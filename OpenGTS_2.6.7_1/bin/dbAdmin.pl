...
#    This may be caused by one/both of the following:
#    1) Permissions issue with SELinux 
...
#    2) MySQL may be restricting output file directories with "--secure-file-priv" startup option.
#       Use the following command to see where output files are allowed:
#          SHOW VARIABLES LIKE "secure_file_priv";
#        or
#          mysql -u gts -p -e "SHOW VARIABLES LIKE 'secure_file_priv'"
#       Change "-dir" to a new subdirectory of the displayed 'secure_file_priv' directory and re-attempt dump.
# -----------------------------------------------------------------------------
...
$IgnoreTable_RGCache   = (defined $opt_excludeRGCache)? $true : $false;
...
if (!$optok || $opt_help) {
    ...
    print "    -dump=<table> [-account=<acct>] [-dir=<dir>] [-db=<DB>]\n";
    print "    -dump -excludeEventData -excludeRGCache [-db=<DB>] [-dir=<dir>]\n";
    print "\n";
...
# --- dump table/database
if (defined $opt_dump) {
    # bin/dbAdmin.pl -dir=/var/lib/mysql/gts_dump -dump=TABLE [-dumpFormat=CSV|TXT|SQL]
    print "\n";
    print "Dump table(s) ...\n";
    my $rtnErr = 0;
    # - make temp directory
    ...
    if ((($opt_dump eq "all") && (!defined $opt_account)) || ($opt_dump eq "") || $ALWAYS_USE_MYSQLDUMP) {
        ...
        if ($IgnoreTable_RGCache)   { $cmd = "$cmd --ignore-table=$DumpDB.RGCache"; }
        ...
        if (defined $opt_dumpFormat) {
            $cmd .= " -dumpFormat=$opt_dumpFormat";
        }
