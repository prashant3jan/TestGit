...
if [ -f "${DCSOPT_CONF}" ] ; then
    # - extract single entry from dcs option config file
    DCS_OPT=`(grep -m1 ^${SERVER_NAME}= ${DCSOPT_CONF} | sed 's/^.*=//')`
    if [ "$DCS_OPT" != "" ]; then
        echo "Initial DCS options [${DCSOPT_CONF}]: ${DCS_OPT}"
        ARGS="${ARGS} ${DCS_OPT}"
    fi
fi
...
#if [ "$MEMORY" == "" ] && [ -f "${DCSMEM_CONF}" ] ; then
#    # - extract single entry from dcs memory config file
#    MEMORY=`(grep -m1 ${SERVER_NAME}= ${DCSMEM_CONF} | sed 's/^.*=//')`
#fi
if [ "$MEMORY" != "" ]; then
    echo "Runtime allocated memory (override): ${MEMORY}"
...
# --- extra args
ARGS="${ARGS} $1 $2 $3" # adds 3 additional spaces if extra-args are blank

# --- interactive
if [ $INTERACTIVE -eq 1 ]; then
    COMMAND="${CMD_JAVA} ${ARGS} -log.file.enable=false"
    # --- run interactively
    echo "Server '${SERVER_NAME}' jar: ${SERVER_JAR}"
