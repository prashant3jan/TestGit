...
/* return RGB structure from 3/4/6/8-digit hex value */
function rgbVal(rgb)
{
    try {
        ...
        if ((rgbLen == 3) || (rgbLen == 4)) {
            // -- "#123", "#1234"
            var Rx = rgb.substr(0,1); // red
            var Gx = rgb.substr(1,1); // green
            var Bx = rgb.substr(2,1); // blue
            if (rgbLen == 3) {
                // -- "#rgb"
                var rE = eval("[ 0x"+Rx+Rx+", 0x"+Gx+Gx+", 0x"+Bx+Bx+" ]");
                return { R:rE[0], G:rE[1], B:rE[2] };
            } else {
                // -- "#rgba"
                var Ax = rgb.substr(3,1); // alpha
                var rE = eval("[ 0x"+Rx+Rx+", 0x"+Gx+Gx+", 0x"+Bx+Bx+", 0x"+Ax+Ax+" ]");
                return { R:rE[0], G:rE[1], B:rE[2], A:rE[3] };
            }
        } else 
        if ((rgbLen == 6) || (rgbLen == 8)) {
            // -- "#123456", "#123456AA"
            var Rx = rgb.substr(0,2); // red
            var Gx = rgb.substr(2,2); // green
            var Bx = rgb.substr(4,2); // blue
            if (rgbLen == 6) {
                // -- "#rrggbb"
                var rE = eval("[ 0x"+Rx+", 0x"+Gx+", 0x"+Bx+" ]");
                return { R:rE[0], G:rE[1], B:rE[2] };
            } else {
                // -- "#rrggbbaa"
                var Ax = rgb.substr(6,2); // alpha
                var rE = eval("[ 0x"+Rx+", 0x"+Gx+", 0x"+Bx+", 0x"+Ax+" ]");
                return { R:rE[0], G:rE[1], B:rE[2], A:rE[3] };
            }
        } else {
        ...
        // -- error (likely due to invalid input string)
        return { R:0, G:0, B:0 };
    }
};
