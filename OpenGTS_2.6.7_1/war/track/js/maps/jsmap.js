    ...
    //shadowURL, shadowSize)
{
    ...
    this.show        = true;    // show pushpin
    this.balloon     = true;    // show info-balloon
    ...
    this.accRadius   = null; // (shape placeholder) Circle
    ...
    this.bgMarker    = null; // (shape placeholder) Marker

    /* route line segment color (RouteSegmentColor) */
    var  hasRC       = ppIcon.routeColor? true : false;
    this.routeSegC   = hasRC? ppIcon.routeColor : null;
    this.routeSeg    = null; // (shape placeholder) Polyline
...
// }
    ...
    for (var dsi = 0; dsi < DataSets.length; dsi++) {
        ...
        /* dataset vars */
        var showPushpin = PUSHPINS_SHOW; // [JSON] PROP_map_pushpins
        var showBalloon = BALLOONS_SHOW; // [JSON] PROP_map_balloons
        var pushPinCnt  = 0;
            ...
            if (evRcd.validGPS) {
                ...
                if (evRcd.timestamp > 0) {
                    ...
                    if (ppObj) {
                        ...
                        lastValidPushpin.show    = showPushpin; // [JSON] PROP_map_pushpins
                        lastValidPushpin.balloon = showBalloon; // [JSON] PROP_map_balloons
                        if (showPushpin) {
        ...
        if (!showPushpin && (lastValidPushpin != null)) {
            lastValidPushpin.show = true; // PROP_map_pushpins [2.4.0-B36]
            pushPinList.push(lastValidPushpin);
        }

        /* !showBalloon? always show at least the last balloon [XML] */
        if (!showBalloon && (lastValidPushpin != null)) {
            lastValidPushpin.balloon = true; // PROP_map_balloons [2.6.6-B71]
        }
...
function jsmParseAJAXPoints_XML(xmlText, recenterMode, replay) // tmz
{
    ...
    for (var dsi = 0; dsi < dataSets.length; dsi++) {
        ...
        /* dataset vars */
        var showPushpin = PUSHPINS_SHOW; // [XML] PROP_map_pushpins
        var showBalloon = BALLOONS_SHOW; // [XML] PROP_map_balloons
        var pushPinCnt  = 0;
        var pushPinList = []; // device pushpins
        var partial     = false;
        ...
        for (var p = startNdx; p < pts.length; p++) {
            ...
            if (evRcd.validGPS) {
                ...
                if (evRcd.timestamp > 0) {
                    ...
                    if (ppObj) {
                        lastValidPushpin = ppObj;
                        lastValidPushpin.show    = showPushpin; // [XML]
                        lastValidPushpin.balloon = showBalloon; // [XML]
                        if (showPushpin) {
                            lastValidPushpin.show = true;
                            pushPinList.push(lastValidPushpin);
        ...
        if (!showPushpin && (lastValidPushpin != null)) {
            lastValidPushpin.show = true; // GTS_2.4.0-B36
            pushPinList.push(lastValidPushpin);
        }

        /* !showBalloon? always show at least the last balloon [XML] */
        if (!showBalloon && (lastValidPushpin != null)) {
            lastValidPushpin.balloon = true; // [2.6.6-B71]
        }
...
/**
*** Pushpin: 
*** Returns a 'orange' pushpin/icon URL based on the event heading
*** @param e  The 'MapEventRecord' object
**/
function evHeadingOrangeURL(e)
{
    ...
    if (e.speedKPH < 1.0) {
        // probably not moving
        return ppImg + "_orange.png";
    } else {
        var x = Math.round(e.heading / 45.0) % 8;
        return ppImg + "_orange_h"+x+".png";
    }
};
