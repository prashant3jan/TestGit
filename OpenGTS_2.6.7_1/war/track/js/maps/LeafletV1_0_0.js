...
function JSMap(mapElement)
{
    ...
    if ((LEAFLET_CUSTOM1_NAME != "") && (LEAFLET_CUSTOM1_URL != "")) {
        ...
        mapLayer[LEAFLET_CUSTOM1_NAME] = map;
        if (!firstMapLayer) { firstMapLayer = map; }
    }
    if ((LEAFLET_CUSTOM2_NAME != "") && (LEAFLET_CUSTOM2_URL != "")) {
        ...
        mapLayer[LEAFLET_CUSTOM2_NAME] = map;
        if (!firstMapLayer) { firstMapLayer = map; }
    }
    ...
    /* initialize traffic overlays */
    var mapOverlay = { };
    if ((LEAFLET_TRAFFIC_NAME != "") && (LEAFLET_TRAFFIC_URL != "")) {
        var traffic = L.tileLayer(
            LEAFLET_TRAFFIC_URL, 
            {
                subdomains: LEAFLET_TRAFFIC_SUBDOMAINS,
                maxZoom: LEAFLET_TRAFFIC_MAX_ZOOM,
            }
        );
        mapOverlay[LEAFLET_TRAFFIC_NAME] = traffic;
    }

    /* create map */
    if (firstMapLayer) {
        this.leafletMap.addLayer(firstMapLayer,mapOverlay); // add first mapLayer (this sets the default displayed map)
        L.control.layers(mapLayer,mapOverlay).addTo(this.leafletMap);
    } else {
