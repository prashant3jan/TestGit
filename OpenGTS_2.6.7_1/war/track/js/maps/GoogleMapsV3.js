...
var DRAW_ROUTE_SEGMENT          = true; // RouteSegmentColor

//var GOOGLE_DOUBLE_CLICK_ZOOM  = false;

var MAP_GOOGLE_ROAD             = "road";
var MAP_GOOGLE_HYBRID           = "hybrid";
var MAP_GOOGLE_SATELLITE        = "satellite";
var MAP_GOOGLE_TERRAIN          = "terrain";
var MAP_OSM_ROAD                = "osm_road";
var MAP_BING_ROAD               = "bing_road";
var MAP_HERE_FLEET              = "here_fleet";
var MAP_HERE_HYBRID             = "here_hybrid";
...
google.maps.Map.prototype.initGTS = function()
{
    ...
    var mv = DEFAULT_VIEW;
    if (mv != "") {
        if (GOOGLE_ROADMAP && (mv == MAP_GOOGLE_ROAD)) {
            mapTypeID = google.maps.MapTypeId.ROADMAP;
        } else
        if (GOOGLE_HYBRID && (mv == MAP_GOOGLE_HYBRID)) {
            mapTypeID = google.maps.MapTypeId.HYBRID;
        } else
        if (GOOGLE_SATELLITE && ((mv == MAP_GOOGLE_SATELLITE) || (mv == "aerial"))) {
            mapTypeID = google.maps.MapTypeId.SATELLITE;
        } else
        if (GOOGLE_TERRAIN && (mv == MAP_GOOGLE_TERRAIN)) {
            mapTypeID = google.maps.MapTypeId.TERRAIN;
        } else
        if (GOOGLE_OPENSTREETMAP && ((mv == MAP_OSM_ROAD) || (mv == "osm"))) {
            mapTypeID = MAP_OSM_ROAD;
        } else
        if (GOOGLE_NOKIAHERE && ((mv == MAP_HERE_FLEET) || (mv == "here"))) {
            mapTypeID = MAP_HERE_FLEET;
        } else
        if (GOOGLE_NOKIAHERE && (mv == MAP_HERE_HYBRID)) {
            mapTypeID = MAP_HERE_HYBRID;
        } else {
            // -- unexpected state (unlikely, if debugged properly)
            // -  (ie. showRoadMap=false and mapView="road")
            mv = ""; // reset and try again below
        }
    }
    // -- default type not specified?  
    if (mv == "") {
        // -- pick a mapTypeID based on what is enabled
        if (GOOGLE_ROADMAP) {
            mapTypeID = google.maps.MapTypeId.ROADMAP;
        } else
        if (GOOGLE_HYBRID) {
        ...
        if (GOOGLE_NOKIAHERE) {
            ...
            // -- nothing enabled? set to Google "road" anyway (unlikely, if debugged properly)
            mapTypeID = google.maps.MapTypeId.ROADMAP; 
        }
    }
    ...
    try {
        ...
        // -- ignore (this catch may not be necessary)
    }
...
google.maps.Map.prototype.addPushpinMarker = function(pp/*JSMapPushpin*/)
{
    ...
    /* route-line segment (RouteSegmentColor) */
    if (pp.routeSeg) { // RouteSegmentColor
        pp.routeSeg.setMap(this);
    }
...
function JSMap(element)
{
    ...
    if (!GOOGLE_MAP_OPTIONS || (GOOGLE_MAP_OPTIONS == "")) {
        GOOGLE_MAP_OPTIONS = {
            mapTypeControlOptions: {
                ...
                mapTypeIds: [  ] // set mapTypeControlOptions below [2.6.6-B11]
            },
            panControl: true, /* new */
            draggable: true,
    ...
    /* add Google RoadMap */
    if (GOOGLE_ROADMAP) {
        GOOGLE_MAP_OPTIONS.mapTypeControlOptions.mapTypeIds.push(google.maps.MapTypeId.ROADMAP);
    }

    /* add Google Hybrid */
    if (GOOGLE_HYBRID) {
        GOOGLE_MAP_OPTIONS.mapTypeControlOptions.mapTypeIds.push(google.maps.MapTypeId.HYBRID);
    }

    /* add Google Satellite/Aerial */
    if (GOOGLE_SATELLITE) {
        GOOGLE_MAP_OPTIONS.mapTypeControlOptions.mapTypeIds.push(google.maps.MapTypeId.SATELLITE);
    }

    /* add Google Terrain */
    if (GOOGLE_TERRAIN) {
        GOOGLE_MAP_OPTIONS.mapTypeControlOptions.mapTypeIds.push(google.maps.MapTypeId.TERRAIN);
    }

    /* add Bing Maps */
    if (GOOGLE_BINGMAP) {
        GOOGLE_MAP_OPTIONS.mapTypeControlOptions.mapTypeIds.push(MAP_BING_ROAD);
    }

    /* add OpenStreetMaps */
    if (GOOGLE_OPENSTREETMAP) {
        GOOGLE_MAP_OPTIONS.mapTypeControlOptions.mapTypeIds.push(MAP_OSM_ROAD);
    }

    /* add NokiaHere [EXPERIMENTAL] */
    if (GOOGLE_NOKIAHERE) {
        GOOGLE_MAP_OPTIONS.mapTypeControlOptions.mapTypeIds.push(MAP_HERE_FLEET);
        GOOGLE_MAP_OPTIONS.mapTypeControlOptions.mapTypeIds.push(MAP_HERE_HYBRID);
    }
    ...
    if (GOOGLE_BINGMAP) {
        ...
        this.gmapGoogleMap.mapTypes.set(MAP_BING_ROAD, new google.maps.ImageMapType({
    ...
    if (GOOGLE_OPENSTREETMAP) {
        ...
        this.gmapGoogleMap.mapTypes.set(MAP_OSM_ROAD, new google.maps.ImageMapType({
    ...
    if (GOOGLE_NOKIAHERE) {
        ...
        var tsNdx  = 0;
        // --
        this.gmapGoogleMap.mapTypes.set(MAP_HERE_FLEET, new google.maps.ImageMapType({
            name: "HERE Fleet",
            getTileUrl: function(C, Z) { 
                var url = "http://"+((++tsNdx%4)+1)+".maps.nlp.nokia.com/maptile/2.1/maptile/newest/normal.day";
              //var url = "http://"+((++tsNdx%4)+1)+".base.maps.cit.api.here.com/maptile/2.1/maptile/newest/normal.day";
                return url+"/"+Z+"/"+C.x+"/"+C.y+"/256/png8?lg=ENG&app_id="+APP_ID+"&app_code="+APP_CD;
            },
            tileSize: new google.maps.Size(256, 256),
            maxZoom: 20
        }));
        // --
        this.gmapGoogleMap.mapTypes.set(MAP_HERE_HYBRID, new google.maps.ImageMapType({
            name: "HERE Hybrid",
            getTileUrl: function(C, Z) { 
                var url = "http://"+((++tsNdx%4)+1)+".maps.nlp.nokia.com/maptile/2.1/maptile/newest/hybrid.day";
              //var url = "http://"+((++tsNdx%4)+1)+".aerial.maps.cit.api.here.com/maptile/2.1/maptile/newest/hybrid.day";
                return url+"/"+Z+"/"+C.x+"/"+C.y+"/256/png8?lg=ENG&app_id="+APP_ID+"&app_code="+APP_CD;
            },
            tileSize: new google.maps.Size(256, 256),
            maxZoom: 20
    ...
    /* "click" */
    google.maps.event.addListener(this.gmapGoogleMap, "click", function(event) { // 'click'
        ...
        if (jsvGeozoneMode && jsvZoneEditable) {
            ...
            if (jsvZoneType == ZONE_POLYGON) {
                ...
                if (true) {
                    ...
                    // (NOT USED) move valid points to new location
...
JSMap.prototype.JSDrawPushpins = function(pushPins, recenterMode, replay)
{
    ...
    for (var i = 0; i < drawPushpins.length; i++) {
        ...
        try {
            var nextPP = DRAW_ROUTE_SEGMENT? this._getNextPP(drawPushpins,i) : null; // RouteSegmentColor
            this._addPushpin(pp, nextPP); // pp/nextPP=JSMapPushpin
        } catch (e) {
...
JSMap.prototype.JSDrawPOI = function(pushPins)
{
    ...
    if ((pushPins != null) && (pushPins.length > 0)) {
        ...
        for (var i = 0; i < pushPins.length; i++) {
            ...
            try {
                this._addPushpin(pp, null); // JSMapPushpin
            } catch (e) {
                if (pushpinErr == null) { pushpinErr = e; }
            }
        }
...
/**
*** Returns the next valid JSMapPushpin after the specified index
*** @param pushPins  An array of JSMapPushpin objects
*** @param ndx       The initial index
**/
JSMap.prototype._getNextPP = function(pushPins, ndx)
{
    if ((pushPins != null) && (ndx >= 0)) {
        for (var i = ndx + 1; i < pushPins.length; i++) {
            var npp = pushPins[i]; // JSMapPushpin
            if ((npp.lat != 0.0) || (npp.lon != 0.0)) {
                return npp;
            }
        }
    }
...
*** @param pp  The JSMapPushpin object to add to the map (should never be null)
*** @param np  The next JSMapPushpin object, or null if we are at the end of the list
**/
JSMap.prototype._addPushpin = function(pp, np) // pp/npp=JSMapPushpin
{
    try {
        var self = this;
        ...
        /* create route-line segment Polyline (RouteSegmentColor) */
        pp.routeSeg = null; // RouteSegmentColor
        if (DRAW_ROUTE_SEGMENT && (np != null)) { // RouteSegmentColor
            var nPT  = jsNewGLatLng(np.lat, np.lon);
            var rSeg = [ gPT, nPT ]; // GLatLng[] (google.maps.LatLng[])
            inzRouteSeg: {
                // -- based on event criteria?
                if ((pp.evRcd != null) && (pp.evRcd.routeC) && (pp.evRcd.routeC != "")) {
                    pp.routeSeg = jsNewGPolyline(rSeg, pp.evRcd.routeC, 5/*width*/, 1.0);
                    break inzRouteSeg;
                }
                // -- based on pushpin attribute?
                if ((pp.routeSegC != null) && (pp.routeSegC != "")) {
                    pp.routeSeg = jsNewGPolyline(rSeg, pp.routeSegC, 5/*width*/, 1.0);
                    break inzRouteSeg;
                }
                // -- exit block (redundant)
                break inzRouteSeg;
            }
        }

        /* create accuracy point-radius shape */
...
JSMap.prototype._replayPushpins = function(replay)
{
    ...
    try { 
        ...
        var ppNdx = this.replayIndex++;
        var pp = this.replayPushpins[ppNdx]; // JSMapPushpin
        pp.hoverPopup = true;
        ...
        var nextPP = DRAW_ROUTE_SEGMENT? this._getNextPP(this.replayPushpins,ppNdx) : null; // RouteSegmentColor
        this._addPushpin(pp, nextPP);
        if (replay && (replay >= 2)) {
