    ...
    extends DBRecord<ReportDeviceList> // not really a database table
{
    ...
    private static volatile long _uniqSequence = 1L;
    ...
    public ReportDeviceList()
    {
        super();
        this._seq = _uniqSequence++;
    }
    ...
    **/
    ...
    protected void _addDevice(User user, DeviceHolder dh)
        throws DBException
    {
        // -- Note: added devices may be subsequently removed by the report "postInitialize()"
        String devID = dh.getDeviceID();
        if ((user == null) || user.isAuthorizedDevice(DBReadWriteMode.READ_ONLY,devID)) {
            Map<String,DeviceHolder> dm = this.getDeviceMap();
            if (dm.containsKey(devID)) {
            ...
            //Print.logWarn("*User not authorized to Device: "+dh.getDevice());
        }
    }
        ...
        throws DBException
    {
        ...
        if ((user == null) || user.isAuthorizedDevice(rwMode,devID)) {
            Map<String,DeviceHolder> dm = this.getDeviceMap();
            if (dm.containsKey(devID)) {
            ...
            //Print.logWarn("User not authorized to Device: "+device);
        }
    }
    ...
    public boolean add(Device device)
    {
        ...
        try {
            ...
            Print.logException("Error adding device: "+device+" [size="+this.size()+"]", dbe);
            return false;
        }
    ...
    public boolean add(String devID[])
    {
        ...
        try {
            ...
            Print.logException("Error adding deviceIDs: [size="+this.size()+"]", dbe);
            return false;
        }
    ...
    public boolean add(java.util.List<String> devIDList)
    {
        ...
        try {
            ...
            Print.logException("Error adding device list: [size="+this.size()+"]", dbe);
            return false;
        }
    ...
    public boolean add(String devID)
    {
        ...
        try {
            ...
            Print.logException("Error adding deviceID: "+devID+" [size="+this.size()+"]", dbe);
            return false;
        }
    ...
    public boolean add(DeviceGroup group)
    {
        ...
        User    user = this.getUser();
        try {
            OrderedSet<String> devIDSet = DeviceGroup.getDeviceIDsForGroup(rwMode, acctID, group.getGroupID(), null/*User*/, false);
            ...
            Print.logException("Error adding DeviceGroup", dbe);
            return false;
        }
    ...
    public void addAllAuthorizedDevices()
    {
        ...
        try {
            ...
            Account acct = this.getAccount();
            OrderedSet<String> list = User.getAuthorizedDeviceIDs(rwMode, acct, usr, false);
