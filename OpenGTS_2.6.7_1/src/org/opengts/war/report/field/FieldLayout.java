...
//     -Added DATA_REPORT_START_TIMESTAMP, DATA_REPORT_END_TIMESTAMP
//     -Added DATA_REPORT_DAY_RANGE, DATA_TRIP_COUNT, DATA_DRIVING_ELAPSED_AVG
//     -Added DATA_SPEED_AVG, DATA_ODOMETER_DELTA_AVG, DATA_DISTANCE_AVG
//     -Added support for using "best" lat/lon for DATA_LATITUDE/DATA_LONGITUDE [2.6.6-B09]
//     -Added support for PTO hours offset to DATA_PTO_HOURS [2.6.6-B24g]
//     -Added DATA_LAST_REFUEL_DATETIME/_LEVEL/_VOLUME/_DISTANCE [2.6.6-B60g]
// ----------------------------------------------------------------------------
    ...
    extends ReportLayout
{
    ...
    public static final String  DATA_REPORT_START_TIMESTAMP = "reportStartTime";        // report start time
    public static final String  DATA_REPORT_END_TIMESTAMP   = "reportEndTime";          // report end time
    public static final String  DATA_REPORT_DAY_RANGE       = "reportDayRange";         // report days between end and start (inclusive)
    ...
    public static final String  DATA_VEHICLE_ID             = "vehicleId";              // field (see also DATA_DEVICE_VIN)
    ...
    public static final String  DATA_GPS_SPEED              = "gpsSpeed";               // field
    ...
    public static final String  DATA_SPEED_AVG              = "speedAvg";               // field/calc
    public static final String  DATA_HEADING                = "heading";                // field
    public static final String  DATA_DISTANCE               = "distance";               // field
    public static final String  DATA_DISTANCE_AVG           = "distanceAvg";            // field/calc
    ...
    public static final String  DATA_ODOMETER_DELTA_AVG     = "odomDeltaAvg";           // field/calc
    ...
    public static final String  DATA_LAST_REFUEL_DATETIME   = "lastRefuelDateTime";     // Device
    public static final String  DATA_LAST_REFUEL_LEVEL      = "lastRefuelLevel";        // Device
    public static final String  DATA_LAST_REFUEL_VOLUME     = "lastRefuelVolume";       // Device
    public static final String  DATA_LAST_REFUEL_DISTANCE   = "lastRefuelDistance";     // Device
  ...
  //public static final String  DATA_STARTSTOP_FUEL         = "startStopFuel";          // field
    ...
    public static final String  DATA_DELTA_FUEL_VOLUME      = "deltaFuelVolume";        // field
    ...
    public static final String  DATA_TRIP_COUNT             = "tripCount";              // field
    ...
    public static final String  DATA_DRIVING_ELAPSED_AVG    = "drivingElapseAvg";       // field/calc
    ...
    public static final String  DATA_DEVICE_VIN             = "deviceVehicleID";        // Device record (see also DATA_VEHICLE_ID)
    ...
    **/
    protected static boolean HasValidGeoPoint(ReportData rd, Object dataObj)
    {
        if (dataObj instanceof FieldData) {
            ...
            if (ReportLayout.GetDefaultBestLatLon(rd)) {
                if (fd.hasValue(DATA_BEST_GEOPOINT)) {
                    return GeoPoint.isValid(fd.getGeoPoint(DATA_BEST_GEOPOINT));
                } else
                if (fd.hasValue(DATA_BEST_LATITUDE) && fd.hasValue(DATA_BEST_LONGITUDE)) {
                    double lat = fd.getDouble(DATA_BEST_LATITUDE);
                    double lon = fd.getDouble(DATA_BEST_LONGITUDE);
                    return GeoPoint.isValid(lat,lon);
                }
                // -- fall through below
            }
    ...
    **/
    protected static GeoPoint GetGeoPoint(ReportData rd, Object dataObj)
    {
        if (dataObj instanceof FieldData) {
            FieldData fd = (FieldData)dataObj;
            if (fd.hasValue(DATA_GEOPOINT)) {
            ...
            if (ReportLayout.GetDefaultBestLatLon(rd)) {
                if (fd.hasValue(DATA_BEST_GEOPOINT)) {
                    return fd.getGeoPoint(DATA_BEST_GEOPOINT);
                } else
                if (fd.hasValue(DATA_BEST_LATITUDE) && fd.hasValue(DATA_BEST_LONGITUDE)) {
                    double lat = fd.getDouble(DATA_BEST_LATITUDE);
                    double lon = fd.getDouble(DATA_BEST_LONGITUDE);
        ...
        extends DataRowTemplate
    {
        ...
        public FieldDataRow() {
            ...
            // -- Report time range
            this.addColumnTemplate(new DataColumnTemplate(DATA_REPORT_START_TIMESTAMP) {
                public Object getColumnValue(int rowNdx, ReportData rd, ReportColumn rc, Object dataObj) {
                    String arg = rc.getArg();
                    FieldData fd = (FieldData)dataObj;
                    if (fd.hasValue(DATA_REPORT_START_TIMESTAMP)) {
                        long ts = fd.getLong(DATA_REPORT_START_TIMESTAMP);
                        if (ts > 0L) {
                            return fd.filterReturnedValue(DATA_REPORT_START_TIMESTAMP,String.valueOf(ts));
                        } else {
                            return rc.getBlankFiller();
                        }
                    } else {
                        ReportConstraints rptCon = rd.getReportConstraints();
                        long startTS = rptCon.getTimeStart();
                        if (startTS > 0L) {
                            return fd.filterReturnedValue(DATA_REPORT_START_TIMESTAMP,String.valueOf(startTS));
                        } else {
                            return rc.getBlankFiller();
                        }
                    }
                ...
                public String getTitle(ReportData rd, ReportColumn rc) {
                    ...
                    return i18n.getString("FieldLayout.reportTimeStart","Report Start") + "\n(Epoch)";
                }
            });
            this.addColumnTemplate(new DataColumnTemplate(DATA_REPORT_END_TIMESTAMP) {
                public Object getColumnValue(int rowNdx, ReportData rd, ReportColumn rc, Object dataObj) {
                    String arg = rc.getArg();
                    FieldData fd = (FieldData)dataObj;
                    if (fd.hasValue(DATA_REPORT_END_TIMESTAMP)) {
                        long ts = fd.getLong(DATA_REPORT_END_TIMESTAMP);
                        if (ts > 0L) {
                            return fd.filterReturnedValue(DATA_REPORT_END_TIMESTAMP,String.valueOf(ts));
                        } else {
                            return rc.getBlankFiller();
                        }
                    } else {
                        ReportConstraints rptCon = rd.getReportConstraints();
                        long endTS = rptCon.getTimeEnd();
                        if (endTS > 0L) {
                            return fd.filterReturnedValue(DATA_REPORT_END_TIMESTAMP,String.valueOf(endTS));
                        } else {
                            return rc.getBlankFiller();
                        }
                    }
            ...
            this.addColumnTemplate(new DataColumnTemplate(DATA_REPORT_DAY_RANGE) {
                public Object getColumnValue(int rowNdx, ReportData rd, ReportColumn rc, Object dataObj) {
                    String arg = rc.getArg();
                    FieldData fd = (FieldData)dataObj;
                    if (fd.hasValue(DATA_REPORT_DAY_RANGE)) {
                        long days = fd.getLong(DATA_REPORT_DAY_RANGE);
                        if (days >= 0L) {
                            return fd.filterReturnedValue(DATA_REPORT_DAY_RANGE,String.valueOf(days));
                        } else {
                            return rc.getBlankFiller();
                        }
                    } else {
                        ReportConstraints rptCon = rd.getReportConstraints();
                        TimeZone tmz     = rptCon.getTimeZone(); // possibly null?
                        DateTime startDT = new DateTime(rptCon.getTimeStart(), tmz);
                        DateTime endDT   = new DateTime(rptCon.getTimeEnd()  , tmz);
                        long     days    = endDT.getDayNumber() - startDT.getDayNumber() + 1L;
                        if (days >= 0L) {
                            return fd.filterReturnedValue(DATA_REPORT_DAY_RANGE,String.valueOf(days));
                        } else {
                            return rc.getBlankFiller();
                ...
                public String getTitle(ReportData rd, ReportColumn rc) {
                    I18N i18n = rd.getPrivateLabel().getI18N(FieldLayout.class);
                    return i18n.getString("FieldLayout.reportDayRange","Report Day") + "\nRange";
                }
            });
            ...
            this.addColumnTemplate(new DataColumnTemplate(DATA_DRIVER_STATUS) {
                public Object getColumnValue(int rowNdx, ReportData rd, ReportColumn rc, Object dataObj) {
                    ...
                    FieldData fd = (FieldData)dataObj;
                    int  drvStat = Driver.DutyStatus_INVALID;
                    if (fd.hasValue(DATA_DRIVER_STATUS)) {
                        drvStat = fd.getInt(DATA_DRIVER_STATUS);
                    } else {
                        Driver driver = fd.getDriver();
            ...
            this.addColumnTemplate(new DataColumnTemplate(DATA_LATITUDE) {
                public Object getColumnValue(int rowNdx, ReportData rd, ReportColumn rc, Object dataObj) {
                    ...
                    FieldData fd = (FieldData)dataObj;
                    double lat;
                    if (fd.hasValue(DATA_GEOPOINT) && GeoPoint.isValid(fd.getGeoPoint(DATA_GEOPOINT))) {
                        lat = fd.getGeoPoint(DATA_GEOPOINT).getLatitude();
                    } else
                    if (fd.hasValue(DATA_LATITUDE)) {
                        lat = fd.getDouble(DATA_LATITUDE);
                    } else
                    if (ReportLayout.GetDefaultBestLatLon(rd)) {
                        if (fd.hasValue(DATA_BEST_GEOPOINT) && GeoPoint.isValid(fd.getGeoPoint(DATA_BEST_GEOPOINT))) {
                            lat = fd.getGeoPoint(DATA_BEST_GEOPOINT).getLatitude();
                        } else
            ...
            this.addColumnTemplate(new DataColumnTemplate(DATA_LONGITUDE) {
                public Object getColumnValue(int rowNdx, ReportData rd, ReportColumn rc, Object dataObj) {
                    ...
                    double lon;
                    if (fd.hasValue(DATA_GEOPOINT) && GeoPoint.isValid(fd.getGeoPoint(DATA_GEOPOINT))) {
                        lon = fd.getGeoPoint(DATA_GEOPOINT).getLongitude();
                    } else
                    ...
                    if (ReportLayout.GetDefaultBestLatLon(rd)) {
                        if (fd.hasValue(DATA_BEST_GEOPOINT) && GeoPoint.isValid(fd.getGeoPoint(DATA_BEST_GEOPOINT))) {
                            lon = fd.getGeoPoint(DATA_BEST_GEOPOINT).getLongitude();
                        } else
            ...
            // -- Last Refuel
            this.addColumnTemplate(new DataColumnTemplate(DATA_LAST_REFUEL_DATETIME) {
                public Object getColumnValue(int rowNdx, ReportData rd, ReportColumn rc, Object dataObj) {
                    String  fmtArg = rc.getArg();
                    FieldData   fd = (FieldData)dataObj;
                    Device  device = fd.getDevice();
                    long timestamp = 0L;
                    if (fd.hasValue(DATA_LAST_REFUEL_DATETIME)) {
                        timestamp = fd.getLong(DATA_LAST_REFUEL_DATETIME);
                    } else
                    if (device != null) {
                        EventData ev = device.getLastFuelRefillEvent();
                        if (ev != null) {
                    ...
                    if (timestamp > 0L) {
                        ...
                        String   fmtStr = !StringTools.isBlank(fmtArg)? fmtArg : rl.getDateTimeFormat(rd.getPrivateLabel());
                        String    dtFmt = dt.format(fmtStr, tz);
                        ColumnValue  cv = new ColumnValue(dtFmt).setSortKey(timestamp);
                        return fd.filterReturnedValue(DATA_LAST_REFUEL_DATETIME,cv);
                    }
                    return rc.getBlankFiller();
                }
                public String getTitle(ReportData rd, ReportColumn rc) {
                    I18N i18n = rd.getPrivateLabel().getI18N(FieldLayout.class);
                    return i18n.getString("FieldLayout.lastRefuelDateTime","Last Refuel\nDate/Time" + "\n${timezone}");
                }
            });
            this.addColumnTemplate(new DataColumnTemplate(DATA_LAST_REFUEL_LEVEL) {
                public Object getColumnValue(int rowNdx, ReportData rd, ReportColumn rc, Object dataObj) {
                    String fmtArg = rc.getArg();
                    FieldData  fd = (FieldData)dataObj;
                    Device device = fd.getDevice();
                    double  level = 0.0;
                    if (fd.hasValue(DATA_LAST_REFUEL_LEVEL)) {
                        level = fd.getDouble(DATA_LAST_REFUEL_LEVEL);
                    } else
                    if (device != null) {
                        EventData ev = device.getLastFuelRefillEvent();
                        if (ev != null) {
                            Device.FuelTankIndex tank = Device.FuelTankIndex.TANK_1;
                            level = ev.getFuelLevel(tank,true/*estimate*/);
                        }
                    }
                    if (level < 0.0) {
                        I18N i18n = rd.getPrivateLabel().getI18N(FieldLayout.class);
                    ...
                    if (level == 0.0) {
                        ...
                        String p = Math.round(level*100.0) + "%";
                        return fd.filterReturnedValue(DATA_LAST_REFUEL_LEVEL, p);  // percent
                    }  
                }
                public String getTitle(ReportData rd, ReportColumn rc) {
                    I18N i18n = rd.getPrivateLabel().getI18N(FieldLayout.class);
                    return i18n.getString("FieldLayout.lastRefuelLevel","Last Refuel\nLevel");
                }
            });
            this.addColumnTemplate(new DataColumnTemplate(DATA_LAST_REFUEL_VOLUME) {
                public Object getColumnValue(int rowNdx, ReportData rd, ReportColumn rc, Object dataObj) {
                    String   fmtArg = rc.getArg();
                    FieldData    fd = (FieldData)dataObj;
                    Device   device = fd.getDevice();
                    double literVol = 0.0;
                    if (fd.hasValue(DATA_LAST_REFUEL_VOLUME)) {
                        literVol = fd.getDouble(DATA_LAST_REFUEL_VOLUME);
                    } else
                    if (device != null) {
                        EventData ev = device.getLastFuelRefillEvent();
                        if (ev != null) {
                            Device.FuelTankIndex tank = Device.FuelTankIndex.TANK_1;
                            double level = ev.getFuelLevel(tank,true/*estimate*/);
                            double cap = device.getFuelCapacity(tank);
                            literVol = cap * level;
                        }
                    }
                    if (literVol > 0.0) {
                        ...
                        if ((fmtArg != null) && fmtArg.startsWith("$")) {
                            // -- cost
                            double cost = Device.CalculateFuelCost(acct, fd.getDevice(), literVol);
                            if (cost > 0.0) {
                                String fmt = fmtArg.substring(1);
                                return fd.filterReturnedValue(DATA_LAST_REFUEL_VOLUME,formatDouble(cost, fmt, "#0.00"));
                            } else {
                                return rc.getBlankFiller();
                            }
                        } else {
                            // -- volume
                            return fd.filterReturnedValue(DATA_LAST_REFUEL_VOLUME,formatDouble(vol, fmtArg, "#0.0"));
                        }
                    } else {
                        return rc.getBlankFiller();
                    }
                ...
                public String getTitle(ReportData rd, ReportColumn rc) {
                    ...
                    I18N i18n = rd.getPrivateLabel().getI18N(FieldLayout.class);
                    String title = i18n.getString("FieldLayout.lastRefuel","Last Refuel");
            ...
            this.addColumnTemplate(new DataColumnTemplate(DATA_LAST_REFUEL_DISTANCE) {
                public Object getColumnValue(int rowNdx, ReportData rd, ReportColumn rc, Object dataObj) {
                    String fmtArg = rc.getArg();
                    FieldData  fd = (FieldData)dataObj;
                    Device device = fd.getDevice();
                    double distKM = -1.0;
                    if (fd.hasValue(DATA_LAST_REFUEL_DISTANCE)) {
                        distKM = fd.getDouble(DATA_LAST_REFUEL_DISTANCE);
                    } else
                    if (device != null) {
                        EventData ev = device.getLastFuelRefillEvent();
                        if (ev != null) {
                            ...
                            if ((refuelKM > 0.0) && (refuelKM <= odomKM)) {
                                distKM = odomKM - refuelKM;
                            }
                        }
                    }
                    if (distKM >= 0.0) {
                        double dist = Account.getDistanceUnits(rd.getAccount()).convertFromKM(distKM);
                        return fd.filterReturnedValue(DATA_LAST_REFUEL_DISTANCE,formatDouble(dist, fmtArg, "#0"));
                    } else {
                        return rc.getBlankFiller();
                    }
                }
                public String getTitle(ReportData rd, ReportColumn rc) {
                    I18N i18n = rd.getPrivateLabel().getI18N(FieldLayout.class);
                    return i18n.getString("FieldLayout.distanceSinceRefuel","Distance\nSince Refuel") + "\n${distanceUnits}";
                }
            });
            ...
            this.addColumnTemplate(new DataColumnTemplate(DATA_GPS_SPEED) {
                public Object getColumnValue(int rowNdx, ReportData rd, ReportColumn rc, Object dataObj) {
                    String arg = rc.getArg();
                    FieldData fd = (FieldData)dataObj;
                    if (fd.hasValue(DATA_GPS_SPEED)) {
                        double kph = fd.getDouble(DATA_GPS_SPEED); // KPH
                        if (kph > 0.0) {
                            Account a = rd.getAccount();
                            return fd.filterReturnedValue(DATA_GPS_SPEED,formatDouble(Account.getSpeedUnits(a).convertFromKPH(kph),arg,"0"));
                        } else {
                            return fd.filterReturnedValue(DATA_GPS_SPEED,"0   ");
                        }
                    } else {
                        return rc.getBlankFiller();
                    }
                ...
                public String getTitle(ReportData rd, ReportColumn rc) {
                    I18N i18n = rd.getPrivateLabel().getI18N(FieldLayout.class);
                    return i18n.getString("FieldLayout.gpsSpeed","GPS Speed") + "\n${speedUnits}";
                }
            });
            ...
            this.addColumnTemplate(new DataColumnTemplate(DATA_SPEED_AVG) {
                public Object getColumnValue(int rowNdx, ReportData rd, ReportColumn rc, Object dataObj) {
                    String arg = rc.getArg();
                    FieldData fd = (FieldData)dataObj;
                    if (fd.hasValue(DATA_SPEED_AVG)) {
                        double kph = fd.getDouble(DATA_SPEED_AVG); // KPH
                        if (kph > 0.0) {
                            Account a = rd.getAccount();
                            return fd.filterReturnedValue(DATA_SPEED_AVG,formatDouble(Account.getSpeedUnits(a).convertFromKPH(kph),arg,"0"));
                        } else {
                            return fd.filterReturnedValue(DATA_SPEED_AVG,"0   ");
                        }
                    } else {
                        // -- TODO: calculate? deltaDistance / deltaTime
                        return rc.getBlankFiller();
                    }
                }
                public String getTitle(ReportData rd, ReportColumn rc) {
                    I18N i18n = rd.getPrivateLabel().getI18N(FieldLayout.class);
                    return i18n.getString("FieldLayout.speed","Speed Avg") + "\n${speedUnits}";
                }
            });
            ...
            this.addColumnTemplate(new DataColumnTemplate(DATA_DISTANCE_AVG) {
                public Object getColumnValue(int rowNdx, ReportData rd, ReportColumn rc, Object dataObj) {
                    String arg = rc.getArg();
                    FieldData fd = (FieldData)dataObj;
                    if (fd.hasValue(DATA_DISTANCE_AVG)) {
                        double dist = fd.getDouble(DATA_DISTANCE_AVG); // kilometers
                        if (dist > 0.0) {
                            dist = Account.getDistanceUnits(rd.getAccount()).convertFromKM(dist);
                            return fd.filterReturnedValue(DATA_DISTANCE_AVG,formatDouble(dist, arg, "#0"));
                        } else {
                            return rc.getBlankFiller();
                        }
                    }
                    // -- trip count
                    if (!fd.hasValue(DATA_TRIP_COUNT)) {
                        return rc.getBlankFiller();
                    }
                    long tripCnt = fd.getLong(DATA_TRIP_COUNT,-1L);
                    if (tripCnt <= 0L) {
                        return rc.getBlankFiller();
                    }
                    ...
                    if (fd.hasValue(DATA_DISTANCE)) {
                        double dist = fd.getDouble(DATA_DISTANCE); // kilometers
                        if (dist >= 0.0) {
                            double avgDist = dist / (double)tripCnt;
                            avgDist = Account.getDistanceUnits(rd.getAccount()).convertFromKM(avgDist);
                            return fd.filterReturnedValue(DATA_DISTANCE_AVG,formatDouble(avgDist, arg, "#0"));
                        } else {
                            return rc.getBlankFiller();
                        }
                    } else {
                ...
                public String getTitle(ReportData rd, ReportColumn rc) {
                    I18N i18n = rd.getPrivateLabel().getI18N(FieldLayout.class);
                    return i18n.getString("FieldLayout.distanceAvg","Distance Avg") + "\n${distanceUnits}";
                }
            });
            ...
            this.addColumnTemplate(new DataColumnTemplate(DATA_ODOMETER_DELTA_AVG) {
                public Object getColumnValue(int rowNdx, ReportData rd, ReportColumn rc, Object dataObj) {
                    String arg = rc.getArg();
                    FieldData fd = (FieldData)dataObj;
                    if (fd.hasValue(DATA_ODOMETER_DELTA_AVG)) {
                        double dist = fd.getDouble(DATA_ODOMETER_DELTA_AVG); // kilometers
                        if (dist > 0.0) {
                            dist = Account.getDistanceUnits(rd.getAccount()).convertFromKM(dist);
                            return fd.filterReturnedValue(DATA_ODOMETER_DELTA_AVG,formatDouble(dist, arg, "#0"));
                        } else {
                            return rc.getBlankFiller();
                        }
                    }
                    ...
                    // -- calculate based on odometer-delta
                    if (fd.hasValue(DATA_ODOMETER_DELTA)) {
                        double dist = fd.getDouble(DATA_ODOMETER_DELTA); // kilometers
                        if (dist >= 0.0) {
                            double avgDist = dist / (double)tripCnt;
                            avgDist = Account.getDistanceUnits(rd.getAccount()).convertFromKM(avgDist);
                            return fd.filterReturnedValue(DATA_ODOMETER_DELTA_AVG,formatDouble(avgDist, arg, "#0"));
                        } else {
                            return rc.getBlankFiller();
                        }
                    }
                    ...
                    if ((startOdom >= 0.0) && (stopOdom >= 0.0) && (stopOdom >= startOdom)) {
                        double deltaOdom = stopOdom - startOdom;
                        double averDist  = deltaOdom / (double)tripCnt;
                        averDist = Account.getDistanceUnits(rd.getAccount()).convertFromKM(averDist);
                        return fd.filterReturnedValue(DATA_ODOMETER_DELTA_AVG,formatDouble(averDist,arg,"#0"));
                    }
                    // -- default
                    return rc.getBlankFiller();
                }
                public String getTitle(ReportData rd, ReportColumn rc) {
                    I18N i18n = rd.getPrivateLabel().getI18N(FieldLayout.class);
                    return i18n.getString("FieldLayout.driven","Driven Avg") + "\n${distanceUnits}";
                }
            });
            ...
            this.addColumnTemplate(new DataColumnTemplate(DATA_TRIP_COUNT) {
                public Object getColumnValue(int rowNdx, ReportData rd, ReportColumn rc, Object dataObj) {
                    String arg = rc.getArg();
                    FieldData fd = (FieldData)dataObj;
                    if (fd.hasValue(DATA_TRIP_COUNT)) {
                        long tripCnt = fd.getLong(DATA_TRIP_COUNT,-1L);
                        if (tripCnt >= 0L) {
                            return fd.filterReturnedValue(DATA_TRIP_COUNT, String.valueOf(tripCnt));
                        } else {
                            return rc.getBlankFiller();
                        }
                    } else {
                ...
                public String getTitle(ReportData rd, ReportColumn rc) {
                    I18N i18n = rd.getPrivateLabel().getI18N(FieldLayout.class);
                    return i18n.getString("FieldLayout.tripCount","Trip\nCount");
                }
            });
            ...
            this.addColumnTemplate(new DataColumnTemplate(DATA_DRIVING_ELAPSED_AVG) {
                public Object getColumnValue(int rowNdx, ReportData rd, ReportColumn rc, Object dataObj) {
                    String arg = rc.getArg();
                    FieldData fd = (FieldData)dataObj;
                    if (fd.hasValue(DATA_DRIVING_ELAPSED_AVG)) {
                        long driveSec = fd.getLong(DATA_DRIVING_ELAPSED_AVG,-1L);
                        if (driveSec >= 0L) {
                            int fmt = FieldLayout.getElapsedFormat(arg, StringTools.ELAPSED_FORMAT_HHMMSS);
                            ColumnValue cv = new ColumnValue(FieldLayout.formatElapsedTime(driveSec,fmt)).setSortKey(driveSec);
                            return fd.filterReturnedValue(DATA_DRIVING_ELAPSED_AVG, cv);
                        } else {
                            return rc.getBlankFiller();
                        }
                    }
                    ...
                    // -- calculate based on driving elapsed
                    if (fd.hasValue(DATA_DRIVING_ELAPSED)) {
                        long totDriveSec = fd.getLong(DATA_DRIVING_ELAPSED,-1L);
                        if (totDriveSec > 0L) {
                            long avgDriveSec = totDriveSec / tripCnt;
                            int fmt = FieldLayout.getElapsedFormat(arg, StringTools.ELAPSED_FORMAT_HHMMSS);
                            ColumnValue cv = new ColumnValue(FieldLayout.formatElapsedTime(avgDriveSec,fmt)).setSortKey(avgDriveSec);
                            return fd.filterReturnedValue(DATA_DRIVING_ELAPSED_AVG, cv);
                        } else {
                            return rc.getBlankFiller();
                        }
                ...
                public String getTitle(ReportData rd, ReportColumn rc) {
                    I18N i18n = rd.getPrivateLabel().getI18N(FieldLayout.class);
                    return i18n.getString("FieldLayout.drivingElapsedAvg","Driving\nElapsed Avg");
                }
            });
            ...
            this.addColumnTemplate(new DataColumnTemplate(DATA_PTO_HOURS) {
                public Object getColumnValue(int rowNdx, ReportData rd, ReportColumn rc, Object dataObj) {
                    ...
                    if (fd.hasValue(DATA_PTO_HOURS)) {
                        hours = fd.getDouble(DATA_PTO_HOURS,0.0);
                    } else
                    if (dev != null) {
                        hours = dev.getLastPtoHours();
                    }
                    if (hours > 0.0) {
                        if (dev != null) {
                            hours += dev.getPtoHoursOffset(); // ok
                        }
                        long sec = Math.round(hours * 3600.0);
                        if (sec >= 0L) {
                            int fmt = FieldLayout.getElapsedFormat(arg, StringTools.ELAPSED_FORMAT_HHHh);
            ...
            // -- Fuel Delta Volume (ie. "Gallons Consumed")
            this.addColumnTemplate(new DataColumnTemplate(DATA_DELTA_FUEL_VOLUME) {
                public Object getColumnValue(int rowNdx, ReportData rd, ReportColumn rc, Object dataObj) {
                    String arg = rc.getArg();
                    FieldData fd = (FieldData)dataObj;
                    if (fd.hasValue(DATA_DELTA_FUEL_VOLUME)) {
                        double literVol = fd.getDouble(DATA_DELTA_FUEL_VOLUME); // Liters
                        if (literVol >= 0.0) {
                            Account acct = rd.getAccount();
                            double vol = Account.getVolumeUnits(acct).convertFromLiters(literVol);
                            return fd.filterReturnedValue(DATA_DELTA_FUEL_VOLUME,formatDouble(vol,arg,"#0"));
                        } else {
                            return rc.getBlankFiller();
                        }
                    } else
                    if (fd.hasValue(DATA_DELTA_FUEL_LEVEL)) {
                        double pct  = fd.getDouble(DATA_DELTA_FUEL_LEVEL); // percent
                        Device dev  = fd.getDevice();
                        double capL = (dev != null)? dev.getFuelCapacity(Device.FuelTankIndex.TOTAL) : 0.0;
                        if (capL > 0.0) {
                            double volL = pct * capL; // Litres
                            double vol  = Account.getVolumeUnits(rd.getAccount()).convertFromLiters(volL);
                            return fd.filterReturnedValue(DATA_DELTA_FUEL_VOLUME,formatDouble(vol,arg,"#0"));
                        } else {
                            return rc.getBlankFiller();
                        }
                        ...
                        if (fd.hasValue(DATA_START_FUEL_LEVEL)) {
                            double pct  = fd.getDouble(DATA_START_FUEL_LEVEL); // percent
                            Device dev  = fd.getDevice();
                            double capL = (dev != null)? dev.getFuelCapacity(Device.FuelTankIndex.TOTAL) : 0.0;
                            if (capL > 0.0) {
                                startFuel = pct * capL;
                        ...
                        if (fd.hasValue(DATA_STOP_FUEL_LEVEL)) {
                            double pct  = fd.getDouble(DATA_STOP_FUEL_LEVEL); // percent
                            Device dev  = fd.getDevice();
                            double capL = (dev != null)? dev.getFuelCapacity(Device.FuelTankIndex.TOTAL) : 0.0;
                            if (capL > 0.0) {
                                stopFuel  = pct * capL;
            ...
            this.addColumnTemplate(new DataColumnTemplate(DATA_FAULT_CODES) {
                public Object getColumnValue(int rowNdx, ReportData rd, ReportColumn rc, Object dataObj) {
                    ...
                    if (StringTools.isBlank(faultStr)) {
                        ...
                        String fc = DTOBDFault.GetFaultString(new RTProperties(faultStr), false/*firstOnly?*/);
