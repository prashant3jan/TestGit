                ...
                for (String m : modules) {
                    if (!StringTools.isBlank(m)) {
                        ...
                        if (m.startsWith("$") && (m.length() > 1)) {
                            // -- check property (must be explicitly set to a 'true' value)
                            String mkey = m.substring(1).trim();
                            if (!RTConfig.getBoolean(mkey,false)) {
                                Print.logDebug("[Report '"+rptName+"'] Property is false: " + mkey);
                                continue nextReport; 
                            }
                        } else {
                ...
                if (ListTools.contains(OPTIONS_TYPE_geozone,rptOptType)) {
                    ...
                    ReportOptionsProvider rptOptProv = new ReportOptionsProvider() {
                        public OrderedMap<String,ReportOption> getReportOptionMap(ReportFactory rptFact, RequestProperties reqState) {
                            ...
                            User    user    = (reqState != null)? reqState.getCurrentUser()    : null;
                            if (account == null) { 
                            ...
                            try {
                                ...
                                if (showCustomOptions) {
                                  ...
                                  //String userID = User.getUserID(user); // fixed NPE [2.6.7-B46k]
                                    DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE; // TODO: could be READ?
                                    String idList[] = Geozone.getGeozoneIDsForAccount(rwMode, account, user, (String[])null); // has Account/User
                                    if (ListTools.isEmpty(idList)) {
                                        Print.logWarn("Account does not contain any Geozones: " + acctID);
                                    } else {
                                        for (String id : idList) {
                                            ...
                                            if (GZ != null) {
                                                ...
                                                Print.logWarn("Account/Geozone not found: " + acctID+"/"+id);
                                            }
                                        }
                                    }
                                } else {
                                    Print.logInfo("List of Geozones not included (showCustomOptions == false) ... ");
                                }
                                return roMap;
                        ...
                        public OrderedMap<String,ReportOption> getReportOptionMap(ReportFactory rptFact, RequestProperties reqState) {
                            ...
                            try {
                                ...
                                if (reqState != null) {
                                    ...
                                    java.util.List<String> idList = reqState.getDeviceGroupIDList(rwMode,true/*inclAll*/);
                                    for (String id : idList) {
                        ...
                        public OrderedMap<String,ReportOption> getReportOptionMap(ReportFactory rptFact, RequestProperties reqState) {
                            ...
                            try {
                                ...
                                if (reqState != null) {
                                    DBReadWriteMode rwMode = DBReadWriteMode.READ_ONLY;
                                    String acctID = account.getAccountID();
                                    OrderedSet<String> driverList = Driver.getDriverIDsForAccount(rwMode, acctID, -1L);
