    ...
    extends HttpServlet
{
    ...
    /* authentication */
    private static final String  PARM_ACCOUNT[]     = { "account"    ,         "a"  };  // Constants.PARM_ACCOUNT;
    private static final String  PARM_USER[]        = { "user"       ,         "u"  };  // Constants.PARM_USER;
    private static final String  PARM_PASSWORD[]    = { "password"   ,         "p"  };  // Constants.PARM_PASSWORD;

    /* command */
    private static final String  PARM_COMMAND[]     = { "command"    , "cmd" , "c"  };  // Constants.PARM_COMMAND;

    /* event retrieval options */
    private static final String  PARM_DEVICE[]      = { "device"     ,         "d"  };  // Constants.PARM_DEVICE;
    private static final String  PARM_GROUP[]       = { "group"      ,         "g"  };  // Constants.PARM_GROUP;
    private static final String  PARM_TIMEZONE[]    = { "timezone"   , "tmz" , "tz" };  // Constants.PARM_TIMEZONE;
    private static final String  PARM_RANGE_FR[]    = { "rangefrom"  , "from", "rf" };  // Constants.PARM_RANGE_FR;
    private static final String  PARM_RANGE_TO[]    = { "rangeto"    , "to"  , "rt" };  // Constants.PARM_RANGE_TO;
    private static final String  PARM_LIMIT[]       = { "limit"      , "lim" , "l"  };  // Constants.PARM_LIMIT;
    private static final String  PARM_VALID_GPS[]   = { "validgps"   , "gps" , "vg" };  // Constants.PARM_VALID_GPS;
    private static final String  PARM_ALL_TAGS[]    = { "alltags"    ,         "at" };
    private static final String  PARM_MAINT[]       = { "maint"      ,         "ma" };  // Maintenance (JSON only)
    private static final String  PARM_OLD_XML[]     = { "oldxml"     , "old" , "ox" };
    private static final String  PARM_EXPIRE[]      = { "expire"     , "exp" , "ex" };

    // ------------------------------------------------------------------------

    /* commands */
    private static final String  CMD_EVENTS[]       = { "events" };
    private static final String  CMD_MAINT[]        = { "maintenance", "maint" };

    /** 
    *** Returns true if the specified command matches any command-alias in the list
    **/
    private static boolean CommandMatch(String cmd, String cmdList[]) 
    {
        if (!ListTools.isEmpty(cmdList)) {
            cmd = StringTools.trim(cmd).toLowerCase();
            if (StringTools.isBlank(cmd)) {
                // -- blank is allowed for "events"
                return CMD_EVENTS[0].equalsIgnoreCase(cmdList[0])? true : false;
            } else {
                // -- check for matching commnd
        ...
        throws ServletException, IOException
    {
        ...
        String  groupList  = Events.getRequestString (request, PARM_GROUP    , "");
        String  command    = Events.getRequestString (request, PARM_COMMAND  , "");
        ...
        boolean inclMaint  = Events.getRequestBoolean(request, PARM_MAINT    , false);
        ...
        if (!StringTools.isBlank(userID)) {
            try {
                if (userID.indexOf("@") > 0) {
                    user = User.getUserForContactEmail(account.getAccountID(), userID, true/*unique*/);
                } else {
        ...
        /* device list */
        if (!StringTools.isBlank(deviceList)) {
            String devIDArray[] = StringTools.parseStringArray(deviceList,",");
        ...
        if (!StringTools.isBlank(groupList)) {
            String groupIDArray[] = StringTools.parseStringArray(groupList,",");
            for (String groupID : groupIDArray) {
                try {
                    OrderedSet<String> devIDList = DeviceGroup.getDeviceIDsForGroup(DBReadWriteMode.READ_ONLY, accountID, groupID, user, false, -1);
        ...
        if (user != null) {
            for (String deviceID : deviceIDSet) {
                try {
                    if (!user.isAuthorizedDevice(DBReadWriteMode.READ_ONLY,deviceID)) {
        ...
        // --------------------------------------
        // -- Command: maintenance
        if (CommandMatch(command,CMD_MAINT)) {
            Print.logError("Maintenance not yet supported: " + accountID + "/" + userID);
            this.errorResponse(response, outFmt, "Maintenance not yet supported");
            return;
        }

        // --------------------------------------
        // --- Command: events (default)
        // CommandMatch(command,CMD_EVENTS)
