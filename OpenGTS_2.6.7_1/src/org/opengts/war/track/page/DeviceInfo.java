...
//     -Added "Engine Displacement" [2.6.6-B10]
//     -Added support for PTO offset [2.6.6-B24g]
//     -Added check for global unique DeviceID when creating a new device (see GetEnforceUniqueDeviceID) [2.6.6-B58q]
//     -Added "Last Received Event" time to device list [2.6.6-B60j]
// ----------------------------------------------------------------------------
    ...
    implements Constants
{
    ...
    private static final boolean SHOW_REPORTED_PTO_HRS      = true;  
    private static final boolean SHOW_MAINTENANCE_ODOM      = true;  // still requires maintenance support
    private static final boolean EDIT_MAINTENANCE_ODOM      = false; // still requires maintenance support
    private static final boolean SHOW_MAINTENANCE_HOURS     = true;  // still requires maintenance support
    private static final boolean SHOW_MAINTENANCE_FIXEDTIME = true;  // still requires maintenance support
  ...
  //public  static final String COMMAND_INFO_REFRESH        = "refreshList";
    ...
    public  static final String PARM_DEV_ENGINE_DISPLACE    = "d_engdisp";
    ...
    public  static final String PARM_ENGINE_HOURS           = "d_enghours";
    public  static final String PARM_PTO_HOURS              = "d_ptohours";
    ...
    public  static final String PARM_MAINT_INTERVFT_        = "d_mntintrft";
    public  static final String PARM_MAINT_LASTFT_          = "d_mntlastft";    // read-only
    public  static final String PARM_MAINT_RESETFT_         = "d_mntresft";     // reset checkbox
  ...
  //public  static final String PARM_WORKORDER_ID           = "d_workid";
    ...
    */
    ...
    private String _updateDeviceTable(RequestProperties reqState)
    {
        ...
        double  engDisplace = AttributeTools.getRequestDouble(request, PARM_DEV_ENGINE_DISPLACE    , 0.0);
        ...
        double  rptEngHrs   = AttributeTools.getRequestDouble(request, PARM_ENGINE_HOURS           , 0.0);
        double  rptPtoHrs   = AttributeTools.getRequestDouble(request, PARM_PTO_HOURS              , 0.0);
      ...
      //String  worderID    = AttributeTools.getRequestString(request, PARM_WORKORDER_ID           , "");
        try {
            ...
            if (!Device.GetEnabledLoadByUniqueID()) {
                // -- clear UniqueID
                selDev.setUniqueID("");
            } else
            ...
            if (engDisplace != selDev.getEngineDisplacement()) {
                selDev.setEngineDisplacement(engDisplace);
            }
            // -- Driver ID
            ...
            if (showAssgnUsr && Device.supportsAssignedUserID()) {
                if (!StringTools.isBlank(prefUsrID)) {
                    try {
                        ...
                        if (!user.isAuthorizedDevice(DBReadWriteMode.READ_WRITE,selDev.getDeviceID())) {
            ...
            if (editSIM && !selDev.getSimPhoneNumber().equals(simPhone)) {
                ...
                if (uniqSimPhone && !StringTools.isBlank(simPhone)) { // [2.6.5-B29]
                    ...
                    java.util.List<String> devList = Device.getDeviceIDsForSimPhoneNumber(DBReadWriteMode.READ_WRITE, simPhone, sep); // "account,device"
            ...
            // -- Reported PtoHours[Offset]
            boolean lastPtoHrsOK = privLabel.getBooleanProperty(PrivateLabel.PROP_DeviceInfo_showReportedPtoHours,SHOW_REPORTED_PTO_HRS);
            if (lastPtoHrsOK && Device.supportsLastPtoHours()) { // Domain.Properties.deviceInfo.showReportedPtoHours=false
                if (rptPtoHrs >= 0.0) {
                    double lastPtoHrs = selDev.getLastPtoHours();
                    double offsetHrs  = rptPtoHrs - lastPtoHrs;
                    if (Math.abs(offsetHrs - selDev.getPtoHoursOffset()) >= 0.01) {
                        selDev.setPtoHoursOffset(offsetHrs);
                    }
                }
            }
            // -- Maintenance Interval
            if (Device.supportsMaintenance()) {
                Account.DistanceUnits distUnits = Account.getDistanceUnits(currAcct);
                // -- Odometer Maintenance 
                boolean maintOdomEdit = privLabel.getBooleanProperty(PrivateLabel.PROP_DeviceInfo_editMaintenanceOdometer,EDIT_MAINTENANCE_ODOM);
                ...
                if (maintOdomOK && Device.supportsMaintenance_KM()) { // Domain.Properties.deviceInfo.showMaintenanceOdometer=false
                    double offsetKM = selDev.getOdometerOffsetKM();
                    int cnt = Device.getMaintCountKM();
                    for (int ndx = 0; ndx < cnt; ndx++) {
                        // -- Odometer Maintenance #X
                        long    mLastUN  = AttributeTools.getRequestLong(request, PARM_MAINT_LASTKM_+ndx  , 0L);
                        double  lastKM   = distUnits.convertToKM((double)mLastUN - offsetKM);
                        long    mIntrvUN = AttributeTools.getRequestLong(request, PARM_MAINT_INTERVKM_+ndx, 0L);
                        double  intrvKM  = distUnits.convertToKM((double)mIntrvUN);
                        boolean mResetKM = !StringTools.isBlank(AttributeTools.getRequestString(request,PARM_MAINT_RESETKM_+ndx,null));
                        if (maintOdomEdit && (lastKM != selDev.getMaintOdometerKM(ndx))) {
                            Print.logInfo("Updating Odometer Maint #"+ndx+": " + lastKM);
                            selDev.setMaintOdometerKM(ndx, lastKM);
                        }
                        selDev.setMaintIntervalKM(ndx,intrvKM);
                        if (mResetKM) {
                            selDev.resetMaintOdometerKM(ndx);
                        }
                    }
                }
                ...
                if (maintHoursOK && Device.supportsMaintenance_HR()) { // Domain.Properties.deviceInfo.showMaintenanceHours=false
                    int cnt = Device.getMaintCountHR();
                    for (int ndx = 0; ndx < cnt; ndx++) {
                        double  mIntrvHR = AttributeTools.getRequestDouble(request, PARM_MAINT_INTERVHR_+ndx, 0.0);
                        double  intrvHR  = mIntrvHR;
                        boolean mResetHR = !StringTools.isBlank(AttributeTools.getRequestString(request,PARM_MAINT_RESETHR_+ndx,null));
                        selDev.setMaintIntervalHR(ndx,intrvHR);
                        if (mResetHR) {
                            selDev.resetMaintEngHoursHR(ndx);
                        }
                    }
                }
                // -- FixedTime Maintenane #n
                boolean maintFixTimOK = privLabel.getBooleanProperty(PrivateLabel.PROP_DeviceInfo_showMaintenanceFixedTime,SHOW_MAINTENANCE_FIXEDTIME);
                if (maintFixTimOK && Device.supportsMaintenance_FT()) { // Domain.Properties.deviceInfo.showMaintenanceFixedTime=false
                    int cnt = Device.getMaintCountFT();
                    for (int ndx = 0; ndx < cnt; ndx++) {
                        long    mIntrvFT = AttributeTools.getRequestLong(request, PARM_MAINT_INTERVFT_+ndx, 0L);
                        long    intrvFT  = mIntrvFT; // Seconds/Days/Months
                        boolean mResetFT = !StringTools.isBlank(AttributeTools.getRequestString(request,PARM_MAINT_RESETFT_+ndx,null));
                        selDev.setMaintIntervalFT(ndx,intrvFT); // Seconds/Days/Months
                        if (mResetFT) {
                            selDev.resetMaintFixedTimeFT(ndx);
                        }
                    }
                }
                // -- maintenance notes
            ...
            */
            ...
            if (showPrefGrp) {
                ...
                if (StringTools.isBlank(prefGrpID)) {
                    ...
                    java.util.List<String> dgList = reqState.getDeviceGroupIDList(DBReadWriteMode.READ_WRITE, false); // user groups
                    if (dg == null) {
            ...
            if (!selDev.hasError()) {
                ...
                java.util.List<String> fullGroupSet = reqState.getDeviceGroupIDList(DBReadWriteMode.READ_WRITE, true);
                // -- add checked groups
        ...
        throws IOException
    {
        ...
        /* device */
        java.util.List<String> devList = reqState.getDeviceIDList(DBReadWriteMode.READ_WRITE, true/*inclInactv*/);
        ...
        boolean editEngDisp  = sysadminLogin || privLabel.hasWriteAccess(currUser, this.getAclName(_ACL_FUEL_CAPACITY));
        boolean viewEngDisp  = editEngDisp   || privLabel.hasReadAccess( currUser, this.getAclName(_ACL_FUEL_CAPACITY));
      ...
      //boolean refreshList  = deviceCmd.equals(COMMAND_INFO_REFRESH);
        ...
        if (deleteDevice) {
            ...
            try {
                ...
                devList = reqState.getDeviceIDList(DBReadWriteMode.READ_WRITE, true/*inclInactv*/);
        ...
        if (newDevice) {
            try {
                ...
                if (Device.GetEnforceUniqueDeviceID()) {
                    // -- check for another matching deviceID system-wide
                    java.util.List<String> acctList = Account.getAccountIDsForDeviceID(DBReadWriteMode.READ_WRITE, newDeviceID);
                    if (!ListTools.isEmpty(acctList)) {
                        throw new DBAlreadyExistsException("Global DeviceID already exists: "+newDeviceID+" ["+StringTools.join(acctList,",")+"]");
                    }
                } else {
                    // -- check for another matching deviceID within this account
                    for (int d = 0; d < devList.size(); d++) {
                        if (newDeviceID.equalsIgnoreCase(devList.get(d))) {
                            throw new DBAlreadyExistsException("Account DeviceID already exists: "+currAcct+"/"+newDeviceID);
                        }
                    }
                }
                // -- create device
                ...
                Print.logInfo("Device '"+currAcctID+"/"+selDevID+"' created");
                // -- add device to Users first explicitly authorized group
                ...
                if (addDevToUsrGrp && (currUser != null)) {
                    java.util.List<String> groupIDList = currUser.getExplicitlyAuthorizedDeviceGroupIDs(DBReadWriteMode.READ_WRITE);
                    String groupID = !ListTools.isEmpty(groupIDList)? groupIDList.get(0) : null;
              ...
              //m = i18n.getString("DeviceInfo.createdUser","New {0} has been created", devTitles); // UserErrMsg
                ...
                Print.logError(dbaee.getMessage());
        ...
        final boolean showFilter         = privLabel.getBooleanProperty(PrivateLabel.PROP_AdminPage_showSearchFilter,false) && Track.jsFileExists(FILTER_JS);
        ...
        /* JavaScript */
        final boolean  _showFilter   = showFilter; // FILTER_JS
        ...
        final java.util.List<String> _deviceList = devList;
            ...
            HTML_CONTENT = new HTMLOutput(CommonServlet.CSS_CONTENT_FRAME, m) {
                public void write(PrintWriter out) throws IOException {
                  ...
                  //String editURL    = DeviceInfo.this.encodePageURL(reqState);//,RequestProperties.TRACK_BASE_URI());
                    ...
                    String timeZoneS  = reqState.getTimeZoneString(null); // _viewActive
                    ...
                    out.write("<span class='"+CommonServlet.CSS_ADMIN_SELECT_TITLE+"'>"+i18n.getString("DeviceInfo.selectDevice","Select a {0}",devTitles)+":</span>\n");
                    ...
                    out.write("<table class='"+CommonServlet.CSS_ADMIN_SELECT_TABLE+"' id='filter-table' cellspacing='0' cellpadding='0' border='0'>\n");
                    out.write(" <thead>\n");
                    ...
                    if (_viewActive) {
                        ...
                        out.write("   <th class='"+CommonServlet.CSS_ADMIN_TABLE_HEADER_COL    +"' nowrap>"+FilterText(i18n.getString("DeviceInfo.lastReceivedEvent","Last Received Event\n{0}",timeZoneS))+"</th>\n");
                    }
                    out.write("  </tr>\n");
                    out.write(" </thead>\n");
                    out.write(" <tbody>\n");
                    ...
                    for (int d = 0; d < _deviceList.size(); d++) {
                        ...
                        try {
                            ...
                            if (dev != null) {
                                ...
                                if (_viewActive) {
                                    boolean readLastEventData = false;
                                    long    lastEvTime  = dev.getLastEventTimestamp(readLastEventData); // may read from EventData
                                    String  lastEvTimeS = FilterText(reqState.formatDateTime(lastEvTime,false/*TimeZone?*/,""));
                                    out.write("   <td class='"+CommonServlet.CSS_ADMIN_TABLE_BODY_COL+"' nowrap>"+active+"</td>\n");
                                    out.write("   <td class='"+CommonServlet.CSS_ADMIN_TABLE_BODY_COL+"' nowrap>"+lastEvTimeS+"</td>\n");
                                }
        ...
        if (_uiEdit || _uiView) {
            ...
            final boolean _editEngDisp = _uiEdit && editEngDisp;
            final boolean _viewEngDisp = _uiView && viewEngDisp;
            HTML_CONTENT = new HTMLOutput(CommonServlet.CSS_CONTENT_FRAME, m) {
                public void write(PrintWriter out) throws IOException {
                    String editURL        = DeviceInfo.this.encodePageURL(reqState);//,RequestProperties.TRACK_BASE_URI());
                    ...
                    boolean showEngDispl  = showFuelCap;
                    ...
                    String lastEventTime   = reqState.formatDateTime(reqState.getLastEventTime()); // _selDev.getLastEventTimestamp()
                    ...
                    if (showAssgnUsr && Device.supportsAssignedUserID()) {
                        ...
                        userMap.put("", "---");
                        try {
                            OrderedSet<User> userList = Device.getAuthorizedUsers(DBReadWriteMode.READ_WRITE,_selDev);
                    ...
                    // -- engine displacement
                    if (showEngDispl && _viewEngDisp) {
                        if (fuelItemDisplayed > 0) { out.println(FormRow_SubSeparator()); }
                        double engDispL = (_selDev!=null)? _selDev.getEngineDisplacement() : 0.0; // Liters
                        String engDispS = StringTools.format(engDispL, "0.0");
                        out.println(FormRow_TextField(PARM_DEV_ENGINE_DISPLACE, _editEngDisp, i18n.getString("DeviceInfo.engineDisplacement","Engine Displacement") +":", engDispS, 10, 10, Account.VolumeUnits.LITERS.toString(locale)));
                        fuelItemDisplayed++;
                    }
                    // ------------------------------------
                    // -- Maintenance Section
                    ...
                    if ((lastOdomOK && Device.supportsLastOdometer())      ||
                        ...
                        Device.supportsMaintenance()                       ||
                        (workHoursOK && Device.supportsHoursOfOperation())   ) {
                        // -- add separator before odometer if maintenance is supported
                        out.println(FormRow_Separator());
                        maintItemDisplayed = 0;
                    }
                    if (lastOdomOK && Device.supportsLastOdometer()) {
                    ...
                    if (lastEngHrsOK && Device.supportsLastEngineHours()) {
                      ...
                      //String lastHrT  = i18n.getString("DeviceInfo.lastEngineHours","Last Engine Hours");
                        ...
                        out.println(FormRow_TextField(PARM_ENGINE_HOURS, _uiEdit, i18n.getString("DeviceInfo.reportEngineHours","Reported Engine Hours") +":" , hourStr, 10, 11, hour_ofs));
                        maintItemDisplayed++;
                    }
                    if (lastPtoHrsOK && Device.supportsLastPtoHours()) {
                        double ptoHR    = (_selDev != null)? _selDev.getLastPtoHours() : 0.0;
                        double offsetHr = (_selDev != null)? _selDev.getPtoHoursOffset() : 0.0;
                        String hourStr  = StringTools.format(ptoHR + offsetHr, "0.00");
                      //String lastHrT  = i18n.getString("DeviceInfo.lastPtoHours","Last PTO Hours");
                        String ofsStr   = StringTools.format(offsetHr,"0.00");
                        String ofsText  = i18n.getString("DeviceInfo.offset","Offset");
                        String hour_ofs = i18n.getString("DeviceInfo.hours","Hours");
                        hour_ofs += " [" + ofsText + " " + ofsStr + "]";
                        out.println(FormRow_TextField(PARM_PTO_HOURS, _uiEdit, i18n.getString("DeviceInfo.reportPtoHours","Reported PTO Hours") +":" , hourStr, 10, 11, hour_ofs));
                        maintItemDisplayed++;
                    }
                    if (maintNotesOK && Device.supportMaintenance_notes()) {
                        // -- Maintenance Notes [Domain.Properties.deviceInfo.showMaintenanceNotes=false]
                    ...
                    if (maintOdomOK && Device.supportsMaintenance_KM()) { // Domain.Properties.deviceInfo.showMaintenanceOdometer=false
                        ...
                        for (int ndx = 0; ndx < cnt; ndx++) {
                            ...
                            out.println(FormRow_TextField(PARM_MAINT_LASTKM_+ndx, maintOdomEdit, i18n.getString("DeviceInfo.mainLast","Last Odometer Maint #{0}",ndxStr) +":" , lastMaintSt, 10, 11, distUnitsStr));
                    ...
                    if (maintHoursOK && Device.supportsMaintenance_HR()) { // Domain.Properties.deviceInfo.showMaintenanceHours=false
                        ...
                        for (int ndx = 0; ndx < cnt; ndx++) {
                            ...
                            out.println(FormRow_TextField(PARM_MAINT_LASTHR_+ndx, false, i18n.getString("DeviceInfo.maintEngHours","Last Eng Hours Maint #{0}",ndxStr) +":" , lastMaintSt, 10, 11));
                    ...
                    // -- FixedTime Maintenance / Interval
                    if (maintFixTimOK && Device.supportsMaintenance_FT()) { // Domain.Properties.deviceInfo.showMaintenanceFixedTime=false
                        out.println(FormRow_Separator());
                        maintItemDisplayed = 0;
                        long offsetFT = 0L; // _selDev.getFixedTimeOffset()
                        int cnt = Device.getMaintCountFT();
                        for (int ndx = 0; ndx < cnt; ndx++) {
                            String ndxStr = String.valueOf(ndx + 1);
                            if (maintItemDisplayed > 0) { out.println(FormRow_SubSeparator()); }
                            Device.MaintUnitsFT units = (_selDev != null)? _selDev.getMaintUnitsFT(ndx) : Device.MaintUnitsFT.SECONDS;
                            long   lastMaintFT = (_selDev != null)? _selDev.getMaintFixedTimeFT(ndx) : 0L; // Seconds/Days
                            long   lastMaint   = lastMaintFT + offsetFT;
                            String lastMaintSt; // TODO: Display as date? (new DateTime(lastMaint)).format("YYYY/MM/DD");
                            DateTime.DateStringFormat dsf = privLabel.getDateStringFormat();
                            switch (units) {
                                default :
                                case SECONDS:   // EpochTime/Seconds
                                    lastMaintSt = (_selDev!=null)?FormatDayNumberTS(dsf,lastMaint,acctTimeZone) : "";
                                    break;
                                case DAYS:   // DayNumber/Days
                                    lastMaintSt = (_selDev!=null)?FormatDayNumber(dsf,lastMaint):"";
                                    break;
                                case MONTHS:   // DayNumber/Months
                                    lastMaintSt = (_selDev!=null)?FormatDayNumber(dsf,lastMaint):"";
                                    break;
                            }
                            out.println(FormRow_TextField(PARM_MAINT_LASTFT_+ndx, false, i18n.getString("DeviceInfo.maintFixedTime","Last Fixed Time Maint #{0}",ndxStr) +":" , lastMaintSt, 10, 11));
                            long   intrvFT = (_selDev != null)? _selDev.getMaintIntervalFT(ndx) : 0L;
                            long   intrv   = intrvFT; // Seconds/Days/Months
                            out.print("<tr>");
                            out.print("<td class='"+CommonServlet.CSS_ADMIN_VIEW_TABLE_HEADER+"' nowrap>"+i18n.getString("DeviceInfo.maintIntervalFT","Fixed Time Maint Interval",ndxStr)+":</td>");
                            out.print("<td class='"+CommonServlet.CSS_ADMIN_VIEW_TABLE_DATA+"'>");
                            out.print(Form_TextField(PARM_MAINT_INTERVFT_+ndx, _uiEdit, String.valueOf(intrv), 10, 11)); // Seconds/Days/Months
                            out.print("&nbsp;");
                            out.print("(" + units.toString(locale) + ")&nbsp;");
                            if (_uiEdit) {
                                out.print(" &nbsp;&nbsp;(" + i18n.getString("DeviceInfo.maintResetFT","Check to Reset Service") + " ");
                                out.print(Form_CheckBox(PARM_MAINT_RESETFT_+ndx, PARM_MAINT_RESETFT_+ndx, _uiEdit, false, null, null));
                                out.print(")");
                            }
                            out.print("</td>");
                            out.print("</tr>\n");
                    ...
                    if (Device.supportsMaintenance() && (reminderOK || servTimeOK)) {
                        out.println(FormRow_Separator());
                        // -- Reminder
                        if (reminderOK) { // Domain.Properties.deviceInfo.showReminderMessage=false
                    ...
                    if (faultCodesOK && Device.supportsFaultCodes()) {
                        ...
                        String fc = !StringTools.isBlank(fs)? DTOBDFault.GetFaultString(new RTProperties(fs),false/*firstOnly?*/) : "";
                        out.println(FormRow_Separator());
                        out.print("<tr>");
                    ...
                    if (ntfyOK) {
                        ...
                        if (SHOW_NOTIFY_SELECTOR           || // always show?
                          ...
                          //out.println(FormRow_TextField(PARM_DEV_RULE_DESC , ntfyEdit  , i18n.getString("DeviceInfo.notifyDesc"  ,"Notify Description")+":", ntfyDesc , 75,  90));
                            ...
                            // -- [OBSOLETE - do not use] EventNotificationEMail
                    ...
                    */
                    ...
                    final java.util.List<String> grpList = reqState.getDeviceGroupIDList(DBReadWriteMode.READ_WRITE,true/*includeAll*/);
