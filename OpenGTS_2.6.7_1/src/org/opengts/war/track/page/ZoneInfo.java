...
//     -Changes no assigned groups to mean "all" geozones.  [2.6.6-B38]
// ----------------------------------------------------------------------------
    ...
    private static String[] getGeozoneIDs(PrivateLabel privLabel, Account currAcct, User currUser)
    {
        ...
        try {
            ...
            if (limitToGroups && !User.isAdminUser(userID)) {
                // -- return only geozones that match the current users authorized groups
                java.util.List<String> dgList = User.getExplicitlyAuthorizedDeviceGroupIDs(rwMode, acctID, userID, -1L);
                if (ListTools.isEmpty(dgList)) {
                    // -- no DeviceGroups assigned to user (none is all? [2.6.6-B38])
                    //return new String[0]; <== removed [2.6.6-B38]
                    return Geozone.getGeozoneIDsForAccount(rwMode, currAcct, currUser, (String[])null); // has Account/User
                } else {
                    // -- get Geozones assigned to a the users DeviceGroup
                    String dgIDs[] = dgList.toArray(new String[dgList.size()]);
                    return Geozone.getGeozoneIDsForAccount(rwMode, currAcct, currUser, dgIDs); // has Account/User
                }
            }
            ...
            public void write(PrintWriter out) throws IOException {
              ...
              //String menuURL    = EncodeMakeURL(reqState,RequestProperties.TRACK_BASE_URI(),PAGE_MENU_TOP);
                ...
                if (_listZones) {
                    ...
                    out.write("<span class='"+CommonServlet.CSS_ADMIN_SELECT_TITLE+"'>"+FilterText(i18n.getString("ZoneInfo.list.selectZone","Select a Geozone"))+":</span>\n");
                    ...
                    if (showAssignedDevGroup && !selZoneIsPOI) {
                        ...
                        if (limitToGroups && !User.isAdminUser(userID)) {
                            ...
                            try {
                                java.util.List<String> grps = User.getExplicitlyAuthorizedDeviceGroupIDs(DBReadWriteMode.READ_WRITE,acctID,userID,-1L);
                                if (!ListTools.isEmpty(grps)) {
                                    devGrps = new Vector<String>(grps);
                                }
                            } catch (DBException dbe) {
                                devGrps = null;
                            }
                            ...
                            try {
                                ...
                                java.util.List<String> grps = DeviceGroup.getDeviceGroupsForAccount(DBReadWriteMode.READ_WRITE,acctID,includeAll);
                                if (!ListTools.isEmpty(grps)) {
                                    devGrps = grps;
                                }
                            } catch (DBException dbe) {
