    ...
    implements Constants
{
    ...
    private boolean         statusCodeTable         = false;
    ...
    protected void postInit()
    {
        ...
        this.statusCodeTable  = false;
        String statusCodesCSV = this.getStringProperty(privLabel,PROP_statusCodes,null);
        if (StringTools.isBlank(statusCodesCSV)) {
            // -- disregard status codes
        } else
        if (statusCodesCSV.equalsIgnoreCase("table")) {
            // -- status codes are to be read from StatusCode table
            this.statusCodeTable = true;
        } else {
            // -- parse explicit list of status codes
    ...
    **/
    protected int[] getStatusCodes(String accountID, String deviceID)
    {
        if (!ListTools.isEmpty(this.statusCodes)) {
            // -- explicit status codes specified
           return this.statusCodes;
        } else
        if (this.statusCodeTable) {
            // -- read status codes from StatusCode table
            try {
                int sc[] = StatusCode.getStatusCodes(accountID, null/*ALL_DEVICES*/);
                return sc; // NOTE: null/empty means that status codes are to be disregarded
            } catch (DBException dbe) {
                Print.logException("Unable to read StatusCodes", dbe);
                return null;
            }
        } else {
            // -- disregard status codes
    ...
    **/
    protected int[] getStatusMarkers(String accountID, String deviceID)
    {
        if (!ListTools.isEmpty(this.statusMarkers)) {
        ...
        throws IOException
    {
        ...
        int autoUpdateRecenterMode = 0; // none
        if (autoUpdateEnabled) {
            String mode;
            String lafV = RTConfig.getString(BasicPrivateLabel.LAF_Map_Auto_RecenterMode, null);
            if (!StringTools.isBlank(lafV)) {
                mode = StringTools.trim(lafV);
            } else {
                mode = this.getStringProperty(privLabel,PrivateLabel.PROP_TrackMap_autoUpdateRecenter,AUTO_RECENTER_ZOOM[0]);
            }
            // --
        ...
        JavaScriptTools.writeJSVar(out, "AutoSkipRadius"            , autoSkipRadius); // used only for replay (not auto-update)
        ...
        throws IOException
    {
        ...
        /* status code/markers */
        reqState.setStatusCodes(this.getStatusCodes(accountID,deviceID)); // may be null
        reqState.setStatusMarkers(this.getStatusMarkers(accountID,deviceID)); // may be null
        ...
        final double  autoSkipRadius; // used for replay, not auto-update
        long _autoIntrv = 0L; // (long)currAcct.getMapAutoUpdInterval();
        long _autoMaxCt = 0L; // (long)currAcct.getMapAutoUpdCount();
        if (cmdName.equals(COMMAND_AUTO_UPDATE)) {
            ...
            if ((v != null) && (v.length == 2)) {
                _autoIntrv = Math.max(v[0], 30L);
                _autoMaxCt = (v[1] > 0L)? Math.min(v[1],999L) : 2L;
            }
        }
        // --
        boolean dftAutoUpdEnabled = this.getBooleanProperty(privLabel, PROP_autoUpdate_enable    , mapProvider.getAutoUpdateEnabled(isFleet));
        boolean dftAutoUpdOnLoad  = this.getBooleanProperty(privLabel, PROP_autoUpdate_onload    , mapProvider.getAutoUpdateOnLoad(isFleet));
        long    dftAutoInterval   = this.getLongProperty(   privLabel, PROP_autoUpdate_interval  , mapProvider.getAutoUpdateInterval(isFleet));
        long    dftAutoMaxCount   = this.getLongProperty(   privLabel, PROP_autoUpdate_count     , mapProvider.getAutoUpdateCount(isFleet));
        double  dftAutoSkipRadius = this.getDoubleProperty( privLabel, PROP_autoUpdate_skipRadius, mapProvider.getAutoUpdateSkipRadius(isFleet));
        //if (!dftAutoUpdEnabled) {
        //    // -- not allowed by map
        ...
        //    autoMaxCount        = 0L;
        //    autoSkipRadius      = 0.0;
        //    //Print.logInfo("AutoUpdate not allowed by map, disabled ...");
        //} else
        if (!autoUpdateOk) {
            // -- user not authorized
            autoUpdateEnabled   = false;
            ...
            autoMaxCount        = 0L;
            autoSkipRadius      = 0.0;
            //Print.logInfo("AutoUpdate not authorized, disabled ...");
        } else
        if (_autoIntrv > 0L) {
            // -- overridden 
            ...
            autoSkipRadius      = 0.0;
            //Print.logInfo("AutoUpdate enabled/overridden by command ...");
        } else
        if (dftAutoUpdEnabled && currAcct.hasMapAutoUpdate()) {
            // -- set to Account values (limited by map properties)
            autoUpdateEnabled   = currAcct.getMapAutoUpdateEnabled(dftAutoUpdEnabled);
            autoUpdateOnLoad    = currAcct.getMapAutoUpdateOnload(dftAutoUpdOnLoad);
            autoInterval        = currAcct.getMapAutoUpdateInterval(dftAutoInterval); // should be >= default?
            autoMaxCount        = currAcct.getMapAutoUpdateCount(dftAutoMaxCount);    // should be <= default?
            autoSkipRadius      = currAcct.getMapAutoUpdateSkipRadius(dftAutoSkipRadius);
            //Print.logInfo("AutoUpdate set by property: page="+autoUpdateEnabled+", map="+dftAutoUpdEnabled);
        } else {
            // -- default to map properties
            autoUpdateEnabled   = dftAutoUpdEnabled;
            autoUpdateOnLoad    = dftAutoUpdOnLoad;
            autoInterval        = dftAutoInterval;
            autoMaxCount        = dftAutoMaxCount;
            autoSkipRadius      = dftAutoSkipRadius;
            //Print.logInfo("AutoUpdate set by property: page="+autoUpdateEnabled+", map="+dftAutoUpdEnabled);
        }
        ...
        HTMLOutput HTML_CONTENT = new HTMLOutput((mapAutoSize? CommonServlet.CSS_CONTENT_MAP_FULL : CommonServlet.CSS_CONTENT_MAP), m) {
            public void write(PrintWriter out) throws IOException {
                String pageName = TrackMap.this.getPageName();
                //Print.logStackTrace("Writing page: "+pageName);
                ...
                if (DeviceChooser.isDeviceChooserUseTable(privLabel)) {
                    ...
                    java.util.List<String> dgList = isFleet? 
                        reqState.getDeviceGroupIDList(DBReadWriteMode.READ_WRITE,true) : 
                        reqState.getDeviceIDList(DBReadWriteMode.READ_WRITE,false);
