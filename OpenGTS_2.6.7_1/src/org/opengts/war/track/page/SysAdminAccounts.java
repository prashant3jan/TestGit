...
//  2018/05/24  Martin D. Flynn
//     -Changed "Active" (Yes/No) list column to "Account Status" (Deleted/Expired/Suspended/Inactive/Active) [2.6.6-B60]
//  2018/09/10  GTS Development Team
//     -Added last Device connected time to device list
//     -Initial support for billing report
//  2020/02/19  GTS Development Team
//     -Added property to allow editing Border-Crossing enable field [PROP_SysAdminAccounts_allowEditBCross]
// ----------------------------------------------------------------------------
    ...
    implements Constants
{
    ...
    private static final boolean SHOW_LOGIN_MSG                 = false;
    private static final boolean SHOW_INACTIVE_MSG              = false;
    ...
    public  static final String PARM_SUBMIT_LOGOUT              = "a_sublogout";
    ...
    public  static final String PARM_ACCT_LOGIN_MSG             = "a_loginmsg";
    public  static final String PARM_ACCT_INACTIVE_MSG          = "a_inactvmsg";
    ...
    public  static final String PARM_COST_ACTIVE_DEV            = "a_costActv";
    public  static final String PARM_COST_INACTIVE_DEV          = "a_costInactv";
        ...
        throws IOException
    {
        ...
        final boolean  showBilling = isSysAdmin && Account.SupportsBilling();
        final boolean  showBillingReportLink = showBilling &&
            privLabel.getBooleanProperty(PrivateLabel.PROP_SysAdminAccounts_showBillingReportLink,false);
        final boolean  showDeviceCounts = // "Device\nCount", getDeviceCount, getLastConnectedDeviceForAccount
            privLabel.getBooleanProperty(PrivateLabel.PROP_SysAdminAccounts_showDeviceCounts,true);
        ...
        String  submitLogout  = AttributeTools.getRequestString(request, PARM_SUBMIT_LOGOUT, "");
        ...
        boolean allowLogout  = allowEdit && privLabel.isSystemAccountsForcedLogoutEnabled(currAcctID);
        ...
        if (selectAccount) {
            ...
            if (SubmitMatch(submitLogout,i18n.getString("SysAdminAccounts.forceLogout","Force Logout"))) {
                if (allowLogout) {
                    if (selAccount == null) {
                        m = i18n.getString("SysAdminAccounts.pleaseSelectAccount","Please select an Account"); // UserErrMsg
                        error = true;
                    ...
                    if (isCurrentAccountSelected) {
                        m = i18n.getString("SysAdminAccounts.cannotForceLogoutAccount","Cannot force logout this Account"); // UserErrMsg
                        error = true;
                        listAccounts = true;
                    } else {
                        logoutAccount = true;
        ...
        if (loginAccount) {
            if (selAccount == null) {
                ...
                try {
                    ...
                    url.addArg(Constants.PARM_SA_RELOGIN_PASS, passcode); // URL only
                    // "./Track?account=ACCOUNT&user=admin&password=PASSWORD&page=menu.top&sar_p=relogin"
                    Print.logInfo("ReLogin URL: " + url);
                    AttributeTools.clearSessionAttributes(request); // invalidate/logout
                    AttributeTools.setSessionLong(     request, Constants.PARM_SA_RELOGIN_TIME, DateTime.getCurrentTimeSec());
        ...
        if (logoutAccount) {
            if (selAccount == null) {
                ...
                HttpSession hs = AttributeTools.getSession(reqState.getHttpServletRequest());
                if (hs != null) {
                    RTConfigContextListener.ForceAccountLogout(hs.getServletContext(), selAccount.getAccountID());
                } else {
                    Print.logError("Unable to determine local HttpSession");
                }
            }
            listAccounts = true;
        }
        ...
        if (updateAccount) {
            if (selAccount == null) {
                ...
                String  loginMsg      = AttributeTools.getRequestString(request, PARM_ACCT_LOGIN_MSG     , "");
                String  inactvMsg     = AttributeTools.getRequestString(request, PARM_ACCT_INACTIVE_MSG  , "");
                ...
                double  costActiveD   = AttributeTools.getRequestDouble(request, PARM_COST_ACTIVE_DEV    , 0.0);
                double  costInactiveD = AttributeTools.getRequestDouble(request, PARM_COST_INACTIVE_DEV  , 0.0);
                ...
                try {
                    ...
                    if (showRetainEvents && isSysAdmin) {
                        // -- TODO: allow selection of months - M1,M3,M6,Y1,Y2,Y3,Y4,Y5,Never
                        long retainSec = 0L;
                        if (retainEvStr.startsWith("-M")) {
                            retainSec = Math.round(StringTools.parseDouble(retainEvStr.substring(2),0.0) * 365.2425 / 12.0);
                        } else
                        if (retainEvStr.startsWith("-Y")) {
                            retainSec = Math.round(StringTools.parseDouble(retainEvStr.substring(2),0.0) * 365.2425);
                        } else 
                        if (ComboOption.isText_YES(locale,retainEvStr)) {
                            retainSec = DateTime.DaySeconds(20.0 * 365.2425); // [2.6.6-B52d] (was "99999999L")
                        } else {
                            retainSec = 0L;
                        }
                        ...
                        if (retainSec > 0L) {
                            if (!selAccount.hasRetainedEventAge()) {
                                selAccount.setRetainedEventAge(retainSec); 
                            } else {
        ...
        final boolean _allowLogout   = allowLogout;
        ...
        final boolean _editBCross    = _editAccount && (isSysAdmin || privLabel.getBooleanProperty(PrivateLabel.PROP_SysAdminAccounts_allowEditBCross,false));
        ...
        HTMLOutput HTML_CONTENT = new HTMLOutput(CommonServlet.CSS_CONTENT_FRAME, m) {
            public void write(PrintWriter out) throws IOException {
              ...
              //String menuURL    = EncodeMakeURL(reqState,RequestProperties.TRACK_BASE_URI(),PAGE_MENU_TOP);
                ...
                if (_listAccounts) {
                  //String devTitles[] = acct.getDeviceTitles(locale, new String[]{"",""});

                    // -- account selection table (Select, Account ID, Account Description)
                    out.write("<span class='"+CommonServlet.CSS_ADMIN_SELECT_TITLE+"'>"+i18n.getString("SysAdminAccounts.selectAccount","Select an Account")+":</span>\n");
                  ...
                  //out.write("   <th class='"+CommonServlet.CSS_ADMIN_TABLE_HEADER_COL    +"' nowrap>"+filter(i18n.getString("SysAdminAccounts.active","Active"))+"</th>\n");
                    out.write("   <th class='"+CommonServlet.CSS_ADMIN_TABLE_HEADER_COL    +"' nowrap>"+filter(i18n.getString("SysAdminAccounts.accountStatus","Account\nStatus"))+"</th>\n");
                    ...
                    out.write("   <th class='"+CommonServlet.CSS_ADMIN_TABLE_HEADER_COL    +"' nowrap>"+filter(i18n.getString("SysAdminAccounts.deviceLastConnected","Device Connected\n{0}",currAcctTZID))+"</th>\n");
                    ...
                    for (int u = 0; u < ListTools.size(_accountList); u++) {
                        ...
                        String acctStatus   = Account.GetActiveStatusDescription(locale, acct);
                        ...
                        // -- most recent connected Device
                        long  lastConnTime = 0L;
                        String connTimeStr = "n/a";
                        if (showDeviceCounts) {
                            try {
                                Device lastConnDev = Device.getLastConnectedDeviceForAccount(acct.getAccountID());
                                lastConnTime = (lastConnDev != null)? lastConnDev.getLastConnectTime() : 0L;
                                connTimeStr  = (lastConnTime > 0L)? new DateTime(lastConnTime,currAcctTZ).format(dtFormat) : i18n.getString("SysAdminAccounts.never","never");
                            } catch (DBException dbe) {
                                lastConnTime = 0L;
                                connTimeStr  = i18n.getString("SysAdminAccounts.unknown","unknown");
                            }
                        }
                        ...
                        String deviceCountS = "--";
                        if (showDeviceCounts) {
                            deviceCountS = String.valueOf(acct.getDeviceCount());
                        }
                        int    loginCount   = 0;
                        ...
                        out.write("   <td class='"+CommonServlet.CSS_ADMIN_TABLE_BODY_COL    +"' nowrap>"+filter(acctStatus)+"</td>\n");
                        if (hasAccountMgr && (isSysAdmin || isAccountMgr)) {
                            String sortID = "";
                            String mgrID;
                            if (isSysAdmin) {
                        ...
                        if (showDeviceCounts) { // device-count
                        ...
                        if (showDeviceCounts) { // last device connect
                        out.write("   <td class='"+CommonServlet.CSS_ADMIN_TABLE_BODY_COL    +"' "+SORTTABLE_SORTKEY+"='"+lastConnTime +"' nowrap>"+filter(connTimeStr)+"</td>\n");
                        }
                    ...
                    if (_allowLogout) {
                        ...
                        out.write("<input type='submit' name='"+PARM_SUBMIT_LOGOUT+"' value='"+i18n.getString("SysAdminAccounts.forceLogout","Force Logout")+"' "+Onclick_ConfirmForceLogout(locale)+">");
                        out.write("</td>\n"); 
                    }
                    if (_allowLogin) {
                        out.write("<td style='padding-left:30px;'>");
                    ...
                    if (isSysAdmin && (showExpiration || showSuspension)) {
                        ...
                        if (showExpiration) {
                            ...
                            out.println(FormRow_TextField(PARM_ACCT_EXPIRE , _editAccount, i18n.getString("SysAdminAccounts.expiration","Expire at Date/Time")+":", expireFmt, 30, 30));
                        }
                        // -- Suspension
                        if (showSuspension) {
                            ...
                            out.println(FormRow_TextField(PARM_ACCT_SUSPEND, _editAccount, i18n.getString("SysAdminAccounts.suspension","Suspend until Date/Time")+":", suspendFmt, 32, 32));
                        }
                    }
    ...
    protected String Onclick_ConfirmForceLogout(Locale locale)
    {
        I18N i18n = I18N.getI18N(SysAdminAccounts.class, locale);
        String confirmLogout = i18n.getString("SysAdminAccounts.confirmForceLogout",
            "Are you sure you want to force the selected Account to log-out?");
        return "onclick=\"return confirm('"+confirmLogout+"');\"";
    }
