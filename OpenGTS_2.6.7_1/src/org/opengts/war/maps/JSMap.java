    ...
    extends MapProviderAdapter
{
    ...
    **/
    private static final Object SKIP_PROPS[] = new Object[] {
        ...
        PROP_map_height,
        PROP_map_fillFrame,
        PROP_map_pushpins,              // "map.showPushpins", "map.pushpins" (if false, only show last pushpin)
        PROP_map_balloons,              // "map.showBalloons", "map.balloons" (if false, only show last info-balloon)
    ...
    **/
        ...
        throws IOException 
    {
        ...
        /* properties */
        RTProperties rtp          = this.getProperties();
        // -- rtp.getBoolean, rtp.getProperty, rtp.getString, rtp.getInt
        ...
        boolean      scrollZoom   = rtp.getBoolean(PROP_scrollWheelZoom,false);
        String       defaultView  = rtp.getString(PROP_map_view,"").toLowerCase();
        boolean      mapLoadText  = rtp.getBoolean(PROP_MAP_LOADING,false);
        String       mapLoadImage = rtp.getString(PROP_MAP_LOADING_IMAGE,null);

        /* pushpins/balloons */
        long         maxPushpins  = this.getMaxPushpins(reqState);
        boolean      showPushpins = rtp.getBoolean(PROP_map_pushpins,true); // false == only last pushpin
        boolean      showBalloons = rtp.getBoolean(PROP_map_balloons,true); // false == only last info-balloon

        /* map callbacks */
        String       createMapCB  = rtp.getString(PROP_createMapCallback,"null");
        String       postMapInzCB = rtp.getString(PROP_postMapInitCallback,"null");

        /* max creation age */
        int          maxCreateAge = rtp.getInt(PROP_map_maxCreationAge,0); // seconds

        /* info-balloon items */
        boolean      showSatCount = rtp.getBoolean(PROP_detail_showSatCount,false);
        boolean      showSpeed    = rtp.getBoolean(PROP_info_showSpeed,true);
        boolean      combSpdHead  = rtp.getBoolean(PROP_combineSpeedHeading,true);
        boolean      showAltitude = rtp.getBoolean(PROP_info_showAltitude,false);
        boolean      showAddress  = reqState.getShowAddress();
        boolean      incBlankAddr = rtp.getBoolean(PROP_info_inclBlankAddress,true);
        boolean      showOptField = rtp.getBoolean(PROP_info_showOptionalFields,true);
        boolean      incBlankOptF = rtp.getBoolean(PROP_info_inclBlankOptFields,true);

        /* route-line */
        boolean      showRoute    = rtp.getBoolean(PROP_map_routeLine,true);
        String       routeColor   = rtp.getString( PROP_map_routeLine_color,"#FF2222");
        boolean      routeArrows  = rtp.getBoolean(PROP_map_routeLine_arrows,false);
        boolean      snapToRoad   = rtp.getBoolean(PROP_map_routeLine_snapToRoad,false); // Google V2 only
        ...
        JavaScriptTools.writeJSVar(out, "CREATE_MAP_CALLBACK"   , createMapCB,false);
        JavaScriptTools.writeJSVar(out, "POST_MAP_INIT_CALLBACK", postMapInzCB,false);
        ...
        JavaScriptTools.writeJSVar(out, "PUSHPINS_SHOW"         , showPushpins); // PROP_map_pushpins
        JavaScriptTools.writeJSVar(out, "BALLOONS_SHOW"         , showBalloons); // PROP_map_balloons
        JavaScriptTools.writeJSVar(out, "MAX_PUSH_PINS"         , maxPushpins);
        JavaScriptTools.writeJSVar(out, "MAX_CREATION_AGE_SEC"  , maxCreateAge);
        ...
        JavaScriptTools.writeJSVar(out, "SHOW_SAT_COUNT"        , showSatCount);
        JavaScriptTools.writeJSVar(out, "SHOW_SPEED"            , showSpeed);
        JavaScriptTools.writeJSVar(out, "COMBINE_SPEED_HEAD"    , combSpdHead);
        JavaScriptTools.writeJSVar(out, "SHOW_ALTITUDE"         , showAltitude);
        JavaScriptTools.writeJSVar(out, "SHOW_ADDR"             , showAddress);
        JavaScriptTools.writeJSVar(out, "INCL_BLANK_ADDR"       , incBlankAddr);
        JavaScriptTools.writeJSVar(out, "SHOW_OPT_FIELDS"       , showOptField);
        JavaScriptTools.writeJSVar(out, "INCL_BLANK_OPT_FIELDS" , incBlankOptF);
        ...
        JavaScriptTools.writeJSVar(out, "SCROLL_WHEEL_ZOOM"     , scrollZoom);
        JavaScriptTools.writeJSVar(out, "DEFAULT_VIEW"          , defaultView);
        JavaScriptTools.writeJSVar(out, "ROUTE_LINE_SHOW"       , showRoute);
        JavaScriptTools.writeJSVar(out, "ROUTE_LINE_COLOR"      , routeColor);
        JavaScriptTools.writeJSVar(out, "ROUTE_LINE_ARROWS"     , routeArrows);
        JavaScriptTools.writeJSVar(out, "ROUTE_SNAP_TO_ROAD"    , snapToRoad); // Google V2 only
        ...
        /* map "Loading ..." */
        JavaScriptTools.writeJSVar(out, "TEXT_LOADING_MAP_POINTS"   , (mapLoadText? i18n.getString("JSMap.loadingMapPoints","Loading Map Points ...") : null));
        JavaScriptTools.writeJSVar(out, "MAP_LOADING_IMAGE_URI"     , mapLoadImage);
        ...
        throws IOException 
    {
        ...
        for (Iterator<String> k = iconMap.keyIterator(); k.hasNext();) {
            ...
            if (!StringTools.isBlank(RC)) {
                // -- route color
                out.write(",");
                out.write(" routeColor:\""+RC+"\"");
            }
            out.write(" }");
