    ...
    extends JSMap
{
  ...
  //private static final String PROP_addOneMapSLAMap[]      = { "leaflet.addOneMapSLAMap"      , "addOneMapSLAMap"       };
    ...
    /* Traffic Overlay Provider */
    private static final String PROP_traffic_Name[]         = { "leaflet.traffic.name"         , "traffic.name"           };
    private static final String PROP_traffic_URL[]          = { "leaflet.traffic.URL"          , "traffic.URL"            };
    private static final String PROP_traffic_Subdomains[]   = { "leaflet.traffic.subdomains"   , "traffic.subdomains"     };
    private static final String PROP_traffic_AuthID[]       = { "leaflet.traffic.authID"       , "traffic.authID"         };
    private static final String PROP_traffic_AuthCode[]     = { "leaflet.traffic.authCode"     , "traffic.authCode"       };
    private static final String PROP_traffic_MaxZoom[]      = { "leaflet.traffic.maxZoom"      , "traffic.maxZoom"        };
    ...
    **/
        ...
        throws IOException
    {
      ...
      //JavaScriptTools.writeJSVar(out, "LEAFLET_ADD_ONEMAP_SLA_MAP" , rtp.getBoolean(PROP_addOneMapSLAMap ,false));
        ...
        // -- Traffic
        {
            String trafficName = rtp.getString(PROP_traffic_Name,"");
            String trafficURL  = rtp.getString(PROP_traffic_URL,"");
            Object subdomains  = this._parseSubdomains(rtp.getString(PROP_traffic_Subdomains,""));
            String authID      = rtp.getString(PROP_traffic_AuthID,"");
            String authCode    = rtp.getString(PROP_traffic_AuthCode,"");
            trafficURL = StringTools.replace(trafficURL, "{authID}"  , authID);
            trafficURL = StringTools.replace(trafficURL, "{authCode}", authCode);
            JavaScriptTools.writeJSVar(out, "LEAFLET_TRAFFIC_NAME"       , trafficName);
            JavaScriptTools.writeJSVar(out, "LEAFLET_TRAFFIC_URL"        , trafficURL);
            JavaScriptTools.writeJSVar(out, "LEAFLET_TRAFFIC_SUBDOMAINS" , subdomains);
            JavaScriptTools.writeJSVar(out, "LEAFLET_TRAFFIC_MAX_ZOOM"   , rtp.getInt(PROP_traffic_MaxZoom,19));
        }
