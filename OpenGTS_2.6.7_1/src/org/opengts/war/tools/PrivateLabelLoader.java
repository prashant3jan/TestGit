    ...
    extends BasicPrivateLabelLoader
{
    ...
    **/
                ...
                if (!StringTools.isBlank(key)) {
                    ...
                    if (!alias.equals("")) {
                        ...
                        if (!iconURL.equals("")) {
                            ...
                            // -- RouteSegmentColor
                            String routeColor = XMLTools.getAttribute(iconElem,ATTR_routeColor,"",false).trim();
                            // -- Pushpin
            ...
            if (jspElemName.equalsIgnoreCase(TAG_JSP)) {
                ...
                if (StringTools.isBlank(name)) {
                    ...
                    if (StringTools.endsWithIgnoreCase(file,".ftl") && !DBConfig.hasFreemarkerPackage()) {
                        ...
                        Print.logInfo("["+xmlFile+"] Ignoring FTL Entry file: " + name);
                    } else {
    ...
    protected void parseTag_WebPages(File xmlFile, String i18nPkgName, BasicPrivateLabel bpl, Element webPages)
    {
        ...
        PrivateLabel pl       = (PrivateLabel)bpl; // non-null
        String   keyPrefix    = XMLTools.getAttribute(webPages,ATTR_rtPropPrefix,null,true);
        String   dftFTL       = XMLTools.getAttribute(webPages,ATTR_ftl         ,null,true); // alternative for "jsp"
        String   dftJSP       = XMLTools.getAttribute(webPages,ATTR_jsp         ,null,true);
        String   dftCssDir    = XMLTools.getAttribute(webPages,ATTR_cssDir      ,null,true);
        String   dftIconDir   = XMLTools.getAttribute(webPages,ATTR_iconDir     ,null,true); // eg. "http://HOST/DIR/"
        String   dftIconDir2  = XMLTools.getAttribute(webPages,ATTR_iconDir2    ,null,true); // eg. "extra/images/mw/"
        String   dftButtonDir = XMLTools.getAttribute(webPages,ATTR_buttonDir   ,null,true);
        //Print.logInfo("ATTR_jsp     : " + dftJSP);
        //Print.logInfo("ATTR_iconDir : " + dftIconDir);
        //Print.logInfo("ATTR_iconDir2: " + dftIconDir2);

        /* default overrides */
        if (!StringTools.isBlank(keyPrefix)) {
            dftFTL       = RTConfig.getString(keyPrefix + ATTR_ftl      , dftFTL);      // Domain.WebPages.ftl=
            dftJSP       = RTConfig.getString(keyPrefix + ATTR_jsp      , dftJSP);      // Domain.WebPages.jsp=default
            dftCssDir    = RTConfig.getString(keyPrefix + ATTR_cssDir   , dftCssDir);   // Domain.WebPages.cssDir=extra/css
            dftIconDir   = RTConfig.getString(keyPrefix + ATTR_iconDir  , dftIconDir);  // Domain.WebPages.iconDir=extra/images
            dftIconDir2  = RTConfig.getString(keyPrefix + ATTR_iconDir2 , dftIconDir2); // Domain.WebPages.iconDir2=
            dftButtonDir = RTConfig.getString(keyPrefix + ATTR_buttonDir, dftButtonDir);// Domain.WebPages.buttonDir=
        }

        /* make sure default directories end with "/" */
        if (!StringTools.isBlank(dftIconDir  ) && !dftIconDir  .endsWith("/")) { dftIconDir   += "/"; }
        if (!StringTools.isBlank(dftIconDir2 ) && !dftIconDir2 .endsWith("/")) { dftIconDir2  += "/"; }
        if (!StringTools.isBlank(dftButtonDir) && !dftButtonDir.endsWith("/")) { dftButtonDir += "/"; }

        /* default to "dftFTL" if "dftJSP" is blank */
        if (StringTools.isBlank(dftJSP) && !StringTools.isBlank(dftFTL)) {
            dftJSP = dftFTL;
        }
    ...
    private void _parseTag_WebPages(File xmlFile, String i18nPkgName, PrivateLabel pl, 
        String dftJSP, String dftCssDir, 
        String dftIconDir, String dftIconDir2,
        String dftButtonDir, 
        String keyPrefix,
        ...
        Map<String,WebPage> pageMap, MenuGroup menuGroup, Map<String,MenuGroup> menuGroupMap)
    {
        ...
        for (int p = 0; p < webPageList.getLength(); p++) {
            ...
            if (wpElemName.equalsIgnoreCase(TAG_Page)) {
                ...
                if (ExtendedDebugLogging()) {
                    Print.logDebug("Parsing Page '"+pageName+"' ...");
                }
                ...
                String  ftlURI     = XMLTools.getAttribute(page,ATTR_ftl        ,null  ,false);
                String  jspURI     = XMLTools.getAttribute(page,ATTR_jsp        ,ftlURI,false);
                String  cssDir     = XMLTools.getAttribute(page,ATTR_cssDir     ,null  ,false);
                String  _iconURI   = XMLTools.getAttribute(page,ATTR_icon       ,null  ,false);
                String  _iconURI2  = XMLTools.getAttribute(page,ATTR_icon2      ,null  ,false);
                String  _buttonURI = XMLTools.getAttribute(page,ATTR_button     ,null  ,false);
                String  _buttonALT = XMLTools.getAttribute(page,ATTR_altButton  ,null  ,false);
                String  _matIcon   = XMLTools.getAttribute(page,ATTR_matIcon    ,null  ,false);
                // -- I18N Strings
                ...
                for (int c = 0; c < chList.getLength(); c++) {
                    ...
                    if (childName.equalsIgnoreCase(TAG_IconImage)) {
                        _iconURI = StringTools.trim(XMLTools.getNodeText(childElem,"",false));
                    } else
                    if (childName.equalsIgnoreCase(TAG_IconImage2)) {
                        _iconURI2 = StringTools.trim(XMLTools.getNodeText(childElem,"",false));
                    } else
                    if (childName.equalsIgnoreCase(TAG_ButtonImage)) {
                        _buttonURI = StringTools.trim(XMLTools.getNodeText(childElem,"",false));
                    } else
                    if (childName.equalsIgnoreCase(TAG_ButtonImageAlt)) {
                        _buttonALT = StringTools.trim(XMLTools.getNodeText(childElem,"",false));
                    } else
                    if (childName.equalsIgnoreCase(TAG_MatIcon)) {
                        _matIcon = StringTools.trim(XMLTools.getNodeText(childElem, "", false));
                    } else
                ...
                String matIcon = _matIcon;
                // -- adjust icon image URL
                String iconURI;
                if (!StringTools.isBlank(_iconURI)) {
                    if (_iconURI.startsWith("/")) {
                        iconURI = _iconURI.substring(1);  // relative to the servlet base
                    } else
                    if (!StringTools.isBlank(dftIconDir)) {
                        iconURI = dftIconDir + _iconURI; 
                    } else {
                        iconURI = _iconURI;
                    }
                } else {
                ...
                // --
                String iconURI2;
                if (!StringTools.isBlank(_iconURI2)) {
                    if (_iconURI2.startsWith("/")) {
                        iconURI2 = _iconURI2.substring(1);  // relative to the servlet base
                    } else
                    if (!StringTools.isBlank(dftIconDir2)) {
                        iconURI2 = dftIconDir2 + _iconURI2; 
                    } else {
                        iconURI2 = _iconURI2;
                    }
                } else
                if (!StringTools.isBlank(dftIconDir2) && !StringTools.isBlank(_iconURI) && !_iconURI.startsWith("/")) {
                    iconURI2 = dftIconDir2 + _iconURI; 
                } else {
                    iconURI2 = null;
                }
                // -- adjust button image URL
                String buttonURI;
                if (!StringTools.isBlank(_buttonURI)) {
                    if (_buttonURI.startsWith("/")) {
                        // -- relative to the servlet base
                        buttonURI = _buttonURI.substring(1);
                    } else
                    if (!StringTools.isBlank(dftButtonDir)) {
                        buttonURI = dftButtonDir + _buttonURI; 
                    } else {
                        buttonURI = _buttonURI;
                    }
                } else {
                    buttonURI = null;
                }
                // --
                String buttonALT;
                if (!StringTools.isBlank(_buttonALT)) {
                    if (_buttonALT.startsWith("/")) {
                        buttonALT = _buttonALT.substring(1);  // relative to the servlet base
                    } else
                    if (!StringTools.isBlank(dftButtonDir)) {
                        buttonALT = dftButtonDir + _buttonALT; 
                    } else {
                        buttonALT = _buttonALT;
                    }
                } else {
                    buttonALT = null;
                }
                ...
                try {
                    ...
                    if (WebPage.class.isAssignableFrom(pageClass)) {
                        ...
                        if (wp instanceof WebPageAdaptor) {
                            ...
                            //Print.logInfo("["+pl.getName()+"] Setting WebPage["+pageName+"] JspURI: " + (!StringTools.isBlank(jspURI)? jspURI: dftJSP));
                            ...
                            wpa.setMenuIconImage2(iconURI2);
                            wpa.setMatIcon(matIcon);
                        ...
                        if (wp.getIsEnabled()) {
                            ...
                            if (pageMap.containsKey(pn)) {
                                printWarn("Domain '"+pl.getName()+"', WebPage name already exists: "+pn);
                            }
                            pageMap.put(pn, wp);
                            if (!StringTools.isBlank(pageAlias) && !pageAlias.equals(pn)) {
                                if (pageMap.containsKey(pageAlias)) {
                                    printWarn("Domain '"+pl.getName()+"', WebPage alias already exists: "+pageAlias);
                                }
                    ...
                    if (optional) {
                        if (ExtendedDebugLogging()) {
                            printInfo("Domain '"+pl.getName()+"', optional WebPage class not found: "+classname);
                        }
                    } else {
            ...
            if (wpElemName.equalsIgnoreCase(TAG_Link)) {
                ...
                String  _iconURI   = XMLTools.getAttribute(link,ATTR_icon       ,null,false);
                String  _iconURI2  = XMLTools.getAttribute(link,ATTR_icon2      ,null,false);
                String  _buttonURI = XMLTools.getAttribute(link,ATTR_button     ,null,false);
                String  _buttonALT = XMLTools.getAttribute(link,ATTR_altButton  ,null,false);
                // -- I18N Strings
                I18N.Text navDesc  = null;
                I18N.Text navTab   = null;
                I18N.Text menuDesc = null;
                ...
                if (!StringTools.isBlank(_iconURI2)) {
                    if (_iconURI2.startsWith("/")) {
                        // -- relative to the servlet base
                        iconURI2 = _iconURI2.substring(1);
                    } else
                    if (!StringTools.isBlank(dftIconDir2)) {
                        iconURI2 = dftIconDir2 + _iconURI2; 
                    } else {
                ...
                if (!StringTools.isBlank(_buttonURI)) {
                    if (_buttonURI.startsWith("/")) {
                        buttonURI = _buttonURI.substring(1);  // relative to the servlet base
                    } else
                    if (!StringTools.isBlank(dftButtonDir)) {
                        buttonURI = dftButtonDir + _buttonURI; 
                    } else {
                ...
                wpu.setMenuIconImage2(iconURI2);
    ...
    protected void parseTag_Reports(File xmlFile, String i18nPkgName, BasicPrivateLabel bpl, Element reports)
    {
        ...
        for (int r = 0; r < reportList.getLength(); r++) {
            ...
            if (!StringTools.isBlank(keyPrefix)) {
                ...
                if (!RTConfig.getBoolean(rk,enabled)) {
                    if (ExtendedDebugLogging()) {
