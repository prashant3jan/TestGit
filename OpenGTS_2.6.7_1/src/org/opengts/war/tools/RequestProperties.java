...
//     -Added specific check for SECTION_STYLESHEET to allow adding addition CSS
// ----------------------------------------------------------------------------
    ...
    implements StringTools.KeyValueMap
{
    ...
    // -- default accountID indicator passed from AccountLogin to Track
    public  static final String      _DEFAULT_ID_           = "(default)"; // must be lower case
    ...
    **/
    ...
    public static String TRACK_BASE_URI()
    {
        if (TRACK_BASE_URI == null) {
            ...
            if (uri == null) { 
                uri = Constants._DEFAULT_BASE_URI; // ie. "./Track"
            }
    ...
    private java.util.List<String>  driverList              = null;
    private java.util.List<String>  deviceList              = null;
    private java.util.List<String>  deviceGrpSet            = null;
    private java.util.List<String>  deviceGrpSetAll         = null;
    ...
    /**
    *** Returns a Map instance containing ACL names as keys,
    *** and AccessLevel as values, for the current page and user.
    *** Returns null if there is no current Account or WebPage.
    **/
    public HashMap<String,AccessLevel> getWebPageAccessLevels()
    {
        PrivateLabel _privLabel = this.getPrivateLabel(); // non-null
        Account      currAcct   = this.getCurrentAccount();
        User         currUser   = this.getCurrentUser(); // may be null
        WebPage      wp         = this.getWebPage();
        if (wp == null) {
            // -- no current WebPage?
            return null;
        } else
        if (currAcct == null) {
            // -- no current account?
            //return null; <== ok without an account (currUser will also be null)
        }
        HashMap<String,AccessLevel> aclMap = new HashMap<String,AccessLevel>();
        // -- main page ACL
        String mainAclN = wp.getAclName(); // could be null
        aclMap.put(StringTools.trim(mainAclN), _privLabel.getAccessLevel(currUser,mainAclN)); // main
        // -- children/sub ACLs
        if (wp instanceof WebPageAdaptor) {
            String childAcl[] = ((WebPageAdaptor)wp).getChildAclList();
            if (!ListTools.isEmpty(childAcl)) {
                for (int i = 0; i < childAcl.length; i++) {
                    String childAclN = wp.getAclName(childAcl[i]); // non-null
                    aclMap.put(StringTools.trim(childAclN), _privLabel.getAccessLevel(currUser,childAclN));
                }
            }
        }
        return aclMap;
    ...
    /* DEBUG: print WebPage access levels */
    public void printWebPageAccessLevels()
    {
        HashMap<String,AccessLevel> map = this.getWebPageAccessLevels();
        Print.logInfo("WebPage Access Levels: " + this.getPageName());
        if (map == null) {
            Print.logInfo("  n/a");
        } else {
            for (String key : map.keySet()) {
                AccessLevel al = map.get(key);
                Print.logInfo("  " + key + " ==> " + al);
            }
        }
    }
    ...
    {
        //Print.logInfo("Setting WebPageURL: " + uri);
        this.webPageURI = uri;
    }
    ...
    public String getWebPageURI()
    {
        if (!StringTools.isBlank(this.webPageURI)) {
            ...
            //Print.logInfo("Returning JSP["+((page!=null)?page.getPageName():"n/a")+"]: " + jsp);
            return jsp;
        }
    }
    ...
    **/
    public static boolean isLoggedInFromSysAdmin(HttpServletRequest request, PrivateLabel _privLabel)
    {
        ...
        long   saTimeSt = AttributeTools.getSessionLong(request,Constants.PARM_SA_RELOGIN_TIME,0L);
        ...
        long saReloginTimeoutSec = DateTime.MinuteSeconds(30);
        if ((saTimeSt + saReloginTimeoutSec) < DateTime.getCurrentTimeSec()) {
    ...
    **/
        ...
        HttpServletRequest request, PrivateLabel _privLabel)
    {
        ...
        String saAcctID  = AttributeTools.getSessionString(request,Constants.PARM_SA_RELOGIN_ACCT,"");
        long   saLoginTS = AttributeTools.getSessionLong(  request,Constants.PARM_SA_RELOGIN_TIME,0L);
    ...
    /* gets the default login account id */
    public String getDefaultLoginAccountID()
    {
        return this.getPrivateLabel().getDefaultLoginAccount();
    }
    ...
    // -- must only be used within a user edit context
    public String[] getUserList() // READ_WRITE
    {
        ...
        if (this.userList == null) {
            try {
                ...
                if (privLabel.hasReadAccess(user, userPage.getAclName(UserInfo._ACL_ALL))) {
                    this.userList = User.getUsersForAccount(rwMode,this.getCurrentAccountID());
                } else
    ...
    public String getSelectedDeviceGroupID()
    {
        ...
        if (!StringTools.isBlank(this.selDeviceGroupID)) {
            ...
            java.util.List<String> grpList = this.getDeviceGroupIDList(rwMode,false);
    ...
    public java.util.List<String> getDeviceGroupIDList(DBReadWriteMode rwMode, boolean inclAll)
    {
        ...
        if (this.deviceGrpSet == null) {
            ...
            if (!User.isAdminUser(user)) {
                ...
                try {
                    user.clearCachedDeviceGroupIDs();
                    java.util.List<String> grpList = user.getExplicitlyAuthorizedDeviceGroupIDs(rwMode);
                    if (!ListTools.isEmpty(grpList)) {
                        ...
                        this.deviceGrpSet    = DeviceGroup.getDeviceGroupsForAccount(rwMode,this.getCurrentAccountID(),false);
                        this.deviceGrpSetAll = new OrderedSet<String>(this.deviceGrpSet); // shallow copy
    ...
    // --
    @Deprecated
    public java.util.List<String> getDeviceGroupIDList(boolean inclAll)
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        return this.getDeviceGroupIDList(rwMode, inclAll);
    }
    ...
    public java.util.List<String> getDeviceIDList(DBReadWriteMode rwMode, boolean inclInactv)
    {
        if (this.deviceList == null) {
            try {
                this.deviceList = User.getAuthorizedDeviceIDs(rwMode, this.getCurrentAccountID(), this.getCurrentUser(), inclInactv);
            } catch (DBException dbe) {
    ...
    // --
    @Deprecated
    public java.util.List<String> getDeviceIDList(boolean inclInactv)
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        return this.getDeviceIDList(rwMode, inclInactv);
    }

    /* return a list of known devices for this account */
    protected java.util.List<String> _getDeviceIDsForSelectedGroup(DBReadWriteMode rwMode, boolean isFleet, boolean inclInactv)
        throws DBException
    {
        if (isFleet) {
            ...
            if (!groupID.equals("") && !groupID.equalsIgnoreCase(DeviceGroup.DEVICE_GROUP_ALL)) {
                OrderedSet<String> dList = DeviceGroup.getDeviceIDsForGroup(rwMode, accountID, groupID, null/*User*/, inclInactv);
    ...
    // --
    @Deprecated
    protected java.util.List<String> _getDeviceIDsForSelectedGroup(boolean isFleet, boolean inclInactv)
        throws DBException
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        return this._getDeviceIDsForSelectedGroup(rwMode, isFleet, inclInactv);
    }
    ...
    /* return a list of known devices for this account */
    public java.util.List<String> getDriverIDList(DBReadWriteMode rwMode)
    {
        if (this.driverList == null) {
            try {
                String acctID = this.getCurrentAccountID();
                this.driverList = Driver.getDriverIDsForAccount(rwMode, acctID, -1L);
            } catch (DBException dbe) {
    ...
    // --
    @Deprecated
    public java.util.List<String> getDriverIDList()
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        return this.getDriverIDList(rwMode);
    }
    ...
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE; // used for user edit
        java.util.List<String> dgList = this.getDeviceGroupIDList(rwMode, inclAll);
    ...
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        java.util.List<String> dgList = this.getDeviceIDList(rwMode, inclInactv);
        java.util.List<IDDescription> idList = new Vector<IDDescription>();
        if (!ListTools.isEmpty(dgList)) {
    ...
    public java.util.List<IDDescription> createDriverIDDescriptionList(IDDescription.SortBy sortBy)
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        java.util.List<String> dgList = this.getDriverIDList(rwMode);
        java.util.List<IDDescription> idList = new Vector<IDDescription>();
        if (!ListTools.isEmpty(dgList)) {
            sortBy = IDDescription.GetSortBy(sortBy); // make sure 'sortBy' is not null
            boolean rtnDispName = sortBy.equals(IDDescription.SortBy.NAME);
    ...
    /* create Device/DeviceGroup IDDescription list */
    // -- used only by DashboardMap.java
    @Deprecated 
    public java.util.List<IDDescription> createIDDescriptionList(
        boolean groupList, 
        IDDescription.SortBy sortBy)
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        boolean inclAll = true, inclInactv = false;
        java.util.List<String> dgList = groupList? 
            this.getDeviceGroupIDList(rwMode,inclAll) : 
            this.getDeviceIDList(rwMode,inclInactv);
        java.util.List<IDDescription> idList = new Vector<IDDescription>();
        if (!ListTools.isEmpty(dgList)) {
            sortBy = IDDescription.GetSortBy(sortBy); // make sure 'sortBy' is not null
            boolean rtnDispName = sortBy.equals(IDDescription.SortBy.NAME);
    ...
    **/
        ...
        throws DBException
    {
        ...
        if (this.isFleet()) {
            ...
            // -- get list of devices
            java.util.List<String> devIDList = this._getDeviceIDsForSelectedGroup(rwMode,true/*fleet*/,false/*inclActv*/);
            ...
            for (int i = 0; i < devIDList.size(); i++) { // apply limit?
                ...
                if (notifyEventsOnly) {
                    ...
                    EventData ev[] = device.getRangeEvents( // rwMode??
    ...
    public String formatDateTime(long timestamp)
    {
        ...
        return this.formatDateTime(timestamp, tmz, true, "");
    }
    ...
    {
        TimeZone tmz = this.getTimeZone();
        return this.formatDateTime(timestamp, tmz, true, dft);
    }
    ...
    /* return the last event time as a string */
    public String formatDateTime(long timestamp, boolean inclTZ, String dft)
    {
        TimeZone tmz = this.getTimeZone();
        return this.formatDateTime(timestamp, tmz, inclTZ, dft);
    }

    /* return the last event time as a string */
    public String formatDateTime(long timestamp, TimeZone tmz, boolean inclTZ, String dft)
    {
        if (timestamp > 0L) {
            DateTime dt = new DateTime(timestamp, tmz);
            return this.formatDateTime(dt, tmz, inclTZ, dft);
        } else {
            return dft;
        }
    }
    ...
    {
        TimeZone tmz = this.getTimeZone();
        return this.formatDateTime(dt, tmz, true, "");
    }
    ...
    {
        TimeZone tmz = this.getTimeZone();
        return this.formatDateTime(dt, tmz, true, dft);
    }
    ...
    /* return the last event time as a string */
    public String formatDateTime(DateTime dt, TimeZone tmz, boolean inclTZ, String dft)
    {
        if ((dt != null) && (dt.getTimeSec() > 0L)) {
            ...
            String dtFmt   = dateFmt + " " + timeFmt + (inclTZ?" z":"");
            return dt.format(dtFmt, tmz);
        } else {
            return dft;
        }
    }
    ...
    **/
        ...
        throws IOException
    {
        ...
        if (key.equalsIgnoreCase(CommonServlet.SECTION_CONTENT_MESSAGE)) {
            ...
            msg = StringTools.htmlFilterText(msg);         // convert "\n" to "<BR>", etc ...
            msg = StringTools.htmlEncodeAnchorMailto(msg); // convert "mailto:EMAIL" to "<a href='mailto:EMAIL'>...
            // -- write message
            pw.write(msg);
            return;
        }
        ...
        /* StyleSheet */
        if (key.equalsIgnoreCase(CommonServlet.SECTION_STYLESHEET)) {
            // -- CSS/style to write all pages
            // -  TODO:
            // -- check for other stylesheet 
            Object obj = request.getAttribute(CommonServlet.SECTION_STYLESHEET);
        ...
        if (StringTools.isBlank(accountID)) {
            ...
            return RTConfigContextListener.GetSessionCount(sc,
                new RTConfigContextListener.HttpSessionFilter() {
                    public boolean filterSession(HttpSession s) {
