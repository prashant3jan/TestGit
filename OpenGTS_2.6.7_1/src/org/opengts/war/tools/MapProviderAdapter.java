    ...
    implements MapProvider
{
    ...
    **/
    ...
    /**
    *** Returns either the URL of the map provider, or a blank string if it's not implemented.
    **/
    public String getMapURL(RequestProperties reqState)
        throws IOException
    {
        // -- override
        return "";
    ...
    **/
    public boolean getAutoUpdateEnabled(boolean isFleet)
    {
        ...
        String autoUpdate;
        {
            String lafV = RTConfig.getString(BasicPrivateLabel.LAF_Map_Auto_Enable, null);
            if (!StringTools.isBlank(lafV)) {
                autoUpdate = lafV; // effects device AND fleet map
            } else {
                autoUpdate = isFleet?
        ...
        // --
        if (StringTools.isBoolean_FALSE(autoUpdate,false)) {
            return false; // "true", "no", "off", "0"
        } else // "true" implied if not false
    ...
    **/
    public boolean getAutoUpdateOnLoad(boolean isFleet)
    {
        ...
        String autoOnload;
        {
            String lafV = RTConfig.getString(BasicPrivateLabel.LAF_Map_Auto_OnLoad, null);
            if (!StringTools.isBlank(lafV)) {
                autoOnload = lafV;
            } else {
                autoOnload = isFleet?
        ...
        // --
        return ((autoEnable != null) && autoEnable.equalsIgnoreCase("onload"))? true : false;
    ...
    **/
    ...
    {
        long autoInterval;
        String lafV = RTConfig.getString(BasicPrivateLabel.LAF_Map_Auto_Interval, null);
        if (!StringTools.isBlank(lafV)) {
            autoInterval = StringTools.parseLong(lafV, 0L);
        } else {
            autoInterval = isFleet?
    ...
    **/
    ...
    {
        long autoCount;
        String lafAutoCnt = RTConfig.getString(BasicPrivateLabel.LAF_Map_Auto_Count, null);
        if (!StringTools.isBlank(lafAutoCnt)) {
            autoCount = StringTools.parseLong(lafAutoCnt, 0L);
        } else {
            autoCount = isFleet?
