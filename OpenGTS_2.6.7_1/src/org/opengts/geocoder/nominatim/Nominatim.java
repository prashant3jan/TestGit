...
//     -Added alias TAG_house_number (same as TAG_house) [2.6.6-B40a]
// ----------------------------------------------------------------------------
    ...
    implements ReverseGeocodeProvider
{
    ...
    protected static final String TAG_house_number              = "house_number";       // 
    ...
    protected static final int    RG_PROVIDER                   = ReverseGeocodeCache.RGPROV_NOMINATIM;
        ...
        Properties props)
    {
        ...
        for (;;) {
            ...
            if (this.rgCache != null) {
                ...
                rg = this.rgCache.getReverseGeocode(RG_PROVIDER, gp, isMoving); // "localeStr" is ignored
                if (rg != null) {
                    isCached = true;
                    break;
                }
        ...
        Properties props) 
    {
        ...
        for (int a = 0; (a < addresspartsList.getLength()); a++) {
            ...
            for (int ac = 0; ac < addresspartsChildren.getLength(); ac++) {
                ...
                if (elemName.equalsIgnoreCase(TAG_house)) { // see also TAG_house_number
                    house_val = XMLTools.getNodeText(elem," ",false);
                } else
                if (elemName.equalsIgnoreCase(TAG_house_number)) { // see also TAG_house
                    house_val = XMLTools.getNodeText(elem," ",false);
                } else
