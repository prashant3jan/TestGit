...
//     -Added checks for ignoring leading superfluous characters [2.6.6-B61b]
//     -Added support for unassigned devices (GTSE) [2.6.6-B61c]
// ----------------------------------------------------------------------------
// ----------------------------------------------------------------------------
    ...
    extends AbstractClientPacketHandler
{
    ...
    **/
    public enum TKDeviceType {
        ...
        VHG760   (10,"VHG760"  ); // EXPERIMENTAL: may not be supported
        // ---
        private int    vv = 0;
    ...
    **/
    public int getActualPacketLength(byte packet[], int packetLen)
    {
        ...
        if ((packetLen == 1) && (packet[0] < ' ')) {
            return 1; // consume/ignore
        }

        /* minimum number of bytes */
        int minBytes = 3; // set to expected minimum number of bytes
        if (packetLen < minBytes) {
            // -- next stop "getActualPacketLength"
        ...
        if ((packet[0] == '[') || (packet[1] == '[')) {
            // TK102B: "[!0000000001.(...)]"
            // -- set offset
            this.offset = (packet[1] == '[')? 1 : 0; // this will re-init multiple times
            // -- at least 14 bytes, to include the "(" character
            if (packetLen < (this.offset + 14)) {
                // -- read at least up to "(" char
                // -  required to skip past binary length byte at index-12
                // -- next stop "getActualPacketLength"
                return (this.offset + 14) | ServerSocketThread.PACKET_LEN_INCREMENTAL_;
            }
        ...
        if ((Character.toUpperCase(packet[0]) == 'I') || (Character.toUpperCase(packet[1]) == 'I')) {
            ...
            // -- set offset
            this.offset = (Character.toUpperCase(packet[1]) == 'I')? 1 : 0;
            // --
            this.tkDevType = TKDeviceType.TK103_2;
            if (PACKET_LEN_END_OF_STREAM) {
                return ServerSocketThread.PACKET_LEN_END_OF_STREAM;
        ...
        this.tkDevType = TKDeviceType.TK102; // could also be TK103_1,VHG760
        if (PACKET_LEN_END_OF_STREAM) {
            return ServerSocketThread.PACKET_LEN_END_OF_STREAM;
        } else {
            return ServerSocketThread.PACKET_LEN_LINE_TERMINATOR;
    ...
    **/
    public byte[] getHandlePacket(byte pktBytes[]) 
    {
        ...
        String s = StringTools.toStringValue(pktBytes,this.offset).trim();

        /* reset offset and new-event */
        // -- reset offset/state for next event
        this.offset = 0;
        this.state  = 0;
        ...
        if ((s.length() == 15) && StringTools.isNumeric(s)) { // 15-digit value
            // -- 1) "ON"
        ...
        /* VHG760 */
        if (s.indexOf("$GPRMC") > 0) { 
            if (s.endsWith("END")) {
                // -- well defined VHG760 packet type
                // -  862828027612345$GPRMC,210500.00,A,4738.8575,N,14210.6933,W,0.0,41.4,040618,,,D*6DEND 
                this.tkDevType = TKDeviceType.VHG760;
                return this.parseInsertRecord_VHG760(s);
            } else {
                // -- missing the trailing "END", but assume it is a VHG760 (or similar) anyway
                // -  862828027612345$GPRMC,210500.00,A,4738.8575,N,14210.6933,W,0.0,41.4,040618,,,D*6D 
                this.tkDevType = TKDeviceType.VHG760;
                return this.parseInsertRecord_VHG760(s);
            }
        }
    ...
    private byte[] parseInsertRecord_TK103_2(String s) // handleCommon
    {
        ...
        if ((dcs != null) && !StringTools.isBlank(eventCode)) {
            ...
            if (code == -9999) {
                ...
                if (eventCode.equalsIgnoreCase("door alarm")) {
                    // -- door open
                    statusCode = StatusCodes.STATUS_DOOR_OPEN_0;
                } else
                if (eventCode.equalsIgnoreCase("sensor alarm")) {
                    // -- shock alert (less than impact?)
                    statusCode = StatusCodes.STATUS_SENSOR_ON;
                } else
                if (eventCode.equalsIgnoreCase("acc alarm")) {
                    // -- Acc alarm
                    statusCode = StatusCodes.STATUS_ALARM_ON;
                } else
                if (eventCode.equalsIgnoreCase("accident alarm")) {
                    // -- accident alarm (impact?)
                    statusCode = StatusCodes.STATUS_IMPACT;
                } else
                if (eventCode.equalsIgnoreCase("bonnet alarm")) {
                    // -- bonnet/hood alarm
                    statusCode = StatusCodes.STATUS_CLASP_ON;
                } else
                if (eventCode.equalsIgnoreCase("footbrake alarm")) {
                    // -- footbrake alarm
                    statusCode = StatusCodes.STATUS_HANDBRAKE_ON;
                } else
                if (eventCode.startsWith("T:")) {
                    // -- temperature alarm
                    statusCode = StatusCodes.STATUS_TEMPERATURE_RANGE_0;
                } else
                if (eventCode.equalsIgnoreCase("oil")) {
                    // -- fuel alarm
                    statusCode = StatusCodes.STATUS_LOW_FUEL;
                } else
                if (eventCode.equalsIgnoreCase("service")) {
                    // -- vehicle maintenance
                    statusCode = StatusCodes.STATUS_MAINTENANCE_0;
                } else
                if (eventCode.startsWith("DTC ")) {
 ...
 // (027043534890BR00
        ...
        /* GPIO input */
        long gpioInput = -1L; // parseInsertRecord_TK103_3
        if (gpioStr.length() >= 8) {
    ...
    **/
    private byte[] parseInsertRecord_VJoy(String s) // handleCommon
    {
        ...
        /* misc */
        double odomKM     = 0.0;
        long   gpioInput  = -1L; // parseInsertRecord_VJoy
        double engTempC   = 0.0;
        double fuelLevel  = -1.0;
        double fuelLevel2 = -1.0;
    ...
    /* reserved GPIO for the TK-nano (if implemented) */
    public static       long    TKNANO_GPIO_ACC             = 0x0001L; // Input #0
    public static       long    TKNANO_GPIO_ENGINE          = 0x0002L; // Input #1
    public static       long    TKNANO_GPIO_ON_BATTERY      = 0x0008L; // Input #3
    public static       long    TKNANO_GPIO_SENSOR_1        = 0x0010L; // Input #4
    public static       long    TKNANO_GPIO_SENSOR_2        = 0x0020L; // Input #5
    // --
    ...
    **/
    private byte[] parseInsertRecord_TKnano_1(String s) // handleCommon
    {
        ...
        int gpsOfs = -1;
        int vehOfs = -1;
        if (eventCode.equals("V1" ) || 
            eventCode.equals("V2" )   ) {
            gpsOfs = 3;
            vehOfs = gpsOfs + 9;
        } else
        if (eventCode.equals("V19")   ) {
            gpsOfs = 3;
            vehOfs = gpsOfs + 12;
        } else
        ...
        /* date/time */
        long hhmmss  = StringTools.parseLong(fld[gpsOfs + 0],0L);   // hhmmss
        long ddmmyy  = StringTools.parseLong(fld[gpsOfs + 8],0L);   // ddmmyy
        long fixtime = this._parseDate_ddmmyy_hhmmss(ddmmyy, hhmmss);
        if (fixtime <= 0L) {
            Print.logWarn("Invalid date: " + fld[gpsOfs + 0] + "/" + fld[gpsOfs + 8]);
            fixtime = DateTime.getCurrentTimeSec(); // default to now
        }

        /* latitude/longitude */
        boolean  validGPS  = fld[4].equalsIgnoreCase("A");
        double   latitude  = Nmea0183.ParseLatitude( fld[gpsOfs + 2], fld[gpsOfs + 3], 0.0);
        double   longitude = Nmea0183.ParseLongitude(fld[gpsOfs + 4], fld[gpsOfs + 5], 0.0);
        GeoPoint geoPoint  = validGPS? new GeoPoint(latitude,longitude) : GeoPoint.INVALID_GEOPOINT;
        long     gpsAge    = 0L;
        double   HDOP      = 0.0;
        int      numSats   = 0;
        ...
        /* speed/heading */
        double speedKPH   = StringTools.parseDouble(fld[gpsOfs + 6],0.0) * SPEED_CONVERSION_TO_KPH;
        double headingDeg = StringTools.parseDouble(fld[gpsOfs + 7],0.0);
        if (speedKPH < MINIMUM_SPEED_KPH) {
            //Print.logInfo("Actual Speed: " + speedKPH);
            speedKPH   = 0.0;
            headingDeg = 0.0;
        ...
        /* vehicle status */
        long vehStatusNeg = StringTools.parseHexLong(fld[vehOfs + 0],0L); // V19
        ...
        long gpioInput = -1L; // parseInsertRecord_TKnano_1
        int statusCode = StatusCodes.STATUS_LOCATION;
    ...
    **/
    private byte[] parseInsertRecord_TKnano_2(byte pktBytes[]) // handleCommon
    {
        ...
        /* parse default status codes */
        long gpioInput = -1L; // parseInsertRecord_TKnano_2
        int statusCode = StatusCodes.STATUS_LOCATION;
        HashSet<Integer> statCodeSet = null;
        // handle "vehStatusNeg" here
        // --
    ...
    **/
    private static long _parseTKnanoVehicleStatus(long vehStatusNeg, Set<Integer> statCodeSet, long gpioInput)
    {
        ...
        // For example:
        //  - Byte-3/Bit-2 "ACC-off": a '0' means it 'happened' and the ACC is OFF (thus '1' is ACC-ON)
        //  - Byte-3/Bit-5 "Engine" : a '0' means it 'happened' and the Engine turned 'ON' (thus '1' is Engine-OFF)
    ...
    **/
    private byte[] parseInsertRecord_TK102B(String s) // handleCommon
    {
        ...
        double   HDOP       = 0.0;
        int      numSats    = 0;
        long     gpioInput  = -1L; // parseInsertRecord_TK102B
    ...
    **/
    private byte[] parseInsertRecord_TK102(String s) // handleCommon
    {
        ...
        long    gpsAge      = 0L;
        double  HDOP        = 0.0;
        long    gpioInput   = -1L; // parseInsertRecord_TK102
        double  engTempC    = 0.0;
        double  fuelLevel   = -1.0;
        double  fuelLevel2  = -1.0;
    ...
    // Amit VHG760: (router)
    /**
    *** VHG760: parse and insert data record 
    **/
    private byte[] parseInsertRecord_VHG760(String s) // handleCommon
    {
        Print.logInfo("Parsing: VHG760 ("+this.tkDevType+") ...");
        ...
        /* find "GPRMC" */
        int rmcX = s.indexOf("$GPRMC");
        if (rmcX < 0) {
            Print.logError("'GPRMC' not found");
            return null;
        }
        int endX = s.endsWith("END")? (sLen - 3) : sLen;
        String gprmcStr = s.substring(rmcX, endX);

        /* get mobile-id */
        int midS = 0;
        int midE = rmcX;
        while ((midE>0)&&((s.charAt(midE-1)=='|')||(s.charAt(midE-1)==','))) { midE--; }
        while ((midS<midE)&&(s.charAt(midS)=='#')) { midS++; }
        this.tkModemID = s.substring(midS, midE);
        if (StringTools.isBlank(this.tkModemID)) {
            Print.logError("ModemID is missing");
            return null;
        }

        /* parse data following GPRMC */
        Nmea0183 gprmc = new Nmea0183(gprmcStr,true/*ignoreCRC*/);
        long    fixtime     = gprmc.getFixtime();
        boolean validGPS    = gprmc.isValidGPS();
        double  latitude    = gprmc.getLatitude();
        double  longitude   = gprmc.getLongitude();
        double  altitudeM   = gprmc.getAltitudeMeters();
        double  headingDeg  = gprmc.getHeading();
        double  speedKPH    = gprmc.getSpeedKPH();
        long    gpsAge      = gprmc.getGpsAge();
        double  HDOP        = gprmc.getHDOP();
        int     numSats     = gprmc.getNumberOfSatellites();
        String  eventCode   = StringTools.trim(gprmc.getEventCode());
        double  odomKM      = 0.0;
        long    gpioInput   = -1L; // parseInsertRecord_VHG760
        double  engTempC    = 0.0;
        double  fuelLevel   = -1.0;
        double  fuelLevel2  = -1.0;
        double  batteryV    = 0.0;
        ...
        if (fixtime <= 0L) {
            if (!validGPS) {
                // -- ignore event with invalid fixtime and invalid location
                Print.logWarn("Invalid date and location, ignoring event (no data) ...");
                return null;
            }
            Print.logWarn("Invalid date, defaulting to current time");
            fixtime = DateTime.getCurrentTimeSec(); // default to now
        }
        ...
        // ------------------------------------------------
        // VHG760: Common data handling below 
        this.handleCommon(this.tkModemID, this.tkModemIDAlt,
            fixtime, statusCode, null,
            geoPoint, gpsAge, HDOP, numSats,
            speedKPH, headingDeg, altitudeM, odomKM,
    ...
    **/
        ...
        CellTower servingCell)
    {
        ...
        Device device    = this.tkDevice; // non-null
        ...
        if (gpioInput >= 0L) {
        Print.logInfo("GPIO-in  : 0x" + StringTools.toHexString(gpioInput,16));
        }
        if (odomKM > 0.0) {
