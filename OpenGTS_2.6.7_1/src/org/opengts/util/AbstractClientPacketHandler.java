...
*** This class maintains state and is therefore not suitable for multi-threaded
*** re-entrant ClientPacketHandler sessions.
**/
    ...
    implements ClientPacketHandler
{
    ...
    **/
    ...
    public void setLoggingEnabled(boolean quiet)
    {
        this.loggingEnabled = quiet;
    }
    ...
    **/
    ...
    private static HashMap<String,Long> LastMessageTime = new HashMap<String,Long>();

    /**
    *** Gets the timestamp of the last sent message
    **/
    private static long _GetLastMessageTime(String msgKey)
    {
        long rtn;
        synchronized (LastMessageTime) {
            Long ts = LastMessageTime.get(msgKey);
            rtn = (ts != null)? ts.longValue() : 0L;
        }
        return rtn;
    }

    /**
    *** Sets the timestamp of the last sent message
    **/
    private static void _SetLastMessageTime(String msgKey, long nowSec)
    {
        synchronized (LastMessageTime) {
            LastMessageTime.put(StringTools.trim(msgKey),new Long(nowSec));
        }
    }
    ...
    *** Send an alert email
    **/
    public boolean sendAlertEmail(String message, long minIntervalSec, String msgKey)
    {

        /* log message */
        //Print.logError(message);
        if ((msgKey != null) && msgKey.startsWith("W")) {
            Print._log(Print.LOG_WARN , 1, message, (Object[])null); // previous stack-frame
        } else {
            Print._log(Print.LOG_ERROR, 1, message, (Object[])null); // previous stack-frame
        }

        /* check last message time */
        long nowTS  = DateTime.getCurrentTimeSec();
        long lastTS = _GetLastMessageTime(msgKey);
        if (nowTS < (lastTS + minIntervalSec)) {
        ...
        String subj = name + ": " + message;
        String body = 
            "Condition detected in module that may require attention.\n" +
            "\n" +
            "Module : " + name + "\n" +
            "Message:\n" +
            "   " + message + "\n" + 
            "";
        if (SendMail.sendSystemEmail(name, subj, body)) {
            // -- update last message time
            _SetLastMessageTime(msgKey, nowTS);
            return true;
        } else {
            return false;
        }
