...
public class TemperatureSet
{
    ...
    **/
    public static String CreateGoogleDataTableJavaScript(boolean F, TemperatureSet... TSList)
    {
        ...
        /* remove null TemperatureSet entries */
        if (TSList == null) {
            Print.logWarn("TemperatureSet list is null");
            TSList = new TemperatureSet[0];
        } else
        if (TSList.length == 0) {
            Print.logWarn("TemperatureSet list is empty");
        } else {
            for (int i = 0; i < TSList.length; i++) {
                if (TSList[i] == null) {
                    Print.logWarn("TemperatureSet list contains a null entry at index# " + i);
                    TSList[i] = new TemperatureSet();
                }
            }
        }
    ...
    **/
    ...
    {
        return this.addTemperature(new Temperature(C, ts));
    }
