    ...
    public static void main(String argv[])
    {
        ...
        if (cmdRecv || cmdEcho) {
            ...
            try {
                ...
                Print.sysPrintln("Waiting for incoming data on %s:%d ...", host, port);
                for (;;) {
                    // -- read bytes
                    byte recv[] = dgm.receive(1000);
                    if (ListTools.isEmpty(recv)) {
                        continue;
                    }
                    // -- host
                    SocketAddress     sa = dgm.getReceivePacket().getSocketAddress();
                    InetAddress recvHost = (sa instanceof InetSocketAddress)? ((InetSocketAddress)sa).getAddress() : null;
                    int         recvPort = dgm.getReceivePacket().getPort();
                    // -- display
                    if (StringTools.isPrintableASCII(recv)) {
                        Print.logInfo("Recv from "+recvHost+"["+recvPort+"]: " + StringTools.toStringValue(recv));
                    } else {
                        Print.logInfo("Recv from "+recvHost+"["+recvPort+"]: 0x" + StringTools.toHexString(recv));
                    }
                    // -- echo
                    if (cmdEcho) {
                        try { Thread.sleep(500L); } catch (Throwable th) { /* ignore */ }
                        Print.sysPrintln("Echoing packet back to sender ...");
                        dgm.setRemoteHost(recvHost, recvPort);
                        dgm.send(recv);
                    }
                }
            } catch (Throwable th) {
