...
public class IButton
{
    ...
    **/
    ...
    {
        // -- value assumed to be 64-bits in Little-Endian format
    ...
    *** Depending on the origin of the iButton value, the "bigEndian" parameter should be set accordingly.
    **/
    public IButton(byte value[], int offset, int length, boolean bigEndian)
    {
        byte V[] = value;
        ...
        if ((V == null) || (V.length == 0)) {
            // -- empty
            Print.logError("Empty/Null iButton-ID hex value");
            this.valueLE = 0L;
            return;
        }
        ...
        int len = (length >= 0)? length : (V.length - ofs);
        if ((len <= 0) || ((ofs + len) > V.length)) {
            // -- invalid offset/length
            Print.logError("Invalid iButton-ID Offset/Length");
            this.valueLE = 0L;
            return;
        }
        ...
        if (len == 8) {
            // -- 64-bit: full length, Little-Endian format
            long ib64    = StringTools.parseLong(V, ofs, len, true, false, 0L);
            long ib64LE  = bigEndian? Payload.reverseByteOrder(ib64,8) : ib64; // make LE
            this.valueLE = ib64LE;
        } else
        if (len == 6) {
            // -- 48-bit: assume missing CRC and FamilyCode (assume FamilyCode 0x01)
            long ib48    = StringTools.parseLong(V, ofs, len, true, false, 0L);
            long ib48LE  = bigEndian? Payload.reverseByteOrder(ib48,6) : ib48; // make LE
            long ib64LE  = IButton.ResetCRC((ib48LE << 8) | 0x01L); 
            this.valueLE = ib64LE;
        } else
        if (len == 4) {
            // -- 32-bit: assume missing CRC, FamilyCode, an 2 MSB bytes of serial#
            long ib32    = StringTools.parseLong(V, ofs, len, true, false, 0L);
            long ib32LE  = bigEndian? Payload.reverseByteOrder(ib32,4) : ib32; // make LE
            long ib64LE  = IButton.ResetCRC((ib32LE << 8) | 0x01L); 
            this.valueLE = ib64LE;
        } else {
            Print.logError("Invalid iButton-ID hex length: " + len);
            this.valueLE = 0L;
        }
