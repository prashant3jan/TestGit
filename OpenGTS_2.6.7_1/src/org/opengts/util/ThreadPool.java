...
//     -Add try-catch around executed job [2.6.7-B46f]
//     -Add 'jobQueue.notifyAll()' when job is completed [2.6.7-B46f]
// ----------------------------------------------------------------------------
...
public class ThreadPool
{
    ...
    **/
    ...
    private int                         activeCount     = 0;  // [2.6.7-B46f]
    ...
    private volatile int                stopThreads     = STOP_NEVER;
    ...
    **/
    ...
    /**
    *** Returns true if there are Job-Queue is empty
    **/
    public boolean isQueueEmpty()
    {
        boolean isEmpty;
        synchronized (this.jobQueue) {
            isEmpty = this.jobQueue.isEmpty()? true : false;
        }
        return isEmpty;
    }
    ...
    **/
    public int getActiveCount()
    {
        ...
        synchronized (this.jobThreadPool) {
            ...
            // -- actual count should match counter
            if (this.activeCount != cnt) {
                // -- should never occur
                Print.logWarn("'activeCount' does not match number of actual running jobs");
            }
        }
        return cnt;
    }
    ...
    **/
    ...
    /**
    *** Check for empty job-queue and active jobs complete.
    *** MUST only be called within a "synchronized(this.jobQueue) {...}" block.
    **/
    private boolean _queueEmpty(boolean chkActiveJobs)
    {
        if (chkActiveJobs && (this.activeCount > 0)) {
            return false;
        } else {
    ...
    /**
    *** Wait until all active jobs are complete
    **/
    public boolean waitUntilEmpty(long timeoutMS, boolean chkActiveJobs)
    {
        boolean rtn = false;
        synchronized (this.jobQueue) {
            if (timeoutMS > 0L) {
                ...
                try {
                    ...
                    while (!this._queueEmpty(chkActiveJobs)) {
                        long alreadyWaitedMS = DateTime.getCurrentTimeMillis() - startTimeMS;
                        if (alreadyWaitedMS >= timeoutMS) {
                            // -- timeout
                            break;
                        }
                        this.jobQueue.wait((int)Math.min((timeoutMS - alreadyWaitedMS), 5000L));
                        if (this.stopThreads == STOP_NOW) {
                            // -- threads are stopping, return now
                            break;
                        }
                    }
            ...
            if (timeoutMS < 0L) {
                ...
                // -- no timeout, return results now
            }
            // -- set returned value
            rtn = !this._queueEmpty(chkActiveJobs)? true : false;
        }
        return rtn;
    }

    /**
    *** Wait here until Job-Queue is empty.
    *** @return True if timeout (or interrupted) and job-queue still has jobs, false otherwise.
    **/
    public boolean waitUntilJobQueueEmpty(long timeoutMS) // waitUntilQueueEmpty
    {
        return this.waitUntilEmpty(timeoutMS, false/*!chkActiveJobs*/);
    }

    /**
    *** Wait until all active jobs are complete
    *** @return True if timeout (or interrupted) and still has jobs, false otherwise.
    **/
    public boolean waitUntilAllJobsComplete(long timeoutMS)
    {
        return this.waitUntilEmpty(timeoutMS, true/*!chkActiveJobs*/);
    }

    // ------------------------------------------------------------------------

    /**
    *** Gets the stop-thread state
    **/
    public boolean isStoppingNow()
    {
        return (this.stopThreads == STOP_NOW)? true : false;
    }
    ...
    **/
    protected void _removeThreadJob(ThreadJob thread)
    {
        if (thread != null) {
            synchronized (this.jobThreadPool) {
                Print.logDebug("Removing Thread: " + thread.getName()); // "New Thread ...
        ...
        public void run() {
            ...
            while (true) {
                ...
                synchronized (this.threadPool.jobQueue) {
                    ...
                    while (this.job == null) {
                        ...
                        if (this.threadPool.jobQueue.size() > 0) { // this.jobQueue
                            ...
                            this.threadPool.activeCount++;
                            if (this.threadPool.jobQueue.isEmpty()) {
                                this.threadPool.jobQueue.notifyAll();
                            }
                        } else
                        if (this.threadPool.stopThreads == STOP_WAITING) {
                            ...
                            int tmoMS = 60000; // maximum wait (probably could be larger)
                            if (this.threadPool.maxIdleAgeMS > 0L) { // [2.6.7-B46f]
                                // -- adjust 'tmoMS' to coincide with remaining 'maxIdleAgeMS'
                                long lastUsedDeltaMS = DateTime.getCurrentTimeMillis() - this.lastUsedTimeMS;
                                long untilIdleMS = this.threadPool.maxIdleAgeMS - lastUsedDeltaMS;
                                if (untilIdleMS <= 0L) {
                                    // -- expired, stop due to excess idle time
                                    stop = true;
                                    break;
                                } else
                                if (untilIdleMS < (long)tmoMS) {
                                    // -- max idle timeout is less than our default wait time
                                    tmoMS = (int)untilIdleMS;
                                }
                            }
                ...
                try { // [2.6.7-B46f]
                    ...
                    Print.logException("Thread("+this.threadPool.getName()+":"+this.getName()+") Job error", th);
                }
                synchronized (this.threadPool.jobQueue) {
                    this.job = null;
                    if (this.threadPool.activeCount > 0) { // [2.6.7-B46f]
                        this.threadPool.activeCount--; // [2.6.7-B46f]
                    } else {
                        // -- should never occur
                        this.threadPool.activeCount = 0; // [2.6.7-B46f]
                    }
                    this.threadPool.jobQueue.notifyAll(); // [2.6.7-B46f]
                }
