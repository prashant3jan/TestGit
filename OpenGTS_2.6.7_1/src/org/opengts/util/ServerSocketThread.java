...
// ----------------------------------------------------------------------------
// Properties used by this module:
//  - ServerSocketThread.reOpenSocketOnUnexpectedClose=true
//  - ServerSocketThread.maximumClientThreadPoolSize
//  - ServerSocketThread.udpReceiveBufferSize
// ----------------------------------------------------------------------------
// Change History:
...
//     -Added support for re-opening unexpected listener socket closures (see REOPEN)
// ----------------------------------------------------------------------------
    ...
    extends Thread
{
  ...
  //public static final int         PACKET_LEN_HTTP_HEADER              = -5;  // read until blank line (\r\n)  (NOT SUPPORTED!)
    ...
    **/
    ...
    private boolean                              isTcpSSL                 = false; // 
    private boolean                              reopenOnUnexpClose       = false; // ServerSocketThread.reOpenSocketOnUnexpectedClose=true
    
    private java.util.List<ServerSessionThread>  clientThreadPool         = null;
    private int                                  maxClientPoolSize        = 0;     // ServerSocketThread.maximumClientThreadPoolSize
    ...
    **/
    private ServerSocketThread()
    {
        ...
        // -- REOPEN: set re-open listen socket on unexpected socket closure
        this.setReopenOnUnexpectedClose(RTConfig.getBoolean(RTKey.ServerSocketThread_reOpenSocketOnUnexpectedClose,false));
    ...
    **/
    public ServerSocketThread(ServerSocket ss) 
    {
        ...
        this.isTcpSSL     = (ss instanceof SSLServerSocket)? true : false; // best guess
    }
    ...
    **/
        ...
        throws IOException 
    {
        ...
        this.isTcpSSL     = false;
        this.serverSocket = ServerSocketThread.createServerSocket(this.bindAddress, this.listenPort);
    }
    ...
    **/
        ...
        throws IOException 
    {
        ...
        this.bindAddress  = (bindAddr != null)? bindAddr : ServerSocketThread.getDefaultBindAddress();
        this.listenPort   = port;
        this.isTcpSSL     = useSSL;
        this.serverSocket = this.isTcpSSL?
            SSLServerSocketFactory.getDefault().createServerSocket(this.listenPort, ListenBacklog, this.bindAddress) :
            ServerSocketFactory   .getDefault().createServerSocket(this.listenPort, ListenBacklog, this.bindAddress);
    }
    ...
    **/
    ...
    /**
    *** Minimum elapsed time between unexpected socket closure failures
    **/
    private static long MinimumReopenTimeElapsedSec = DateTime.MinuteSeconds(5); // REOPEN

    /**
    *** Sets flag to re-open listen socket on unexpected socket closure.
    **/
    public void setReopenOnUnexpectedClose(boolean reopenOnClose) // REOPEN
    {
        this.reopenOnUnexpClose = reopenOnClose;
    }

    /**
    *** Gets flag indicating re-open listen socket on unexpected socket closure.
    **/
    public boolean getReopenOnUnexpectedClose() // REOPEN
    {
        return this.reopenOnUnexpClose;
    }
    ...
    **/
    ...
    /**
    *** Returns true if this ServerSockeThread instance listens on a UDP socket
    **/
    public boolean isUDP() 
    {
        return (this.datagramSocket != null)? true : false;
    }
    ...
    *** @return The ServerSocket handle
    **/
    ...
    /**
    *** Returns true if this ServerSockeThread instance listens on a TCP socket
    **/
    public boolean isTCP() 
    {
        return (this.serverSocket != null)? true : false;
    }

    /**
    *** Returns true if this ServerSockeThread instance listens on a TCP SSL socket
    **/
    public boolean isSSL() 
    {
        return (this.isTCP() && this.isTcpSSL)? true : false;
    }
    ...
    public void run() 
    {
        ...
        while (true) {
            ...
            try {
                ...
                // -- shutdown support
                long nowSec = DateTime.getCurrentTimeSec(); // time of SocketException
                long deltaSec = nowSec - lastReopenTime; // since last re-open attempt
                if (this.serverSocket != null) { // this.isTCP()
                    // -- REOPEN: TCP
                    //SocketAddress addr = this.serverSocket.getLocalSocketAddress();
                    ...
                    Print.logException("Unexpected TCP socket shutdown on "+addr+":"+portStr, se);
                    if (!this.getReopenOnUnexpectedClose()) {
                        // -- will not attempt to re-open
                    } else
                    if (deltaSec < MinimumReopenTimeElapsedSec) {
                        // -- previous re-open attempt was too recent
                        Print.logError("Last TCP re-open attempt occurred only "+deltaSec+" seconds ago.");
                        Print.logError("TCP re-open will not be attempted to prevent possible endless-loop.");
                    } else {
                        // -- re-open TCP listener socket
                        lastReopenCnt++;
                        InetAddress bAddr = this.getBindAddress();
                        int         bPort = this.getLocalPort();
                        Print.logWarn("Attempt [#"+lastReopenCnt+"] to re-open TCP listener socket ["+bAddr+":"+bPort+"] ...");
                        // -- close (just to be sure)
                        try { // this.serverSocket.isClosed()?
                            this.serverSocket.close();
                        } catch (Throwable th) {
                            // -- ignore (an exception is likely)
                        }
                        // -- re-open
                        try { 
                            this.serverSocket = null; // clear first
                            this.serverSocket = this.isSSL()?
                                SSLServerSocketFactory.getDefault().createServerSocket(bPort, ListenBacklog, bAddr) :
                                ServerSocketFactory   .getDefault().createServerSocket(bPort, ListenBacklog, bAddr);
                            lastReopenTime = nowSec;
                            continue;
                        } catch (BindException be) { 
                            // -- BindException: Address already in use 
                            Print.logException("Unable to bind to new TCP server socket", be);
                        } catch (Throwable th) {
                            // -- unknown error
                            Print.logException("Unable to create new TCP server socket", th);
                        }
                    }
                } else
                if (this.datagramSocket != null) { // this.isUDP()
                    // -- REOPEN: UDP
                    //SocketAddress addr = this.datagramSocket.getLocalSocketAddress();
                    InetAddress addr = this.datagramSocket.getLocalAddress();
                    int port = this.datagramSocket.getLocalPort(); // should be same as 'this.listenPort'
                    if (port <= 0) { port = this.getLocalPort(); }
                    String portStr = (port <= 0)? "?" : String.valueOf(port);
                    Print.logException("Unexpected UDP socket shutdown on "+addr+":"+portStr, se);
                    if (!this.getReopenOnUnexpectedClose()) {
                        // -- will not attempt to re-open
                    } else
                    if (deltaSec < MinimumReopenTimeElapsedSec) {
                        // -- previous re-open attempt was too recent
                        Print.logError("Last UDP re-open attempt occurred only "+deltaSec+" seconds ago.");
                        Print.logError("UDP re-open will not be attempted to prevent possible endless-loop.");
                    } else {
                        // -- re-open UDP listener socket
                        lastReopenCnt++;
                        InetAddress bAddr = this.getBindAddress();
                        int         bPort = this.getLocalPort();
                        Print.logWarn("Attempt [#"+lastReopenCnt+"] to re-open UDP listener socket ["+bAddr+":"+bPort+"] ...");
                        // -- close (just to be sure)
                        try { // this.datagramSocket.isClosed()?
                        ...
                        // -- re-open
                        try { 
                            this.datagramSocket = null; // clear first
                            this.datagramSocket = ServerSocketThread.createDatagramSocket(bAddr,bPort);
                            lastReopenTime = nowSec;
                            continue; // re-try
                        } catch (BindException be) { 
                            // -- BindException: Address already in use 
                            Print.logException("Unable to bind to new UDP server socket", be);
                        } catch (Throwable th) {
                            // -- unknown error
                            Print.logException("Unable to create new UDP server socket", th);
                        }
                    }
                } else {
        ...
        /* final log */
        if (LogEnable) {
            String proto     = this.isTCP()? "TCP" : this.isUDP()? "UDP" : "???";
            InetAddress addr = this.getBindAddress();
            int port         = this.getLocalPort();
            Print.logInfo(proto + " listener on "+addr+":"+port+" has terminated.");
        }
    ...
    **/
    ...
    {
        final long timeoutMS = (tmoMS >= 1000L)? tmoMS : 1000L; // at least 1-second
        // --
        if (LogEnable) {
            String mode = this.isTCP()? "TCP" : this.isUDP()? "UDP" : "???";
            int port = this.getLocalPort();
            Print.logWarn(mode+" listener shutdown on port "+port+" ...");
        }
        // --
    	try {
    ...
    **/
    ...
    /** 
    *** ClientSocket.
    *** Instances of this class are created to handle a single incoming connection.
    **/
    private class ClientSocket // new ClientSocket(...)
    {
        ...
        private RTProperties   rtpState    = null;  // to maintain arbitrary state
        private boolean        terminate   = false; // hint to session handler, may be ignored
        // -- TCP Constructor
        ...
        // -- maintain state
        public boolean hasRTProperties() {
            return (this.rtpState != null)? true : false;
        }
        public RTProperties getRTProperties() {
            if (this.rtpState == null) {
                this.rtpState = new RTProperties();
            }
            return this.rtpState;
        }
        // -- hint to terminate session
    ...
    *** SessionInfo
    **/
    public interface SessionInfo // implements SessionInfo
    {
        ...
        public boolean              hasClientSocketState(); 
        public RTProperties         getClientSocketState(); // may be null
        public void                 setClientSocketTerminate(boolean terminate); 
        public boolean              getClientSocketTerminate();
    ...
    **/
        ...
        implements SessionInfo
    {
        ...
        private ClientSocket        client               = null; // this.client
        ...
        //    super("ClientSession");
        //    this.client = new ClientSocket(client); // (this.setClientSocket(new ClientSocket(client))) TCP?
        //    this.start(); // java.lang.OutOfMemoryError: unable to create new native thread
        //}
        ...
        public ServerSessionThread(ClientSocket clientSock, boolean startThread) {
            super("ClientSession" + "_" + 
                (startThread?
                    ...
                    "XXX")); // this.getName()
            this.client = clientSock; // (this.setClientSocket(clientSock)) new ClientSocket thread
            if (startThread) {
                this.start(); // java.lang.OutOfMemoryError: unable to create new native thread
            }
        }
        ...
        **/
        ...
        public boolean setClientIfAvailable(ClientSocket clientSocket) {
            ...
            synchronized (this.runLock) {
                if (!this.hasStarted) {
                    ...
                    if (this.client != null) { // (this.hasClientSocket())
                        // -- should not occur
                        Print.logWarn("Overwriting current client!");
                    }
                    this.client = clientSocket; // (this.setClientSocket(clientSocket))
        ...
        public boolean hasClientSocketState() {
            return (this.client != null)? this.client.hasRTProperties() : false;
        }

        public RTProperties getClientSocketState() {
            return (this.client != null)? this.client.getRTProperties() : null;
        }

        public void setClientSocketTerminate(boolean terminate) {
            if (this.client != null) {
                this.client.setTerminate(terminate);
            }
        }

        public boolean getClientSocketTerminate() {
            return (this.client != null)? this.client.getTerminate() : false;
        }
        ...
        public void close() throws IOException {
            ...
            synchronized (this.runLock) {
                if (this.client != null) { // (this.hasClientSocket())
                    ...
                    this.client = null; // (this.setClientSocket(null))
        ...
        **/
        ...
        public void run() {
            ...
            while (true) {
                ...
                synchronized (this.runLock) {
                    while (this.client == null) { // (!this.hasClientSocket())
                ...
                synchronized (this.runLock) {
                    this.client = null; // (setClientSocket)
        ...
        public void handleClientSession(ClientSocket clientSock) { 
            // -- "this.client" will be null if this is an InputStream session,
            // -  otherwise "this.client" will be same as "clientSock"
    ...
    **/
    ...
    public static void main(String argv[])
    {
        ...
        try {
            ...
            Print.logError("Unknown host: " + host);
            System.exit(1);
        }
        try {
            Print.logInfo("Opening TCP listener on "+host+":"+port);
            ServerSocketThread sst = new ServerSocketThread(bindAddr, port, false/*ssl?*/);
            MinimumReopenTimeElapsedSec = 4L;
            sst.setLoggingEnabled(true);
            sst.setName("TCPListener_" + port);
            sst.start(); // java.lang.OutOfMemoryError: unable to create new native thread
            /*
            if (UNEXPECTED_CLOSE_TEST) {
                OSTools.sleepSec(3);
                sst.setReopenOnUnexpectedClose(true); // REOPEN
                for (int i = 1; i <= 3; i++) {
                    Print.logInfo("Closing TCP socket #"+i+" ...");
                    sst.serverSocket.close();
                    OSTools.sleepSec((i == 2)? 1 : 5);
                }
            }
            */
        } catch (IOException ioe) {
