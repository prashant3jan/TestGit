...
//     -Added TIRE_UNDER_PRESSURE_2, TIRE_OVER_PRESSURE_2
// ----------------------------------------------------------------------------
...
public class TireState
{
    ...
    *** @param axleTireNdx  The index of the tire on the axle (top-view, front-forward, left-to-right) (ie. 0..3)
    ...
    *** @param tireNdx  The tire index (top-view, front-forward, left-to-right)
    ...
    **/
    ...
    public enum TemperatureUnits {
        ...
        public double  convertFromC(double c)       { return this.isF()? C2F(c) : c; }
        public double  convertToC(double c)         { return this.isF()? F2C(c) : c; }
    ...
    **/
    ...
    {
        return !Double.isNaN(F)? ((F - 32.0) * 5.0 / 9.0) : Double.NaN;
    }
    ...
    **/
    ...
    {
        return !Double.isNaN(C)? ((C * 9.0 / 5.0) + 32.0) : Double.NaN;
    }
    ...
    public static final long    TIRE_UNDER_PRESSURE_2   = 0x0004L;  // [q] severe under pressure
    public static final long    TIRE_OVER_PRESSURE_2    = 0x0008L;  // [Q] severe over pressure
    ...
    public static final long    TIRE_LOST_SIGNAL        = 0x1000L;  // [s]
    ...
    public static long ParseTireStatus(String ss)
    {
        ...
        for (char ch : StringTools.trim(ss).toCharArray()) {
            switch (ch) {
                ...
                // -- severe under/over pressure
                case 'q': stat |= TIRE_UNDER_PRESSURE | TIRE_UNDER_PRESSURE_2; break;
                case 'Q': stat |= TIRE_OVER_PRESSURE  | TIRE_OVER_PRESSURE_2;  break;
                // -- low/high temperature
                case 't': stat |= TIRE_LOW_TEMPERATURE;                        break;
    ...
    **/
    public static String GetTireStatusString(long status)
    {
        ...
        // -- (severe) under pressure
        if ((status & TIRE_UNDER_PRESSURE_2) != 0L) {
            sb.append("q");
        } else
        if ((status & TIRE_UNDER_PRESSURE) != 0L) {
            sb.append("p");
        } 
        // -- (severe) over pressure
        if ((status & TIRE_OVER_PRESSURE_2) != 0L) {
            sb.append("Q");
        } else
    ...
    **/
    ...
    /**
    *** Returns a parsable String representation of the TireState array
    **/
    public static String toString(TireState tsa[])
    {
        if (ListTools.isEmpty(tsa)) { // fix (removed "!") [2.6.7-B47t]
            return "";
        } else {
            StringBuffer sb = new StringBuffer();
            for (TireState ts : tsa) {
