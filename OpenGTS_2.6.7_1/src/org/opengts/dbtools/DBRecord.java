...
public abstract class DBRecord<gDBR extends DBRecord<gDBR>>
{
    ...
    **/
    public static <T extends DBRecord<T>> long getRecordCount(DBReadWriteMode rwMode, DBFactory<T> fact)
        throws DBException
    {
        return DBRecord.getRecordCount(rwMode, fact, "");
    }
    // --
    @Deprecated
    ...
    *** @throws DBException If a general DB error occurs
    **/
    public static <T extends DBRecord<T>> long getRecordCount(DBReadWriteMode rwMode, DBFactory<T> fact, StringBuffer where)
        throws DBException
    {
        return DBRecord.getRecordCount(rwMode, fact, ((where!=null)?where.toString():null));
    }
    // --
    @Deprecated
    ...
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        return DBRecord.getRecordCount(rwMode, fact, where);
    }
    ...
    *** @throws DBException If a general DB error occurs
    **/
    public static <T extends DBRecord<T>> long getRecordCount(DBReadWriteMode rwMode, DBFactory<T> fact, String where)
        throws DBException
    {
        ...
        /* return actual count */
        return DBRecord.getRecordCount(rwMode, new DBSelect<T>(fact, where)); // "unchecked call"
    ...
    *** @throws DBException If a general DB error occurs
    **/
    public static <T extends DBRecord<T>> long getRecordCount(DBReadWriteMode rwMode, DBFactory<T> fact, DBWhere where)
        throws DBException
    {
    ...
    *** @throws DBException If a general DB error occurs
    **/
    public static <T extends DBRecord<T>> long getRecordCount(DBReadWriteMode rwMode, DBSelect<T> dsel)
        throws DBException
    {
        ...
        if (innodb) {
            ...
            if (!dsel.hasWhere()) {
                ...
                return fact.getMySQLTableStatus(rwMode);
            } else
            ...
            // -- Only large DB tables should be InnoDB, so read from the 'READ' mode database.
            // -  If the Account/Device/etc table is also InnoDB, there is a very small window of
            // -  opportunity for an inaccurate result to be returned.
            if (!rwMode.equals(DBReadWriteMode.READ_ONLY)) {
                rwMode = DBReadWriteMode.READ_ONLY;
                Print.logWarn("Warn: Using 'COUNT(*)' for InnoDB query: " + utableName + " (using '"+rwMode+"' mode DB)");
            }
        }
    ...
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        return DBRecord.getRecordCount(rwMode, dsel);
    }
  ...
  //private   DBFieldValues             fieldVals               = null;
    ...
    **/
    protected DBRecord(DBRecordKey<gDBR> key)
    {
        ...
        if (this.recordKey != null) {
            this.recordKey._setDBRecordVar(this); // "unchecked cast"
        }
    }
    ...
    public DBRecordKey<gDBR> getRecordKey()
    {
        if (this.recordKey == null) {
            ...
            try {
                ...
                // dbe.printException();
                Print.logException("Get RecordKey error", dbe);
                return null;
            }
        }
    ...
    **/
    @SuppressWarnings("unchecked")
    public static <T extends DBRecord<T>> DBRecord<T>[] _getRecords(
        DBReadWriteMode rwMode,
        DBSelect<T> dsel, 
        DBRecordHandler<T> rcdHandler)
        throws DBException
    {
        return DBRecord.select(rwMode, dsel, rcdHandler);
    }
    ...
    *** @throws DBException If a DB access error occurs
    **/
    public static <T extends DBRecord<T>> T[] getRecords(
        DBReadWriteMode rwMode,
        DBFactory<T> fact, 
        String where,
        ...
        throws DBException
    {
        return DBRecord.select(rwMode, fact, where, null, orderBy, ascending, -1L, -1L, null);
    }
    ...
    **/
    public static <T extends DBRecord<T>> T[] getRecords(
        ...
        throws DBException
    {
        return DBRecord.select(rwMode, fact, where, addtnlSel, orderBy, ascending, limit, offset, null);
    }
    ...
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        return DBRecord._getRecords(rwMode, dsel, rcdHandler);
    }
    ...
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        return DBRecord.getRecords(rwMode, fact, where, orderBy, ascending);
    }
    ...
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        return DBRecord.getRecords(rwMode, fact, where, addtnlSel, orderBy, ascending, limit, offset);
    }
    ...
    *** @throws DBException If a DB access error occurs
    **/
    protected static <T extends DBRecord<T>> T[] select(
        DBReadWriteMode rwMode,
        DBFactory<T> fact, 
        String where, String addtnlSel, 
        String orderBy[], boolean ascending,
    ...
    **/
    protected static <T extends DBRecord<T>> T[] select(
        ...
        throws DBException
    {
        return DBRecord.select(rwMode, dsel, null);
    }
    ...
    **/
        ...
        throws DBException
    {
        ...
        try {
            ...
            rs   = stmt.getResultSet();
            // -- debug
            //Print.logInfo("'"+dsel.getUntranslatedTableName()+"' read DBConnection: ["+dbc.getReadWriteMode()+"] " + dbc.getUri());
    ...
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        return DBRecord.select(rwMode, fact, where, addtnlSel, orderBy, ascending, limit, offset, rcdHandler);
    }
    ...
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        return DBRecord.select(rwMode, dsel);
    }
    ...
    **/
    public gDBR reload(String... fldNames)
    {
        ...
        try {
            // -- reload from "READ_WRITE" database
            // -  there is a small window of opportunity that a reload from the 'READ' DB 
            // -  could miss recent data written to the 'WRITE' DB.  So for now we always
            // -  reload from the 'WRITE' DB.
            DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
            return this._reload(rwMode, fldNames);
        } catch (DBException dbe) {
            //dbe.printException();
            //Print.logException("Reload error", dbe);
            Print.logError("Reload error: " + dbe);
            return null;
        }
    }
    ...
    /**
    *** Reload the contents of this record from the DB
    *** @param rwMode   The read/write database from which to reload the record.
    *** @param fldNames The list of field names to update (null for all fields).
    *** @return This DBRecord, or null if key not found.
    *** @throws DBException If a general DB error occurs
    **/
    @SuppressWarnings("unchecked")
    public gDBR _reload(DBReadWriteMode rwMode, String... fldNames)
        throws DBException
    {
    ...
    /**
    *** Gets the Record corresponding to the specified column value
    *** @param rwMode      Read/Write mode
    *** @param dbFld       The alternate key DBField
    *** @param lookupKey   The lookup key value
    *** @param limit       The maximum number of records to return
    *** @param allowMult   True to allow multiple returned results, false to throw DBDuplicateFoundException on multiple matches
    *** @return The loaded DBRecord instance, or null if the lookup key was not found
    *** @throws DBException if a database error occurs
    **/
    @SuppressWarnings("unchecked")
    public static <T extends DBRecord<T>> T[] _loadByAlternateKey(
        DBReadWriteMode rwMode, 
        DBFactory<T> dbFact, String altFldName, 
        String lookupKey, long limit, boolean allowMult)
        throws DBException
    {
        // select(
        ...
        /* field/column */
        DBField dbFld = dbFact.getField(altFldName);
        if (dbFld == null) {
            // -- specified field does not exist in table
            Print.logError("Lookup requested on invalid field: "+utableName+"."+altFldName);
            return null;
        }

        /* alternate key? */
        // -- warn if not an alternate key
        if (!dbFld.isAlternateKey() && !dbFld.isPrimaryKey()) {
            Print.logWarn("Lookup requested on non-alternate-key field: "+utableName+"."+altFldName);
        }

        /* invalid lookup key? */
        // -- a non-blank lookup key is requried
        if (StringTools.isBlank(lookupKey)) {
            return null; // just say it doesn't exist
        }

        /* select */
        DBSelect<T> dsel = new DBSelect<T>(dbFact);
        DBWhere dwh = dsel.createDBWhere();
        dsel.setWhere(dwh.WHERE_(
            dwh.EQ(altFldName, lookupKey)
        ));
        dsel.setLimit(limit);
        ...
        /* select/return */
        T rcds[] = DBRecord.select(rwMode, dsel, null);
        if ((rcds == null) || (rcds.length <= 0)) {
            // -- not found
            return null;
        } else
        if (!allowMult && (rcds.length > 1)) {
            // -- throw DBDuplicateFoundException
            String errMsg = "Duplicate key match ("+altFldName+"): "+lookupKey;
            throw new DBDuplicateFoundException(errMsg, rcds);
        } else {
            // -- single record, or duplicates ok
            return rcds;
        }
    ...
    **/
    ...
    /**
    *** Gets the most recent update time for a specified table, of any account/device
    *** @param factory The DBFactory of the table from which the last update time is obtained.
    ...
    **/
        ...
        throws DBException
    {
        ...
        /* get last update from "READ_WRITE" database */
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        // -- the "write" DB has the most accurate 'lastUpdateTime' information
    ...
    **/
    ...
    /**
    *** Returns a new last update time field (in Epoch seconds)
    ...
    **/
    ...
    /**
    *** Gets the last update time (in Epoch seconds)
    ...
    **/
    ...
    /**
    *** Sets the last update time of this record (in Epoch seconds)
    *** @param time The last update time
    *** @return True if this record contained a last update time field to set
    **/
    protected boolean setLastUpdateTime(long time) // setLastUpdateTimeMS
    {
        // -- not all tables will have this field
        ...
        throws DBException
    {
        ...
        try {
            ...
            if (DBAudit.EnableTableAudit()) {
                DBAudit.addDBAudit_insert(insAcctID, insUserID, -1L, this);
            }
    ...
    **/
        ...
        throws DBException
    {
        ...
        try {
            ...
            /* DBAudit update */
            if (DBAudit.EnableTableAudit()) {
                DBAudit.addDBAudit_update(updAcctID, updUserID, -1L, this, updFldSet);
            }
    ...
    **/
    ...
    //    throws SQLException, DBException
    //{
    //    DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE; // depends on prior operation
    //    Statement stmt = null;
    //    DBConnection dbc = null;
    //    try {
    //        dbc  = DBConnection.getDBConnection(rwMode);
    ...
    *** @param fldNames  The set of field names to include
    *** @param inclMask  Mask indicating the type of information to include in the JSON object
    *** @return The JSON Object containing requested fields
    **/
    public JSON._Object toJSON(Set<String> fldNames, int inclMask)
    {
        ...
        /* what to include */
        boolean inclTable = ((inclMask & DBFactory.INCL_TABLE) != 0); // incl table name
        boolean inclBlank = ((inclMask & DBFactory.INCL_BLANK) != 0); // incl fields with blank values
        boolean inclType  = ((inclMask & DBFactory.INCL_TYPE ) != 0); // not used
        boolean inclTitle = ((inclMask & DBFactory.INCL_TITLE) != 0); // not used
        boolean inclKey   = ((inclMask & DBFactory.INCL_KEY  ) != 0); // not used
        ...
        for (int i = 0; i < fld.length; i++) {
            if (fld[i].isPrimaryKey()) {
                ...
                JSON._KeyValue fldKV = DBFactory.writeJSON_DBField(fld[i], inclMask, value);
                flds.addKeyValue(fldKV);
            }
        }
    ...
    /**
    *** Encodes this DBRecord into RTProperties
    *** @param fldNames  The set of field names to include
    *** @return The RTProperties instance
    **/
    public RTProperties toRTProperties(Set<String> fldNames)
    {
        DBRecordKey<gDBR> recKey     = this.getRecordKey();
        String            utableName = recKey.getUntranslatedTableName();
        DBField           fld[]      = recKey.getFields();       // ALL fields
        ...
        /* begin RTProperties object */
        RTProperties rtp = new RTProperties();
        rtp.setName(utableName);

        /* first all primary keys */
        // -- TODO: check fld[i].isPrimaryKey() to sort primary keys to the front
        for (int i = 0; i < fld.length; i++) {
            String fName = fld[i].getName();
            if ((fldNames != null) && !fldNames.contains(fName)) {
                continue;
            }
            String fValS = fldVals.getFieldValueAsString(fName);
            rtp.setString(fName, fValS);
        }
    ...
    **/
    ...
    /**
    *** Clears the temporary properties 
    **/
    public void clearTemporaryProperties()
    {
        if (this.tempProps != null) {
            this.tempProps.clearProperties();
        }
    }
    ...
    *** @return True if this DBRecord has a temporary properties instance defined
    **/
    public void printTemporaryProperties(String header, String pfx)
    {
        ...
        if ((rtp != null) && !rtp.isEmpty()) {
            ...
            String _pfx = (pfx != null)? pfx : "";
            for (Object k : rtp.getPropertyKeys()) {
                Print.sysPrintln(_pfx+StringTools.padRight(k.toString(),' ',20)+" --> "+rtp.getProperty(k,null));
            }
        }
    }
    ...
    /**
    *** Adds the primary keys for this record to the specified JSON Object instance.
    *** @param jsonObj  The JSON object to which the primary keys are added.  
    ***                 If null, a new JSON object will be created and returned.
    *** @return The JSON object to which the primary keys were added
    **/
    public JSON._Object addPrimaryKeyJSON(JSON._Object jsonObj)
    {
        // -- default JSON object
        JSON._Object _jsonObj = (jsonObj != null)? jsonObj : new JSON._Object();
        // -- table name
        String utableName = this.getUntranslatedTableName();
        DBFactory<?> dbFact = DBFactory.getFactoryByName(utableName);
        if (dbFact == null) {
            // -- unlikely, unless this DBRecord doesn't point to any table
            Print.logError("DBFactory not found for table: " + utableName);
            return _jsonObj;
        }
        // -- Primary key fields
        DBField dbKeyFlds[] = dbFact.getKeyFields();
        if (ListTools.isEmpty(dbKeyFlds)) {
            // -- unlikely, every table has at least one key
            Print.logError("Table key fields not found: " + utableName);
            return _jsonObj;
        }
        // -- add primary key field values
        for (DBField dbf : dbKeyFlds) {
            String fldName = dbf.getName();
            Object fldVal = dbf.getFieldValue(this);
            if (fldVal != null) {
                _jsonObj.addKeyValue(fldName,fldVal);
            }
        }
        // -- return JSON object
        return _jsonObj;
