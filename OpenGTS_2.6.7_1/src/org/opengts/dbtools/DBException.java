    ...
    extends Exception
{
    ...
    **/
    ...
    /**
    *** Returns true if the specified exception is a MySQL PacketTooBigException
    *** IE. "com.mysql.jdbc.PacketTooBigException"
    *** @param th  The Exception/Throwable to test
    *** @return True if the cause is a MySQL PacketTooBigException
    **/
    public boolean isPacketTooBigException()
    {
        // Possible exceptions/messages:
        //  com.mysql.jdbc.PacketTooBigException
        // Notes:
        //  - PacketTooBigException may be due to having "saveRawDataPackets" set true and having large event packets
        //  - The occurance of this error can be mitigated my increasing the "max_allowed_packet" property in "my.cnf"
        Throwable th = this.getCause();
        if (th == null) {
            return false;
        } else {
            String cn  = StringTools.className(th);     // com.mysql.jdbc.PacketTooBigException
            String msg = th.getMessage().toLowerCase(); // "Packet for query is too large ..."
            if (( cn.indexOf("PacketTooBigException"        ) >= 0) ||    // exception class name
                (msg.indexOf("packet for query is too large") >= 0)   ) { // lower-case compare
                return true;
            } else {
                return false;
            }
