...
public abstract class DBRecordKey<gDBR extends DBRecord<gDBR>>
{
    ...
    *** Clone this instance
    *** Notes:
    ***     - The "record" instance is not cloned
    ***     - The "missingDelegate" instance is not cloned
    **/
    @SuppressWarnings("unchecked")
    public DBRecordKey<gDBR> clone() // [2.6.6-B36]
    {
        try {
            DBFieldValues fldVals = this.getFieldValues(); // non null
            DBFieldValues keyVals = this.getKeyValues(); // non null
            Class<DBRecordKey<gDBR>> dbkClass = (Class<DBRecordKey<gDBR>>)this.getClass(); // DBRecordKey<gDBR extends DBRecord<gDBR>>
            DBRecordKey<gDBR> dbk = dbkClass.newInstance();
            dbk.taggedFields = (this.taggedFields != null)? new HashSet<String>(this.taggedFields) : null;
            dbk.ignOnInsert  = this.ignOnInsert;
            dbk.ignOnUpdate  = this.ignOnUpdate;
            dbk.fieldValues  = fldVals.clone(dbk); // non null
            dbk.fieldValues.clearFieldValues(); // clear non-primary-key fields
            dbk.keyValues    = (keyVals == fldVals)? dbk.fieldValues : keyVals.clone(dbk); // non null
            return dbk;
        } catch (InstantiationException ie) {
            Print.logException("Unable to clone DBRecordKey", ie);
            return null; // may cause more errors (ie NPE)
        } catch (IllegalAccessException iae) {
            Print.logException("Unable to clone DBRecordKey", iae);
            return null; // may cause more errors (ie NPE)
        }
    }
    ...
    /**
    *** Sets the delegate to use for missing fields
    **/
    public void setMissingDelegate(DBFieldValues missingDel)
    {
        this.missingDelegate = missingDel;
    }

    /**
    *** Sets the delegate to use for missing fields
    **/
    public DBFieldValues getMissingDelegate()
    {
        return this.missingDelegate;
    }
    ...
    **/
    ...
    /**
    *** @see DBFactory#hasMissingFields()
    **/
    public boolean hasMissingFields()
    {
        return this.getFactory().hasMissingFields();
    }

    /**
    *** Returns true if the specified field exists as a "missing field" the DBFactory
    ...
    **/
    ...
    {
        return this.getFactory().isMissingField(fldName);
    }

    // ------------------------------------------------------------------------

    /**
    *** @see DBFactory#getExistingFields()
    **/
    public DBField[] getExistingFields()
    {
        return this.getFactory().getExistingFields(); // not-null, may return 'getFields()'
    }
        ...
        if (this.keyValues == null) {
            ...
            try {
                if (DBField.AllowUpdateKeyFields()) {
                    DBFieldValues keyDelegate = this.getFieldValues(); // getKeyValues
                    this.keyValues = new DBFieldValues(this, keyDelegate, null); // DataDelegate
    ...
    **/
    ...
    /**
    *** Sets the DBFieldValue instance for this DBRecordKey.
    *** - DBFieldValue must not be null
    *** - DBFieldValue must not have been initialized with a DBRecordKey
    *** - DBFieldValue must not have a MissingDelegate
    *** - This instance method "getFieldValues()" must not have already been called
    **/
        ...
        throws DBException
    {
        if (fldVals == null) {
            // -- cannot be null
            throw new DBException("DBFieldValues is null");
        } else
        if (fldVals.hasRecordKey()) {
            // -- DBFieldValues has already been initialized with another DBRecordKey
            throw new DBException("Specified DBFieldValues already has a DBRecordKey");
        } else
        if (fldVals.hasMissingDelegate()) {
            // -- unlikely if hasRecordKey is false
            throw new DBException("Specified DBFieldValues has a defined MissingDelegate");
        } else
        if (this.fieldValues != null) {
            // -- this instance has already initialized its DBFieldValues
            throw new DBException("'<DBRecordKey>.getFieldValues()' has already been called");
        } else {
            fldVals.setRecordKey(this,null,null); // throws DBException
            this.fieldValues = fldVals;
            this.keyValues   = null; // likely already null
            this.getKeyValues();
        }
    }
    ...
    **/
    public DBFieldValues getFieldValues()
    {
        if (this.fieldValues == null) {
            try {
                DBFieldValues missingDelegate = this.getMissingDelegate(); // may be null
                this.fieldValues = new DBFieldValues(this, null, missingDelegate); // MissingDelegate
                this.fieldValues.setName(this.getUntranslatedTableName()+"_Fields");
            } catch (DBException dbe) {
                // -- will not occur
                Print.logException("Unexpected error", dbe);
            }
    ...
    **/
    public boolean exists(DBReadWriteMode rwMode)
        throws DBException
    {
        try {
            return this._exists(rwMode, null, DBWhere.KEY_FULL); // full primary key lookup
            ...
            throw new DBException("Record existence " + tn + "='" + this + "'", sqe);
        }
    }
    // --
    ...
    **/
    public boolean exists(DBReadWriteMode rwMode, int whereKeyType)
        throws DBException
    {
        try {
            return this._exists(rwMode, null, whereKeyType); // primary key lookup
    ...
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        return this.exists(rwMode, whereKeyType);
    }
    ...
    **/
    public boolean exists(DBReadWriteMode rwMode, boolean fullKeyOnly)
        throws DBException
    {
        try {
    ...
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        return this.exists(rwMode, fullKeyOnly);
    }
    ...
    **/
    public boolean altIndexExists(DBReadWriteMode rwMode, String indexName)
        throws DBException
    {
        try {
            if (indexName == null) { indexName = DBProvider.DEFAULT_ALT_INDEX_NAME; }
            return this._exists(rwMode, indexName, DBWhere.KEY_FULL);  // alternate key lookup
        } catch (SQLException sqe) {
            String tn = this.getUntranslatedTableName();
            throw new DBException("Record existence " + tn + "='" + this + "'", sqe);
        }
    ...
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        return this.altIndexExists(rwMode, indexName);
    }
    ...
    **/
    protected boolean _exists(DBReadWriteMode rwMode, String altIndexName, int whereKeyType)
    ...
    *** @param whereKeyType WHERE key type: DBWhere.KEY_FULL, DBWhere.KEY_PARTIAL_FIRST, DBWhere.KEY_PARTIAL_ALL
    **/
        ...
        throws SQLException, DBException
    {
        ...
        /* DBAudit delete */
        if (DBAudit.EnableTableAudit() && (whereKeyType == DBWhere.KEY_FULL)) {
            String delAcctID = DBRecord.GetCurrentAccount();
            String delUserID = DBRecord.GetCurrentUser();
            DBAudit.addDBAudit_delete(delAcctID, delUserID, -1L, this);
        }
    ...
    **/
    ...
    /* package */ gDBR _getDBRecordVar()
    {
        return this.record; // may be null
    }

    @SuppressWarnings("unchecked")
    /* package */ void _setDBRecordVar(DBRecord<gDBR> rcd)
    {
    ...
    **/
    ...
    {
        try {
            return this._getDBRecord();
        } catch (DBException dbe) {
            // -- internal error, unlikely to occur, errors already displayed
    ...
    **/
    public gDBR getDBRecord(boolean reload, String... fldNames)
    {
        ...
        try {
            DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
            return this._getDBRecord(reload, rwMode, fldNames);
        } catch (DBException dbe) {
            Print.logError("'getDBRecord' error: " + dbe);
            return null;
        }
    }
    ...
    /**
    *** Gets the DBRecord associated with this key.
    *** Creates an empty record if not yet defined.
    *** @return The DBRecord associated with this key
    **/
    public gDBR _getDBRecord()
        throws DBException
    {
        if (this.record == null) {
            try {
                ...
                Print.logStackTrace("Implementation error - unable to create DB record", dbe);
                throw dbe;
            }
        }
        return this.record;
    ...
    *** @param reload   If the record should be reloaded before it is returned
    *** @param fldNames The specific field-names to reload (null to reload all)
    *** @return The DBRecord associated with this key (does not return null)
    **/
    public gDBR _getDBRecord(boolean reload, DBReadWriteMode rwMode, String... fldNames)
        throws DBException
    {

        /* create record */
        gDBR rcd = this._getDBRecord(); // may throw DBException

        /* reload */
        if (reload) {
            // -- 'reload' is ignored if key does not exist
            rcd._reload(rwMode, fldNames); // may throw DBException
        }
    ...
    *** @return The string representation of this object
    **/
    public String toString(String keySep, String nullVal) 
    {
        DBField kf[] = this.getKeyFields();
        if (kf.length == 0) { 
            return (nullVal != null)? nullVal : "<null>"; 
        } else {
            DBFieldValues fv = this.getFieldValues();
            StringBuffer sb = new StringBuffer();
            for (int i = 0; i < kf.length; i++) {
                if (i > 0) { sb.append(keySep); }
