...
//     -Fixed 'dumpTable' to convert BLOB fields to hex Strings [2.6.7-B06g]
    ...
    implements DBRecordListener<gDBR>
{
  ...
  //public static final int MSQL_ERR_UNKNOWN_COLUMN     =     ?;  // does not appear to be supported by MSQL 
    ...
    **/
    ...
    { // -- getFactory(String tn), getDBFactory(String tn), ...
    ...
    **/
    ...
    private volatile boolean                        fieldArrayReady     = false;
    private DBField                                 fieldArray[]        = null;     // optimization
    private OrderedMap<String,DBField>              existingMap         = null;
    private DBField                                 existingArray[]     = null;     // optimization
    ...
    **/
        ...
        boolean editable, boolean viewable) 
    {
        ...
        if (DBFactory.factoryHandler != null) {
            try {
                // -- this adds any additional optional fields that may enabled by configuration
        ...
        this.existingMap = new OrderedMap<String,DBField>();

        /* table exists? */
        // -- eliminates error stacktraces issued by "hasExistingColumn" when table doesn't exist
        boolean tableExists;
        try {
            tableExists = this.tableExists(); // exists
            if (!tableExists) {
                Print.logError("Table does not exist: " + this.utableName);
            }
        } catch (DBException dbe) {
            Print.logError("Error checking for table existance: " + this.utableName + " ["+dbe.getMessage()+"]");
            tableExists = false;
        }

        /* accumulate primary/alternate key fields */
        OrderedMap<String,DBField> existsMap = new OrderedMap<String,DBField>();
        ...
        for (DBField fld : preList) {
            ...
            if (this.fieldMap.containsKey(fn)) {
                ...
                //Print.logStackTrace("ERROR: Already defined: " + this.utableName + "." + fn);
                continue;
            }
            ...
            // -- exists?
            if (!tableExists) {
                // -- table does not exist, thus no existing fields
                hasMissing = true;
            } else
            if (this.hasExistingColumn(fn)) {
                // -- defined and exists in table
                existsMap.put(fn,fld);
            } else {
                // -- defined, but missing from table
                Print.logDebug("Column is missing from table: " + this.utableName + "." + fn);
                hasMissing = true;
            }
        ...
        } // iterate through table fields/columns
        ...
        if (hasMissing) {
            ...
            this.existingMap   = existsMap;
            this.existingArray = this.existingMap.valueArray(DBField.class); 
        } else {
            // -- no missing fields
            this.existingMap   = null; // nothing is missing, no need for this
            this.existingArray = null; // nothing is missing, no need for this
        }
        ...
        /* table audit? */
        this.auditTable = DBAudit.EnableTableAudit(this.utableName);
        if (this.auditTable) {
            Print.logInfo("DBAudit enabled for table: " + this.utableName);
        }
    ...
    **/
    ...
    /**
    *** Returns true if this table should be audited for changes (insert/update/delete) via DBAudit
    *** @return True if this table should be audited, false otherwise
    **/
        ...
        throws DBException
    {
        synchronized (this.existingFieldLock) {
            if (update || (this.existingFieldMap == null)) {
                DBField fld[] = this.getExistingColumns(update); // throws DBException
                if (fld != null) {
    ...
    **/
    public boolean hasExistingColumn(String name) // see also "hasField(String)"
    {
        try {
          ...
          //Print.logException("Error hecking for existing column: " + name, dbe);
          //Print.logError("Error checking for existing column: "+utableName+"."+name+ " ["+dbe.getMessage()+"]");
            return false;
        }
    }
    ...
    **/
    public boolean validateColumns(int validateMask, boolean refreshColumns)
    {
        ...
        for (int i = 0; i < colDefined.length; i++) {
            ...
            if (colDefined[i].isUTF8MB4() && !existingField.isUTF8MB4()) { // [2.6.7-B45s]
                // -- Defined column is UTF8MB4, but actual column is not
                if (!DBField.IgnoreColumnError(utableName,columnName)) {
                    if (displayErrors) {
                        Print.logInfo("ERROR - " + utableName + ": Column '" + columnName + "' UTF8MB4 mismatch '" + definedCS + "' != '" + actualCS + "'");
                    }
                    typeMismatchColumns.add(colDefined[i]);
                    columnsOK = false;
                } else {
                    if (displayErrors) {
                        Print.logInfo("WARNING - " + utableName + ": Column '" + columnName + "' UTF8MB4 mismatch '" + definedCS + "' != '" + actualCS + "' (ignored)");
                    }
                }
            } else
            if (!colDefined[i].isUTF8MB4() && existingField.isUTF8MB4()) { // [2.6.7-B45s]
                // -- Actual column is UTF8MB4, but Defined column is not
                if (checkColumnEncoding) {
                    if (alterColumnTypes) {
                        if (!DBField.IgnoreColumnError(utableName,columnName)) {
                            if (displayErrors) {
                        ...
                        if (displayErrors) {
                            Print.logInfo("WARNING - " + utableName + ": Column '" + columnName + "' UTF8MB4 mismatch '" + definedCS + "' != '" + actualCS + "'");
                        }
                    }
                }
            }
    ...
    **/
    private String _columnInfo(int i, DBField f, boolean isDefined)
    {
        ...
        if (f.isUTF8MB4()) { type += " utf8mb4"; } // [2.6.7-B45s]
        ...
        if (!this.fieldArrayReady) {
            ...
            synchronized (this.fieldMap) {
                if (!this.fieldArrayReady) { // test again inside lock
                    // -- defined fields
                    this.fieldArray = this.fieldMap.valueArray(DBField.class); // DBField[]
                    // -- mark as initialized
    ...
    *** @return True if any fields are missing
    **/
    public boolean hasMissingFields()
    {
        // -- 'this.existingMap' will be null if nothing is missing
        return (this.existingMap != null)? true : false;
    }

    /**
    *** Returns true if the specified field exists as a "missing field" this DBFactory
    *** @param name  A field name to test
    *** @return True if the specified field is a "missing field", false otherwise
    **/
    public boolean isMissingField(String name)
    {
        if (!this.hasField(name)) {
            // -- field is not even defined, thus is missing
            return true;
        } else
        if (this.existingMap == null) {
            // -- nothing is missing
            return false;
        } else {
            // -- missing iff not in 'this.existingMap'
            String n = this.getMappedFieldName(name);
            return ((n == null) || !this.existingMap.containsKey(n))? true : false;
        }
    }
    ...
    /**
    *** Returns an array of all defined and existing DBFields in this DBFactory
    *** @return An array of defined and existing DBFields
    **/
    public DBField[] getExistingFields()
    {
        if (this.existingArray == null) {
            // -- nothing is missing iff 'this.existingArray' is null
            return this.getFields(); // all fields
        } else {
            return this.existingArray;
        }
    }
    ...
    **/
        ...
        throws DBException 
    {
        ...
        if (rcdKey != null) {
            ...
            gDBR rcd = (gDBR)rcdKey._getDBRecord();
        ...
        throws DBException
    {
        ...
        try {
            ...
            if (errCode == SQLERR_UNKNOWN_DATABASE) { // MySQL: ?
                String dbName = DBProvider.getDBName(rwMode);
    ...
    **/
    public long getMySQLTableStatus(DBReadWriteMode rwMode)
        throws DBException
    {
    ...
    **/
    public boolean validateTable(boolean inclWarn)
    {
        ...
        try {
            ...
            try {
                DBSelect<gDBR> dsel = new DBSelect<gDBR>(this);
                dbc  = DBConnection.getDBConnection(DBReadWriteMode.READ_ONLY);
    ...
    **/
        ...
        throws SQLException, DBException
    {
        ...
        DBReadWriteMode rwMode = DBReadWriteMode.ALTER; // DBReadWriteMode.READ_WRITE
        ...
        if (dbp.getID() == DBProvider.DB_MYSQL) {
            ...
            //  - Show current table column character sets:
            //      mysql> select TABLE_SCHEMA,TABLE_NAME,COLUMN_NAME,CHARACTER_SET_NAME,COLLATION_NAME FROM information_schema.`COLUMNS` WHERE table_schema="<DBNAME>" AND table_name="<TABLENAME>";
            ...
            for (;ndx < cols.length; ndx++){
                ...
                if (colMap.containsKey(colName)) {
                    ...
                    //   MySQL: ALTER TABLE <TableName> MODIFY <column> <column> <type> [CHARACTER SET utf8 [COLLATE utf8_general_ci]]
                    sb.append(" CHANGE "); // sb.append(" MODIFY ");
                    sb.append(colName).append(" ").append(col.getFieldDefinition());
                    ...
                    // -- character set
                    if (col.isUTF8MB4()) { // [2.6.7-B45s]
                        sb.append(" CHARACTER SET utf8mb4"); // [2.6.7-B45s]
                        sb.append(" COLLATE utf8mb4_bin"); // [2.6.7-B45s]
                    } else
                    if (col.isUTF8()) {
                        sb.append(" CHARACTER SET utf8");
                        sb.append(" COLLATE utf8_general_ci"); // [2.6.7-B45s]
                    }
                    // -- 
        ...
        if (sb.length() > 0) {
            ...
            try {
                ...
                long deltaMS = System.currentTimeMillis() - startTS;
                Print.logInfo("... SQL 'ALTER TABLE ...' completed ["+deltaMS+" ms]");
            } finally {
                DBConnection.release(dbc);
            }
        }
    ...
    **/
    protected void _dropColumn(DBField col)
        throws SQLException, DBException
    {
        DBReadWriteMode rwMode = DBReadWriteMode.ALTER;
        String colName = col.getName();
        Map<String,DBField> colMap = this.getExistingColumnMap(false);
    ...
    *** @throws DBException   If a database error occurs
    **/
    public long dumpTable(File toFile, boolean append)
        throws DBException
    {
        return this.dumpTable(toFile, append, /*DBSelect*/null, /*String[]*/null);
    }
    ...
    *** @throws DBException   If a database error occurs
    **/
    public long dumpTable(File toFile, boolean append, String where)
        throws DBException
    {
        ...
        return this.dumpTable(toFile, append, dbSel, /*String[]*/null);
    }
    ...
    *** @throws DBException   If a database error occurs
    **/
    public long dumpTable(File toFile, boolean append, DBSelect<gDBR> dsel)
        throws DBException
    {
        return this.dumpTable(toFile, append, dsel, /*String[]*/null);
    }
    ...
    *** @throws DBException   If a database error occurs
    **/
    public long dumpTable(File toFile, boolean append, DBSelect<gDBR> dsel, String fldn[])
        throws DBException
    {
        ...
        /* dump to file */
        Writer dumpOutStream = null;
        boolean closeStream = true;
        long dumpedRcdCount = 0L;
        try {
            ...
            /* dump */
            dumpedRcdCount = this._dumpTable(dumpOutStream, inclHeader, dsel, fldn, outputFmt);
    ...
    **/
    public long _dumpTable(
        Writer dumpOutStream, boolean inclHeader, 
        DBSelect<gDBR> dsel, String fldn[], int outFmt)
        throws DBException
    { // rs
        ...
        try {
            ...
            if (inclHeader) {
                ...
                dumpOutStream.write(sbData.toString()); // must be a full string including "\n"
            }
            sbData.setLength(0);
            ...
            while (rs.next()) {
                ...
                if (outFmt == DUMP_FORMAT_CSV) {
                    ...
                    for (int i = 0; i < fields.length; i++) {
                        ...
                        String v;
                        try {
                            Object r = fields[i].getResultSetValue(rs); //throws SQLException
                            v = DBFieldValues.toStringValue(r); // fix [2.6.7-B06g]
                        } catch (SQLException sqe) { // -- likely "Column 'xxxx' not found"
                            v = "";
                        }
                ...
                if (outFmt == DUMP_FORMAT_XML) {
                    ...
                    for (int i = 0; i < fields.length; i++) {
                        ...
                        try {
                            Object r = fields[i].getResultSetValue(rs); //throws SQLException
                            v = DBFieldValues.toStringValue(r);
                        } catch (SQLException sqe) { // -- likely "Column 'xxxx' not found"
                            v = "";
                        }
                        DBFactory.writeXML_DBField(sbData, 2*indent, fields[i], false/*inclInfo*/, v);
                    }
        ...
        throws DBException
    {
        ...
        try {
            ...
            for (;;rowNumber++) {
                ...
                String failReason = this._loadInsertRecord(oldFieldNames, rowValues,
    ...
    **/
    public String _loadInsertRecord(String oldFieldNames[], String rowValues[], 
        DBLoadValidator validator, boolean insertRecord, boolean overwriteExisting)
        throws DBException, SQLException, IOException
    {
        // -- returns:
        // -    "v" = failed validation
        // -    "i" = insertRecord is false
        // -    "e" = exists and overwriteExisting is false
        ...
        DBRecord<?>    dbRcd     = dbRcdKey._getDBRecord();
    ...
    **/
        ...
        throws DBException, SQLException, IOException
    {
        ...
        // -- now check for record existence 
    ...
    **/
    public String getIndexType()
    {
        ...
        if (this.indexType == null) {
            try {
                this.getMySQLTableStatus(rwMode);
            } catch (DBException dbe) {
                // continue below
            }
    ...
    *** @throws DBException   If a database error occurs
    **/
    public long getRecordCount(DBReadWriteMode rwMode, String where, boolean actualCount)
        throws DBException
    {
        ...
        if (!StringTools.isBlank(where)) {
            ...
            return DBRecord.getRecordCount(rwMode, new DBSelect<gDBR>(this, where)); // "unchecked call"
        } else
        if (actualCount) {
            // -- possibly non-efficient for InnoDB
            return DBRecord.getRecordCount(rwMode, new DBSelect<gDBR>(this, "")); // "unchecked call"
        } else {
            // -- use table status, may be estimated count for InnoDB
            return this.getMySQLTableStatus(rwMode);
        }
    ...
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        return this.getRecordCount(rwMode, where, actualCount);
    }
    ...
    *** @throws DBException   If a database error occurs
    **/
    public long getRecordCount(DBReadWriteMode rwMode, DBWhere where, boolean actualCount)
        throws DBException
    {
    ...
    **/
    ...
    /**
     *** Create JSON object of specified fields
     *** @param fld The DBFields to include
     *** @param fldVals The field column values
     *** @return The StringBuffer
     **/
    ...
    /**
    *** Create an JSON KeyValue instance of specified DBField
    *** @param fld      The DBField to create a tag for
    *** @param inclInfo True if field datatype/description should be included
    *** @param value    The value of the tag element
    *** @return The StringBuffer comtaining the XML
