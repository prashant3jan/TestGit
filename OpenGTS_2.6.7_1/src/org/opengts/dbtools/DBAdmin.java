...
public class DBAdmin
{
        ...
        String utableName, boolean existing)
    {
        ...
        for (Iterator<String> i = factMap.keyIterator(); i.hasNext();) {
            ...
            for (int c = 0; c < colDef.length; c++) {
                ...
                boolean isUTF8MB4 = colDef[c].isUTF8MB4(); // [2.6.7-B45s]
                boolean isAuto    = colDef[c].isAutoIncrement();
                boolean isUTF8    = colDef[c].isUTF8();
                if (isAuto || isUTF8 || isUTF8MB4) {
                    sqlType += " [";
                    // -- auto
                    if (isAuto) {
                        sqlType += "auto"; 
                    ...
                    // -- character set
                    if (isUTF8MB4) {  // [2.6.7-B45s]
                        if (isAuto) { sqlType += ","; }
                        sqlType += "utf8mb4";   // [2.6.7-B45s]
                    } else
                    if (isUTF8) {
                        if (isAuto) { sqlType += ","; }
                        sqlType += "utf8"; 
    ...
    **/
    ...
    {
        DBAdmin._printTableSchema(docWidth, header, utableName, true/*existing/actual*/);
    }
    ...
    **/
    public static File getDumpDirectory()
    {
        ...
        String dirName = DEFAULT_DIRECTORY + File.separator + DBProvider.getDBName(rwMode);
    ...
    **/
    ...
    public  static final String ARG_RWMODE[]        = new String[] { "rwMode"    , "mode"    };
    ...
    public  static final String ARG_DB_PROPS[]      = new String[] { "dbProperties", "dbProps" };
  ...
  //public  static final String ARG_RELOAD[]        = new String[] { "reload"        };
    ...
    **/
    ...
    public static <T extends DBRecord<T>> DBAdminExec execCommands() // may not build on Java6
    {
        ...
        /* read/write mode */
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        if (RTConfig.hasProperty(ARG_RWMODE)) {
            String rwm = RTConfig.getString(ARG_RWMODE, "");
            if (!StringTools.isBlank(rwm) && rwm.equalsIgnoreCase("read")) {
                rwMode = DBReadWriteMode.READ_ONLY;
            }
        }

        /* DEBUG: show DB properties for specified R/W mode */
        if (RTConfig.getBoolean(ARG_DB_PROPS,false)) {
            // --
            String sfx = rwMode.keySuffix();
            Print.sysPrintln(RTKey.DB_NAME+sfx                 +"="+RTConfig.getString(DBConnection.AppendRWMode(RTKey.DB_NAME,rwMode)));
            Print.sysPrintln(RTKey.DB_DATASOURCE_CLASS+sfx     +"="+RTConfig.getString(DBConnection.AppendRWMode(RTKey.DB_DATASOURCE_CLASS,rwMode)));
            Print.sysPrintln(RTKey.DB_DATASOURCE_MAX_ACTIVE+sfx+"="+RTConfig.getString(DBConnection.AppendRWMode(RTKey.DB_DATASOURCE_MAX_ACTIVE,rwMode))); // Long
            Print.sysPrintln(RTKey.DB_DATASOURCE_MAX_IDLE+sfx  +"="+RTConfig.getString(DBConnection.AppendRWMode(RTKey.DB_DATASOURCE_MAX_IDLE,rwMode))); // Long
            Print.sysPrintln(RTKey.DB_DATASOURCE_MAX_WAIT+sfx  +"="+RTConfig.getString(DBConnection.AppendRWMode(RTKey.DB_DATASOURCE_MAX_WAIT,rwMode))); // Long
            Print.sysPrintln(RTKey.DB_URL+sfx                  +"="+RTConfig.getString(DBConnection.AppendRWMode(RTKey.DB_URL,rwMode)));
            Print.sysPrintln(RTKey.DB_URL_DB+sfx               +"="+RTConfig.getString(DBConnection.AppendRWMode(RTKey.DB_URL_DB,rwMode)));
            Print.sysPrintln(RTKey.DB_HOST+sfx                 +"="+RTConfig.getString(DBConnection.AppendRWMode(RTKey.DB_HOST,rwMode)));
            Print.sysPrintln(RTKey.DB_PORT+sfx                 +"="+RTConfig.getString(DBConnection.AppendRWMode(RTKey.DB_PORT,rwMode)));
            Print.sysPrintln(RTKey.DB_USER+sfx                 +"="+RTConfig.getString(DBConnection.AppendRWMode(RTKey.DB_USER,rwMode)));
            Print.sysPrintln(RTKey.DB_PASS+sfx                 +"="+RTConfig.getString(DBConnection.AppendRWMode(RTKey.DB_PASS,rwMode)));
            // --
            Print.sysPrintln(RTKey.DB_DBCONNECTION_POOL        +"="+RTConfig.getBoolean(RTKey.DB_DBCONNECTION_POOL));
            Print.sysPrintln(RTKey.DB_PROVIDER                 +"="+RTConfig.getString( RTKey.DB_PROVIDER));
            Print.sysPrintln(RTKey.DB_UTF8                     +"="+RTConfig.getBoolean(RTKey.DB_UTF8));
            Print.sysPrintln(RTKey.DB_MYSQL_TBLEXIST_SEL_COUNT +"="+RTConfig.getBoolean(RTKey.DB_MYSQL_TBLEXIST_SEL_COUNT));
            Print.sysPrintln(RTKey.DB_TABLE_NAME_PREFIX        +"="+RTConfig.getString( RTKey.DB_TABLE_NAME_PREFIX));
            Print.sysPrintln(RTKey.DB_ALLOW_UPDATE_KEY_FIELDS  +"="+RTConfig.getBoolean(RTKey.DB_ALLOW_UPDATE_KEY_FIELDS));
            Print.sysPrintln(RTKey.DB_INCLUDE_LAST_UPDATE_ACCT +"="+RTConfig.getBoolean(RTKey.DB_INCLUDE_LAST_UPDATE_ACCT));
            Print.sysPrintln(RTKey.DB_INCLUDE_LAST_UPDATE_USER +"="+RTConfig.getBoolean(RTKey.DB_INCLUDE_LAST_UPDATE_USER));
            Print.sysPrintln(RTKey.DB_EMAIL_CONNECTION_ERRORS  +"="+RTConfig.getBoolean(RTKey.DB_EMAIL_CONNECTION_ERRORS));
            Print.sysPrintln(RTKey.DB_SHOW_SQL                 +"="+RTConfig.getBoolean(RTKey.DB_SHOW_SQL));
            Print.sysPrintln(RTKey.DB_SHOW_CONNECTIONS         +"="+RTConfig.getBoolean(RTKey.DB_SHOW_CONNECTIONS));
            Print.sysPrintln(RTKey.DB_TABLE_LOCKING            +"="+RTConfig.getBoolean(RTKey.DB_TABLE_LOCKING));
            Print.sysPrintln(RTKey.DB_INACTIVE_TIMEOUT         +"="+RTConfig.getLong(   RTKey.DB_INACTIVE_TIMEOUT));
            // --
            return DBAdminExec.EXIT;
        }

        /* if specified, the argument specified for "-db" MUST match 'DBProvider.getDBName(rwMode)' */
        if (RTConfig.hasProperty(ARG_DB)) {
            ...
            if (!dbName.equals(dbArg)) {
                Print.logError("Argument DB ("+dbArg+") does not match config DB["+rwMode+"]: "+dbName);
        ...
        if (RTConfig.getBoolean(ARG_CREATE_DB, false)) {
            ...
            try {
                DBProvider.createDatabase(rwMode,rootUser,rootPass);
                ...
                if (sqlMsg.indexOf("already exists" ) >= 0) { // PostgreSQL: ?
                    ...
                    Print.logSQLError("DB create error [" + DBProvider.getDBUri(rwMode,false) + "]", sqe);
                    return DBAdminExec.ERROR;
                }
            } catch (DBException dbe) {
                Print.logException("DB create error [" + DBProvider.getDBUri(rwMode,false) + "]", dbe);
            }
        }
        ...
        if (RTConfig.getBoolean(ARG_GRANT, false)) {
            ...
            String rootPass = RTConfig.getString(ARG_ROOT_PASS, null);
            try {
                DBProvider.grantDBUser(rootUser, rootPass); // DBReadWriteMode.GRANT
            } catch (SQLException sqe) {
                Print.logSQLError("DB grant error [" + DBProvider.getDBUri(rwMode,false) + "]", sqe);
                return DBAdminExec.ERROR;
            } catch (DBException dbe) {
                Print.logException("DB grant error [" + DBProvider.getDBUri(rwMode,false) + "]", dbe);
            }
        }
        ...
        /* connect to db */
        DBConnection dbc = DBConnection.getDBConnection(DBReadWriteMode.READ_WRITE);
        try {
            try {
                dbc.getConnection();
            } finally {
            ...
            String uri = DBProvider.getDBUri(dbc.getReadWriteMode(),true); // includes dbName
        ...
        */
        ...
        if (dumpTable != null) {
            ...
            // -- dump tables
            Vector<File> emptyDumpFiles = new Vector<File>();
            for (Iterator<String> i = tableList.iterator(); i.hasNext(); ) {
                ...
                // -- header
                Print.logInfo("Dumping "+StringTools.padRight(utableName,' ',16)+" ==> "+dumpFile+((where!=null)?" ["+where+"]":""));
                // -- DBFactory for table
                @SuppressWarnings("unchecked")
                ...
                if (fact != null) {
                    ...
                    try {
                        ...
                        rcdCnt = fact.dumpTable(dumpFile, append, dbSel); // "unchecked conversion"
                        Print.logInfo("  ... dumped " + rcdCnt + " records");
                        if (rcdCnt == 0L) {
                            emptyDumpFiles.add(dumpFile);
                        }
                    } catch (DBException dbe) {
            ...
            // -- has empty dump files?
            if (!ListTools.isEmpty(emptyDumpFiles)) {
                Print.logInfo("");
                Print.logInfo("The following dumped table files were empty:");
                StringBuffer sb = new StringBuffer();
                for (File edf : emptyDumpFiles) {
                    if (sb.length() != 0) { sb.append(" "); }
                    sb.append(edf.getName());
                }
                Print.logInfo(sb.toString()); // StringTools.join(emptyDumpFiles," "));
                Print.logInfo("");
            }
        }
