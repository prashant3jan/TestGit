...
public class DBConnection
{
    ...
    /**
    *** Appends the specified mode ("%read","%write") to the property.
    **/
    public static String[] AppendRWMode(String propKey, DBReadWriteMode rwMode)
    {
        rwMode = DBReadWriteMode.getDefaultReadWriteMode(rwMode);
        // -- normalize property key
        String pk = StringTools.trim(propKey);
        int p = pk.indexOf(DBReadWriteMode.RW_PREFIX_CHAR);
        if (p >= 0) {
            pk = pk.substring(0,p);
        }
        // -- check for no property key specified
        if (StringTools.isBlank(pk)) {
            return new String[0];
        }
        // -- return { "propKey%RWMode", "propKey" }
        return new String[] { (pk + rwMode.keySuffix()), pk };
    }
    ...
    **/
    ...
    private static DataSource   DBDataSource_read       = null;
    private static DataSource   DBDataSource_write      = null;

    private static DataSource GetDataSource(DBReadWriteMode rwMode)
    {
        rwMode = DBReadWriteMode.getDefaultReadWriteMode(rwMode);
        if (rwMode.isWrite()) {
            return DBDataSource_write;
        } else
        if (DBDataSource_read == null) {
            return DBDataSource_write;
        } else {
            return DBDataSource_read;
        }
    }

    private static DataSource InitDataSource(DBReadWriteMode rwMode)
    {
        ...
        rwMode = DBReadWriteMode.getDefaultReadWriteMode(rwMode);
        String dataSrcClass = RTConfig.getString(AppendRWMode(RTKey.DB_DATASOURCE_CLASS,rwMode),null);
        if (!StringTools.isBlank(dataSrcClass)) {
            Print.logInfo("Attempting to load DataSource("+rwMode+"): " + dataSrcClass);
                ...
                for (int i = 0; i < bdsClassName.length; i++) {
                    try {
                        ...
                        if (th.getMessage().indexOf("GenericObjectPool") >= 0) {
                            ...
                            Print.logException("DataSource("+rwMode+"): Apache 'Pool' Component missing! (see http://commons.apache.org/pool/)", th);
                        } else
                        if (RTConfig.isDebugMode()) {
                            //Print.logException("Error loading '"+bdsClassName[i]+"'", th);
                            Print.logInfo("DataSource("+rwMode+"): Error loading '"+bdsClassName[i]+"'");
            ...
            if (bdsClass != null) {
                ...
                String urlStr     = DBProvider.getDBUri(rwMode, true);
                String user       = DBProvider.getDBUsername(rwMode);
                String password   = DBProvider.getDBPassword(rwMode);
                int    maxActive  = RTConfig.getInt( AppendRWMode(RTKey.DB_DATASOURCE_MAX_ACTIVE,rwMode),   100);
                int    maxIdle    = RTConfig.getInt( AppendRWMode(RTKey.DB_DATASOURCE_MAX_IDLE  ,rwMode),    30);
                long   maxWait    = RTConfig.getLong(AppendRWMode(RTKey.DB_DATASOURCE_MAX_WAIT  ,rwMode), 10000L);
                try {
                    ...
                    Print.logWarn("Unable to load DataSource("+rwMode+"): " + th);
                }
            } else {
                Print.logWarn("DataSource("+rwMode+") class not found: " + dataSrcClass);
            }
    ...
    static {
        ...
        /* DataSource configuration */
        DBConnection.DBDataSource_read  = DBConnection.InitDataSource(DBReadWriteMode.READ_ONLY );
        DBConnection.DBDataSource_write = DBConnection.InitDataSource(DBReadWriteMode.READ_WRITE);

        /* display message */
        if (DBConnection.DBDataSource_write != null) {
            Print.logInfo("Loaded DataSource: %s ...", StringTools.className(DBConnection.DBDataSource_write));
        } else {
            Print.logDebug("DriverManager will be used for new connections");
            DBReadWriteMode rwMode = DBReadWriteMode.getDefaultReadWriteMode(null);
            String keySfx = rwMode.keySuffix();
            Print.logDebug(RTKey.DB_PROVIDER    +keySfx+" = " + RTConfig.getString(AppendRWMode(RTKey.DB_PROVIDER,rwMode)));
            Print.logDebug(RTKey.DB_URL         +keySfx+" = " + RTConfig.getString(AppendRWMode(RTKey.DB_URL     ,rwMode)));
            Print.logDebug(RTKey.DB_URL_DB      +keySfx+" = " + RTConfig.getString(AppendRWMode(RTKey.DB_URL_DB  ,rwMode)));
            Print.logDebug("DBProvider.getDBUri"+keySfx+" = " + DBProvider.getDBUri(rwMode,true));
        }
    ...
    **/
    ...
    /**
    *** Class containing a connection URI/User/Password
    **/
    public static class ConnectionURI
    {
        ...
        private DBReadWriteMode rwMode = null;
        public ConnectionURI(String uri, 
            String user, String pass, 
            DBReadWriteMode rwMode) {
            this.uri    = uri;
            this.user   = user;
            this.pass   = pass;
            this.rwMode = DBReadWriteMode.getDefaultReadWriteMode(rwMode);
;
        }
        ...
        public DBReadWriteMode getReadWriteMode() {
            return DBReadWriteMode.getDefaultReadWriteMode(this.rwMode);
        }
    }

    // --------------------------------

    public static ConnectionURI getConnectionURI(DBReadWriteMode rwMode, boolean inclDBName)
    {
        rwMode = DBReadWriteMode.getDefaultReadWriteMode(rwMode);
        String uri = DBProvider.getDBUri(rwMode, inclDBName);
        String usr = DBProvider.getDBUsername(rwMode);
        String pwd = DBProvider.getDBPassword(rwMode);
        return new ConnectionURI(uri, usr, pwd, rwMode);
    }

    public static ConnectionURI getConnectionURI(DBReadWriteMode rwMode)
    {
        return DBConnection.getConnectionURI(rwMode, true);
    }

    // --------------------------------

    public static ConnectionURI getConnectionURI_read(boolean inclDBName)
    {
        return DBConnection.getConnectionURI(DBReadWriteMode.READ_ONLY, inclDBName);
    }

    public static ConnectionURI getConnectionURI_read()
    {
        return DBConnection.getConnectionURI_read(true);
    }

    // --------------------------------

    public static ConnectionURI getConnectionURI_write(boolean inclDBName)
    {
        return DBConnection.getConnectionURI(DBReadWriteMode.READ_WRITE, inclDBName);
    }

    public static ConnectionURI getConnectionURI_write()
    {
        return DBConnection.getConnectionURI_write(true);
    }

    // ------------------------------------------------------------------------
    // ------------------------------------------------------------------------

    /* list of all connections to be released during cleanup */
    protected static Collection<DBConnection>   dbConnectionList = null;

    /**
    *** Gets the list of DBConnections to be released during cleanup
    **/
    protected static Collection<DBConnection> _getDBConnectionList(boolean create)
    {
        if ((DBConnection.dbConnectionList == null) && create) {
            DBConnection.dbConnectionList = new Vector<DBConnection>();
        }
        return DBConnection.dbConnectionList;
    }
    ...
    protected static ThreadLocalMap<String,DBConnection> dbConnectionMap_read  = null;
    protected static ThreadLocalMap<String,DBConnection> dbConnectionMap_write = null;

    /**
    *** Gets the per-thread DBConnection instance
    *** (should not be used within Tomcat or other servlet engine)
    **/
    protected static ThreadLocalMap<String,DBConnection> _getDBConnectionMap(DBReadWriteMode rwMode, boolean create)
    {
        rwMode = DBReadWriteMode.getDefaultReadWriteMode(rwMode);
        // -- "DBConnection.ConnectionPoolLock" lock required
        if (rwMode.isWrite()) {
            if ((DBConnection.dbConnectionMap_write == null) && create) {
                DBConnection.dbConnectionMap_write = new ThreadLocalMap<String,DBConnection>("DBConnection:write");
            }
            return DBConnection.dbConnectionMap_write;
        } else {
            if ((DBConnection.dbConnectionMap_write == null) && create) {
                DBConnection.dbConnectionMap_read = new ThreadLocalMap<String,DBConnection>("DBConnection:read");
            }
            return DBConnection.dbConnectionMap_read;
        }
    }
    ...
    protected static Map<String,ArrayList<DBConnection>> dbConnectionPool_read  = null;
    protected static Map<String,ArrayList<DBConnection>> dbConnectionPool_write = null;

    /**
    *** Gets the connection pool map for the specified DBReadWriteMode
    **/
    protected static Map<String,ArrayList<DBConnection>> _getDBConnectionPool(DBReadWriteMode rwMode, boolean create)
    {
        rwMode = DBReadWriteMode.getDefaultReadWriteMode(rwMode);
        // -- "DBConnection.ConnectionPoolLock" lock required
        if (rwMode.isWrite()) {
            if ((DBConnection.dbConnectionPool_write == null) && create) {
                DBConnection.dbConnectionPool_write = new HashMap<String,ArrayList<DBConnection>>();
            }
            return DBConnection.dbConnectionPool_write;
        } else {
            if ((DBConnection.dbConnectionPool_read == null) && create) {
                DBConnection.dbConnectionPool_read = new HashMap<String,ArrayList<DBConnection>>();
            }
            return DBConnection.dbConnectionPool_read;
        }
    }
    ...
    *** @param rwMode The DBReadWriteMode
    **/
    private static DBConnection _getDBConnection(DBReadWriteMode rwMode, String uri)
    {
        rwMode = DBReadWriteMode.getDefaultReadWriteMode(rwMode);
        //if (true) { DBConnection.closeAllConnections(); return null; } // <== DEBUG USE ONLY!
        // -- "DBConnection.ConnectionPoolLock" lock required
        if (uri == null) {
            return null;
        } else
        if (DBCONNECTION_POOL) {
            // -- Pooled Connections
            Map<String,ArrayList<DBConnection>> connPool = DBConnection._getDBConnectionPool(rwMode,false);
            if (connPool != null) {
                ArrayList<DBConnection> dbcArray = connPool.get(uri);
                if (dbcArray != null) {
            ...
            ThreadLocalMap<String,DBConnection> connMap = DBConnection._getDBConnectionMap(rwMode,false);
            return (connMap != null)? connMap.get(uri) : null;
        }
    }
    ...
    **/
    protected static void _saveDBConnection(DBConnection dbc)
    {
        // -- "DBConnection.ConnectionPoolLock" lock required
        if (dbc != null) {
            DBReadWriteMode rwMode = dbc.getReadWriteMode();
            String uri = dbc.getUri();
            if (DBCONNECTION_POOL) {
                // -- Pooled Connections
                Map<String,ArrayList<DBConnection>> connPool = DBConnection._getDBConnectionPool(rwMode,true);
                ArrayList<DBConnection> dbcArray = connPool.get(uri);
                if (dbcArray == null) {
                    dbcArray = new ArrayList<DBConnection>();
                    connPool.put(uri, dbcArray);
                }
                dbcArray.add(dbc);
            } else {
                // -- ThreadLocal connections
                ...
                ThreadLocalMap<String,DBConnection> connMap = DBConnection._getDBConnectionMap(rwMode,true);
                connMap.put(uri, dbc);
            }
            // -- save list of DBConnections
            Collection<DBConnection> connList = DBConnection._getDBConnectionList(true);
            connList.add(dbc);
        }
    }
    ...
    **/
    public static void closeAllConnections()
    {
        ...
        /* close connections */
        int closed = 0;
        Collection<DBConnection> connList = DBConnection._getDBConnectionList(false);
        if (connList != null) {
            for (DBConnection dbc : connList) {
        ...
        //if (dbConnectionMap_write != null) {
        //    dbConnectionMap_write.releaseResources(); // ThreadLocal
        //    dbConnectionMap_write = null;
        //}
    ...
    **/
    protected static DBConnection _getDBConnection(DBReadWriteMode rwMode, String uri, String user, String pass)
    {
        rwMode = DBReadWriteMode.getDefaultReadWriteMode(rwMode);
        if (uri != null) {
            ...
            synchronized (DBConnection.ConnectionPoolLock) {
                dbc = DBConnection._getDBConnection(rwMode, uri);
                if (dbc == null) {
                    // -- new connection
                    String threadName = Thread.currentThread().getName();
                    dbc = new DBConnection(uri, user, pass, rwMode);
                    dbc.threadName = StringTools.trim(threadName);
                    DBConnection._saveDBConnection(dbc);
                    dbConnMsg = "New DBConnection("+rwMode+") [" + threadName + "] " + uri;
                } else {
                    // -- reused connection
                    //if (DBCONNECTION_POOL) {
            ...
            if (previousLock != null) {
                Print.logException("DBConnection("+rwMode+") connection not released!", previousLock);
            }
            return dbc;
        } else {
            return null;
    ...
    /**
    *** Gets the DBConnection for the specified DBReadWriteMode
    **/
    public static DBConnection getDBConnection(DBReadWriteMode rwMode)
    {
        rwMode = DBReadWriteMode.getDefaultReadWriteMode(rwMode);
        String uri = DBProvider.getDBUri(rwMode,true);
        String usr = DBProvider.getDBUsername(rwMode);
        String pwd = DBProvider.getDBPassword(rwMode);
        return DBConnection._getDBConnection(rwMode, uri, usr, pwd);
    }
    ...
    *** Called by <code>RTConfigContextListener.contextDestroyed</code> when a servlet is undeployed.
    **/
    public static void _closeUnusedPooledConnections(DBReadWriteMode rwMode)
    {
        // -- "DBConnection.ConnectionPoolLock" lock required
        Map<String,ArrayList<DBConnection>> connPool = DBConnection._getDBConnectionPool(rwMode,false);
        if (connPool != null) {
            for (String uri : connPool.keySet()) {
                ArrayList<DBConnection> dbcArray = connPool.get(uri);
                if (dbcArray != null) {
                    for (int d = 0; d < dbcArray.size(); d++) {
                        DBConnection dbc = dbcArray.get(d);
    ...
    **/
    ...
    {
        synchronized (DBConnection.ConnectionPoolLock) {
            DBConnection._closeUnusedPooledConnections(DBReadWriteMode.READ_ONLY );
            DBConnection._closeUnusedPooledConnections(DBReadWriteMode.READ_WRITE);
        }
    }
    ...
    private DBReadWriteMode rwMode              = DBReadWriteMode.READ_WRITE;
    ...
    *** @param rwMode The DBReadWriteMode
    **/
    protected DBConnection(String uri, String user, String pass, DBReadWriteMode rwMode)
    {
        this.dbUri    = uri;
        this.userName = user;
        this.password = pass;
    ...
    **/
    ...
    /**
    *** Gets the DBReadWriteMode for this DBConnection
    **/
    public DBReadWriteMode getReadWriteMode()
    {
        return DBReadWriteMode.getDefaultReadWriteMode(this.rwMode);
    ...
    **/
    public String getUri()
    {
        return (this.dbUri != null)? this.dbUri : DBProvider.getDBUri(this.getReadWriteMode(),true);
    }
    ...
    **/
    public String getUser()
    {
        if (this.userName == null) {
            return DBProvider.getDBUsername(this.getReadWriteMode());
        } else
    ...
    **/
    public String getPassword()
    {
        if (this.password == null) {
            return DBProvider.getDBPassword(this.getReadWriteMode());
        } else {
    ...
    **/
    public Connection getConnection()
        throws SQLException
    {
        DBReadWriteMode rwMode = this.getReadWriteMode();
            ...
            try {
                ...
                if (RTConfig.getBoolean(RTKey.DB_EMAIL_CONNECTION_ERRORS)) {
                    ...
                    if ((nowTime - LAST_EMAIL_SENT_AT) > DateTime.MinuteSeconds(10)) {
                        ...
                        SendMail.sendSysadminEMail(subj, body);
    ...
    **/
    ...
    {
        try {
            if (ShowExecutedSQL) {  // getShowExecutedSQL()
                // -- show SQL query
