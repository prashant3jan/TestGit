...
//  2018/05/21  GTS Developent Team
//     -Added "GetDataTypeLength_String(...)"
// ----------------------------------------------------------------------------
...
public class DBField
{
    ...
    **/
    ...
    /**
    *** The UTF8[MB4] character set
    **/
    private static final String  UTF8MB4_CHARACTER_SET  = "utf8mb4"; // [2.6.7-B45s]
  //private static final String  UTF8_CHARACTER_SET     = UTF8MB4_CHARACTER_SET;
    ...
    public static final String ATTR_UTF8MB4         = "utf8mb4";    // UTF8MB4 character set [Boolean]
    ...
    **/
    ...
    public static class DBFieldTemplate // IndexTemplate
    {
        private String      fName  = null;
        private Class<?>    fClass = null;
        private String      fType  = null;
        private I18N.Text   fTitle = null;
        private String      fAttr  = null;
        private DBFieldTemplate() {
            super();
        }
        public DBFieldTemplate(String name, Class<?> clazz, String type, String title, String attr) {
            this();
            String _ttl = StringTools.trim(title);
            if (_ttl.indexOf("{0}") < 0) {
                _ttl += " #{0}";
            }
            this._init(name, clazz, type, new I18N.Text(_ttl), attr);
        }
        public DBFieldTemplate(String name, Class<?> clazz, String type, I18N.Text title, String attr) {
            this();
            this._init(name, clazz, type, title, attr);
        }
        private void _init(String name, Class<?> clazz, String type, I18N.Text title, String attr) {
            this.fName  = StringTools.trim(name);
            this.fClass = clazz;
            this.fType  = StringTools.trim(type);
            this.fTitle = title;
            this.fAttr  = StringTools.trim(attr);
        }
        public String getIndexString(int ndx, int len) {
            return StringTools.format(ndx, "%0"+len+"d");
        }
        public String getFieldName(int ndx) {
            int s = this.fName.indexOf('#');
            int e = -1;
            if (s >= 0) {
                for (e = s + 1; (e < this.fName.length()) && (this.fName.charAt(e) == '#'); e++);
                String ndxS = this.getIndexString(ndx,e - s);
                return this.fName.substring(0,s)+ndxS+this.fName.substring(e);
            } else {
                return this.fName + ndx;
            }
        }
        public DBField getDBField(int ndx) {
            final String _ndx = String.valueOf(ndx);
            final String _fnm = this.getFieldName(ndx);
            return new DBField(_fnm,this.fClass,this.fType,this.fTitle,this.fAttr) {
                public String getTitle(Locale locale) {
                    I18N.Text T = this.getTitle();
                    if (T == null) {
                        return "#" + _ndx;
                    } else
                    if (T.hasDefaultParameters()) {
                        return T.toString(locale, _ndx);
                    } else {
                        return T.toString(locale) + " #" + _ndx;
                    }
                }
        ...
        public String toString() {
            return this.fName + "(" + this.fType + ") : " + this.fTitle;
        }
    } // DBFieldTemplate

    /**
    *** Create an array containing multiple instances of the specified DBFieldTemplates
    **/
    public static DBField[] CreateDBFieldArray(int count, DBFieldTemplate... dbfTemplate)
    {
        //Print.logInfo("Count="+count + ", DBFieldTemplate length=" + dbfTemplate.length);
        if ((count > 0) && !ListTools.isEmpty(dbfTemplate)) {
            Vector<DBField> dbfList = new Vector<DBField>();
            for (int ndx = 0; ndx < count; ndx++) {
                for (DBFieldTemplate dt : dbfTemplate) {
                    if (dt != null) {
                        dbfList.add(dt.getDBField(ndx));
                    }
                }
            }
            return dbfList.toArray(new DBField[dbfList.size()]);
        } else {
    ...
    **/
        ...
        String charSet, Set<String> indexNames)
    {
        ...
        if (StringTools.isBlank(charSet)) {
            ...
            String cs = charSet.toLowerCase().trim();
            this.charSet = cs;
            /*
            if (cs.startsWith("utf8")) {
                if (cs.equals("utf8mb4")) {
                    this.charSet = UTF8MB4_CHARACTER_SET;
                } else {
                    this.charSet = UTF8_CHARACTER_SET;
                }
            } else {
                this.charSet = cs;
            }
    ...
    **/
    public DBField(String fldName, Class<?> javaClass, String dataType, I18N.Text title, String attr)
    {
        ...
        /* character set */
        if (RTConfig.getBoolean(RTKey.DB_UTF8)) {
            if (this.getBooleanAttribute(ATTR_UTF8,false)) {
                this.charSet = UTF8_CHARACTER_SET; // UTF8MB4_CHARACTER_SET
            } else
            if (this.getBooleanAttribute(ATTR_UTF8MB4,false)) { // [2.6.7-B45s]
                this.charSet = UTF8MB4_CHARACTER_SET;
            }
        }
    ...
    **/
    public boolean isUTF8()
    {
        ...
        if (this.charSet.equalsIgnoreCase("utf8")) { // 2.6.7-B45s
            return true;
        } else {
            // -- ie "utf8_general_ci"
            return this.charSet.startsWith("utf8_"); // 2.6.7-B45s
        }
    }

    /**
    *** Returns true if this field is defined as character set UTF8MB4 [2.6.7-B45s]
    *** @return True if this field is defined as character set UTF8MB4
    **/
    public boolean isUTF8MB4() // [2.6.7-B45s]
    {
        if (this.charSet == null) {
            return false;
        } else 
        if (this.charSet.equalsIgnoreCase("utf8mb4")) {
            return true;
        } else {
            // -- ie "utf8mb4_bin"
            return this.charSet.startsWith("utf8mb4_");
        }
    }
    ...
    **/
    ...
    /**
    *** Gets the String length of the specified "STRING[X]" data type
    *** Returns '0' if not a STRING data type.
    *** @return The length of the String data type
    **/
    public static int GetDataTypeLength_String(String stringDT)
    {
        String dt = StringTools.trim(stringDT).toUpperCase();
        if (dt.startsWith(TYPE_STRING + "[")) {
            String x = dt.substring(TYPE_STRING.length() + 1);
            return StringTools.parseInt(x, 0);
        } else {
    ...
    **/
    public int getStringLength()
    {
        if (this.stringLength < 0) {
            this.stringLength = DBField.GetDataTypeLength_String(this.getDataType());
        }
    ...
    **/
    public boolean equals(Object other)
    {
        ...
        if (this.isUTF8MB4() != fld.isUTF8MB4()) { // [2.6.7-B45s]
            return false;
        }
