...
//     -Added "getDBProperties()"
//     -Added support for separating MySQL read/write connections. [2.6.7-B45]
...
public class DBProvider
{
    ...
    public static final String      TYPE_MTEXT              = "MTEXT"; // CLOB (65535 bytes)
    ...
    public static final int         SQL_MTEXT               = 18;   // TYPE_MTEXT
    ...
    protected static DBProvider MySQL_Old_Provider = new DBProvider(
        ...
        new String[] {
            ...
            /* SQL_MTEXT     */ "MEDIUMTEXT",         // max (2^24 - 1) bytes
        }
    );
    ...
    protected static DBProvider MySQL_MyISAM_Provider = new DBProvider(
        ...
        new String[] {
            ...
            /* SQL_VARCHAR   */ "VARCHAR",
            /* SQL_DATETIME  */ "DATETIME",
            /* SQL_MTEXT     */ "MEDIUMTEXT",         // max (2^16 - 1) bytes
        }
    );
    ...
    protected static DBProvider PostgreSQL_Provider = new DBProvider(
        ...
        new String[] {
            ...
            /* SQL_MTEXT     */ "TEXT",               // max (216 - 1) bytes
        }
    );  
    ...
    protected static DBProvider MS_SQLServer_Provider = new DBProvider(
        ...
        new String[] {
            ...
            /* SQL_VARCHAR   */ "VARCHAR",
            /* SQL_DATETIME  */ "DATETIME",
            /* SQL_MTEXT     */ "TEXT",               // max (2^31 - 1) bytes
        }
    );
    ...
    **/
    public static String getDBHost(DBReadWriteMode rwMode)
    {
        return RTConfig.getString(DBConnection.AppendRWMode(RTKey.DB_HOST,rwMode));
    }
    ...
    **/
    public static int getDBPort(DBReadWriteMode rwMode)
    {
        return DBProvider.getProvider().getDefaultPort(rwMode);
    }
    ...
    **/
    public static String getDBName(DBReadWriteMode rwMode)
    {
        rwMode = DBReadWriteMode.getDefaultReadWriteMode(rwMode);
        if (RTConfig.hasProperty(DBConnection.AppendRWMode(RTKey.DB_NAME,rwMode),false)) {
            return RTConfig.getString(DBConnection.AppendRWMode(RTKey.DB_NAME,rwMode), "");
        } else {
            String name = RTConfig.getString(DBConnection.AppendRWMode(RTKey.DB_NAME,rwMode));
    ...
    **/
    public static String getDBUsername(DBReadWriteMode rwMode)
    {
        rwMode = DBReadWriteMode.getDefaultReadWriteMode(rwMode);
        if (RTConfig.hasProperty(DBConnection.AppendRWMode(RTKey.DB_USER,rwMode),false)) {
            return RTConfig.getString(DBConnection.AppendRWMode(RTKey.DB_USER,rwMode), "");
        } else {
            String user = RTConfig.getString(DBConnection.AppendRWMode(RTKey.DB_USER,rwMode));
    ...
    **/
    public static String getDBPassword(DBReadWriteMode rwMode)
    {
        rwMode = DBReadWriteMode.getDefaultReadWriteMode(rwMode);
        if (RTConfig.hasProperty(DBConnection.AppendRWMode(RTKey.DB_PASS,rwMode),false)) {
            return RTConfig.getString(DBConnection.AppendRWMode(RTKey.DB_PASS,rwMode));
        } else {
            String pass = RTConfig.getString(DBConnection.AppendRWMode(RTKey.DB_PASS,rwMode));
  ...
  //private static String  JDBC_URL_DB[]    = new String[] { RTKey.DB_URL_DB };
    ...
    **/
    public static String getDBUri(DBReadWriteMode rwMode, boolean inclDBName)
    {
        rwMode = DBReadWriteMode.getDefaultReadWriteMode(rwMode);
        ...
        String urlStr = null;
        if (inclDBName) {
            urlStr = RTConfig.getString(DBConnection.AppendRWMode(RTKey.DB_URL_DB,rwMode),"").trim();
            if (!StringTools.isBlank(urlStr)) {
                // -- explicitly specified in runtime properties
                return urlStr;
            }
            // -- start with standard connection url
            urlStr = RTConfig.getString(DBConnection.AppendRWMode(RTKey.DB_URL,rwMode),"").trim();
        } else {
            urlStr = RTConfig.getString(DBConnection.AppendRWMode(RTKey.DB_URL,rwMode),"").trim();
            if (!StringTools.isBlank(urlStr)) {
                // -- explicitly specified in runtime properties
        ...
        switch (dbProv.getID()) {
            case DB_MYSQL: {
                ...
                if (uri.length() == 0) {
                    uri.append("jdbc:mysql://").append(getDBHost(rwMode)).append(":").append(getDBPort(rwMode)).append("/");
                }
                if (inclDBName) {
                    uri.append(getDBName(rwMode));
                }
                break;
            }
            case DB_POSTGRESQL: {
                ...
                if (uri.length() == 0) {
                    uri.append("jdbc:postgresql://").append(getDBHost(rwMode)).append(":").append(getDBPort(rwMode)).append("/");
                }
                if (inclDBName) {
                    uri.append(getDBName(rwMode));
                }
            ...
            case DB_DERBY: {
                ...
                if (uri.length() == 0) {
                    // TODO:
                    uri.append("jdbc:derby://").append(getDBHost(rwMode)).append(":").append(getDBPort(rwMode)).append("/");
                    uri.append(getDBName(rwMode)).append(";create=true");
                }
                break;
            }
            case DB_SQLSERVER: {
                ...
                if (uri.length() == 0) {
                    uri.append("jdbc:sqlserver://").append(getDBHost(rwMode)).append(":").append(getDBPort(rwMode));
                }
                if (inclDBName && (uri.indexOf("databaseName=") < 0)) {
                    uri.append(";databaseName=").append(getDBName(rwMode)); 
                }
                break;
            }
            default : {
    ...
    **/
    public static String getDataTypeFromSqlType(String sqlType)
    {
        if (sqlType != null) {
            ...
            if (sqlType.startsWith(sqlTypes[SQL_MTEXT])) {
                return TYPE_MTEXT;
            } else
    ...
    **/
        ...
        throws DBException
    {
        ...
        try {
            ...
            Print.logError("Unable to retrieve table index information ["+utableName+"]: " + dbe.getMessage());
            Print.logException("Unable to retrieve table index information ["+utableName+"]: ", dbe); // + dbe.getMessage());
        }
        ...
        try {
            ...
            while (rs.next()) {
                ...
                */
                ...
                String charSet = StringTools.trim(rs_GetString(rs,colName_CharSet,dftCharSet)); // latin1_swedish_ci, utf8_general_ci, utf8mb4_bin
    ...
    **/
    public static void createDatabase(DBReadWriteMode rwMode, String rootUser, String rootPass)
        throws DBException, SQLException
    {
        ...
        if (rwMode.equals(DBReadWriteMode.READ_ONLY)) {
            Print.logWarn("Unexpected Read/Write mode: " + rwMode);
        ...
        /* connection */
        String dbUri = DBProvider.getDBUri(rwMode, false);
        DBConnection dbc = null;
        ...
        switch (DBProvider.getProvider().getID()) {
            case DB_MYSQL    : {
                ...
                dbc = DBConnection._getDBConnection(DBReadWriteMode.READ_WRITE, dbUri, rootUser, rootPass);
                try {
        ...
        throws SQLException, DBException
    {
        ...
        for (int fx = 0; fx < flds.length; fx++) {
            ...
            if (dbProvID == DB_MYSQL) {
                ...
                // -- character set
                if (flds[fx].isUTF8MB4()) { // [2.6.7-B45s]
    ...
    **/
    public static void grantDBUser(String rootUser, String rootPass)
        throws DBException, SQLException
    {
        DBReadWriteMode rwMode = DBReadWriteMode.GRANT;
        String grantUser = DBProvider.getDBUsername(rwMode);
        String grantPass = DBProvider.getDBPassword(rwMode);
        String dbName    = DBProvider.getDBName(rwMode);
        ...
        /* connection */
        String dbUri = DBProvider.getDBUri(rwMode,false); // DBReadWriteMode.[GRANT|ALTER|READ_WRITE]
        DBConnection dbc = DBConnection._getDBConnection(rwMode, dbUri, rootUser, rootPass);
    ...
    *** @throws DBException   If a database error occurs
    **/
    public static String getDefaultCharacterSet(DBReadWriteMode rwMode)
        throws DBException, SQLException
    {
        String user   = DBProvider.getDBUsername(rwMode);
        String pass   = DBProvider.getDBPassword(rwMode);
        String dbName = DBProvider.getDBName(rwMode);
        ...
        throws DBException
    {
        ...
        try {
            ...
            if (forceLock || DBProvider.isTableLockingEnabled()) {
                ...
                DBConnection dbc = null;
                try {
                    dbc = DBConnection.getDBConnection(rwMode); // depends on next operation
                    dbc.executeUpdate(sb.toString());
                } finally {
                    DBConnection.release(dbc);
                }
    ...
    **/
        ...
        throws DBException
    {
        ...
        try {
            if (DBProvider.isTableLockingEnabled() || !DBProvider.lockLevel.empty()) {
                ...
                DBConnection dbc = null;
                try {
                    dbc = DBConnection.getDBConnection(rwMode); // depends on previous operation
    ...
    /** 
    *** Reads the Properties/Variables from the DBProvider
    **/
    public static Map<String,String> getDBProperties()
    {
        try {
            return DBProvider._getDBProperties();
        } catch (DBException dbe) {
            Print.logError("Unable to get DB properties: " + dbe);
            return null;
        }
    }
    ...
    *** Reads the Properties/Variables from the DBProvider
    **/
    public static Map<String,String> _getDBProperties()
        throws DBException
    {
        ...
        try {
            dbc  = DBConnection.getDBConnection(DBReadWriteMode.READ_ONLY);
            stmt = dbc.execute("SHOW VARIABLES;");
            rs   = stmt.getResultSet();
            while (rs.next()) {
            ...
            throw new DBException("Unable to get properties", sqe);
        } finally {
            if (rs   != null) { try { rs.close();   } catch (Throwable t) {} }
            if (stmt != null) { try { stmt.close(); } catch (Throwable t) {} }
            DBConnection.release(dbc);
    ...
    **/
    public int getDefaultPort(DBReadWriteMode rwMode)
    {
        rwMode = DBReadWriteMode.getDefaultReadWriteMode(rwMode);
        if (RTConfig.hasProperty(DBConnection.AppendRWMode(RTKey.DB_PORT,rwMode),false)) {
            return RTConfig.getInt(DBConnection.AppendRWMode(RTKey.DB_PORT,rwMode));
        } else {
