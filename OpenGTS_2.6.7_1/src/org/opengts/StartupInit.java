...
//     -Added support for setting "networkaddress.cache.negative.ttl" (see DNS_CACHE_TIMEOUT).
// ----------------------------------------------------------------------------
package org.opengts;
    ...
    implements DBConfig.DBInitialization, DBFactory.CustomFactoryHandler
{
    ...
    **/
    public StartupInit()
    {
        ...
        {
            // -- dns.cache.timeou=30
            int dnsTmoSec = RTConfig.getInt(RTKey.DNS_CACHE_TIMEOUT, -999); // "dns.cache.timeout"
            if (dnsTmoSec >= 0) {
                // -- https://docs.oracle.com/javase/7/docs/technotes/guides/net/properties.html
                // -- https://docs.oracle.com/javase/8/docs/technotes/guides/net/properties.html
                // --   - "-1" is a valid value (meaning "cache forever"), however this is already  
                // -      the default so we ignore anyting less-than 0 here.
                // -    - This code MUST be executed before any other netrowking operations
                try {
                    String dnsTmoSecStr = String.valueOf(dnsTmoSec);
                    // -- "networkaddress.cache.ttl" (default -1)
                    // -    -1:CacheForever, >=0:CacheSeconds
                    java.security.Security.setProperty("networkaddress.cache.ttl", dnsTmoSecStr);
                    // -- "networkaddress.cache.negative.ttl" (default is 10)
                    // -    -1:CacheForever, 0:NeverCache, >0:CacheSeconds [2.6.7-B20s]
                    java.security.Security.setProperty("networkaddress.cache.negative.ttl", dnsTmoSecStr);
                    Print.logDebug("Set DNS caching timeout: " + dnsTmoSec + " seconds");
                    // -- these value can also be set in the "$JAVA_HOME/jre/lib/security/java.security" file
    ...
    **/
    public <T extends DBRecord<T>> java.util.List<DBField> selectFields(DBFactory<T> factory, java.util.List<DBField> fields)
    {
        ...
        if (tblName.equalsIgnoreCase(Account.TABLE_NAME())) {
            ...
            // -- startupInit.Account.BillingInfo=true
            addDBFields(tblName, fields, Account.OPTCOLS_BillingInfo                    , false, Account.BillingInfo);
            ...
            addDBFields(tblName, fields, Account.OPTCOLS_MaintFieldInfo                 , false, Account.MaintFields_Odom());
            addDBFields(tblName, fields, Account.OPTCOLS_MaintFieldInfo                 , false, Account.MaintFields_Hours());
            addDBFields(tblName, fields, Account.OPTCOLS_MaintFieldInfo                 , false, Account.MaintFields_FixedTime());
        ...
        if (tblName.equalsIgnoreCase(Device.TABLE_NAME())) {
            ...
            addDBFields(tblName, fields, Device.OPTCOLS_MaintFieldInfo                  , false, Device.MaintFields_Odom());
            addDBFields(tblName, fields, Device.OPTCOLS_MaintFieldInfo                  , false, Device.MaintFields_Hours());
            addDBFields(tblName, fields, Device.OPTCOLS_MaintFieldInfo                  , false, Device.MaintFields_FixedTime());
            addDBFields(tblName, fields, Device.OPTCOLS_MaintFieldInfo                  , false, Device.MaintFieldInfo);
        ...
        if (tblName.equalsIgnoreCase(EventData.TABLE_NAME())) {
            ...
            addDBFields(tblName, fields, EventData.OPTCOLS_RotationalAttitude           , false, EventData.RotationalAttitude);
            ...
            addDBFields(tblName, fields, EventData.OPTCOLS_SnowPlowData                 , false, EventData.SnowPlowData);
            ...
            // startupInit.EventData.PersonalHealth=true
            addDBFields(tblName, fields, EventData.OPTCOLS_PersonalHealth               , false, EventData.PersonalHealth);
            // startupInit.EventData.AutoLicensePlateRecog=true
            addDBFields(tblName, fields, EventData.OPTCOLS_AutoLicensePlateRecog        , false, EventData.AutoLicensePlateRecog);
            // -- 
            return fields;
        }

        /* EventDataExtra */
        if (tblName.equalsIgnoreCase(EventDataExtra.TABLE_NAME())) {
            // -- startupInit.EventDataExtra.OverflowExtra_1=true
          //addDBFields(tblName, fields, EventDataExtra.OPTCOLS_OverflowExtra_1         , false, EventDataExtra.OverflowExtra_1);
            return fields;
        }
    ...
    **/
    protected void addDBFields(String tblName, java.util.List<DBField> tblFields, String key, boolean defaultAdd, DBField customFields[], int maxCount)
    {
        ...
        /* add fields */
        if (!addFields) {
            // -- quietly skip custom fields
        } else
        if (customFields == null) {
            // -- no custom fields to add
            Print.logStackTrace("'customFields' is null! Table=" + tblName + " Key=" + key);
        } else
        if (customFields.length == 0) {
            // -- no custom fields to add
            Print.logWarn("'customFields' is empty! Table=" + tblName + " Key=" + key);
        } else {
            // -- add custom fields
            boolean CHECK_FOR_DUPLICATES_ON_ADD = RTConfig.isDebugMode();
            int cnt = ((maxCount >= 0) && (maxCount <= customFields.length))? maxCount : customFields.length;
            for (int i = 0; i < cnt; i++) {
                // -- omit nulls [2.6.6-B70]
                if (customFields[i] == null) {
                    continue;
                }
                // -- already defined (duplicate)?
                boolean isDuplicate = false;
                if (CHECK_FOR_DUPLICATES_ON_ADD) {
                    String custFN = customFields[i].getName();
                    duplicateCheck:
                    for (DBField dbf : tblFields) {
                        if (custFN.equalsIgnoreCase(dbf.getName())) {
                            Print.logStackTrace("Duplicate field name defined: " + tblName + "." + custFN);
                            isDuplicate = true;
