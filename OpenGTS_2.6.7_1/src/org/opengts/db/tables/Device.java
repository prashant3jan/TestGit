...
//     -Added FLD_lastOdometerKM to optimize GPS odometer calculations.
...
//  2018/05/21  Martin D. Flynn
//     -Added KEY_PTO_STATE
//     -Added KEY_REFUEL_DATETIME, KEY_REFUEL_LEVEL, KEY_REFUEL_VOLUME, KEY_REFUEL_DISTANCE [2.6.6-B60g]
//  2018/09/10  GTS Development Team
//     -Added support for multiple maintenance odometer/hour entries [2.6.6-B61c]
//     -Added KEY_MAINT_DUE_HR [2.6.6-B62h]
//  2020/02/19  GTS Development Team
//     -Increased FLD_fuelTankProfile/FLD_fuelTankProfile2 size to 400 chars (was 320) [2.6.7-B43j]
// ----------------------------------------------------------------------------
               ...
               JSON.JSONBean
{
    ...
    private static int LogEventDataInsertion = Print.LOG_INFO; // Print.LOG_UNDEFINED;

    /**
    *** Sets the logging level for EventData insertions
    **/
    ...
    /**
    *** Gets the logging level for EventData insertions
    **/
    public static int GetLogEventDataInsertion()
    {
        return Device.LogEventDataInsertion;
    }
    ...
    **/
    ...
    private static       int    VehicleIDColumnLength               = -1; // FLD_vehicleID
  ...
  //private static       int    FuelPropertiesColumnLength          = -1; // FLD_fuelProperties
    ...
    public static final String  OPTCOLS_MaintFieldInfo              = "startupInit.Device.MaintFieldInfo";
    ...
    **/
    public enum FuelTankIndex implements EnumTools.IntValue {
        TANK_1(0),
        TANK_2(1),
        TANK_3(2), // not supported
        TANK_4(3), // not supported
        TANK_5(4), // not supported
        TOTAL(-1);
        private int nn = -1;
    ...
    **/
    ...
    private static final String NAME_TANK_3[]   = { "tank3", "tank-3", "tank_3" };
    private static final String NAME_TANK_4[]   = { "tank4", "tank-4", "tank_4" };
    private static final String NAME_TANK_5[]   = { "tank5", "tank-5", "tank_5" };
    ...
    **/
    public static FuelTankIndex GetFuelTankIndex(String name, FuelTankIndex dft)
    {
        ...
        /* tank #3 */
        if (ListTools.contains(NAME_TANK_3, n)) {
            return FuelTankIndex.TANK_3;
        }

        /* tank #4 */
        if (ListTools.contains(NAME_TANK_4, n)) {
            return FuelTankIndex.TANK_4;
        }

        /* tank #5 */
        if (ListTools.contains(NAME_TANK_5, n)) {
            return FuelTankIndex.TANK_5;
        }
    ...
    **/
    ...
    // ------------------------------------------------------------------------
    // enforce unique DeviceID

    private static volatile int  EnforceUniqueDeviceID = -1;
    private static volatile int  LoadByDeviceID        = -1;
    private static volatile int  LoadByUniqueID        = -1;

    /**
    *** Returns true if unique DeviceIDs should be enforced acrosss all Accounts.
    *** A returned "true" value does not guarantee that DeviceID's are actually unique, it only
    *** provides a hint to the Device/Vehicle Admin page (and others) that it should check for 
    *** unique deviceID's prior to adding a new device.
    *** Defaults to false.
    **/
    public static boolean GetEnforceUniqueDeviceID()
    {
        if (Device.EnforceUniqueDeviceID < 0) {
            synchronized (DBConfig.PROP_Device_enforceUniqueDeviceID) {
                if (Device.EnforceUniqueDeviceID < 0) {
                    boolean v = RTConfig.getBoolean(DBConfig.PROP_Device_enforceUniqueDeviceID, false);
                    Device.EnforceUniqueDeviceID = v? 1 : 0;
                }
            }
        }
        return (Device.EnforceUniqueDeviceID > 0)? true : false;
    }

    /**
    *** Returns false if "GetEnforceUniqueDeviceID()" is false.
    *** Otherwise checks the value of the "Device.loadByDeviceID" property.
    *** Defaults to true, if "GetEnforceUniqueDeviceID()" is true.
    **/
    public static boolean GetEnabledLoadByDeviceID()
    {
        if (Device.LoadByDeviceID < 0) {
            synchronized (DBConfig.PROP_Device_loadByDeviceID) {
                if (Device.LoadByDeviceID < 0) {
                    if (!Device.GetEnforceUniqueDeviceID()) {
                        // -- UniqueDeviceID is required for LoadByDeviceID
                        Device.LoadByDeviceID = 0; // false
                    } else {
                        // -- default to "false"
                        boolean v = RTConfig.getBoolean(DBConfig.PROP_Device_loadByDeviceID,true);
                        Device.LoadByDeviceID = v? 1 : 0;
                    }
                }
            }
        }
        return (Device.LoadByDeviceID > 0)? true : false;
    }

    /**
    *** Returns true if "GetEnabledLoadByDeviceID()" is false.
    *** Otherwise checks the value of the "Device.loadByUniqueID" property.
    *** Defaults to true, if "GetEnabledLoadByDeviceID()" is false.
    **/
    public static boolean GetEnabledLoadByUniqueID()
    {
        if (Device.LoadByUniqueID < 0) {
            synchronized (DBConfig.PROP_Device_loadByUniqueID) {
                if (Device.LoadByUniqueID < 0) {
                    if (!Device.GetEnabledLoadByDeviceID()) {
                        // -- if loadByDeviceID is false, default to true
                        Device.LoadByUniqueID = 1; // true;
                    } else {
                        // -- default to "true"
                        boolean v = RTConfig.getBoolean(DBConfig.PROP_Device_loadByUniqueID,true);
                        Device.LoadByUniqueID = v? 1 : 0;
                    }
                }
            }
        }
        return (Device.LoadByUniqueID > 0)? true : false;
    }
    ...
    /**
    *** Gets the configured maximum allowed odometer value
    ...
    **/
    ...
    private static Object        PastEventDateLock   = new Object();
    private static volatile int  PastEventDateAction = PAST_DATE_UNDEFINED;
    private static volatile long PastEventDateMaxSec = -999L;
    ...
    **/
    public static int pastEventDateAction()
    {
        if (PastEventDateAction == PAST_DATE_UNDEFINED) {
            synchronized (PastEventDateLock) {
                if (PastEventDateAction == PAST_DATE_UNDEFINED) {
    ...
    **/
    ...
    {
        if (PastEventDateMaxSec < 0L) {
            synchronized (PastEventDateLock) {
                if (PastEventDateMaxSec < 0L) {
                    // -- "Device.pastDate.maximumSec="
    ...
    **/
    ...
    // ------------------------------------------------------------------------
    // ------------------------------------------------------------------------
    // -- Device JSONBean implementation that places the Device JSON object
    // -  within another object with key name "Device"

    public JSON.JSONBean getJSONBeanTable() // getDeviceBean()
    {
        return new JSON.JSONBean() {
            @JSONBeanGetter(name="Device")
    ...
    public static final String FLD_driverTime            = "driverTime";            // driver timestamp
  ...
  //public static final String FLD_fuelCO2PerLiter       = "fuelCO2PerLiter";       // CO2 emissions per Liter (kg)
    public static final String FLD_engineDisplacement    = "engineDisplacement";    // Engine displacement Liters
    public static final String FLD_speedLimitKPH         = "speedLimitKPH";         // Maximum speed km/h
    public static final String FLD_speedOffsetKPH        = "speedOffsetKPH";        // Speed offset km/h
  ...
  //public static final String FLD_smsGatewayProps       = "smsGatewayProps";       // SMS gateway properties
    ...
    public static final String FLD_iconURL               = "iconURL";               // device icon/image URL
    public static final String FLD_displayColor          = "displayColor";          // display color (maps, reports, etc).
    public static final String FLD_routeSegColorSel      = "routeSegColorSel";      // RouteSegmentColor selector
    ...
    public static final String FLD_firstConnectTime      = "firstConnectTime";      // first connect time
    public static final String FLD_lastRevGeocodeTime    = "lastRevGeocodeTime";    // last reverse-geocode time
    ...
    public static final String FLD_ptoHoursOffset        = "ptoHoursOffset";        // offset to reported PTO hours
    // -- First registration time
    public static final String FLD_registrationTime      = "registrationTime";      // time of first registration
    ...
    private static DBField FieldInfo[] = {
        ...
        new DBField(FLD_vehicleID            , String.class        , DBField.TYPE_STRING(24)  , I18N.getString(Device.class,"Device.fld.vehicleID"            , "VIN"                         ), "edit=2 altkey=vin"),
        ...
        new DBField(FLD_driverStatus         , Integer.TYPE        , DBField.TYPE_INT32       , I18N.getString(Device.class,"Device.fld.driverStatus"         , "Driver Status"               ), "edit=2"),
        new DBField(FLD_driverTime           , Long.TYPE           , DBField.TYPE_UINT32      , I18N.getString(Device.class,"Device.fld.driverTime"           , "Driver Time"                 ), "edit=2 format=time"),
        ...
        new DBField(FLD_fuelTankProfile      , String.class        , DBField.TYPE_STRING(400) , I18N.getString(Device.class,"Device.fld.fuelTankProfile"      , "Fuel Tank Profile"           ), "edit=2"),
        new DBField(FLD_fuelTankProfile2     , String.class        , DBField.TYPE_STRING(400) , I18N.getString(Device.class,"Device.fld.fuelTankProfile"      , "Fuel Tank Profile"           ), "edit=2"),
      //new DBField(FLD_fuelProperties       , String.class        , DBField.TYPE_STRING(200) , I18N.getString(Device.class,"Device.fld.fuelProperties"       , "Fuel Properties"             ), "edit=2"),
        new DBField(FLD_engineDisplacement   , Double.TYPE         , DBField.TYPE_DOUBLE      , I18N.getString(Device.class,"Device.fld.engineDisplacement"   , "Engine Displacement Liters"  ), "edit=2 format=#0.0"),
        ...
        new DBField(FLD_speedOffsetKPH       , Double.TYPE         , DBField.TYPE_DOUBLE      , I18N.getString(Device.class,"Device.fld.speedOffsetKPH"       , "Speed Offset km/h"           ), "edit=2 format=#0.0"),
        ...
        new DBField(FLD_iconURL              , String.class        , DBField.TYPE_STRING(150) , I18N.getString(Device.class,"Device.fld.iconURL"              , "Icon/Image URL"              ), "edit=2"),
        ...
        new DBField(FLD_routeSegColorSel     , String.class        , DBField.TYPE_STRING(80)  , I18N.getString(Device.class,"Device.fld.routeSegColorSel"     , "Route Segment Color selector"), "edit=2"), // RouteSegmentColor
        ...
        new DBField(FLD_firstConnectTime     , Long.TYPE           , DBField.TYPE_UINT32      , I18N.getString(Device.class,"Device.fld.firstConnectTime"     , "First Connect Time"          ), "format=time"),
        new DBField(FLD_lastRevGeocodeTime   , Long.TYPE           , DBField.TYPE_UINT32      , I18N.getString(Device.class,"Device.fld.lastRevGeocodeTime"   , "Last Reverse-Geocode Time"   ), "format=time"),
      ...
      //new DBField(FLD_ipAddressLocal       , DTIPAddress.class   , DBField.TYPE_STRING(32)  , I18N.getString(Device.class,"Device.fld.ipAddressLocal"       , "Local IP Address"            ), ""),
        ...
        new DBField(FLD_engineHoursOffset    , Double.TYPE         , DBField.TYPE_DOUBLE      , I18N.getString(Device.class,"Device.fld.engineHoursOffset"    , "Engine Hours Offset"         ), "format=#0.0"),
        ...
        new DBField(FLD_ptoHoursOffset       , Double.TYPE         , DBField.TYPE_DOUBLE      , I18N.getString(Device.class,"Device.fld.ptoHoursOffset"       , "PTO Hours Offset"            ), "format=#0.0"),
        new DBField(FLD_registrationTime     , Long.TYPE           , DBField.TYPE_UINT32      , I18N.getString(Device.class,"Device.fld.registrationTime"     , "Tegistration Time"           ), "format=time"),
        // -- Common fields
  ...
  //public static final String FLD_rentalAllowedKM       = "rentalAllowedKM";       // rental allowed kilometers
    ...
    // -  startupInit.Device.MaintFieldInfo=true
    public static final String FLD_maintDescriptionKM_  = "maintDescriptionKM";    // odometer maint #X description
    public static final String FLD_maintLabelKM_         = "maintLabelKM";          // odometer maint #X label/description
    public static final String FLD_maintIntervalKM_      = "maintIntervalKM";       // odometer maint #X interval distance to next
    public static final String FLD_maintPreNotifyKM_     = "maintPreNotifyKM";      // odometer maint #X pre-notify offset
    public static final String FLD_maintOdometerKM_      = "maintOdometerKM";       // odometer maint #X last Odometer 
    ...
    public static final DBField[] MaintFields_Odom() {  // [2.6.6-B61c]
        ...
        if (MaintFields_Odom_ == null) {
            MaintFields_Odom_ = DBField.CreateDBFieldArray(
                ...
                new DBFieldTemplate(FLD_maintLabelKM_      , String.class, DBField.TYPE_STRING(40), I18N.getString(Device.class,"Device.fld.maintLabelKM"    ,"#{0} Maint Label (Odom)"       ), "edit=2"),
                new DBFieldTemplate(FLD_maintIntervalKM_   , Double.TYPE , DBField.TYPE_DOUBLE    , I18N.getString(Device.class,"Device.fld.maintIntervalKM" ,"#{0} Maint Interval (Odom)"    ), "format=#0.0 edit=2"),
                new DBFieldTemplate(FLD_maintPreNotifyKM_  , Double.TYPE , DBField.TYPE_DOUBLE    , I18N.getString(Device.class,"Device.fld.maintPreNotifyKM","#{0} Maint PreNotify (Odom)"   ), "format=#0.0 edit=2"),
                new DBFieldTemplate(FLD_maintOdometerKM_   , Double.TYPE , DBField.TYPE_DOUBLE    , I18N.getString(Device.class,"Device.fld.maintOdometerKM" ,"#{0} Last Maint Odometer"      ), "format=#0.0")
                );
        }
        return MaintFields_Odom_;
    }
    public static final String FLD_maintDescriptionHR_ = "maintDescriptionHR";    // hours maint #X description
    public static final String FLD_maintLabelHR_         = "maintLabelHR";          // hours maint #X label/description
    public static final String FLD_maintIntervalHR_      = "maintIntervalHR";       // hours maint #X interval hours to next
    public static final String FLD_maintPreNotifyHR_     = "maintPreNotifyHR";      // hours maint #X pre-notify offset
    public static final String FLD_maintEngHoursHR_      = "maintEngHoursHR";       // hours maint #X last EngineHours
    ...
    public static final DBField[] MaintFields_Hours() {  // [2.6.6-B61c]
        ...
        if (MaintFields_Hours_ == null) {
            MaintFields_Hours_ = DBField.CreateDBFieldArray(
                ...
                new DBFieldTemplate(FLD_maintLabelHR_      , String.class, DBField.TYPE_STRING(40), I18N.getString(Device.class,"Device.fld.maintLabelHR"    ,"#{0} Maint Label (Hours)"      ), "edit=2"),
                new DBFieldTemplate(FLD_maintIntervalHR_   , Double.TYPE , DBField.TYPE_DOUBLE    , I18N.getString(Device.class,"Device.fld.maintIntervalHR" ,"#{0} Maint Interval (Hours)"   ), "format=#0.0 edit=2"),
                new DBFieldTemplate(FLD_maintPreNotifyHR_  , Double.TYPE , DBField.TYPE_DOUBLE    , I18N.getString(Device.class,"Device.fld.maintPreNotifyHR","#{0} Maint PreNotify (Hours)"  ), "format=#0.0 edit=2"),
                new DBFieldTemplate(FLD_maintEngHoursHR_   , Double.TYPE , DBField.TYPE_DOUBLE    , I18N.getString(Device.class,"Device.fld.maintEngHoursHR" ,"#{0} Last Maint Hours"         ), "format=#0.0")
                );
        }
        return MaintFields_Hours_;
    }
    public static final String FLD_maintLabelFT_         = "maintLabelFT";          // FixedTime maint #X label/description
    public static final String FLD_maintIntervalFT_      = "maintIntervalFT";       // FixedTime maint #X interval seconds to next
    public static final String FLD_maintPreNotifyFT_     = "maintPreNotifyFT";      // FixedTime maint #X pre-notify offset
    public static final String FLD_maintFixedTimeFT_     = "maintFixedTimeFT";      // FixedTime maint #X last Timestamp
    ...
    public static final DBField[] MaintFields_FixedTime() {  // [2.6.7-B03]
        ...
        if (MaintFields_FixedTime_ == null) {
            MaintFields_FixedTime_ = DBField.CreateDBFieldArray( // [2.6.7-B03]
                Device.getMaintCountFT(), // <-- RTConfig init required
                new DBFieldTemplate(FLD_maintLabelFT_      , String.class, DBField.TYPE_STRING(40), I18N.getString(Device.class,"Device.fld.maintLabelFT"    ,"#{0} Maint Label (Time)"    ), "edit=2"),
                new DBFieldTemplate(FLD_maintIntervalFT_   , Long.TYPE   , DBField.TYPE_UINT32    , I18N.getString(Device.class,"Device.fld.maintIntervalFT" ,"#{0} Maint Interval (Time)" ), "edit=2"),
                new DBFieldTemplate(FLD_maintPreNotifyFT_  , Long.TYPE   , DBField.TYPE_UINT32    , I18N.getString(Device.class,"Device.fld.maintFixedTimeFT","#{0} Maint PreNotify (Time)"), "edit=2"),
                new DBFieldTemplate(FLD_maintFixedTimeFT_  , Long.TYPE   , DBField.TYPE_UINT32    , I18N.getString(Device.class,"Device.fld.maintFixedTimeFT","#{0} Last Maint Time"       ), "format=time")
                );
        }
        return MaintFields_FixedTime_;
    }
    /* OBSOLETE
  ...
  //public static final String FLD_maintEngHoursHR0      = "maintEngHoursHR0";      // hours maint #0 last EngineHours
    */
    ...
    public static final DBField MaintFieldInfo[] = {
        /* OBSOLETE
    ...
    public static final String FLD_lastDataPushTime      = "lastDataPushTime";      // timestamp of last EventData record push (to another server)
    public static final String FLD_lastDataPullTime      = "lastDataPullTime";      // timestamp of last EventData record pull (from another server)
    ...
    public static final DBField DataPushInfo[]           = {
        ...
        new DBField(FLD_lastDataPullTime     , Long.TYPE           , DBField.TYPE_UINT32      , I18N.getString(Device.class,"Device.fld.lastDataPullTime"     , "Last Event Pull Time (sec)"  ), "format=time edit=2"),
    ...
    public static final String FLD_controllerModel       = "controllerModel";       // controller model (plow, speader, etc)
    public static final DBField AttributeInfo[]          = {
        ...
        new DBField(FLD_controllerModel      , String.class        , DBField.TYPE_STRING(30)  , I18N.getString(Device.class,"Device.fld.controllerModel"      , "Controller Model"            ), "edit=2"),
    };
  ...
  //public static final String FLD_endDayTimestamp 	 	 = "endDayTimestamp"; // usage appears to be same as FLD_lastEventTimestamp
    ...
    public static DBFactory<Device> getFactory()
    {
        if (factory == null) {
          ...
          //factory.setLogMissingColumnWarnings(RTConfig.getBoolean(DBConfig.PROP_Device_logMissingColumns,true));
            ...
            // -- FLD_vehicleID max length
            DBField vehIdFld = factory.getField(FLD_vehicleID);
            Device.VehicleIDColumnLength = (vehIdFld != null)? vehIdFld.getStringLength() : 0;
    ...
    **/
    ...
    {
        Set<String> flds = super._markRecordAs_Deleted();
        if (!ListTools.isEmpty(flds)) { // non-empty if deleted
            if (!StringTools.isBlank(this.getUniqueID())) {
                this.setUniqueID(null);
                flds.add(FLD_uniqueID);
            }
            if (!StringTools.isBlank(this.getDriverID())) {
                this.setDriverID(null);
                flds.add(FLD_driverID);
            }
        }
        return flds;
    }
    ...
    /**
    *** Returns true if a User GroupID has been assigned for this Device
    **/
    ...
    **/
    ...
    /**
    *** Returns a collection of DeviceGroup IDs to which this Device is assigned
    *** @param inclAll  True to include the "ALL" group in the list of IDs, false to not include "ALL".
    *** @return A Collection of DeviceGroup IDs to which this Device is assigned
    **/
    public Collection<String> getAssignedGroupIDs(DBReadWriteMode rwMode, boolean inclAll)
        throws DBException
    {
        String acctID = this.getAccountID();
        String devID  = this.getDeviceID();
        return DeviceGroup.getDeviceGroupsForDevice(rwMode, acctID, devID, inclAll);
    }

    /**
    *** Returns a collection of DeviceGroup IDs to which this Device is assigned (including "ALL")
    *** @return A Collection of DeviceGroup IDs to which this Device is assigned (including "ALL")
    **/
    public Collection<String> getAssignedGroupIDs(DBReadWriteMode rwMode)
        throws DBException
    {
        return this.getAssignedGroupIDs(rwMode, true);
    }

    @JSONBeanGetter(name="groupId", tags="getDeviceList")
    public String[] getGroupIDs(DBReadWriteMode rwMode)
    {
        rwMode = DBReadWriteMode.getDefaultReadWriteMode(rwMode);
        try {
            Collection<String> gid = this.getAssignedGroupIDs(rwMode, false);
            return gid.toArray(new String[gid.size()]);
        } catch (DBException dbe) {
            Print.logError("Could not get group ids for %s: %s", this.getDeviceID(), dbe.toString());
            return new String[0];
        }
    }
    ...
    **/
    ...
    /**
    *** Returns the motion/ignition state.
    **/
    @JSONBeanGetter(name="status", tags="getDeviceList")
    public int getStatus()
    {
        // -- TODO: this should return a more descriptive value, such as an enum or String value.
        // -  Or a bitmask that would separately inticate motion and ignition state (by bit)
        Device device = this;
        int answer = -1;
        double speedOffset = device.getParkedMaxSpeedKPH();
        double speed = device.getLastValidSpeedKPH();
        int ignitionIndex = device.getIgnitionIndex();
        boolean ignitionOff = device.isIgnitionOff(ignitionIndex);
        if(speed > speedOffset && ignitionOff)
        {
            // -- ignition-off, and moving
            answer = 3;
        }
        else if(speed > speedOffset && !ignitionOff)
        {
        ...
        {
            // -- ignition-off, and stopped
            answer = 2;
        }
        else if(speed <= speedOffset && !ignitionOff)
        {
            // -- ignition-on, and stopped
            answer = 0;
        }
    ...
    **/
    @JSONBeanGetter(ignore="$blank")
    public String getVehicleID() // VIN (see also "getDeviceVID()")
    {
    ...
    **/
    public void setVehicleID(String v)
    {
        ...
        /* trim and replace invalid characters in VIN */
        String VIN = StringTools.toStringValue(StringTools.trim(v),'?'); // this also replaces spaces
        if (VIN.length() > Device.VehicleIDColumnLength) {
            // -- truncate to max length (<DBFactory>.getField(FLD_vehicleID).getStringLength())
            VIN = VIN.substring(0, Device.VehicleIDColumnLength);
        }

        /* ignore specific VIN values */
        if (VIN.equals("0")) {
    ...
    /**
    *** Gets the Device corresponding to the specified VehicleID
    *** It is up to the caller to check whether this Device or Account are inactive.
    *** @param vehID  The Vehicle-ID of the device
    *** @return The loaded Device instance, or null if the Device was not found
    *** @throws DBException if a database error occurs
    **/
    public static Device loadDeviceByVehicleID(String vehID)
        throws DBException // DBDuplicateFoundException
    {
        return Device._loadDeviceByVehicleID(vehID, null);
    }
    ...
    *** It is up to the caller to check whether this Device or Account are inactive.
    *** @param vehID     The Vehicle-ID of the device
    *** @param fldVehID  The lookup field/column name (blank to use "vehicleID")
    *** @param showMult  If true, display multiple/duplicate VIN matches.
    *** @return The loaded Device instance, or null if the Device was not found
    *** @throws DBException if a database error occurs
    **/
    public static Device _loadDeviceByVehicleID(String vehID, String fldVehID)
        throws DBException // DBDuplicateFoundException
    {
        long limit = 3L;
        Device dev[] = Device._loadDevicesByVehicleID(vehID, fldVehID, limit);
        return !ListTools.isEmpty(dev)? dev[0] : null;
    }
    ...
    *** @throws DBException if a database error occurs
    **/
    public static Device[] _loadDevicesByVehicleID(String vehID, String fldVehID, long limit)
        throws DBException
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_ONLY;
        // -- default "vehicleID" field name
        if (StringTools.isBlank(fldVehID)) {
            fldVehID = Device.FLD_vehicleID;
        }
        // -- get/reurn device(s)
        Device dev[] = DBRecord._loadByAlternateKey(
            rwMode,
            Device.getFactory(), fldVehID,
            vehID, limit, false);
        return dev;
    }
    ...
    *** Gets the Device corresponding to the specified VehicleID
    *** It is up to the caller to check whether this Device or Account are inactive.
    *** @param rwMode    Read/Write mode
    *** @param dbFld     The alternate key DBField
    *** @param lookupKey The lookup key value
    *** @return The loaded Device instance, or null if the Device was not found
    *** @throws DBException if a database error occurs
    **/
    @Deprecated
    public static Device _loadDeviceByColumnValue(DBReadWriteMode rwMode, DBField dbFld, String lookupKey)
        throws DBException
    {
        rwMode = DBReadWriteMode.getDefaultReadWriteMode(rwMode); // READ_ONLY;
        ...
        /* invalid value? */
        if (StringTools.isBlank(lookupKey)) {
            return null; // just say it doesn't exist
        }

        /* column name */
        String dbFldName = dbFld._getName();
        if (!dbFld.isAlternateKey() && !dbFld.isPrimaryKey()) {
            Print.logWarn("Lookup requested on non-alternate key field: " + dbFldName);
        }

        /* read device for vehicle-id */
        Device       dev = null;
        ...
        try {
            ...
            dsel.setWhere(dwh.WHERE_(
                dwh.EQ(dbFldName, lookupKey)
            ));
            dsel.setLimit(2);
            // -- Note: The index on the column may not enforce uniqueness
            // -  (since null/empty values may be allowed)
            ...
            while (rs.next()) {
                ...
                if (rs.next()) {
                    Print.logError("Found multiple occurances of this column value: '"+dbFldName+"' ==> "+lookupKey);
                }
            ...
            throw new DBException("Getting Device column value: '"+dbFldName+"' ==> "+lookupKey, sqe);
        } finally {
    ...
    **/
    ...
    @JSONBeanGetter(name="licenseExpiration", tags="deviceAdminDetail")
    public String getLicenseExpire_units()
    {
        long dn = this.getLicenseExpire();
        if (dn < 0L) {
            return "";
        } else {
            BasicPrivateLabel bpl = Account.getContextPrivateLabel(this.getAccount());
            DateTime.DateStringFormat dsf = (bpl != null)? bpl.getDateStringFormat() : null;
            String dateYMD = (new DayNumber(dn)).format(DayNumber.DATE_FORMAT_YMD_1);
            return (dsf != null)? dsf.convertFromYMD(dateYMD) : dateYMD;
        }
    }
    ...
    **/
    public boolean hasValidDriverID()
    {
        String drvID = this.getDriverID();
        if (StringTools.isBlank(drvID)) {
            return false;
        } else {
            return Driver.isValidDriverID(drvID)? true : false;
        }
    }
    ...
    **/
    @JSONBeanGetter(ignore="$null", name="driver", tags="getDeviceList")
    ...
    **/
    ...
    {
        int ds = (v >= Driver.DutyStatus_UNKNOWN)? v : Driver.DutyStatus_UNKNOWN;
        this.setFieldValue(FLD_driverStatus, ds);
        this.addOtherChangedFieldNames(FLD_driverStatus);
    }

    /**
    *** Sets the Driver.DutyStatue enumerated type value 
    **/
    public void setDriverStatus(Driver.DutyStatus v)
    {
        int ds = (v != null)? v.getIntValue() : Driver.DutyStatus_INVALID;
        this.setDriverStatus(ds);
    }

    // --------------------------------

    /**
    *** Returns true if an on-duty driver-id is defined for this Device
    *** @param allowUnknown If true, an UNKNOWN DutyStatus will be considered OnDuty, 
    ***                     otherwise UNKNOWN will be considered OffDuty.
    *** @return True if this Device record defines a DriverID, with on-duty status
    **/
    public boolean hasOnDutyDriver(boolean allowUnknown)
    {
        return Driver.isOnDuty(this.getDriverID(), this.getDriverStatus(), allowUnknown);
    }

    /**
    *** Returns true if an driver-id is defined for this Device, with On-Duty or "Personal" status
    *** @return True if this Device record defines a DriverID, with On-Duty or "Personal" status
    **/
    public boolean hasAssignedDriver(boolean allowUnknown)
    {
    ...
    /**
    *** Returns true if a driver-time is defined for this Device
    *** @return True if a driver-time is defined for this Device
    **/
    public boolean hasDriverTime()
    {
        return (this.getDriverTime() > 0L)? true : false;
    }

    /**
    *** Gets the driver change time of this Device (in Unix Epoch time format)
    *** @return The driver change time of this Device, or '0' if undefined.
    **/
    public long getDriverTime()
    {
        Long v = (Long)this.getFieldValue(FLD_driverTime);
        return (v != null)? v.longValue() : 0L;
    }

    /**
    *** Sets the driver change time of this Device (in Unix Epoch time format)
    *** @param v The driver change time of this Device, or '0' if undefined.
    **/
    public void setDriverTime(long v)
    {
        this.setFieldValue(FLD_driverTime, (v >= 0L)? v : 0L);
    }
    ...
    **/
    ...
    /**
    *** Gets the approximate Fuel Economy, in the preferred Account Economy units
    *** @return The approximate Fuel Economy
    **/
    @JSONBeanGetter(ignore="<=0")
    public double getFuelEconomy_units()
    {
        Account.EconomyUnits econUnits = Account.getEconomyUnits(this.getAccount());
        return econUnits.convertFromKPL(this.getFuelEconomy());
    }

    /**
    *** Sets the approximate Fuel Economy, in the preferred Account Economy units
    *** @param v The approximate Fuel Economy
    **/
    public void setFuelEconomy_units(double v)
    {
        Account.EconomyUnits econUnits = Account.getEconomyUnits(this.getAccount());
        this.setFuelEconomy(econUnits.convertToKPL(v));
    }
    ...
    **/
    ...
    /**
    *** Gets the approximate Fuel Consumption Rate, in the preferred Account volume units, per hour
    *** @return The approximate Fuel Consumption Rate
    **/
    @JSONBeanGetter(ignore="<=0")
    public double getFuelRatePerHour_units()
    {
        Account.VolumeUnits volmUnits = Account.getVolumeUnits(this.getAccount());
        return volmUnits.convertFromLiters(this.getFuelRatePerHour());
    }

    /**
    *** Sets the approximate Fuel Consumption Rate, in the preferred Account volume units, per hour
    *** @param v The approximate Fuel Consumption Rate
    **/
    public void setFuelRatePerHour_units(double v)
    {
        Account.VolumeUnits volmUnits = Account.getVolumeUnits(this.getAccount());
        this.setFuelRatePerHour(volmUnits.convertToLiters(v));
    }
    ...
    **/
    ...
    /**
    *** Gets the Fuel cost, per preferred Account volume units
    *** @return The Fuel cost per preferred Account volume units
    **/
    @JSONBeanGetter(ignore="<=0")
    public double getFuelCostPerLiter_units()
    { 
        double costPerLiter = this.getFuelCostPerLiter();
        if (costPerLiter > 0.0) {
            Account.VolumeUnits volmUnits = Account.getVolumeUnits(this.getAccount());
            double litersPerUnit = volmUnits.convertToLiters(1.0); // Liters/Unit
            double costPerUnit   = costPerLiter * litersPerUnit; // $/Liter * Liters/Unit = $/Unit
            return costPerUnit;
        } else {
            return 0.0;
        }
    ...
    /**
    *** Sets the Fuel cost, per preferred Account volume units
    *** @param v The Fuel cost per preferred Account volume units
    **/
    public void setFuelCostPerLiter_units(double v)
    {
        if (v > 0.0) {
            Account.VolumeUnits volmUnits = Account.getVolumeUnits(this.getAccount());
            double unitsPerLiter = volmUnits.convertFromLiters(1.0); // Units/Liter
            double costPerLiter  = v/*costPerUnit*/ * unitsPerLiter; // $/Unit * Units/Liter = $/Liter
            this.setFuelCostPerLiter(costPerLiter);
        } else {
            this.setFuelCostPerLiter(0.0);
        }
    }
    ...
    **/
    @JSONBeanGetter(ignore="<=0", unitsEnum="Account.VolumeUnits")
    ...
    **/
    ...
    /**
    *** Gets the tank #1 Fuel Capacity, in the preferred account volume units
    *** @return The tank #1 Fuel Capacity
    **/
    @JSONBeanGetter(ignore="<=0")
    public double getFuelCapacity_units() // tank #1
    {
        Account.VolumeUnits volmUnits = Account.getVolumeUnits(this.getAccount());
        return volmUnits.convertFromLiters(this.getFuelCapacity());
    }

    /**
    *** Sets the tank #1 Fuel Capacity, in the preferred account volume units
    *** @param v The tank #1 Fuel Capacity
    **/
    public void setFuelCapacity_units(double v) // tank #1
    {
        Account.VolumeUnits volmUnits = Account.getVolumeUnits(this.getAccount());
        this.setFuelCapacity(volmUnits.convertToLiters(v));
    }
    ...
    **/
    ...
    /**
    *** Gets the tank #2 Fuel Capacity, in the preferred account volume units
    *** @return The tank #2 Fuel Capacity
    **/
    @JSONBeanGetter(ignore="<=0")
    public double getFuelCapacity2_units() // tank #2
    {
        Account.VolumeUnits volmUnits = Account.getVolumeUnits(this.getAccount());
        return volmUnits.convertFromLiters(this.getFuelCapacity2());
    }

    /**
    *** Sets the tank #2 Fuel Capacity, in the preferred account volume units
    *** @param v The tank #2 Fuel Capacity
    **/
    public void setFuelCapacity2_units(double v) // tank #2
    {
        Account.VolumeUnits volmUnits = Account.getVolumeUnits(this.getAccount());
        this.setFuelCapacity2(volmUnits.convertToLiters(v));
    }
    ...
    public double getFuelCapacity(Device.FuelTankIndex tank)
    {
        if (tank != null) {
            switch (tank) {
                ...
                case TANK_3 : return 0.0;                       // tank #3 (not supported)
                case TANK_4 : return 0.0;                       // tank #4 (not supported)
                case TANK_5 : return 0.0;                       // tank #5 (not supported)
    ...
    public String getFuelTankProfile(Device.FuelTankIndex tank)
    {
        if (tank != null) {
            switch (tank) {
                ...
                case TANK_3 : return ""; // not supported
                case TANK_4 : return ""; // not supported
                case TANK_5 : return ""; // not supported
                case TOTAL  : return ""; // no profile
            }
        }
        return "";
    }
    ...
    */
    ...
    /**
    *** Gets the engine displacement, in Liters
    *** @return The engine displacement, in Liters
    **/
    @JSONBeanGetter(ignore="<=0")
    public double getEngineDisplacement()
    {
        Double v = (Double)this.getFieldValue(FLD_engineDisplacement);
        return (v != null)? v.doubleValue() : 0.0;
    }

    /**
    *** Sets the engine displacement, in Liters
    *** @param v The engine displacement, in Liters
    **/
    public void setEngineDisplacement(double v)
    {
        this.setFieldValue(FLD_engineDisplacement, (v >= 0.0)? v : 0.0);
    }

    /**
    *** Returns true if this device has a defined engine displacement
    **/
    public boolean hasEngineDisplacement()
    {
        return (this.getEngineDisplacement() > 0.0)? true : false;
    }
    
    // --------------------------------

    /**
    *** Gets the engine displacement, in the preferred Account volume units
    *** @return The engine displacement
    **/
    @JSONBeanGetter(ignore="<=0")
    public double getEngineDisplacement_units()
    {
        Account.VolumeUnits volmUnits = Account.getVolumeUnits(this.getAccount());
        return volmUnits.convertFromLiters(this.getEngineDisplacement());
    }

    /**
    *** Sets the engine displacement, in the preferred Account volume units
    *** @param v The engine displacement
    **/
    public void setEngineDisplacement_units(double v)
    {
        Account.VolumeUnits volmUnits = Account.getVolumeUnits(this.getAccount());
        this.setEngineDisplacement(volmUnits.convertToLiters(v));
    }
    ...
    **/
    ...
    /**
    *** Gets the assigned speed limit for this device, in the preferred account distance units
    *** @return The assigned speed limit for this device
    **/
    @JSONBeanGetter(ignore="<=0")
    public double getSpeedLimitKPH_units()
    {
        Account.DistanceUnits distUnits = Account.getDistanceUnits(this.getAccount());
        return distUnits.convertFromKM(this.getSpeedLimitKPH());
    }

    /**
    *** Sets the assigned speed limit for this device, in the preferred account distance units
    *** @param v The assigned speed limit for this device
    **/
    public void setSpeedLimitKPH_units(double v)
    {
        Account.DistanceUnits distUnits = Account.getDistanceUnits(this.getAccount());
        this.setSpeedLimitKPH(distUnits.convertToKM(v));
    }

    // --------------------------------

    /**
    *** Gets the assigned speed offset for this device, in km/h
    *** @return The assigned speed offset for this device, in km/h.  
    ***     If the "speedOffsetKPH" does not yet exist in the Device table, then
    ***     then 8.04672 km/h (5 mph) will be returned.
    **/
    @JSONBeanGetter(ignore="<=0")
    public double getSpeedOffsetKPH()
    {
        Double v = (Double)this.getFieldValue(FLD_speedOffsetKPH);
        return (v != null)? Math.max(0.0,v.doubleValue()) : (5.0 * GeoPoint.KILOMETERS_PER_MILE);
    }

    /**
    *** Sets the assigned speed offset for this device, in km/h
    *** @param v The assigned speed offset for this device, in km/h
    **/
    public void setSpeedOffsetKPH(double v)
    {
        this.setFieldValue(FLD_speedOffsetKPH, (v >= 0.0)? v : 0.0);
    }

    /**
    *** Returns true if this device has a speed offset assigned
    **/
    public boolean hasSpeedOffsetKPH()
    {
        return (this.getSpeedOffsetKPH() > 0.0)? true : false;
    }

    // --------------------------------

    /**
    *** Gets the assigned speed offset for this device, in the preferred account distance units
    ***     If the "speedOffsetKPH" does not yet exist in the Device table, then
    ***     then 8.04672 km/h (5 mph) will be returned.
    *** @return The assigned speed offset for this device
    **/
    @JSONBeanGetter(ignore="<=0")
    public double getSpeedOffsetKPH_units()
    {
        Account.DistanceUnits distUnits = Account.getDistanceUnits(this.getAccount());
        return distUnits.convertFromKM(this.getSpeedOffsetKPH());
    }

    /**
    *** Sets the assigned speed offset for this device, in the preferred account distance units
    *** @param v The assigned speed offset for this device
    **/
    public void setSpeedOffsetKPH_units(double v)
    {
        Account.DistanceUnits distUnits = Account.getDistanceUnits(this.getAccount());
        this.setSpeedOffsetKPH(distUnits.convertToKM(v));
    }
    ...
    *** Returns true if the specified speed exceeds the speed limit of this Device instance
    **/
    public boolean isSpeeding(double speedKPH)
    {
        double offsetKPH = this.getSpeedOffsetKPH();
        return this.isSpeeding(speedKPH, offsetKPH);
    }
    ...
    **/
    public static boolean IsSpeeding(EventData ed, double dftLimitKPH, double dftOffsetKPH, boolean checkGeozones)
    {
        ...
        Device device = ed.getDevice(); // should not be null, but check anyway
        double evSpeedKPH = ed.getSpeedKPH();

        /* use device offset? */
        if ((dftOffsetKPH <= 0.0) && (device != null) && device.hasSpeedOffsetKPH()) {
            dftOffsetKPH = device.getSpeedOffsetKPH();
        }
        ...
        if ((dftLimitKPH > 0.0) && (evSpeedKPH > (dftLimitKPH + dftOffsetKPH))) {
            ...
            if (device != null) {
                ...
                rtp.setProperty("Speeding.type"  ,new I18N.Text("parameter")); // Fatigue
                rtp.setProperty("Speeding.limit" ,new I18N.Text(StringTools.format(dftLimitKPH,"0.0")));
                rtp.setProperty("Speeding.offset",new I18N.Text(StringTools.format(dftOffsetKPH,"0.0")));
            }
            return true; // exceeded specified limit
        }

        // -- 2: device-speed
        if ((device != null) && device.isSpeeding(evSpeedKPH,dftOffsetKPH)) {
            //Print.logInfo("Speeding: > device limit");
            RTProperties rtp = device.getTemporaryProperties();
            rtp.setProperty("Speeding.type"  ,new I18N.Text("device"));
            rtp.setProperty("Speeding.limit" ,new I18N.Text(StringTools.format(device.getSpeedLimitKPH(),"0.0")));
            rtp.setProperty("Speeding.offset",new I18N.Text(StringTools.format(dftOffsetKPH,"0.0")));
        ...
        if (ed.hasSpeedLimitKPH()) {
            ...
            if ((limKPH > 0.0) && (evSpeedKPH > (limKPH + dftOffsetKPH))) {
                ...
                if (device != null) {
                    RTProperties rtp = device.getTemporaryProperties();
                    rtp.setProperty("Speeding.type"  ,new I18N.Text("posted"));
                    rtp.setProperty("Speeding.limit" ,new I18N.Text(StringTools.format(limKPH,"0.0")));
                    rtp.setProperty("Speeding.offset",new I18N.Text(StringTools.format(dftOffsetKPH,"0.0")));
                }
        ...
        if (checkGeozones) {
            ...
            if ((GZ != null) && GZ.isDeviceInGroup(ed.getDeviceID())) {
                ...
                if ((limKPH > 0.0) && (evSpeedKPH > (limKPH + dftOffsetKPH))) {
                    ...
                    if (device != null) {
                        RTProperties rtp = device.getTemporaryProperties();
                        rtp.setProperty("Speeding.type"  ,new I18N.Text("geozone"));
                        rtp.setProperty("Speeding.limit" ,new I18N.Text(StringTools.format(limKPH,"0.0")));
                        rtp.setProperty("Speeding.offset",new I18N.Text(StringTools.format(dftOffsetKPH,"0.0")));
                    }
                    return true;
    ...
    **/
    public static boolean IsExcessiveRpm(EventData ed, long dftMaxRPM)
    {
        ...
        if ((dftMaxRPM > 0L) && (evEngineRpm > dftMaxRPM)) {
            ...
            if (device != null) {
                RTProperties rtp = device.getTemporaryProperties();
                rtp.setProperty("ExcessRPM.type"  ,new I18N.Text("parameter"));
                rtp.setProperty("ExcessRPM.limit" ,new I18N.Text(StringTools.format(dftMaxRPM,"0")));
            }
            return true; // exceeded specified limit
        }
        ...
        if (device != null) { // device.hasMaximumRpm()
            ...
            if ((maxRPM > 0L) && (evEngineRpm > maxRPM)) {
                ...
                if (device != null) {
                    RTProperties rtp = device.getTemporaryProperties();
                    rtp.setProperty("ExcessRPM.type"  ,new I18N.Text("device"));
                    rtp.setProperty("ExcessRPM.limit" ,new I18N.Text(StringTools.format(maxRPM,"0")));
                }
    ...
    **/
    ...
    /**
    *** Gets the assigned "Plan Distance" for this device, in the preferred account distance units
    *** @return The assigned "Plan Distance" for this device
    **/
    public double getPlanDistanceKM_units()
    {
        Account.DistanceUnits distUnits = Account.getDistanceUnits(this.getAccount());
        return distUnits.convertFromKM(this.getPlanDistanceKM());
    }

    /**
    *** Sets the assigned "Plan Distance" for this device, in the preferred account distance units
    *** @param v The assigned "Plan Distance" for this device
    **/
    public void setPlanDistanceKM_units(double v)
    {
        Account.DistanceUnits distUnits = Account.getDistanceUnits(this.getAccount());
        this.setPlanDistanceKM(distUnits.convertToKM(v));
    }
    ...
    **/
    ...
    public static final long    MAX_ACTIVE_AGE_SEC      = DateTime.HourSeconds(24);

    /**
    *** Returns true if this Device has a last notify time that is younger than the specified age
    **/
    public boolean hasCurrentNotify(long maxAgeSec, long nowTimeSec)
    {
        long lastNotifyTime = this.getLastNotifyTime();
        if (lastNotifyTime <= 0L) {
            return false;
        } else {
            return ((lastNotifyTime + maxAgeSec) > nowTimeSec)? true : false;
        }
    }

    /**
    *** Returns true if this Device has a last notify time that is younger than the specified age
    *** specified in the current context PrivateLabel, and the current system time.
    **/
    @JSONBeanGetter(ignore="false")
    public boolean hasCurrentNotify()
    {
        Account acct = this.getAccount();
        BasicPrivateLabel bpl = (acct != null)? acct.getContextPrivateLabel() : null;
        long maxAgeSec = (bpl != null)? // unlikely to be null
            bpl.getLongProperty(BasicPrivateLabel.PROP_DeviceAlerts_maxActiveAlertAge,MAX_ACTIVE_AGE_SEC) :
            MAX_ACTIVE_AGE_SEC;
        return this.hasCurrentNotify(maxAgeSec, DateTime.getCurrentTimeSec());
    }
    ...
    **/
    public void setLastNotifyEvent(long timestamp, String ruleID, boolean updateNow)
        throws DBException
    {
        if (timestamp >= 0L) {
            Print.logDebug("[Timestamp] ts="+timestamp+" sc="+StatusCodes.GetDesc(StatusCodes.STATUS_NONE)+" rule="+ruleID+" [upd="+updateNow+"]");
            this.setLastNotifyTime(timestamp);                  // FLD_lastNotifyTime
            this.setLastNotifyCode(StatusCodes.STATUS_NONE);    // FLD_lastNotifyCode
        } else {
            Print.logDebug("[Timestamp] ts="+0L+" sc="+StatusCodes.GetDesc(StatusCodes.STATUS_NONE)+" rule="+ruleID+" [upd="+updateNow+"]");
    ...
    **/
    ...
    {
        if (event != null) {
            Print.logDebug("[Event] ts="+event.getTimestamp()+" sc="+StatusCodes.GetDesc(event.getStatusCode())+" rule="+ruleID+" [upd="+update+"]");
            ...
            Print.logDebug("[Event] ts="+0L+" sc="+StatusCodes.GetDesc(StatusCodes.STATUS_NONE)+" rule="+ruleID+" [upd="+update+"]");
            this.setLastNotifyTime(0L);                         // FLD_lastNotifyTime
            this.setLastNotifyCode(StatusCodes.STATUS_NONE);    // FLD_lastNotifyCode
        }
        this.setLastNotifyRule(ruleID);                         // FLD_lastNotifyRule
    ...
    **/
    public void clearLastNotifyEvent(boolean update)
        throws DBException
    {
        this.setLastNotifyEvent(null/*EventData*/, ""/*RuleID*/, update); // clear
    }
    ...
    public String getNotifyEmail(boolean inclAccount, boolean inclUser, boolean inclGroup)
    {
        ...
        if (inclUser) {
            ...
            try {
                ...
                if (!user.isAuthorizedDevice(rwMode,this.getDeviceID())) { // DBException
                    // skip (not authorized)
                } else {
        ...
        if (inclGroup && DeviceGroup.supportsNotification()) {
            try {
                String acctID = this.getAccountID();
                String devID  = this.getDeviceID();
                Collection<String> grpIDs = DeviceGroup.getDeviceGroupsForDevice(rwMode, acctID, devID, false);
    ...
    **/
    @JSONBeanGetter(name="isParked", tags="getDeviceList")
    ...
    **/
    ...
    /**
    *** Gets the parked maximum speed, in the preferred account distance units
    *** @return The parked maximum speed
    **/
    public double getParkedMaxSpeedKPH_units()
    {
        Account.DistanceUnits distUnits = Account.getDistanceUnits(this.getAccount());
        return distUnits.convertFromKM(this.getParkedMaxSpeedKPH());
    }

    /**
    *** Sets the parked maximum speed, in the preferred account distance units
    *** @param v The parked maximum speed
    **/
    public void setParkedMaxSpeedKPH_units(double v)
    {
        Account.DistanceUnits distUnits = Account.getDistanceUnits(this.getAccount());
        this.setParkedMaxSpeedKPH(distUnits.convertToKM(v));
    }
    ...
    **/
    public String getModemID() // aka. getMobileID
    {
        if (StringTools.isBlank(this.modemID)) {
            ...
            if (CHECK_IMEI_FOR_MODEM_ID && !StringTools.isBlank(imei)) {
                ...
                this.modemID = this._getUniqueID_comp()[1];
            }
        }
        return this.modemID;
    }
    ...
    **/
    public String getUniqueID()
    {
        if (this.hasTransport()) {
            return this.getTransport().getUniqueID();
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            this.getTransport().setUniqueID(v);
        } else {
    ...
    **/
    ...
    /**
    *** Separate the UniqueID into the prefix and mobileID
    *** @return 2-element String array containing prefix and mobileID
    **/
    private String[] _getUniqueID_comp() // see "getModemID()"
    {
        // -- "PREFIX_MOBILEID" ==> { "PREFIX", "MOBILEID" }
        String uidComp[] = { "", "" };
        String uid = this.getUniqueID();
        if (uid.length() > 0) {
            int p = uid.indexOf("_");
            if (p < 0) { p = uid.indexOf("-"); }
            if (p < 0) {
                uidComp[1] = uid;
            } else {
                uidComp[0] = uid.substring(0,p);
                uidComp[1] = uid.substring(p+1);
            }
        }
        return uidComp;
    }
    ...
    /**
    *** Gets the UniqueID prefix (sans the "_","-" separator)
    **/
    public String getUniqueIDPrefix()
    {
        return this._getUniqueID_comp()[0];
    }
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getDeviceCode();
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            this.getTransport().setDeviceCode(v);
        } else {
    ...
    /**
    *** Returns a Collection of matching Device.Key instances for the specified DCS name
    *** @param dcsName   The DCS name ("deviceCode")
    *** @return And array of Device Keys for the specified device-code (DCS name)
    **/
    public static Collection<Device.Key> getDeviceKeysForDeviceCode(String dcsName)
        throws DBException
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_ONLY;
        ...
        if (StringTools.isBlank(dcsName)) {
            throw new DBException("DCS name not specified");
        }

        /* read devices */
        java.util.List<Device.Key> devList = new Vector<Device.Key>();
        DBConnection dbc = null;
        Statement   stmt = null;
        ResultSet     rs = null;
        try {
            ...
            /* select */
            // DBSelect: SELECT * FROM Device WHERE (deviceCode='tk10x')
            DBSelect<Device> dsel = new DBSelect<Device>(Device.getFactory());
            ...
            DBWhere dwh = dsel.createDBWhere();
            if (dcsName.startsWith(DCServerFactory.DCSNAME_SERVLET_PREFIX)) {
                // -- "DCSNAME" or "w-DCSNAME"
                dsel.setWhere(dwh.WHERE_(
                    dwh.OR(
                        dwh.EQ(Device.FLD_deviceCode,dcsName), // "w-gprmc"
                        dwh.EQ(Device.FLD_deviceCode,dcsName.substring(DCServerFactory.DCSNAME_SERVLET_PREFIX.length()))
                    )
                ));
            } else {
                // -- "DCSNAME"
                dsel.setWhere(dwh.WHERE_(
                    dwh.EQ(Device.FLD_deviceCode,dcsName)
                ));
            }
            dsel.setOrderByFields(Device.FLD_accountID, Device.FLD_deviceID);
            ...
            while (rs.next()) {
                String acctID = rs.getString(FLD_accountID);
                String devID  = rs.getString(FLD_deviceID);
                devList.add(new Device.Key(acctID,devID));
            }

        } catch (SQLException sqe) {
            throw new DBException("Get Device deviceCode", sqe);
        } finally {
            if (rs   != null) { try { rs.close();   } catch (Throwable t) {} }
            if (stmt != null) { try { stmt.close(); } catch (Throwable t) {} }
            DBConnection.release(dbc);
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getDeviceType();
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            this.getTransport().setDeviceType(v);
        } else {
    ...
    /**
    *** Gets the Device-Type (model) for this Device
    *** @param dftUidPfx  If true, and the deviceType field is blank, return the UniqueID prefix
    *** @return The device type or uniqueID prefix
    **/
    public String getDeviceType(boolean dftUidPfx)
    {
        String devType = this.getDeviceType();
        if (!StringTools.isBlank(devType)) {
            return devType;
        }
        return dftUidPfx? this._getUniqueID_comp()[0] : "";
    }
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getDcsPropertiesID();
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            this.getTransport().setDcsPropertiesID(v);
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getDcsCommandHost();
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            this.getTransport().setDcsCommandHost(v);
        } else {
    ...
    **/
    public String getDcsCommandState()
    {
        // -- command=COMMAND timestamp=EPOCH state=300
        if (this.hasTransport()) {
            return this.getTransport().getDcsCommandState();
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            this.getTransport().setDcsCommandState(v);
        } else {
    ...
    **/
    ...
    /**
    *** Returns true if this Device has a defined icon/image URL
    *** @return True if this Device has a defined icon/image URL
    **/
    public boolean hasIconURL()
    {
        return !StringTools.isBlank(this.getIconURL());
    }

    /**
    *** Gets the Icon/Image-URL for this Device
    *** @return The Icon/Image-URL for this Device
    **/
    public String getIconURL()
    {
        String v = (String)this.getOptionalFieldValue(FLD_iconURL);
        return StringTools.trim(v);
    }

    /**
    *** Sets the Icon/Image-URL for this Device
    *** @param v The Icon/Image-URL for this Device
    **/
    public void setIconURL(String v)
    {
        this.setOptionalFieldValue(FLD_iconURL, StringTools.trim(v));
    }

    // --------------------------------

    private PushpinIcon devicePushpinIcon;

    @JSONBeanGetter(name="pushpin", tags="getDeviceList")
    public PushpinIcon getPushpinIcon()
    {
        return this.devicePushpinIcon;
    }

    public void setPushpinIcon(OrderedMap<String, PushpinIcon> iconMap)
    {
        String pushpinId = this.getPushpinID();
        if(pushpinId.equals("")) 
        {
            pushpinId = "red";
        }
        this.devicePushpinIcon = iconMap.get(pushpinId);
    }
    ...
    **/
    @JSONBeanGetter(name="pushpinId", tags="getDeviceList")
    ...
    **/
    ...
    /**
    *** Returns true if this Device has a specific defined map route display color.
    *** @return True if this Device has a specific defined map route display color.
    **/
    ...
    /**
    *** Gets the defined map route display color
    *** @return The defined map route display-color
    **/
    ...
    /**
    *** Gets the defined map route display-color, or returns the specified default color
    *** if no display-color is defined.
    ...
    **/
    ...
    /**
    *** Sets the map route display color
    *** @param v The map route display color
    **/
    ...
    **/
    ...
    {
        //return "((mph >= 65)? \"cyan\": \"\")"; // test/debug only
        String sel = this.getRouteSegColorSel();
        if (!StringTools.isBlank(sel)) {
            return sel;
        } else {
            // -- check Account
            Account acct = this.getAccount(); // should not be null
            return (acct != null)? acct.getRouteSegmentColorSelector() : "";
        }
    }
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getSimPhoneNumber();
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            this.getTransport().setSimPhoneNumber(v);
        } else {
    ...
    **/
    public static java.util.List<String> getDeviceIDsForSimPhoneNumber(DBReadWriteMode rwMode, String simPhone)
        throws DBException
    {
        return Device.getDeviceIDsForSimPhoneNumber(rwMode, simPhone, (char)0);
    }
    ...
    *** @return And array of Device-IDs for the specified SIM phone number
    **/
    public static java.util.List<String> getDeviceIDsForSimPhoneNumber(DBReadWriteMode rwMode, String simPhone, char sepCH)
        throws DBException
    {
        rwMode = DBReadWriteMode.getDefaultReadWriteMode(rwMode);
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getSimID();
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getSmsEmail();
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            this.getTransport().setSmsEmail(v);
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getImeiNumber();
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            this.getTransport().setImeiNumber(v);
        } else {
    ...
    **/
    public boolean validateDataKey(String _dk) // aka. "authPIN", "AuthorizationPIN"
    {
    ...
    **/
    ...
    /**
    *** Gets the last event count
    **/
    //@JSONBeanGetter(name="lastEventCount")
    //public long getLastEventCount()
    //{
    //    Long v = (Long)this.getFieldValue(FLD_lastEventCount);
    //    return (v != null)? v.longValue() : 0L;
    //}
    ...
    **/
    @JSONBeanGetter(name="lastInputState", tags="getDeviceList")
    public long getLastInputState()
    {
        if (this.hasTransport()) {
            return this.getTransport().getLastInputState();
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            this.getTransport().setLastInputState(v);
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getLastOutputState();
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            this.getTransport().setLastOutputState(v);
        } else {
    ...
    **/
    ...
    {
        if (tank != null) {
            switch ((tank != null)? tank : Device.FuelTankIndex.TOTAL) {
                case TANK_1 : return this.getLastFuelLevel();
                case TANK_2 : return this.getLastFuelLevel2();
                case TANK_3 : return 0.0; // not supported
                case TANK_4 : return 0.0; // not supported
                case TANK_5 : return 0.0; // not supported
                case TOTAL  : {
                    if (this._fuelLevelTotal > 0.0) {
                        ...
                        return (allCapL > 0.0)? (this.getLastFuelRemain(Device.FuelTankIndex.TOTAL) / allCapL) : 0.0;
                    }
                }
            }
        }
    ...
    **/
    public void setLastFuelLevel(Device.FuelTankIndex tank, double v)
    {
        if (tank != null) {
            switch (tank) {
                ...
                case TANK_3 : /*NO-OP*/;                 break;
                case TANK_4 : /*NO-OP*/;                 break;
                case TANK_5 : /*NO-OP*/;                 break;
                case TOTAL  : {
                    this._fuelLevelTotal = (v > 0.0)? v : 0.0; 
                    break;
                }
            }
        }
    ...
    /**
    *** Gets the last fuel remaining (Litres) for all tanks based on the fuel levels and capacities.
    *** Note: this is based on the last fuel-level and fuel tank capacity, not the last "fuelRemain"
    *** value stored in the EventData table.
    **/
    public double getLastFuelRemain(Device.FuelTankIndex tank)
    {
        if (tank != null) {
            switch ((tank != null)? tank : Device.FuelTankIndex.TOTAL) {
                case TANK_1 : return this.getLastFuelLevel(tank) * this.getFuelCapacity(tank);
                case TANK_2 : return this.getLastFuelLevel(tank) * this.getFuelCapacity(tank);
                case TANK_3 : return 0.0; // not supported
                case TANK_4 : return 0.0; // not supported
                case TANK_5 : return 0.0; // not supported
                case TOTAL  : {
    ...
    /**
    *** Gets the last fuel total recieved (in Liters)
    *** @return The last fuel total recieved (in Liters)
    **/
    ...
    /**
    *** Sets the last fuel total recieved (in Liters)
    *** @param v The last fuel total recieved (in Liters)
    **/
    ...
    /**
    *** Gets the last fuel total recieved, in the preferred account volume units
    *** @return The last fuel total recieved
    **/
    public double getLastFuelTotal_units()
    {
        Account.VolumeUnits volmUnits = Account.getVolumeUnits(this.getAccount());
        return volmUnits.convertFromLiters(this.getLastFuelTotal());
    }

    /**
    *** Sets the last fuel total recieved, in the preferred account volume units
    *** @param v The last fuel total recieved
    **/
    public void setLastFuelTotal_units(double v)
    {
        Account.VolumeUnits volmUnits = Account.getVolumeUnits(this.getAccount());
        this.setLastFuelTotal(volmUnits.convertToLiters(v));
    }
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getIgnitionIndex();
        } else {
    ...
    **/
    public void setIgnitionIndex(int v)
    {
        ...
        if (this.hasTransport()) {
            this.getTransport().setIgnitionIndex(v);
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getCodeVersion();
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            this.getTransport().setCodeVersion(v);
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getFeatureSet();
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            this.getTransport().setFeatureSet(v);
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getIpAddressValid();
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            this.getTransport().setIpAddressValid(v);
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getIpAddressCurrent();
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            this.getTransport().setIpAddressCurrent(v);
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getRemotePortCurrent();
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            this.getTransport().setRemotePortCurrent(v);
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getListenPortCurrent();
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            this.getTransport().setListenPortCurrent(v);
        } else {
    ...
    **/
    @JSONBeanGetter(name="lastLatitude", tags="getDeviceList")
    ...
    **/
    @JSONBeanGetter(name="lastLongitude", tags="getDeviceList")
    ...
    ***     The pertinent Device fields (timestamp, age, location, speed, heading) will
    ***     also be updated, however the changed device records are not saved.
    *** @return The last known valid GeoPoint for this Device
    **/
            ...
            try {
                ...
                if ((lastEv != null) && lastEv.isValidGeoPoint()) {
                    ...
                    if (this.getLastOdometerKM() <= 0.0) {
                        ...
                        this.setLastOdometerKM(odomKM); // FLD_lastOdometerKM (may still be 0.0)
                    }
                }
            } catch (DBException dbe) {
                // ignore error
    ...
    **/
    public static class NearbyDevice
    {
        ...
        @JSONBeanGetter(name="deviceId", tags="getDeviceList")
        ...
        public Device getDevice() throws DBException {
            if (this.device == null) {
                DBReadWriteMode rwMode = DBReadWriteMode.READ_ONLY;
                this.device = Device.getDevice(rwMode,this.getAccount(),this.getDeviceID());
            } 
            return this.device;
        }
        @JSONBeanGetter(name="deviceName", tags="getDeviceList")
    ...
    **/
    public Map<String,Device.NearbyDevice> getNearbyDevices(DBReadWriteMode rwMode, double radiusM, long maxAgeSec, User userAuth)
        throws DBException
    {
        rwMode = DBReadWriteMode.getDefaultReadWriteMode(rwMode);
        Account account = this.getAccount();
    ...
    *** @param gp             The GPS location (required)
    *** @param maxRadiusM     The maximum radius in meters (zero for no limit)
    ...
    **/
    public static Map<String,NearbyDevice> GetNearbyDeviceMap(
        DBReadWriteMode rwMode,
        Account account,
        String targetDevID, boolean inclTargetID, 
        long startTime, long endTime,
        GeoPoint gp, double maxRadiusM, 
        ...
        throws DBException
    {
        ...
        if (maxRadiusM > 0.0) {
            bounds = new GeoBounds(maxRadiusM, gp);
            if (!bounds.isValid()) {
                // -- invalid bounds
        ...
        try {
            ...
            while (rs.next()) {
              //String acctID  = rs.getString(Device.FLD_accountID); <== all from same account
                ...
                if ((userAuth != null) && !userAuth.isAuthorizedDevice(rwMode,devID)) {
                    // -- user not authorized
                } else {
                    // -- save deviceID
    ...
    **/
    ...
    /** 
    *** Returns the last valid street address, based on the last known location
    *** @return The last valid street address
    **/
    @JSONBeanGetter(name="lastAddress", tags="getDeviceList")
    public String getLastValidAddress()
    {
        //String accountID  = this.getAccountID();
        //String deviceID   = this.getDeviceID();
        //long   timestamp  = this.getLastGPSTimestamp();
        //int    statusCode = 0; // this.getLastGPSStatusCode();
        // -------
        try {
            EventData ev = this.getLastEvent(true);
            return (ev != null)? ev.getAddress() : "";
        } catch (DBException dbe) {
            // -- DB access error
            return "";
        }
        // -- it is possible that this method might return an address for a location
        // -    that does not match "this.getLastValidLocation()"
        // -- TODO: optimize   
    }
    ...
    /**
    *** Returns true if the "lastSubdivision" field is present.
    *** @return True if the "lastSubdivision" field is present.
    **/
    public static boolean supportsLastSubdivision()
    {
        return Device.getFactory().hasField(FLD_lastSubdivision);
    }
    ...
    **/
    ...
    {
        String v = (String)this.getOptionalFieldValue(FLD_lastSubdivision);
        return StringTools.trim(v);
    }
    ...
    **/
    ...
    {
        this.setOptionalFieldValue(FLD_lastSubdivision, StringTools.trim(v));
    }
    ...
    **/
    @JSONBeanGetter(name="lastSpeedKPH", tags="getDeviceList")
    ...
    **/
    ...
    /**
    *** Gets the last valid speed, in the preferred account distance units
    *** @return The last valid speed
    **/
    @JSONBeanGetter(name="lastSpeedKPH_units", tags="getDeviceList")
    public double getLastValidSpeedKPH_units()
    {
        double speedKPH = this.getLastValidSpeedKPH();
        if (speedKPH >= 0.0) {
            Account.DistanceUnits distUnits = Account.getDistanceUnits(this.getAccount());
            return distUnits.convertFromKM(speedKPH);
        } else {
            return -1.0;
        }
    }
    ...
    /**
    *** Sets the last valid speed, in the preferred account distance units
    *** @param v The last valid speed
    **/
    public void setLastValidSpeedKPH_units(double v)
    {
        if (v >= 0.0) {
            Account.DistanceUnits distUnits = Account.getDistanceUnits(this.getAccount());
            this.setLastValidSpeedKPH(distUnits.convertToKM(v));
        } else {
            this.setLastValidSpeedKPH(-1.0);
        }
    }
    ...
    **/
    @JSONBeanGetter(name="lastHeading", tags="getDeviceList")
    ...
    **/
    @JSONBeanGetter(name="lastGPSTimestamp", tags="getDeviceList")
    ...
    **/
    ...
    /**
    *** Sets the last event timestamp (without regard to valid/invalid GPS)
    ...
    **/
    ...
    /**
    *** Gets the last event timestamp
    *** @param checkEventData If true, and the "lastEventTimestamp" is 0, then get the
    *** last event timestamp by reading the last existing EventData record.
    *** @return The last event timestamp
    **/
    public long getLastEventTimestamp(boolean checkEventData)
    {
        long lastEvTS = this.getLastEventTimestamp();
        if ((lastEvTS <= 0L) && checkEventData) {
            try {
                EventData ev = this.getLastEvent(false);
                if (ev != null) {
                    lastEvTS = ev.getTimestamp();
                }
            } catch (DBException dbe) {
                // -- DB access error
            }
    ...
    **/
    ...
    /**
    *** Sets the last status code received for this device (without regard to valid/invalid GPS)
    ...
    **/
    public java.util.List<GeozoneTransition> checkGeozoneTransitions(long eventTime, GeoPoint eventGP)
    {
        ...
        boolean GET_SINGLE_GEOZONE_FOR_DEVICE    = true; // [2.4.9-B15]  (only used if GET_CATEGORY_GEOZONES_FOR_DEVICE is false)
    ...
    **/
    ...
    /**
    *** Gets the last distance value, in the preferred account distance units
    *** @return The last distance value
    **/
    public double getLastDistanceKM_units()
    {
        Account.DistanceUnits distUnits = Account.getDistanceUnits(this.getAccount());
        return distUnits.convertFromKM(this.getLastDistanceKM());
    }

    /**
    *** Sets the last distance value, in the preferred account distance units
    *** @param distKM  The last distance value
    **/
    public void setLastDistanceKM_units(double dist)
    {
        Account.DistanceUnits distUnits = Account.getDistanceUnits(this.getAccount());
        this.setLastDistanceKM(distUnits.convertToKM(dist));
    }
            ...
            try {
                ...
                if ((lastEv != null) && lastEv.isValidGeoPoint()) {
                    ...
                    if (this.getLastValidLocation() == null) {
                        this.setLastValidLocation(
                            lastEv.getTimestamp(),      // FLD_lastGPSTimestamp
                            lastEv.getGpsAge(),         // n/a [2.6.2-B44]
                            lastEv.getGeoPoint(),       // FLD_lastValidLatitude/FLD_lastValidLongitude
                            lastEv.getSpeedKPH(),       // FLD_lastValidSpeedKPH
                            lastEv.getHeading());       // FLD_lastValidHeading
                    }
    ...
    **/
    ...
    /**
    *** Gets the last odometer value, in the preferred account distance units
    *** @return The last odometer value
    **/
    public double getLastOdometerKM_units()
    {
        Account.DistanceUnits distUnits = Account.getDistanceUnits(this.getAccount());
        return distUnits.convertFromKM(this.getLastOdometerKM());
    }

    /**
    *** Sets the last odometer value, in the preferred account distance units
    *** @param odomKM  The last odometer value
    **/
    public void setLastOdometerKM_units(double odom)
    {
        Account.DistanceUnits distUnits = Account.getDistanceUnits(this.getAccount());
        this.setLastOdometerKM(distUnits.convertToKM(odom));
    }
    ...
    **/
    ...
    /**
    *** Gets the displayed odometer offset, in the preferred account distance units
    *** @return The displayed odometer offset
    **/
    public double getOdometerOffsetKM_units()
    {
        Account.DistanceUnits distUnits = Account.getDistanceUnits(this.getAccount());
        return distUnits.convertFromKM(this.getOdometerOffsetKM());
    }

    /**
    *** Sets the displayed odometer offset, in the preferred account distance units
    *** @param v The displayed odometer offset
    **/
    public void setOdometerOffsetKM_units(double v)
    {
        Account.DistanceUnits distUnits = Account.getDistanceUnits(this.getAccount());
        this.setOdometerOffsetKM(distUnits.convertToKM(v));
    }
    ...
    **/
    ...
    /**
    *** Gets the displayed PTO-hours offset
    *** @return The displayed PTO-hours offset
    **/
    public double getPtoHoursOffset()
    {
        return this.getOptionalFieldValue(FLD_ptoHoursOffset, 0.0);
    }

    /**
    *** Sets the displayed PTO-hours offset
    *** @param v The displayed PTO-hours offset
    **/
    public void setPtoHoursOffset(double v)
    {
        this.setOptionalFieldValue(FLD_ptoHoursOffset, v);
    }
    ...
    *** Specified fault code must be in RTProperties String format.
    ...
    **/
    public void appendLastFaultCode(String v)
    {
        ...
        if (!DTOBDFault.IsTypeEqual(newFC,oldFC)) {
            // -- types differ, take new type (display warning?)
            this.setLastFaultCode(v);
            return;
        } 
        ...
        if (DTOBDFault.IsOBDII(oldFC)) {
            // -- 'newFC' is assumed to also be OBJII
            ...
            for (String dtc : newDTC) {
                ...
                if (!ListTools.contains(oldDTC,dtc)) {
                    oldDTC = ListTools.add(oldDTC,dtc); // array add
                    changed = true;
                }
            }
            if (changed) {
                oldFC.removeProperties(DTOBDFault.PROP_DTC); // remove keys
    ...
    **/
    ...
    {
        Long v = (Long)this.getFieldValue(FLD_registrationTime);
        return (v != null)? v.longValue() : 0L;
    }
    ...
    **/
    public void setRegistrationTime(long v)
    {
        this.setFieldValue(FLD_registrationTime, v);
    }

    // ------------------------------------------------------------------------

    /**
    *** Returns true if the device timestamp is before the specified event timestamp
    **/
    private boolean _isBeforeTS(long devTS, long evTS)
    {
        if (devTS > 0L) {
            // -- we have had a prior event
            if (evTS <= 0L) {
                // -- no current event timestamp, but we have had a prior event
                return true;
            } else
            if (devTS < evTS) {
                // -- we have a current event, and prior event is 'before' current event
                return true;
            }
        }
        return false;
    ...
    /**
    *** Returns true if this Device has any events events prior to the specified timestamp.
    **/
    public boolean hasPriorEvents(long timestamp, boolean checkTable)
    {
        // -- [2019/10/29] Not tested

        /* check Device fields first */
        if (this._isBeforeTS(this.getRegistrationTime()   , timestamp)) { // FLD_registrationTime
            return true;
        } else
        if (this._isBeforeTS(this.getLastEventTimestamp() , timestamp)) { // FLD_lastEventTimestamp
            return true;
        } else
        if (this._isBeforeTS(this.getLastIgnitionOnTime() , timestamp)) {
            return true;
        } else
        if (this._isBeforeTS(this.getLastIgnitionOffTime(), timestamp)) {
            return true;
        } else
        if (this._isBeforeTS(this.getLastEngineOnTime()   , timestamp)) {
            return true;
        } else
        if (this._isBeforeTS(this.getLastEngineOffTime()  , timestamp)) {
            return true;
        } else
        if (this._isBeforeTS(this.getLastStartTime()      , timestamp)) {
            return true;
        } else
        if (this._isBeforeTS(this.getLastStopTime()       , timestamp)) {
            return true;
        }
        ...
        if (checkTable) {
            try {
                EventData ev = this.getFirstEvent(0L,false);
                if ((ev != null) && this._isBeforeTS(ev.getTimestamp(),timestamp)) {
                    return true;
                }
            } catch (DBException dbe) {
                Print.logError("Unable to determine if Device has events [" + dbe);
            }
        }
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getPendingPingCommand();
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            this.getTransport().setPendingPingCommand(v);
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getLastPingTime();
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            this.getTransport().setLastPingTime(v);
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getTotalPingCount();
        } else {
    ...
    **/
    public void setTotalPingCount(int v)
    {
        ...
        if (this.hasTransport()) {
            this.getTransport().setTotalPingCount(v);
        } else {
    ...
    **/
    ...
    {
      //DBRecord dbRec = this.hasTransport()? (DBRecord)this.getTransport() : (DBRecord)this;
        DBRecord dbRec = (DBRecord)this.getDataTransport();

        /* refresh current value */
        if (reload) {
            // -- in case another Device 'ping' has changed this value already
            dbRec.reload(Device.FLD_totalPingCount);
        }
        ...
        if (update) {
            try {
                dbRec.update( // may throw DBException
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getMaxPingCount();
        } else {
    ...
    **/
    public void setMaxPingCount(int v)
    {
        v = (v > 0xFFFF)? 0xFFFF : (v > 0)? v : 0; // limit to 16-bit
        if (this.hasTransport()) {
            this.getTransport().setMaxPingCount(v);
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getExpectAck();
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            this.getTransport().setExpectAck(v);
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getExpectAckCode();
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            this.getTransport().setExpectAckCode(v);
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getLastAckCommand();
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            this.getTransport().setLastAckCommand(v);
        } else {
    ...
    **/
    public boolean clearExpectCommandAck(boolean didAck, boolean update)
    {
      ...
      //this.setLastAckResponse("");
        ...
        /* clear ACK command */
        if (this.hasTransport()) {
            Transport xport = this.getTransport();
            try {
                xport.update(
                    Transport.FLD_expectAck, 
                    Transport.FLD_expectAckCode, 
                    Transport.FLD_lastAckTime
                    );
                return true;
            } catch (DBException dbe) {
                Print.logException("Unable to set Transport.lastAck...", dbe);
                return false;
            }
        } else
        if (update) {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getLastAckTime();
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            this.getTransport().setLastAckTime(v);
        } else {
    ...
    // ------------------------------------------------------------------------
    // -- The meaning of the bits in the "dcsConfigMask" is completely defined by
    // -  the specifie DCS module used by the device.  However, should the DCS
    // -  wish to use a "standard" definitions, they are free to do so using the
    // -  following bit mask: (note: full length is 32 bits)
    
    public static long DCSMASK_OBD_32BIT_MASK       = 0xFFFFFFFFL;
    public static int  DCSMASK_OBD_ODOMETER_BIT     = 0;
    public static long DCSMASK_OBD_ODOMETER_MASK    = 1L << DCSMASK_OBD_ODOMETER_BIT;
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getDcsConfigMask();
        } else {
    ...
    /**
    *** Gets the value of a specific DCS configuration mask bit.
    *** Usage defined by specific DCS.
    *** @return The boolean value od the specified DCS configuration mask bit
    **/
    public boolean getDcsConfigMaskBit(int bitNdx)
    {
        long dcsMask = this.getDcsConfigMask(); // getDcsConfigMaskBit
        long chkMask = (1L << bitNdx);
        return ((dcsMask & chkMask) != 0L)? true : false;
    }
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            this.getTransport().setDcsConfigMask(v);
        } else {
    ...
    /**
    *** Sets the value of a specific DCS configuration mask bit.
    *** Usage defined by specific DCS.
    **/
    public void setDcsConfigMaskBit(int bitNdx, boolean v)
    {
        long dcsMask = this.getDcsConfigMask(); // setDcsConfigMaskBit
        long chkMask = (1L << bitNdx);
        if (v) {
            dcsMask |= chkMask;
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getDcsConfigString();
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            this.getTransport().setDcsConfigString(v);
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getLastDuplexConnectTime();
        } else {
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            this.getTransport().setLastDuplexConnectTime(v);
        } else {
    ...
    /**
    *** Gets the last Reverse-Geocode time
    *** @return The last Reverse-Geocode time
    **/
    public long getLastRevGeocodeTime()
    {
        Long v = (Long)this.getFieldValue(FLD_lastRevGeocodeTime);
        return (v != null)? v.longValue() : 0L;
    }

    /**
    *** Sets the last Reverse-Geocode time
    *** @param v The last Reverse-Geocode time
    **/
    public void setLastRevGeocodeTime(long v)
    {
        this.setFieldValue(FLD_lastRevGeocodeTime, (v > 0L)? v : 0L);
    }

    // --------------------------------

    /**
    *** Checks the previous ReverseGeocode time against the current timestamp
    **/
    public boolean checkLastRevGeocodeTime(ReverseGeocodeProvider rgp, boolean isMoving, long timestamp)
    {
        long minElapsed = (rgp != null)? rgp.getMinimumElapsedSec() : 0L;
        if (minElapsed > 0L) {
            long lastRG = this.getLastRevGeocodeTime();
            if ((timestamp - lastRG) < minElapsed) {
                // -- too soon since last reverse-geocode
                return false;
            }
        }
        return true;
    ...
    /**
    *** Sets the time for the most recent Reverse-Geocode
    **/
    public void updateLastRevGeocodeTime(boolean isMoving, long timestamp, boolean update)
    {
        this.setLastRevGeocodeTime(timestamp);
        if (update) {
            try {
                this.update(Device.FLD_lastRevGeocodeTime);
            } catch (DBException dbe) {
    ...
    private static long MAX_suspendFirstConnectNotify   = 120L; // 2 minutes
    private static long DFT_suspendFirstConnectNotify   = 1L;   // default 1 second

    /**
    *** Returns true if notifications should be suspended for this Device if this
    *** is the first connection as-of the specified timestamp.
    *** [see also 'getAllowNotify()']
    **/
    public boolean suspendFirstConnectNotify(long asOfTS)
    {

        /* first connect not initialized? */
        long fcTS = this.getFirstConnectTime();
        if (fcTS < 0L) {
            // -- column does not exist, disregard suspend query
            return false;
        } else
        if (fcTS == 0L) {
            // -- column exists, assume no previous events 
        } else {
            // -- check max delta time (to avoid reading "Device.suspendFirstConnectNotify") */
            if (fcTS < (asOfTS - MAX_suspendFirstConnectNotify)) {
                // -- 'asOfTS' is more than X-seconds _after_ first connect
                return false; // do not suspend
            }
        }
        // -- is first-connect, or within X-seconds of first-connect

        /* check property "Device.suspendFirstConnectNotify" */
        String suspVal = RTConfig.getString(DBConfig.PROP_Device_suspendFirstConnectNotify, null);
        if (StringTools.isBlank(suspVal)      ||    // not specified
            suspVal.equalsIgnoreCase("false") ||    // explicitly "false"
            suspVal.equalsIgnoreCase("no"   )   ) { // explicitly "no"
            // -- explicitly disabled (could have specified "-1")
            return false; // do not suspend
        }

        /* get delta */
        // -- specifying "true" (or any String) will return the default delta-sec value
        // -- specifying "0" will yield an enabled result with '0' delta-seconds
        long deltaSec = StringTools.parseLong(suspVal, DFT_suspendFirstConnectNotify);
        if (deltaSec < 0L) {
            // -- specified -1 delta-seconds indicates disabled
            return false; // do not suspend
        }
        // -- suspend is enabled after this point, check delta-sec window

        /* return delta-sec check result */
        if ((fcTS <= 0L) || (fcTS >= (asOfTS - deltaSec))) {
            // -- no first-connect time, or first-connect time is within delta-sec of 'asOfTS'
            return true; // suspend
        } else {
            // -- first-connect time is outside specified delta-sec window
            return false; // do not suspend
        }
    ...
    /**
    *** Returns true if this is the Device's first connection.
    *** (returns false if the "firstConnectTime" column does not exist)
    *** @return True if this is the Device's first connection
    **/
    public boolean isFirstConnection(long asOfTS)
    {
        long fcTS = this.getFirstConnectTime();
        if (fcTS < 0L) {
            // -- field does not exist, disregard first-connect query
            return false;
        } else 
        if (fcTS == 0L) {
            // -- no first-connect time, thus first connection
            return true;
        } else 
        if (fcTS >= asOfTS) {
            // -- first-connect time is >= asOf timestamp, thus first connection
            return true;
        } else {
            // -- asOf timestamp is after first-connect time, thus not first connection
            return false;
        }
    }
    ...
    *** @return True if this is the Device's first connection
    **/
    public boolean hasFirstConnectionTime()
    {
        long fcTS = this.getFirstConnectTime();
        return (fcTS > 0L)? true : false;
    }

    /**
    *** Gets the first connection time
    *** @return The first connection time
    **/
    public long getFirstConnectTime()
    {
        Long v = (Long)this.getFieldValue(FLD_firstConnectTime);
        return (v != null)? v.longValue() : -1L; // -1 indicates that the column doesn't exist
    }

    /**
    *** Sets the first connection time
    *** @param v The first connection time
    **/
    public void setFirstConnectTime(long v)
    {
        try {
            this.setFirstConnectTime(v, false); // no update now
        } catch (DBException dbe) {
            // -- will not occur
            Print.logException("Unexpected error", dbe);
        }
    }
    ...
    *** @param v The first connection time
    **/
    public void setFirstConnectTime(long v, boolean updateNow)
        throws DBException
    {
        long _v   = (v > 0L)? v : 0L;
        long fcTS = this.getFirstConnectTime();
        if ((fcTS >= 0L) && (_v != fcTS)) {
            this.setFieldValue(FLD_firstConnectTime, _v);
            if (updateNow) {
                this.update(FLD_firstConnectTime); // throws DBException
            } else {
                this.addOtherChangedFieldNames(FLD_firstConnectTime);
            }
        }
    }
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            return this.getTransport().getLastTotalConnectTime();
        } else {
    ...
    **/
    public void setLastTotalConnectTime(long v)
    {
        long _v = (v > 0L)? v : 0L;
        if (this.hasTransport()) {
            this.getTransport().setLastTotalConnectTime(_v);
        } else {
            this.setFieldValue(FLD_lastTotalConnectTime, _v);
        }
    }
    ...
    **/
    ...
    /**
    *** Sets the last TCP connection time
    *** @param v The last TCP connection time
    **/
    ...
    /**
    *** Gets the last connected Device for the specified Account
    *** @param acctId  The Account ID
    *** @return The last connected Device, or null if no device has connected
    *** @throws DBExeption
    **/
    public static Device getLastConnectedDeviceForAccount(String acctId)
        throws DBException
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_ONLY;
        ...
        try {
            ...
            /* select */
            // DBSelect: SELECT * FROM Device WHERE (accountID='acct') ORDER BY lastTotalConnectTime desc limit 1
            DBSelect<Device> dsel = new DBSelect<Device>(Device.getFactory());
            DBWhere dwh = dsel.createDBWhere();
            dsel.setWhere(dwh.WHERE(
                dwh.EQ(Device.FLD_accountID,acctId)
            ));
            dsel.setOrderByFields(Device.FLD_lastTotalConnectTime);
            ...
            rs = stmt.getResultSet();
            while (rs.next()) {
                String ACC = rs.getString(FLD_accountID);
                String DEV = rs.getString(FLD_deviceID);
                device = new Device(new Device.Key(ACC,DEV));
                device.setAllFieldValues(rs);
                break; // only 1 record
            }
    ...
    **/
    @JSONBeanGetter(name="contactPhone", tags="getDeviceList")
    ...
    **/
    ...
    // ------------------------------------------------------------------------
    // ------------------------------------------------------------------------
    // -- supports any maintenance fields

    /**
    *** Returns true if the any maintenance fields are supported
    **/
    public static boolean supportsMaintenance()
    {
        if (Device.supportMaintenance_notes()) {
            return true;
        } else
        if (Device.supportsMaintenance_KM()) {
            return true;
        } else
        if (Device.supportsMaintenance_HR()) {
            return true;
        } else
        if (Device.supportsMaintenance_FT()) {
            return true;
        } else {
            return false;
        }
    ...
    // ------------------------------------------------------------------------
    // ------------------------------------------------------------------------
    // -- MaintKM: Odometer maintenance fields

    /**
    *** Gets an array of all KM/Odometer Maintenance instances.
    **/
    public Maintenance[] getAllMaintenanceKM()
    {
        return Maintenance.getAllItems(Maintenance.Type.KM, this);
    }

    /**
    *** Gets a list of due KM/Odometer Maintenance instances
    **/
    public java.util.List<Maintenance> getDueMaintenanceKM()
    {
        return Maintenance.getAllDue(Maintenance.Type.KM, this);
    }

    // ------------------------------------------------------------------------

    private static final int MAXIMUM_MAINT_ODOM_COUNT = 20;
    private static final int DEFAULT_MAINT_ODOM_COUNT =  2;

    /**
    *** Gets the number of odometer maintenance fields to support
    *** @return The number of odometer maintenance fields to support
    **/
    public static int getMaintCountKM() // Device.getMaintOdometerCount()
    {
        int mc = RTConfig.getInt(DBConfig.PROP_Device_maintenanceOdometerCount, DEFAULT_MAINT_ODOM_COUNT);
        return Math.min(Math.max(mc,0),MAXIMUM_MAINT_ODOM_COUNT);
    }

    /**
    *** Returns true if the specific odometer maintenance index is supported
    **/
    public static boolean supportsMaintenance_KM(int ndx)
    {
        if (Device.getMaintCountKM() <= ndx) {
            return false; 
        } else {
            return Device.getFactory().hasField(FLD_maintOdometerKM_+ndx);
        }
    }
    ...
    *** Returns true if the specific odometer maintenance index is supported
    **/
    public static boolean supportsMaintenance_KM()
    {
        return Device.supportsMaintenance_KM(0);
    }

    // --------------------------------

    // -- "MaintKM.index", "MaintKM.label"
    private static String KEY_MaintKM_due_      = "MaintKM.due_";     // [String] 
    private static String KEY_MaintKM_due       = "MaintKM.due";      // [I18N.Text] KEY_MAINT_DUE_KM
    private static String KEY_MaintKM_untilDue_ = "MaintKM.untilDue_";// [Double] 

    private int MaintKM_IndexDueState[] = null;

    /**
    *** Gets the MaintDue-KM list
    **/
    private int[] getMaintDueStateKM()
    {
        if (MaintKM_IndexDueState == null) {
            MaintKM_IndexDueState = new int[Device.getMaintCountKM()];
            ListTools.initArray(MaintKM_IndexDueState, -1);
        }
        return MaintKM_IndexDueState;
    }

    /**
    *** Checks the rule-trigger state of the specified Odometer maintenance<br>
    *** (Used for Rule '$MaintKM' trigger state caching)
    **/
    public boolean checkRuleTriggeredMaintKM(int ndx, double intvKM, double offsKM, boolean triggerIfDueNow)
    {
        // -- check index
        if ((ndx < 0) || (ndx >= Device.getMaintCountKM())) {
            // -- invalid index
            Print.logDebug("$MaintKM("+ndx+"): ["+this.getDeviceID()+"] Invalid index: " + ndx);
            return false;
        }
        // -- check interval (km)
        if (intvKM <= 0.0) {
            // -- no interval
            Print.logDebug("$MaintKM("+ndx+"): ["+this.getDeviceID()+"] No Interval");
            return false;
        }
        // -- current maintenance value
        double currKM = this.getLastOdometerKM(); // ed.getOdometerKM();
        if (currKM <= 0.0) {
            // -- no current odometer
            Print.logDebug("$MaintKM("+ndx+"): ["+this.getDeviceID()+"] No Odometer");
            return false; // will likely not occur
        }
        // -- next maintenance value
        double lastKM = this.getMaintOdometerKM(ndx);
        double nextKM = lastKM + intvKM;
        Print.logDebug("$MaintKM("+ndx+"): ["+this.getDeviceID()+"] Curr="+currKM+" Last="+lastKM+", Interval="+intvKM+", Offset="+offsKM);
        // -- set due
        int due = 0; // 0=no, 1=soon, 2=now
        RTProperties rtp = this.getTemporaryProperties();
        if ((currKM + ((offsKM>0.0)?offsKM:0.0)) >= nextKM) {
            if (currKM < nextKM) {
                // -- checking due soon
                due = 1; // soon
            } else
            if (offsKM <= 0.0) {
                // -- checking past due, and it is past due
                due = 2; // now
            } else
            if (triggerIfDueNow) {
                // -- checking due soon, but trigger if past-due
                due = 2; // now
            } else {
                // -- checking due soon, but suppress past-due trigger
                //due = 2; // now, but suppress trigger
            }
        }
        // -- is due?
        if (due > 0) {
            // -- set index due flag
            this.getMaintDueStateKM()[ndx] = 1; // true
            // -- add label
            String ndxDesc  = this.getMaintLabelKM(ndx);
            String listDesc = StringTools.trim(rtp.getString(KEY_MaintKM_due_,null));
            if (!StringTools.isBlank(listDesc)) { listDesc += ", "; } // ie. "Tires, EngineOil, AirFilter"
            listDesc += ndxDesc;
            rtp.setString(  KEY_MaintKM_due_, listDesc);
            rtp.setProperty(KEY_MaintKM_due , new I18N.Text(listDesc)); // ${MaintKM.due}
        }
        // -- remaining distance until actually due
        double remnKM = (nextKM > currKM)? (nextKM - currKM) : 0.0;
        Object untilDueKM = rtp.getProperty(KEY_MaintKM_untilDue_,null);
        if (!(untilDueKM instanceof Number)) {
            rtp.setProperty(KEY_MaintKM_untilDue_, new Double(remnKM));
        } else
        if (remnKM < ((Number)untilDueKM).doubleValue()) {
            rtp.setProperty(KEY_MaintKM_untilDue_, new Double(remnKM));
        }
        return (due > 0)? true : false;
    }
    ...
    *** @return True if the Maintenance distance for the specified index is due
    **/
    public boolean isMaintDueKM(int ndx, double deltaKM)  // getMaintRemainingKM
    {
        // -- does not support HR maintenance?
        //if (!Device.supportsMaintenance_KM()) { <-- already built-in to the checks below
        //    return false;
        //}
        // -- invalid index
        int mdi[] = this.getMaintDueStateKM();
        if ((ndx < 0) || (ndx >= mdi.length)) {
            return false;
        }
        // -- initialize?
        if (mdi[ndx] < 0) {
            double remnKM = this.getMaintRemainingKM(ndx);
            mdi[ndx] = ((remnKM >= 0.0) && (remnKM <= deltaKM))? 1 : 0;
        }
        // -- return
        return (mdi[ndx] > 0)? true : false;
    }

    /**
    *** Gets the index of the next Odometer maintenance that will become due.<br>
    *** Returns '-1' if none will become due
    *** @param maxKM The maximum next due threshold (0 for past due only)
    *** @return An array of next due indices
    **/
    public int[] getMaintNextDueKM(double maxKM)
    {
        Vector<Tuple.Pair<Integer,Double>> matchTP = new Vector<Tuple.Pair<Integer,Double>>();
        // -- populate list
        int cnt = Device.getMaintCountKM();
        for (int i = 0; i < cnt; i++) {
            double remnKM = this.getMaintRemainingKM(i);
            if (remnKM < 0.0) {
                // -- maintenance bucket not used
            } else 
            if ((maxKM >= 0.0) && (remnKM > maxKM)) {
                // -- due value is too far away
            } else {
                // -- due soon
                matchTP.add(new Tuple.Pair<Integer,Double>(new Integer(i),new Double(remnKM)));
            }
        }
        if (matchTP.size() <= 0) {
            return new int[0];
        }
        // -- sort by km (smallest to largest)
        ListTools.sort(matchTP, new Comparator<Tuple.Pair<Integer,Double>>() {
            public int compare(Tuple.Pair<Integer,Double> tp1, Tuple.Pair<Integer,Double> tp2) {
                double n1 = tp1.b.doubleValue(); // km
                double n2 = tp2.b.doubleValue(); // km
                if (n1 == n2) {
                    return 0;
                } else {
                    return (n1 < n2)? -1 : 1;
        ...
        int ndx[] = new int[matchTP.size()];
        for (int i = 0; i < ndx.length; i++) {
            ndx[i] = matchTP.get(i).a.intValue();
        }
        return ndx;
    }
    ...
    /**
    *** Gets a String of comma-separated maint-odom due
    **/
    private String getMaintDueLabelsKM(String sep) // getMaintDueLabels_odom
    {
        String _sep = (sep != null)? sep : ",";
        StringBuffer labelSB = new StringBuffer();
        int cnt = Device.getMaintCountKM();
        for (int i = 0; i < cnt; i++) {
            if (this.isMaintDueKM(i,0.0)) {
                if (labelSB.length() > 0) { labelSB.append(_sep); }
                labelSB.append(this.getMaintLabelKM(i));
            }
        }
    ...
    /**
    *** Gets an array of all Odometer Maintenance Labels
    **/
    ...
    **/
    ...
    {
        return (ndx >= 0)? this.getMaintLabelKM(ndx,("Odometer#"+(ndx+1))) : "";
    }
    ...
    **/
    public String getMaintLabelKM(int ndx, String dft)
    {
        ...
        // -- 1) try "Device.maintLabelKM#" field
        v = this.getOptionalFieldValue(FLD_maintLabelKM_+ndx, (String)null);
        if (!StringTools.isBlank(v)) {
            return v;
        }
        // -- 2) [OBSOLETE] try "Device.maintDescriptionKM#" field
        v = this.getOptionalFieldValue(FLD_maintDescriptionKM_+ndx, (String)null);
        if (!StringTools.isBlank(v)) {
            return v;
        }
        // -- 3) try Account.getMaintLabelKM ...
        Account a = this.getAccount();
        v = (a != null)? a.getMaintLabelKM(ndx,null) : null;
        if (!StringTools.isBlank(v)) {
            return v;
        }
        // -- 4) return default
    ...
    **/
    ...
    /**
    *** Gets an array of Odometer Maintenance values
    **/
    @JSONBeanGetter(expandArray="true", ignore="<=0", unitsEnum="Account.DistanceUnits")
    public double[] getMaintOdometerKM()
    {
        int cnt = Device.getMaintCountKM();
        double maintOdomKM[] = new double[cnt];
        for (int i = 0; i < maintOdomKM.length; i++) {
            maintOdomKM[i] = this.getMaintOdometerKM(i);
        }
        return maintOdomKM;
    }
    ...
    **/
    ...
    {
        return this.getOptionalFieldValue(FLD_maintOdometerKM_+ndx, 0.0);
    }
    ...
    *** @return The maintenance odometer (in kilometers)
    **/
    public void setMaintOdometerKM(int ndx, double v)
    {
        if (v < this.getMaxOdometerKM()) {
            this.setOptionalFieldValue(FLD_maintOdometerKM_+ndx, ((v >= 0.0)? v : 0.0));
        }
    }
    ...
    **/
    ...
    {
        this.setMaintOdometerKM(ndx, this.getLastOdometerKM());
        this.addOtherChangedFieldNames(FLD_maintOdometerKM_+ndx);
    }

    // --------------------------------

    /**
    *** Gets an array of Odometer Maintenance intervals
    **/
    @JSONBeanGetter(expandArray="true", ignore="<=0", unitsEnum="Account.DistanceUnits")
    public double[] getMaintIntervalKM()
    {
        int cnt = Device.getMaintCountKM();
        double maintIntrvKM[] = new double[cnt];
        for (int i = 0; i < maintIntrvKM.length; i++) {
            maintIntrvKM[i] = this.getMaintIntervalKM(i);
        }
        return maintIntrvKM;
    }
    ...
    **/
    ...
    {
        double v = this.getOptionalFieldValue((FLD_maintIntervalKM_+ndx), 0.0);
        return (v >= 0.0)? v : 0.0;
    }

    /**
    *** Sets the last maintenance interval for the specified index
    *** @param ndx  The index of the maintenance interval to return
    *** @param v    The maintenance interval value to set
    *** @return The maintenance interval (in kilometers)
    **/
    public void setMaintIntervalKM(int ndx, double v)
    {
        this.setOptionalFieldValue((FLD_maintIntervalKM_+ndx), ((v >= 0.0)? v : 0.0));
    }

    // --------------------------------

    /**
    *** Gets an array of Odometer Maintenance values, in the preferred Account distance units
    **/
    @JSONBeanGetter(expandArray="true", ignore="<=0")
    public double[] getMaintOdometerKM_units()
    {
        Account.DistanceUnits distUnits = Account.getDistanceUnits(this.getAccount());
        int cnt = Device.getMaintCountKM();
        double maintOdom[] = new double[cnt];
        for (int i = 0; i < maintOdom.length; i++) {
            maintOdom[i] = distUnits.convertFromKM(this.getMaintOdometerKM(i));
        }
        return maintOdom;
    }

    /**
    *** Gets the last maintenance odometer for the specified index, in the preferred Account distance units
    *** @param ndx  The index of the maintenance odometer to return
    *** @return The maintenance odometer
    **/
    public double getMaintOdometerKM_units(int ndx)
    {
        Account.DistanceUnits distUnits = Account.getDistanceUnits(this.getAccount());
        return distUnits.convertFromKM(this.getMaintOdometerKM(ndx));
    }

    /**
    *** Sets the last maintenance odometer for the specified index, in the preferred account distance units
    *** @param ndx  The index of the maintenance odometer to return
    *** @return The maintenance odometer (in kilometers)
    **/
    public void setMaintOdometerKM_units(int ndx, double v)
    {
        Account.DistanceUnits distUnits = Account.getDistanceUnits(this.getAccount());
        this.setMaintOdometerKM(ndx, distUnits.convertToKM(v));
    }

    // --------------------------------

    /**
    *** Gets an array of Odometer Maintenance intervals, in the preferred account distance units
    **/
    @JSONBeanGetter(expandArray="true", ignore="<=0")
    public double[] getMaintIntervalKM_units()
    {
        Account.DistanceUnits distUnits = Account.getDistanceUnits(this.getAccount());
        int cnt = Device.getMaintCountKM();
        double maintIntrv[] = new double[cnt];
        for (int i = 0; i < maintIntrv.length; i++) {
            maintIntrv[i] = distUnits.convertFromKM(this.getMaintIntervalKM(i));
        }
        return maintIntrv;
    }

    /**
    *** Gets the last maintenance interval for the specified index, in the preferred account distance units
    *** @param ndx  The index of the maintenance interval to return
    *** @return The maintenance interval
    **/
    public double getMaintIntervalKM_units(int ndx)
    {
        Account.DistanceUnits distUnits = Account.getDistanceUnits(this.getAccount());
        return distUnits.convertFromKM(this.getMaintIntervalKM(ndx));
    }

    /**
    *** Sets the last maintenance interval for the specified index, in the preferred account distance units
    *** @param ndx  The index of the maintenance interval to return
    *** @param v    The maintenance interval value to set
    *** @return The maintenance interval
    **/
    public void setMaintIntervalKM_units(int ndx, double v)
    {
        Account.DistanceUnits distUnits = Account.getDistanceUnits(this.getAccount());
        this.setMaintIntervalKM(ndx, distUnits.convertToKM(v));
    }

    // --------------------------------

    /**
    *** Gets an array of Odometer Pre-Notification offsets
    **/
    @JSONBeanGetter(expandArray="true", ignore="<=0")
    public double[] getMaintPreNotifyKM()
    {
        int cnt = Device.getMaintCountKM();
        double maintPreNotifyKM[] = new double[cnt];
        for (int i = 0; i < maintPreNotifyKM.length; i++) {
            maintPreNotifyKM[i] = this.getMaintPreNotifyKM(i);
        }
        return maintPreNotifyKM;
    }

    /**
    *** Gets the last maintenance Pre-Notification offset for the specified index
    *** @param ndx  The index of the maintenance Pre-Notification offset to return
    *** @return The maintenance Pre-Notification offset (in kilometers)
    **/
    public double getMaintPreNotifyKM(int ndx)
    {
        double v = this.getOptionalFieldValue((FLD_maintPreNotifyKM_+ndx), 0.0);
        return (v >= 0.0)? v : 0.0;
    }

    /**
    *** Sets the last maintenance Pre-Notification offset for the specified index
    *** @param ndx  The index of the maintenance Pre-Notification offset to return
    *** @return The maintenance Pre-Notification offset (in kilometers)
    **/
    public void setMaintPreNotifyKM(int ndx, double v)
    {
        this.setOptionalFieldValue((FLD_maintPreNotifyKM_+ndx), v);
    }

    // --------------------------------

    /**
    *** Gets an array of Odometer Maintenance Remaining
    **/
    @JSONBeanGetter(expandArray="true", ignore="<=0")
    public double[] getMaintRemainingKM() // getMaintenanceRemainingKM
    {
        int cnt = Device.getMaintCountKM();
        double remainKM[] = new double[cnt];
        for (int i = 0; i < remainKM.length; i++) {
            remainKM[i] = this.getMaintRemainingKM(i);
        }
        return remainKM;
    }
    ...
    **/
    public double getMaintRemainingKM(int ndx) // isMaintDueKM
    {
        if (Device.supportsMaintenance_KM(ndx)) {
            double currKM = this.getLastOdometerKM();
            if (currKM > 0.0) {
                double intvKM = this.getMaintIntervalKM(ndx);
                if (intvKM > 0.0) {
                    double lastKM = this.getMaintOdometerKM(ndx); // last maintenance KM
                    double nextKM = lastKM + intvKM;
                    return (nextKM > currKM)? (nextKM - currKM) : 0.0; // Math.max((nextKM - currKM), 0.0);
                }
            }
        }
        // -- periodic maintenance not supported/available
        return -1.0;
    }

    // ------------------------------------------------------------------------
    // ------------------------------------------------------------------------
    // -- MainHR: Hours maintenance fields

    /**
    *** Gets an array of all HR/Engine-Hours Maintenance instances
    **/
    public Maintenance[] getAllMaintenanceHR()
    {
        return Maintenance.getAllItems(Maintenance.Type.HR, this);
    }

    /**
    *** Gets a list of due HR/Engine-Hours Maintenance instances
    **/
    public java.util.List<Maintenance> getDueMaintenanceHR()
    {
        return Maintenance.getAllDue(Maintenance.Type.HR, this);
    }

    // ------------------------------------------------------------------------

    private static final int MAXIMUM_MAINT_HOURS_COUNT = 20;
    private static final int DEFAULT_MAINT_HOURS_COUNT =  1;

    /**
    *** Gets the number of supported maintenance engine-hour fields
    *** @return The number of supported maintenance engine-hour fields
    **/
    public static int getMaintCountHR() // Device.getMaintHoursCount() [2.6.6-B61c]
    {
        int mc = RTConfig.getInt(DBConfig.PROP_Device_maintenanceHoursCount, DEFAULT_MAINT_HOURS_COUNT);
        return Math.min(Math.max(mc,0),MAXIMUM_MAINT_HOURS_COUNT);
    }

    /**
    *** Returns true if the specific engine-hours maintenance index is supported
    **/
    public static boolean supportsMaintenance_HR(int ndx)
    {
        if (Device.getMaintCountHR() <= ndx) {
            return false; 
        } else {
            return Device.getFactory().hasField(FLD_maintEngHoursHR_+ndx);
        }
    }
    ...
    *** Returns true if the specific engine-hours maintenance index is supported
    **/
    public static boolean supportsMaintenance_HR()
    {
        return Device.supportsMaintenance_HR(0);
    }

    // --------------------------------

    // -- "MaintHR.index", "MaintHR.label"
    private static String KEY_MaintHR_due_      = "MaintHR.due_";     // [String] 
    private static String KEY_MaintHR_due       = "MaintHR.due";      // [I18N.Text] KEY_MAINT_DUE_HR
    private static String KEY_MaintHR_untilDue_ = "MaintHR.untilDue_";// [Double] 

    private int MaintHR_IndexDueState[] = null;

    /**
    *** Gets the MaintDue-HR list
    **/
    private int[] getMaintDueStateHR()
    {
        if (MaintHR_IndexDueState == null) {
            MaintHR_IndexDueState = new int[Device.getMaintCountHR()];
            ListTools.initArray(MaintHR_IndexDueState, -1);
        }
        return MaintHR_IndexDueState;
    }

    /**
    *** Checks the rule-trigger state of the specified Engine-Hours maintenance<br>
    *** (Used for Rule '$MaintHR' trigger state caching)
    **/
    public boolean checkRuleTriggeredMaintHR(int ndx, double intvHR, double offsHR, boolean triggerIfDueNow)
    {
        // -- check index
        if ((ndx < 0) || (ndx >= Device.getMaintCountHR())) {
            // -- invalid index
            Print.logDebug("$MaintHR("+ndx+"): ["+this.getDeviceID()+"] Invalid index: " + ndx);
            return false;
        }
        // -- check interval (hr)
        if (intvHR <= 0.0) {
            // -- no interval
            Print.logDebug("$MaintHR("+ndx+"): ["+this.getDeviceID()+"] No Interval");
            return false;
        }
        // -- current maintenance value
        double currHR = this.getLastEngineHours(); // ed.getEngineHours();
        if (currHR <= 0.0) {
            // -- no current engine-hours
            Print.logDebug("$MaintHR("+ndx+"): ["+this.getDeviceID()+"] No Engine-Hours");
            return false; // will likely not occur
        }
        // -- next maintenance value
        double lastHR = this.getMaintEngHoursHR(ndx);
        double nextHR = lastHR + intvHR;
        Print.logDebug("$MaintHR("+ndx+"): ["+this.getDeviceID()+"] Curr="+currHR+" Last="+lastHR+", Interval="+intvHR+", Offset="+offsHR);
        // -- set due
        int due = 0; // 0=no, 1=soon, 2=now
        RTProperties rtp = this.getTemporaryProperties();
        if ((currHR + ((offsHR>0.0)?offsHR:0.0)) >= nextHR) {
            if (currHR < nextHR) {
                // -- checking due soon
                due = 1; // soon
            } else
        ...
        if (due > 0) {
            // -- set index due flag
            this.getMaintDueStateHR()[ndx] = 1; // true
            // -- add label
            String ndxDesc  = this.getMaintLabelHR(ndx);
            String listDesc = StringTools.trim(rtp.getString(KEY_MaintHR_due_,null));
            if (!StringTools.isBlank(listDesc)) { listDesc += ", "; } // ie. "Tires, EngineOil, AirFilter"
            listDesc += ndxDesc;
            rtp.setString(  KEY_MaintHR_due_, listDesc);
            rtp.setProperty(KEY_MaintHR_due , new I18N.Text(listDesc)); // ${MaintHR.due}
        }
        // -- remaining hours until actually due
        double remnHR = (nextHR > currHR)? (nextHR - currHR) : 0.0;
        Object untilDueHR = rtp.getProperty(KEY_MaintHR_untilDue_,null);
        if (!(untilDueHR instanceof Number)) {
            rtp.setProperty(KEY_MaintHR_untilDue_, new Double(remnHR));
        } else
        if (remnHR < ((Number)untilDueHR).doubleValue()) {
            rtp.setProperty(KEY_MaintHR_untilDue_, new Double(remnHR));
        }
        return (due > 0)? true : false;
    }
    ...
    *** @return True if the Maintenance engine hours for the specified index is due
    **/
    public boolean isMaintDueHR(int ndx, double deltaHR) // getMaintRemainingHR
    {
        // -- does not support HR maintenance?
        //if (!Device.supportsMaintenance_HR()) { <-- already built-in to the checks below
        //    return false;
        //}
        // -- invalid index
        int mdi[] = this.getMaintDueStateHR();
        if ((ndx < 0) || (ndx >= mdi.length)) {
            return false;
        }
        // -- initialize?
        if (mdi[ndx] < 0) {
            double remnHR = this.getMaintRemainingHR(ndx);
            mdi[ndx] = ((remnHR >= 0.0) && (remnHR <= deltaHR))? 1 : 0;
        }
        // -- return
        return (mdi[ndx] > 0)? true : false;
    }
    ...
    /**
    *** Gets the index of the next Engine-Hour maintenance that will become due.<br>
    *** Returns null/empty if none will become due
    *** @param maxHR The maximum next due threshold (0 for past due only)
    *** @return An array of next due indices
    **/
    public int[] getMaintNextDueHR(double maxHR)
    {
        Vector<Tuple.Pair<Integer,Double>> matchTP = new Vector<Tuple.Pair<Integer,Double>>();
        // -- populate list
        int cnt = Device.getMaintCountHR();
        for (int i = 0; i < cnt; i++) {
            double remnHR = this.getMaintRemainingHR(i);
            if (remnHR < 0.0) {
                // -- maintenance bucket not used
            } else 
            if ((maxHR >= 0.0) && (remnHR > maxHR)) {
                // -- due value is too far away
            } else {
                // -- due soon
                matchTP.add(new Tuple.Pair<Integer,Double>(new Integer(i),new Double(remnHR)));
            }
        }
        if (matchTP.size() <= 0) {
            return new int[0];
        }
        // -- sort by hr (smallest to largest)
        ListTools.sort(matchTP, new Comparator<Tuple.Pair<Integer,Double>>() {
            public int compare(Tuple.Pair<Integer,Double> tp1, Tuple.Pair<Integer,Double> tp2) {
                double n1 = tp1.b.doubleValue(); // hr
                double n2 = tp2.b.doubleValue(); // hr
                if (n1 == n2) {
                    return 0;
                } else {
                    return (n1 < n2)? -1 : 1;
    ...
    *** Gets a String of comma-separated maint-odom due
    **/
    private String getMaintDueLabelsHR(String sep) // getMaintDueLabels_hours
    {
        String _sep = (sep != null)? sep : ",";
        StringBuffer labelSB = new StringBuffer();
        int cnt = Device.getMaintCountHR();
        for (int i = 0; i < cnt; i++) {
            if (this.isMaintDueHR(i,0.0)) {
                if (labelSB.length() > 0) { labelSB.append(_sep); }
                labelSB.append(this.getMaintLabelHR(i));
            }
        }
        return labelSB.toString();
    }
    ...
    **/
    ...
    /**
    *** Gets the engine-hours maintenance descrption for the specified index
    ...
    **/
    ...
    {
        return (ndx >= 0)? this.getMaintLabelHR(ndx,("Hours#"+(ndx+1))) : "";
    }
    ...
    **/
    public String getMaintLabelHR(int ndx, String dft)
    {
        ...
        // -- 2) [OBSOLETE] try "Device.maintDescriptionHR#" field
        v = this.getOptionalFieldValue(FLD_maintDescriptionHR_+ndx, (String)null);
        if (!StringTools.isBlank(v)) {
            return v;
        }
        // -- 3) try Account.getMaintLabelHR ...
        Account a = this.getAccount();
        v = (a != null)? a.getMaintLabelHR(ndx,null) : null;
        if (!StringTools.isBlank(v)) {
            return v;
        }
        // -- 4) return default
    ...
    /**
    *** Sets the engine-hours maintenance descrption for the specified index
    ...
    **/
    ...
    /**
    *** Gets an array of Engine-Hours Maintenance values
    **/
    @JSONBeanGetter(expandArray="true", ignore="<=0")
    public double[] getMaintEngHoursHR()
    {
        int cnt = Device.getMaintCountHR();
        double maintHrs[] = new double[cnt];
        for (int i = 0; i < maintHrs.length; i++) {
            maintHrs[i] = this.getMaintEngHoursHR(i);
        }
        return maintHrs;
    }
    ...
    **/
    ...
    {
        return this.getOptionalFieldValue(FLD_maintEngHoursHR_+ndx, 0.0);
    }
    ...
    **/
    public void setMaintEngHoursHR(int ndx, double v)
    {
        if (v < this.getMaxRuntimeHours()) {
            this.setOptionalFieldValue((FLD_maintEngHoursHR_+ndx), ((v >= 0.0)? v : 0.0));
        }
    }
    ...
    **/
    ...
    {
        this.setMaintEngHoursHR(ndx,this.getLastEngineHours());
        this.addOtherChangedFieldNames(FLD_maintEngHoursHR_+ndx);
    }

    // --------------------------------

    /**
    *** Gets an array of Engine-Hour Maintenance values
    **/
    @JSONBeanGetter(expandArray="true", ignore="<=0")
    public double[] getMaintIntervalHR()
    {
        int cnt = Device.getMaintCountHR();
        double maintIntrvHrs[] = new double[cnt];
        for (int i = 0; i < maintIntrvHrs.length; i++) {
            maintIntrvHrs[i] = this.getMaintIntervalHR(i);
        }
        return maintIntrvHrs;
    }
    ...
    **/
    ...
    {
        double v = this.getOptionalFieldValue(FLD_maintIntervalHR_+ndx, 0.0);
        return (v >= 0.0)? v : 0.0;
    }
    ...
    *** Gets the maintenance engine-hours interval for the specified index
    *** @param ndx  The maintenance engine-hours index
    *** @param v    The maintenance engine-hours value to set
    *** @return The maintenance engine-hours interval
    **/
    public void setMaintIntervalHR(int ndx, double v)
    {
        this.setOptionalFieldValue((FLD_maintIntervalHR_+ndx), ((v >= 0.0)? v : 0.0));
    }

    // --------------------------------

    /**
    *** Gets an array of Engine-Hour Maintenance Pre-Notification offsets
    **/
    @JSONBeanGetter(expandArray="true", ignore="<=0")
    public double[] getMaintPreNotifyHR()
    {
        int cnt = Device.getMaintCountHR();
        double maintPreNotifyHR[] = new double[cnt];
        for (int i = 0; i < maintPreNotifyHR.length; i++) {
            maintPreNotifyHR[i] = this.getMaintPreNotifyHR(i);
        }
        return maintPreNotifyHR;
    }

    /**
    *** Gets the maintenance engine-hours pre-notification offset for the specified index
    *** @param ndx  The maintenance engine-hours index
    *** @return The maintenance engine-hours pre-notification offsets
    **/
    public double getMaintPreNotifyHR(int ndx)
    {
        double v = this.getOptionalFieldValue(FLD_maintPreNotifyHR_+ndx, 0.0);
        return (v >= 0.0)? v : 0.0;
    }

    /** 
    *** Sets the maintenance engine-hours pre-notification offsets for the specified index
    *** @param ndx  The maintenance engine-hours index
    *** @param v    The maintenance engine-hours pre-notification offset value to set
    *** @return The maintenance engine-hours pre-notification offsets
    **/
    public void setMaintPreNotifyHR(int ndx, double v)
    {
        this.setOptionalFieldValue((FLD_maintPreNotifyHR_+ndx), ((v >= 0.0)? v : 0.0));
    }

    // --------------------------------

    /**
    *** Gets an array of Hour Maintenance Remaining
    **/
    @JSONBeanGetter(expandArray="true", ignore="<=0")
    public double[] getMaintRemainingHR() // getMaintenanceRemainingHR
    {
        int cnt = Device.getMaintCountHR();
        double remainHR[] = new double[cnt];
        for (int i = 0; i < remainHR.length; i++) {
            remainHR[i] = this.getMaintRemainingHR(i);
        }
        return remainHR;
    }
    ...
    *** Returns zero if past due, or if periodic maintenance is not supported.
    **/
    public double getMaintRemainingHR(int ndx) // isMaintDueHR
    {
        if (Device.supportsMaintenance_HR(ndx)) {
            double currHR = this.getLastEngineHours(); // current HR
            if (currHR > 0.0) {
                double intvHR = this.getMaintIntervalHR(ndx);
                if (intvHR > 0.0) {
                    double lastHR = this.getMaintEngHoursHR(ndx); // last maintenance HR
                    double nextHR = lastHR + intvHR;
                    return (nextHR > currHR)? (nextHR - currHR) : 0.0; // Math.max((nextHR - currHR), 0.0);
                }
            }
        }
        // -- periodic maintenance not supported/available
    ...
    // ------------------------------------------------------------------------
    // ------------------------------------------------------------------------
    // -- MaintFT: FixedTime maintenance fields

    /**
    *** Gets an array of all FT/FixedTime Maintenance instances
    **/
    public Maintenance[] getAllMaintenanceFT()
    {
        return Maintenance.getAllItems(Maintenance.Type.FT, this);
    }

    /**
    *** Gets a list of due FT/FixedTime Maintenance instances
    **/
    public java.util.List<Maintenance> getDueMaintenanceFT()
    {
        return Maintenance.getAllDue(Maintenance.Type.FT, this);
    }

    // ------------------------------------------------------------------------

    private static final int MAXIMUM_MAINT_TIME_COUNT = 10;
    private static final int DEFAULT_MAINT_TIME_COUNT =  0;

    /**
    *** Gets the number of supported maintenance fixed-time fields
    *** @return The number of supported maintenance fixed-time fields
    **/
    public static int getMaintCountFT() // Device.getMaintFixedTimeCount() [2.6.6-B63]
    {
        int mc = RTConfig.getInt(DBConfig.PROP_Device_maintenanceFixedTimeCount, DEFAULT_MAINT_TIME_COUNT);
        return Math.min(Math.max(mc,0),MAXIMUM_MAINT_TIME_COUNT);
    }

    /**
    *** Returns true if the specific fixed-time maintenance index is supported
    **/
    public static boolean supportsMaintenance_FT(int ndx)
    {
        if (Device.getMaintCountFT() <= ndx) {
            return false; 
        } else {
            return Device.getFactory().hasField(FLD_maintFixedTimeFT_+ndx);
        }
    }
    ...
    *** Returns true if the specific fixed-time maintenance index is supported
    **/
    public static boolean supportsMaintenance_FT()
    {
        return Device.supportsMaintenance_FT(0);
    }

    // --------------------------------

    public enum MaintUnitsFT implements EnumTools.StringLocale, EnumTools.IntValue {
        SECONDS (0, I18N.getString(Device.class,"Device.MaintFT.units.seconds","Seconds"), "0"   ),
        DAYS    (1, I18N.getString(Device.class,"Device.MaintFT.units.days"   ,"Days"   ), "0.0" ),
        MONTHS  (2, I18N.getString(Device.class,"Device.MaintFT.units.months" ,"Months" ), "0.00");
        // ---
        private int         vv = 0;
        private I18N.Text   dd = null;
        private String      ff = null;
        MaintUnitsFT(int v, I18N.Text d, String f) { vv = v; dd = d; ff = f; }
        public int       getIntValue()             { return vv; }
        public I18N.Text getText()                 { return dd; }
        public String    toString()                { return dd.toString(); }
    ...
    /**
    *** Gets the FixedTime maint Units
    **/
    public MaintUnitsFT getMaintUnitsFT(int ndx)
    {
        // -- Options for the Units of lastFT, intvFT, and offsFT:
        // -    0: Seconds:
        // -        lastFT = Epoch timestamp
        // -        intvFT = seconds
        // -        offsFT = seconds
        // -    1: Days
        // -        lastFT = DayNumber
        ...
        // -        intvFT = months
        // -        offsFT = days
        MaintUnitsFT dft = MaintUnitsFT.SECONDS;
        String vs = RTConfig.getString(DBConfig.PROP_Device_maintenanceFixedTimeUnits, null);
        Print.logDebug("MaintFT Units Key: " + DBConfig.PROP_Device_maintenanceFixedTimeUnits);
        MaintUnitsFT units;
        if (StringTools.isBlank(vs)) {
            units = dft;
        } else
        if (Character.isDigit(vs.charAt(0))) {
            int v = StringTools.parseInt(vs,dft.getIntValue());
            units = EnumTools.getValueOf(MaintUnitsFT.class, v , dft);
        } else {
            units = EnumTools.getValueOf(MaintUnitsFT.class, vs, dft);
        }
        Print.logDebug("MaintFT Units ("+vs+"): " + units);
        return units;
    }

    // --------------------------------

    // -- "MaintFT.index", "MaintFT.label"
    private static String KEY_MaintFT_due_      = "MaintFT.due_";     // [String] 
    private static String KEY_MaintFT_due       = "MaintFT.due";      // [I18N.Text] KEY_MAINT_DUE_FT
    private static String KEY_MaintFT_untilDue_ = "MaintFT.untilDue_";// [Long] 
    private static String KEY_MaintFT_units_    = "MaintFT.units";    // [String] Days|Seconds

    private int MaintFT_IndexDueState[] = null;

    /**
    *** Gets the MaintDue-FT list
    **/
    private int[] getMaintDueStateFT()
    {
        if (MaintFT_IndexDueState == null) {
            MaintFT_IndexDueState = new int[Device.getMaintCountFT()];
            ListTools.initArray(MaintFT_IndexDueState, -1);
        }
        return MaintFT_IndexDueState;
    }

    /**
    *** Checks the rule-trigger state of the specified Fixed-Time maintenance<br>
    *** (Used for Rule '$MaintFT' trigger state caching)
    **/
    public boolean checkRuleTriggeredMaintFT(int ndx, long intvFT, long offsFT, boolean triggerIfDueNow)
    {
        // -- check index
        if ((ndx < 0) || (ndx >= Device.getMaintCountFT())) {
            // -- invalid index
            Print.logDebug("$MaintFT("+ndx+"): ["+this.getDeviceID()+"] Invalid index: " + ndx);
            return false;
        }
        // -- check interval (seconds/days/months)
        if (intvFT <= 0L) {
            // -- no interval
            Print.logDebug("$MaintFT("+ndx+"): ["+this.getDeviceID()+"] No Interval");
            return false;
        }
        // -- FixedTime maintenance units and current EpochTime/DayNumber
        RTProperties rtp = this.getTemporaryProperties();
        MaintUnitsFT units = this.getMaintUnitsFT(ndx); // non-null
        TimeZone tmz = Account.getTimeZone(this.getAccount(), DateTime.GMT);
        long lastFT = this.getMaintFixedTimeFT(ndx); // EpochTime/DayNumber
        long currFT; // EpochTime/DayNumber
        long nextFT; // EpochTime/DayNumber
        switch (units) {
            default:
            case SECONDS :
                currFT = DateTime.getCurrentTimeSec();
                nextFT = lastFT + intvFT; // EpochTime + Seconds
                break;
            case DAYS    :
                currFT = DateTime.getCurrentDayNumber(tmz);
                nextFT = lastFT + intvFT; // DayNumber + Days
                break;
            case MONTHS  :
                currFT = DateTime.getCurrentDayNumber(tmz);
                nextFT = (new DayNumber(lastFT)).getMonthDelta((int)intvFT); // DayNumber + MonthDays
                break;
        }
        rtp.setProperty(KEY_MaintFT_units_, units.getText());
        Print.logDebug("$MaintFT("+ndx+"): ["+this.getDeviceID()+"] Units="+units+" Curr="+currFT+" Last="+lastFT+", Interval="+intvFT+", Offset="+offsFT);
        // -- set due
        int due = 0; // 0=no, 1=soon, 2=now
        if ((currFT + ((offsFT>0L)?offsFT:0L)) >= nextFT) {
            if (currFT < nextFT) {
                // -- checking due soon
                due = 1; // soon
            } else
            ...
            if (triggerIfDueNow) {
                // -- checking due soon, and trigger if past-due
                due = 2; // now
            } else {
                // -- checking due soon, but suppress past-due trigger
                //due = 2; // now, but suppress trigger
        ...
        if (due > 0) {
            // -- set index due flag
            this.getMaintDueStateFT()[ndx] = 1; // true
            // -- add label
            String ndxDesc  = this.getMaintLabelFT(ndx);
            String listDesc = StringTools.trim(rtp.getString(KEY_MaintFT_due_,null));
            if (!StringTools.isBlank(listDesc)) { listDesc += ", "; } // ie. "Tires, EngineOil, AirFilter"
            listDesc += ndxDesc;
            rtp.setString(  KEY_MaintFT_due_, listDesc);
            rtp.setProperty(KEY_MaintFT_due , new I18N.Text(listDesc)); // ${MaintFT.due}
        }
        // -- remaining time until actually due
        long remnFT = (nextFT > currFT)? (nextFT - currFT) : 0L;
        Object untilDueFT = rtp.getProperty(KEY_MaintFT_untilDue_,null);
        if (!(untilDueFT instanceof Number)) {
            // -- first until-due entry
            rtp.setProperty(KEY_MaintFT_untilDue_, new Long(remnFT));
        } else
        if (remnFT < ((Number)untilDueFT).longValue()) {
            // -- smaller/sooner until-due entry
            // -  Note: this assumes all FT units are the same
            rtp.setProperty(KEY_MaintFT_untilDue_, new Long(remnFT));
        }
        return (due > 0)? true : false;
    }
    ...
    /**
    *** Returns true if the Maintenance Fixed-Time for the specified index is due
    *** @param ndx      The Maintenance Fixed-Time index
    *** @param deltaFT  The delta fixed-time to check
    *** @return True if the Maintenance Fixed-Time for the specified index is due
    **/
    public boolean isMaintDueFT(int ndx, long deltaFT)  // getMaintRemainingFT
    {
        // -- does not support FT maintenance?
        //if (!Device.supportsMaintenance_FT()) { <-- already built-in to the checks below
        //    return false;
        //}
        // -- invalid index
        int mdi[] = this.getMaintDueStateFT();
        if ((ndx < 0) || (ndx >= mdi.length)) {
            return false;
        }
        // -- initialize?
        if (mdi[ndx] < 0) {
            long remnFT = this.getMaintRemainingFT(ndx);
            mdi[ndx] = ((remnFT >= 0L) && (remnFT <= deltaFT))? 1 : 0;
        }
        // -- return
        return (mdi[ndx] > 0)? true : false;
    }
    ...
    /**
    *** Gets the index of the next Fixed-Time maintenance that will become due.<br>
    *** Returns '-1' if none will become due
    *** @param maxFT The maximum next due threshold (0 for past due only)
    *** @return An array of next due indices
    **/
    public int[] getMaintNextDueFT(long maxFT)
    {
        Vector<Tuple.Pair<Integer,Long>> matchTP = new Vector<Tuple.Pair<Integer,Long>>();
        // -- populate list
        int cnt = Device.getMaintCountFT();
        for (int i = 0; i < cnt; i++) {
            long remnFT = this.getMaintRemainingFT(i);
            if (remnFT < 0L) {
                // -- maintenance bucket not used
            } else 
            if ((maxFT >= 0L) && (remnFT > maxFT)) {
                // -- due value is too far away
            } else {
                // -- due soon
                matchTP.add(new Tuple.Pair<Integer,Long>(new Integer(i),new Long(remnFT)));
            }
        }
        if (matchTP.size() <= 0) {
            return new int[0];
        }
        // -- sort by ft (smallest to largest)
        ListTools.sort(matchTP, new Comparator<Tuple.Pair<Integer,Long>>() {
            public int compare(Tuple.Pair<Integer,Long> tp1, Tuple.Pair<Integer,Long> tp2) {
                long n1 = tp1.b.longValue(); // ft
                long n2 = tp2.b.longValue(); // ft
                if (n1 == n2) {
                    return 0;
                } else {
                    return (n1 < n2)? -1 : 1;
    ...
    /**
    *** Gets an array of current maint-fixed-time due.
    *** Returns null if no maintenance due
    **/
    private String getMaintDueLabelsFT(String sep) 
    {
        String _sep = (sep != null)? sep : ",";
        StringBuffer labelSB = new StringBuffer();
        int cnt = Device.getMaintCountFT();
        for (int i = 0; i < cnt; i++) {
            if (this.isMaintDueFT(i,0L)) {
                if (labelSB.length() > 0) { labelSB.append(_sep); }
                labelSB.append(this.getMaintLabelFT(i));
            }
        }
        return labelSB.toString();
    }
    ...
    /**
    *** Gets an array of Fixed-Time Maintenance Labels
    **/
    @JSONBeanGetter(expandArray="true", ignore="$blank")
    ...
    **/
    ...
    {
        return (ndx >= 0)? this.getMaintLabelFT(ndx,("FixedTime#"+(ndx+1))) : "";
    }
    ...
    **/
    public String getMaintLabelFT(int ndx, String dft)
    {
        ...
        // -- 2) [OBSOLETE] try "Device.maintDescriptionFT#" field
        //v = this.getOptionalFieldValue(FLD_maintDescriptionFT_+ndx, (String)null);
        //if (!StringTools.isBlank(v)) {
        //    return v;
        //}
        // -- 3) try Account.getMaintLabelFT ...
        Account a = this.getAccount();
        v = (a != null)? a.getMaintLabelFT(ndx,null) : null;
        if (!StringTools.isBlank(v)) {
            return v;
        }
        // -- 4) return default
    ...
    /**
    *** Sets the fixed-time maintenance descrption for the specified index
    ...
    **/
    ...
    /**
    *** Gets an array of Fixed-Time Maintenance timestamps
    **/
    @JSONBeanGetter(expandArray="true", ignore="<=0")
    public long[] getMaintFixedTimeFT()
    {
        int cnt = Device.getMaintCountFT();
        long maintTimes[] = new long[cnt];
        for (int i = 0; i < maintTimes.length; i++) {
            maintTimes[i] = this.getMaintFixedTimeFT(i);
        }
        return maintTimes;
    }

    /**
    *** Gets the Maintenance Fixed-Time for the specified index
    *** @param ndx  The maintenance fixed-time index
    *** @return The maintenance fixed-time timestamp
    **/
    public long getMaintFixedTimeFT(int ndx)
    {
        return this.getOptionalFieldValue(FLD_maintFixedTimeFT_+ndx, 0L);
    }

    /** 
    *** Sets the Maintenance Fixed-Time for index #
    *** @param v The Maintenance Fixed-Time for index #
    **/
    public void setMaintFixedTimeFT(int ndx, long v)
    {
        this.setOptionalFieldValue((FLD_maintFixedTimeFT_+ndx), ((v >= 0L)? v : 0L));
    }

    /** 
    *** Resets the Maintenance Fixed-Time for the specified index
    *** @param ndx  The Maintenance Fixed-Time index
    **/
    public void resetMaintFixedTimeFT(int ndx)
    {
        TimeZone tmz = Account.getTimeZone(this.getAccount(),DateTime.getGMTTimeZone()); 
        long val;
        switch (this.getMaintUnitsFT(ndx)) {
            default:
            case SECONDS:  // EpochTime/Seconds
                val = DateTime.getCurrentTimeSec();
                break;
            case DAYS:  // DayNumber/Days
                val = DateTime.getCurrentDayNumber(tmz);
                break;
            case MONTHS:  // DayNumber/Months
                val = DateTime.getCurrentDayNumber(tmz);
                break;
        }
        this.setMaintFixedTimeFT(ndx,val);
        this.addOtherChangedFieldNames(FLD_maintFixedTimeFT_+ndx);
    }

    // --------------------------------

    /**
    *** Gets an array of Fixed-Time Maintenance values
    **/
    @JSONBeanGetter(expandArray="true", ignore="<=0")
    public long[] getMaintIntervalFT()
    {
        int cnt = Device.getMaintCountFT();
        long maintIntrvFT[] = new long[cnt];
        for (int i = 0; i < maintIntrvFT.length; i++) {
            maintIntrvFT[i] = this.getMaintIntervalFT(i);
        }
        return maintIntrvFT;
    }

    /**
    *** Gets the Maintenance Fixed-Time interval for the specified index
    *** @param ndx  The Maintenance Fixed-Time index
    *** @return The Maintenance Fixed-Time interval
    **/
    public long getMaintIntervalFT(int ndx)
    {
        long v = this.getOptionalFieldValue(FLD_maintIntervalFT_+ndx, 0L);
        return (v >= 0L)? v : 0L;
    }

    /** 
    *** Sets the Maintenance Fixed-Time interval for the specified index
    *** @param ndx  The Maintenance Fixed-Time index
    *** @param v    The Maintenance Fixed-Time value to set
    *** @return The Maintenance Fixed-Time interval
    **/
    public void setMaintIntervalFT(int ndx, long v)
    {
        this.setOptionalFieldValue((FLD_maintIntervalFT_+ndx), ((v >= 0L)? v : 0L));
    }

    // --------------------------------

    /**
    *** Gets an array of Fixed-Time Maintenance Pre-Notification Offsets
    **/
    @JSONBeanGetter(expandArray="true", ignore="<=0")
    public long[] getMaintPreNotifyFT()
    {
        int cnt = Device.getMaintCountFT();
        long maintPreNotifyFT[] = new long[cnt];
        for (int i = 0; i < maintPreNotifyFT.length; i++) {
            maintPreNotifyFT[i] = this.getMaintPreNotifyFT(i);
        }
        return maintPreNotifyFT;
    }

    /**
    *** Gets the Maintenance Fixed-Time Pre-Notification Offset for the specified index
    *** @param ndx  The Maintenance Fixed-Time index
    *** @return The Maintenance Fixed-Time Pre-Notification Offset
    **/
    public long getMaintPreNotifyFT(int ndx)
    {
        long v = this.getOptionalFieldValue(FLD_maintPreNotifyFT_+ndx, 0L);
        return (v >= 0L)? v : 0L;
    }

    /** 
    *** Sets the Maintenance Fixed-Time Pre-Notification Offset for the specified index
    *** @param ndx  The Maintenance Fixed-Time index
    *** @param v    The Maintenance Fixed-Time value to set
    *** @return The Maintenance Fixed-Time interval
    **/
    public void setMaintPreNotifyFT(int ndx, long v)
    {
        this.setOptionalFieldValue((FLD_maintPreNotifyFT_+ndx), ((v >= 0L)? v : 0L));
    }

    // --------------------------------

    /**
    *** Gets the maintenance due Fixed-Time
    **/
    public long getMaintDueFT(int ndx)
    {
        long intvFT = this.getMaintIntervalFT(ndx); // Seconds/Days/Months
        if (intvFT > 0L) {
            TimeZone tmz = Account.getTimeZone(this.getAccount(),DateTime.getGMTTimeZone()); 
            long lastFT = this.getMaintFixedTimeFT(ndx); // EpochTime/DayNumber
            long currFT; // EpochTime/DayNumber
            ...
            switch (this.getMaintUnitsFT(ndx)) {
                ...
                case SECONDS :
                    currFT = DateTime.getCurrentTimeSec();
                    nextFT = lastFT + intvFT; // next EpochTime
                    break;
                case DAYS    :
                    currFT = DateTime.getCurrentDayNumber(tmz);
                    nextFT = lastFT + intvFT; // next DayNumber
                    break;
                case MONTHS  :
                    currFT = DateTime.getCurrentDayNumber(tmz);
                    nextFT = (new DayNumber(lastFT)).getMonthDelta((int)intvFT); // next DayNumber
                    break;
            }
            return nextFT; // past due if (nextFT <= currFT)
        }
        // -- periodic maintenance not supported/available
        return -1L;
    }
    ...
    /**
    *** Gets the maintenance current Fixed-Time
    **/
    public long getMaintCurrentFT(int ndx)
    {
        long intvFT = this.getMaintIntervalFT(ndx); // Seconds/Days/Months
        if (intvFT > 0L) {
            TimeZone tmz = Account.getTimeZone(this.getAccount(),DateTime.getGMTTimeZone()); 
            ...
            return currFT; // current fixed-time
        }
        // -- periodic maintenance not supported/available
        return -1L;
    }
    ...
    /**
    *** Gets an array of Fixed-Time Maintenance Remaining
    **/
    @JSONBeanGetter(expandArray="true", ignore="<=0")
    public long[] getMaintRemainingFT() // getMaintenanceRemainingFT
    {
        int cnt = Device.getMaintCountFT();
        long remainFT[] = new long[cnt];
        for (int i = 0; i < remainFT.length; i++) {
            remainFT[i] = this.getMaintRemainingFT(i);
        }
        return remainFT;
    }

    /**
    *** Gets the number of fixed-time Seconds/Days (depending on units) until the next maintenance interval is due.
    *** Returns zero if past due, or -1 if periodic maintenance is not supported.
    **/
    public long getMaintRemainingFT(int ndx) // isMaintDueFT
    {
        if (Device.supportsMaintenance_FT(ndx)) {
            long intvFT = this.getMaintIntervalFT(ndx); // Seconds/Days/Months
            if (intvFT > 0L) {
                TimeZone tmz = Account.getTimeZone(this.getAccount(),DateTime.getGMTTimeZone()); 
                long lastFT = this.getMaintFixedTimeFT(ndx); // EpochTime/DayNumber
                ...
                return (nextFT > currFT)? (nextFT - currFT) : 0L; // Math.max((nextFT - currFT), 0L);
            }
        }
        // -- periodic maintenance not supported/available
        return -1L;
    ...
    public static boolean supportMaintenance_notes()
    {
        return Device.getFactory().hasField(FLD_maintNotes)? true : false;
    }
    ...
    **/
    ...
    // 1) Enable (in "config.conf") optional Device MaintFieldInfo fields
    //      startupInit.Device.MaintFieldInfo=true
    ...
    **/
    ...
    {
        // -- FLD_reminderType column may not currently be part of the Device record
    ...
    **/
    public String getThermalProfile()
    {
        ...
        // -- temp0=Frontyard:130/130 temp1=Backyard:130/130 temp2=Upstairs:130/130 temp3=Downstars:130/130 temp4=130/130 temp5=130/130 temp6=130/130 temp7=130/130 
    ...
    **/
    ...
    /**
    *** Get temperature profile range array
    **/
    public TemperatureRange[] getThermalProfileRange()
    {
        // -- T0=Frontyard:130/130 T1=Backyard:130/130 T2=Upstairs:130/130 T3=Downstars:130/130 T4=130/130 T5=130/130 T6=130/130 T7=130/130 
        RTProperties tempRTP = new RTProperties(this.getThermalProfile());
        int tCnt = EventData.getActualThermoAverageCount();
        TemperatureRange range[] = new TemperatureRange[tCnt];
        for (int i = 0; i < tCnt; i++) {
            String tKey[] = { "T"+i, "temp"+i };
            range[i] = TemperatureRange.ParseTemperatureRange(tempRTP.getString(tKey,null)); // may be null
        }
        return range;
    }

    /**
    *** Get temperature profile range labels
    **/
    public String[] getThermalProfileLabels()
    {
        String labels[] = ListTools.initArray(new String[EventData.getActualThermoAverageCount()],"");
        TemperatureRange range[] = this.getThermalProfileRange();
        if (range != null) {
            for (int i = 0; i < range.length; i++) {
                labels[i] = TemperatureRange.getLabel(range[i], "");
            }
        }
        return labels;
    }
    ...
    *** Get temperature profile range labels
    **/
    public String getThermalProfileLabel(int ndx, String dft)
    {
        TemperatureRange range[] = this.getThermalProfileRange();
        if ((ndx >= 0) && (ndx < ListTools.size(range))) {
            return TemperatureRange.getLabel(range[ndx], dft);
        } else {
            return dft;
        }
    }
    ...
    **/
    ...
    /**
    *** Gets the Controller Model/ID, or blank if not defined
    *** @return The Controller Model/ID
    **/
    public String getControllerModel()
    {
        String v = (String)this.getFieldValue(FLD_controllerModel);
        return StringTools.trim(v);
    }

    /**
    *** Sets the Controller Model/ID
    *** @param v The Controller Model/ID
    **/
    public void setControllerModel(String v)
    {
        this.setFieldValue(FLD_controllerModel, StringTools.trim(v));
    }
    ...
    **/
    public static boolean IsELogEnabled(Device dev, boolean showReason)
    {
        ...
        if (dev.isDeleted()) {
            if (showReason) { Print.logDebug("Device is deleted: " + dev.getAccountID() + "/" + dev.getDeviceID()); }
            return false;
        } else
        if (!dev.isActive()) {
    ...
    **/
    public static Device loadDeviceBySubscriberID(String subID)
        throws DBException
    {
        // -- READ_WRITE here because it is possible that a DCS module may receive two events
        // -  from a device in close succession.  Using READ_WRITE ensures that subsequent
        // -  reloads will get the latest updated information (odometer, engine-hours, etc).
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE; // READ_ONLY;
    ...
    **/
    ...
    /**
    *** Returns true if DataPullTime is supported
    *** @return True if DataPullTime is supported
    **/
    public static boolean SupportsDataPullTime()
    {
        return Device.getFactory().hasField(FLD_lastDataPullTime);
    }

    /**
    *** Gets the last data pull time
    *** @return The last data pull time
    **/
    public long getLastDataPullTime()
    {
        Long v = (Long)this.getFieldValue(FLD_lastDataPullTime);
        return (v != null)? v.longValue() : 0L;
    }

    /**
    *** Sets the last data pull time
    *** @param v The last data pull time
    **/
    public void setLastDataPullTime(long v)
    {
        this.setFieldValue(FLD_lastDataPullTime, v);
    }
    ...
    **/
    ...
    /** 
    *** Gets the DCS active state of this Device record
    **/
    public boolean isDCSActive()
    {
        ...
        if (this.isActive()) { // also checks "isDeleted()" again
            // -- device is !deleted and active
            return true;
        } else
        if (RTConfig.getBoolean(DBConfig.PROP_Device_insertEventsIfInactive,false)) {
            // -- device is !active, however save events anyway
            return true;
        } else {
            // -- device is !active, do not save events
            return false;
        }
    }
    ...
    /**
    *** Gets the short name of this Device (same as "getDisplayName()")
    **/
    ...
    private int recurse_getPreferredDescription = 0;
    
    private static final String PREF_DESCRIPTION_KEY_LIST[] = { "licensePlate", "device" };

    /**
    *** Gets the preferred configured display description
    **/
    public String getPreferredDescription()
    {
        return this.getPreferredDescription(PREF_DESCRIPTION_KEY_LIST);
    }

    /**
    *** Gets the preferred configured display description
    **/
    public String getPreferredDescription(String keyList[])
    {
        // -- called recursively?
        if (this.recurse_getPreferredDescription > 0) {
            return ""; // break recursion
        }
        // -- look for non-blank description
        if (!ListTools.isEmpty(keyList)) {
            ...
            try {
                this.recurse_getPreferredDescription++;
                for (String k : keyList) {
                    String desc = this.getKeyFieldValue(k, null, bpl);
    ...
    **/
    //@JSONBeanGetter()
    //public final String getDeviceID()  <-- is "final" in DeviceRecord
    //{
    ...
    **/
    ...
    /**
    *** Return the short Vehicle-ID this Device
    **/
    @JSONBeanGetter(name="VIN",ignore="$blank")
    public String getDeviceVID() // see also "getVehicleID()"
    {
        return super.getDeviceVID();
    }
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            //return this.getTransport().getDescription();
    ...
    **/
    ...
    {
        if (this.hasTransport()) {
            //this.getTransport().setDescription(v);
            super.setDescription(v);
        } else {
            super.setDescription(v);
        }
    }
    ...
    **/
    ...
    {
        // -- if the output format of this method changes, "parse_EventLog(..)" must also change to match.
        ...
        sb.append("Event: ");
        if (ev != null) {
            // -- [INFO_|01/28 18:34:51|Device.log_EventData:14946] Event: 2015/02/27|12:32:45|PST|1425069165, AccountID|DeviceID|UniqueID, 0xF020|Location|GeozoneID, Latitude|Longitude, Speed|Heading
            // -  eg. "[INFO_|05/03 09:26:25|Device.log_EventData:12115] Event: 2019/05/03|09:26:21|AST|1556889981, demo|test|ui_12345, 0xF112|EnRoute, 18.24808|-66.04222, 33.0|11"
    ...
    private static String C_INFO_           = "[INFO_|";
    private static String C_Event_          = "Event: ";
    private static String RTP_EventTime     = "EventTime";
    private static String RTP_SystemTime    = "SystemTime";
    private static String RTP_AccountID     = "AccountID";
    private static String RTP_DeviceID      = "DeviceID";
  //private static String RTP_MobileID      = "MobileID";
    private static String RTP_UniqueID      = "UniqueID";
    private static String RTP_StatusCode    = "StatusCode";
    private static String RTP_GeozoneID     = "GeozoneID";
    private static String RTP_GeoPoint      = "GeoPoint";
    private static String RTP_Speed         = "Speed";
    private static String RTP_Heading       = "Heading";

    /**
    *** Parses the event log information into an RTProperties instance
    *** @param evLog  The event log String
    **/
    public static RTProperties parse_EventLog(String evLog)
    {
        // -- [INFO_|01/28 18:34:51|Device.log_EventData:14946] Event: 2015/02/27|12:32:45|PST|1425069165, AccountID|DeviceID|UniqueID, 0xF020|Location|GeozoneID, Latitude|Longitude, Speed|Heading
        int year = 0;
        ...
        if (evLog == null) {
            ...
            int p = evLog.indexOf(C_Event_);
            if (p < 0) {
                return null;
            }
            evf = StringTools.split(evLog.substring(p + C_Event_.length()), ',');
            if (ListTools.isEmpty(evf)) {
                return null;
            }
        }
        ...
        /* system time */
        String _sysTime = null;
        if (evLog.startsWith(C_INFO_)) {
            int p = evLog.indexOf("|", C_INFO_.length());
            if (p > 0) {
                _sysTime = evLog.substring(C_INFO_.length()+1,p).replace(' ', ',');
            }
        }
        // -- year and timezone added below ...

        /* "Event: " fields */
        // -- event time: evf[0] = "2015/02/27|12:32:45|PST|1425069165"
        if ((evf.length > 0) && evf[0].startsWith("20")) {
            String v[] = StringTools.split(evf[0],'|');
            long evTS = (v.length > 3)? StringTools.parseLong(v[3],0L) : 0L;
            rtp.setLong(RTP_EventTime, evTS);
            // -- set system time
            if (!StringTools.isBlank(_sysTime)) {
                String sysT = null;
                try {
                    String tmzStr = (v.length > 2)? v[2] : null; // timestamp
                    TimeZone tmz = DateTime.getTimeZone(tmzStr, null);
                    DateTime nowDT = new DateTime();
                    int yyyy = (year > 1900)? year : nowDT.getYear();
                    sysT = yyyy + "/" + _sysTime + "," + tmzStr; // "02/27,12:32:45" ==> "2020/02/27,12:32:45,PST"
                    DateTime sysDT = DateTime.parseArgumentDate(sysT, tmz, null);
                    if (sysDT.getTimeSec() > nowDT.getTimeSec()) {
                        // -- calculated time is in the future, use last year time
                        sysT = (yyyy - 1) + "/" + _sysTime + "," + tmzStr;
                        sysDT = DateTime.parseArgumentDate(sysT, tmz, null);
                    }
                    long sysTS = sysDT.getTimeSec();
                    rtp.setLong(RTP_SystemTime, sysTS);
                } catch (DateTime.DateParseException dpe) {
                    // -- unlikely
                    Print.logException("Unable to parse time: " + sysT, dpe);
                }
            }
        } else {
            Print.logError("Invalid event time: >" + evf[0] + "<");
        }
        // -- AccountID|DeviceID|UniqueID: evf[1] = "AccountID|DeviceID|UniqueID"
        if (evf.length > 1) {
            String v[] = StringTools.split(evf[1],'|');
            // -- AccountID
            if (v.length > 0) {
                rtp.setString(RTP_AccountID, v[0]);
            }
            // -- DeviceID
            if (v.length > 1) {
                rtp.setString(RTP_DeviceID, v[1]);
            }
            // -- UniqueID/MobileID
            if (v.length > 2) {
                rtp.setString(RTP_UniqueID, v[2]);
            }
        }
        // -- StatusCode|Description|GeozoneID: evf[2] = "0xF020|Location|GeozoneID"
        if (evf.length > 2) {
            String v[] = StringTools.split(evf[2],'|');
            // -- StatusCode
            if (v.length > 0) {
                String sc = v[0] + ((v.length > 1)? ("|" + v[1]) : "");
                rtp.setString(RTP_StatusCode, sc);
            }
            // -- GeozoneID
            if (v.length > 2) {
                rtp.setString(RTP_GeozoneID, v[2]);
            }
        }
        // -- Latitude|Longitude: evf[3] = "Latitude|Longitude"
        if (evf.length > 3) {
            String v[] = StringTools.split(evf[3],'|');
            if (v.length > 1) {
                rtp.setString(RTP_GeoPoint, v[0] + "/" + v[1]);
            }
        }
        // -- Speed|Heading: evf[4] = "Speed|Heading"
        if (evf.length > 4) {
            String v[] = StringTools.split(evf[4],'|');
            // -- Speed
            if (v.length > 0) {
                rtp.setString(RTP_Speed, v[0]);
            }
            // -- Heading
            if (v.length > 1) {
                rtp.setString(RTP_Heading, v[1]);
            }
        }
    ...
    private static boolean FIX_WEEK_ROLLOVER = true; // valid only from 2019/04/06 to 2038/01/18

    /**
    *** Parse event information from the " Event: " log file line.
    *** @param evS  The "Event:" log file line
    **/
    public static EventData parse_log_EventData(String evS)
    {
        return Device.parse_log_EventData(evS, -1L, -1L);
    }
    ...
    *** @param evS  The "Event:" log file line
    **/
    public static EventData parse_log_EventData(String evS, long fromTS, long toTS)
    {
        // [INFO_|05/03 09:26:25|Device.log_EventData:12115] Event: 2019/05/03|09:26:21|AST|1556889981, demo|test|ui_12345, 0xF112|EnRoute, 18.24808|-66.04222, 33.0|11
        DBReadWriteMode rwMode = DBReadWriteMode.READ_ONLY;

        /* extract String following "Event: " */
        String ev;
        if (evS.startsWith("Event: ")) {
            ev = evS.substring("Event: ".length()).trim();
        } else {
            int e = (evS != null)? evS.indexOf(" Event: ") : -1;
            if (e < 0) {
                // -- not an "Event:" log line
                Print.logError("Not an ' Event: ' log line: " + evS);
                return null;
            }
            ev = evS.substring(e + " Event: ".length()).trim();
        }

        /* split on event components */
        String evF[] = StringTools.split(ev, ',');
        if (evF.length < 5) {
            Print.logError("Insufficient number of event components: " + ev);
            return null;
        }
        ...
        long timestamp;
        {
            // -- "2019/05/03|09:26:21|AST|1556889981"
            String ts[] = StringTools.split(evF[0],'|');
            timestamp = (ts.length > 3)? StringTools.parseLong(ts[3],0L) : 0L;
            // -- adjust timestamp here if needed
            if (FIX_WEEK_ROLLOVER && ((timestamp & 0x80000000L) != 0L)) { // high-32bit on, indicates week-rollover problem
                // -- WEEK_ROLLOVER: hack to fix the week-roller problem
                // -  Offset obtained from ATrack doc "Time Adjustment.pdf"
                // -  4091246111 + (-2536444800) ==> 1554801311 
                // -- NOTE: this should not be in 2038 or after
                timestamp += -2536444800L; // WEEK_ROLLOVER_OFFSET;
            }
            // -- check timestamp
            if (timestamp <= 0L) {
                Print.logError("Invalid timestamp: " + evF[0]);
                return null;
            } else 
            if ((fromTS > 0L) && (timestamp < fromTS)) {
                Print.logInfo("Timestamp is before 'from' time range: " + timestamp);
                return null;
            } else
            if ((toTS > 0L) && (timestamp > toTS)) {
                Print.logInfo("Timestamp is after 'to' time range: " + timestamp);
                return null;
            }
        }
        // -- Account/Device
        ...
        String uniqueID;
        {
            String ad[] = StringTools.split(evF[1],'|');
            accountID = (ad.length > 0)? StringTools.trim(ad[0]) : "";
            deviceID  = (ad.length > 1)? StringTools.trim(ad[1]) : "";
            uniqueID  = (ad.length > 2)? StringTools.trim(ad[2]) : "";
        }
        // -- Status Code
        int statusCode;
        {
            String st[] = StringTools.split(evF[2],'|');
            statusCode = (st.length > 0)? StringTools.parseInt(st[0],-1) : -1;
            if (statusCode <= 0) {
                Print.logWarn("Invalid status code: " + evF[2]);
        ...
        GeoPoint geoPoint;
        {
            String ll[] = StringTools.split(evF[3],'|');
            double lat = (ll.length > 0)? StringTools.parseDouble(ll[0],999.0) : 999.0;
            double lon = (ll.length > 1)? StringTools.parseDouble(ll[1],999.0) : 999.0;
        ...
        double heading;
        {
            String sh[] = StringTools.split(evF[4],'|');
            speedKPH = (sh.length > 0)? StringTools.parseDouble(sh[0],0.0) : 0.0;
            heading  = (sh.length > 1)? StringTools.parseDouble(sh[1],0.0) : 0.0;
        }
        ...
        try {
            ...
            if (account == null) {
                ...
                Print.logError("Account does not exist: " + accountID);
                return null;
            }
        } catch (DBException dbe) {
            // -- error reading Account
            Print.logError("Error reading Account: " + dbe);
            return null;
        }
        // -- get Device
        Device device;
        try {
            device = Device.getDevice(rwMode, account, deviceID);
            if (device == null) {
                // -- Device does not exist
                Print.logError("Device does not exist: " + accountID + "/" + deviceID);
                return null;
            }
        } catch (DBException dbe) {
            // -- error reading Device
            Print.logError("Error reading Device: " + dbe);
            return null;
        }

        /* final event component adjustments */
        if (statusCode < 0/*StatusCodes.STATUS_IGNORE*/) {
            Print.logWarn("Ignoring event status-code: " + evS);
            return null;
        }
        if (statusCode == StatusCodes.STATUS_NONE) {
            statusCode = (speedKPH > 0.0)? StatusCodes.STATUS_MOTION_ENROUTE : StatusCodes.STATUS_LOCATION;
        }

        /* create/return EventData */
    ...
    **/
        ...
        throws DBException
    {
        ...
        /* log event insertion */
        if (Device.GetLogEventDataInsertion() >= Print.LOG_WARN) {
            // -- LOG_WARN, LOG_INFO, LOG_DEBUG
            this.log_EventData(Device.GetLogEventDataInsertion(), evdb);
        }
        ...
        if (sc == StatusCodes.STATUS_GEOFENCE_DEPART) {
            ...
            if ((zone != null) && zone.isCorridorStart(evdb)) {
                ...
                this.setActiveCorridor(corridorID); // Mod: Device.FLD_activeCorridor
    ...
    **/
        ...
        throws DBException // if database insert exception occurs
    {
        ...
        /* set device copy (before modifications) */
        if (this.getDeferRuleCheckToPostInsert()) {
            // -- save current snapshot/copy of this Device instance
            // -  done to allow deferred EventData rule triggers
            try {
                Device.Key dk = new Device.Key(acctID, devID);
                Device devCopy = dk._getDBRecord();
                devCopy.setAllFieldValues(this); // (possible DBException?) copy non-key fields from original
                devCopy.setAccount(account);
                evdb.setDeviceCopy(devCopy); // pre-rule-trigger copy 
            } catch (DBException dbe) {
                // -- will not occur
                Print.logError("Unexpected error saving Device record snapshot/copy: " + dbe);
                evdb.setDeviceNoCopy(this); // pre-rule-trigger
            }
            // -- "checkEventRules" may be called in "_postEventInsertionProcessing" thread
        } else {
            // -- cache this Device instance
            evdb.setDeviceNoCopy(this); // pre-rule-trigger
        ...
        /* first-connect time */
        long firstConnectTime = this.getFirstConnectTime();
        if (firstConnectTime < 0L) {
            // -- "firstConnectTime" column doesn't exist, skip
        } else
        if (firstConnectTime == 0L) {
            // -- "firstConnectTime" not yet initialized (used by "suspendFirstConnectNotify")
            // -- attempt to read the first EventData record
            // -  this section needed to handle cases where the Device table has not yet
            // -  been updated with the first-connect time.
            DBFactory<EventData> edFact = EventData.getFactory();
            DBField fldCreateTime = edFact.getField(EventData.FLD_creationTime);
            if ((fldCreateTime != null) && fldCreateTime.isAlternateKey()) { 
                // -- column "creationTime" exists, and has an alternate key
                try {
                    EventData ev[] = EventData.getEventsByCreationTime(acctID,devID,0L,0L,1L);
                    if ((ev != null) && (ev.length > 0)) {
                        firstConnectTime = ev[0].getCreationTime();
                    }
                } catch (DBException dbe) {
                    Print.logError("Error getting first EventData 'creationTime' event [" + dbe);
                }
            } else {
                // -- Since the "creationTime" column is not keyed here, read the first
                // -  EventData record by "timestamp", then get the event "creationTime".
                // -- This does not guarantee that the obtained "creationTime" represents
                // -  the actual first created event.  If an event was ever received that had
                // -  a timestamp way in the past, then then there could be many records that
                // -  were created prior to the obtained creation-time.
                try {
                    EventData ev = this.getFirstEvent(0L,false); // first 'timestamp' event
                    if (ev != null) {
                        firstConnectTime = ev.getCreationTime();
                    }
                } catch (DBException dbe) {
                    Print.logError("Error getting first EventData 'timestamp' event [" + dbe);
                }
            }
            // -- first-connect initialized?
            try {
                if (firstConnectTime <= 0L) {
                    // -- stilll not yet initialized, use last-connect time
                    long lastConnectTime = this.getLastTotalConnectTime();
                    if (lastConnectTime > 0L) {
                        // -- if defined
                        firstConnectTime = lastConnectTime;
                    } else {
                        // -- else use current time
                        firstConnectTime = DateTime.getCurrentTimeSec();
                    }
                }
                this.setFirstConnectTime(firstConnectTime, true/*updateNow*/);
            } catch (DBException dbe) {
                Print.logException("Error updating first-connect time", dbe);
            }
        }
        ...
        if (evdb.isValidGeoPoint()) {
            // -- already have a valid location, no need to get cell-tower location
        } else
        if (!evdb.canUpdateCellTowerLocation()) {
            // -- we don't have an available cell-tower to look up
        } else {
        ...
        try {
            ...
            if (updFields != null) {
                ...
                if (Device.supportsLastSubdivision()) {
                    // -- update Device "lastSubdivision"
                    String subDiv = evdb.getSubdivision();
                    if (!StringTools.isBlank(subDiv)) {
                        this.setLastSubdivision(subDiv); // Mod: Device.FLD_lastSubdivision
                        try {
        ...
        */
        ...
        if ((lastIgnOff > 0L) && (lastIgnOff == lastIgnOn)) {
            ...
            if (ignNdx < 0) {  // [2.5.8-B37]
                ...
                this.setLastIgnitionOnTime(0L);  //Mod: Device. FLD_lastIgnitionOnTime
            } else
            if (ignNdx < StatusCodes.IGNITION_INPUT_INDEX) {  // [2.5.8-B37]
                ...
                if (this.getLastInputState(ignNdx)) {
                    ...
                    this.setLastIgnitionOffTime(0L);  // Mod: Device.FLD_lastIgnitionOffTime
                } else {
        ...
        if (lastEngOn < lastEngOff) {
            ...
            this.setLastEngineOnTime(0L); // Mod: Device.FLD_lastEngineOnTime
        } else
        if (lastEngOff < lastEngOn) {
            // -- last engine-off is BEFORE engine-on, clear engine-off
            lastEngOff = 0L; // last state confirmed engine-off
            this.setLastEngineOffTime(0L); // Mod: Device.FLD_lastEngineOffTime
        } else
        ...
        if (lastPtoOn < lastPtoOff) {
            ...
            if (lastPtoOn > 0L) { // should already be "0", but check anyway
                ...
                this.setLastPtoOnTime(0L); // Mod: Device.FLD_lastPtoOnTime
            }
            // -- PTO was off
            lastPtoState = 0; // last=OFF
        } else
        if (lastPtoOff < lastPtoOn) {
            ...
            if (lastPtoOff > 0L) { // should already be "0", but check anyway
                ...
                this.setLastPtoOffTime(0L); // Mod: Device.FLD_lastPtoOffTime
            }
            // -- PTO was on
            lastPtoState = 1; // last=ON
        } else
        if ((lastPtoOff > 0L) && (lastPtoOff == lastPtoOn)) {
        ...
        if (statusCode == StatusCodes.STATUS_PTO_OFF) {
            ...
            if (evdb.getPtoEngaged()) {
                ...
                Print.logDebug("PTO-Off status-code received ...");
            }
            evdb.setPtoEngaged(false);
            newPtoState = 0; // new=OFF
        } else 
        if (evdb.getPtoEngaged()) {
            // -- event indicates PTO is ON
            Print.logDebug("PTO-On indicated by event");
            newPtoState = 1; // new=ON
        } else {
        ...
        if (Device.GetSimulatePtoHours(this)) {
            ...
            if (evdb.getPtoHours() > 0.0) {
              ...
              //Print.logDebug("["+acctID+"/"+devID+"] SimPtoHours["+lastPtoState+"/"+newPtoState+"]: Event PTO-hours already set (leaving as-is): "+evdb.getPtoHours()+" ["+ptoHours+"]");
            } else
            if (hasPtoSt && (ptoHours > 0.0)) {
                // -- save current PTO-hours as event PTO-hours
                Print.logDebug("["+acctID+"/"+devID+"] SimPtoHours["+lastPtoState+"/"+newPtoState+"]: Event PTO-hours set to "+ptoHours+" hours (based on PTO On/Off)");
              ...
              //Print.logDebug("["+acctID+"/"+devID+"] SimPtoHours["+lastPtoState+"/"+newPtoState+"]: No available PTO hours");
                evdb.setPtoHours(ptoHours);
                // -- saved in Device record below (setLastPtoHours)
            }
        }
        ...
        Driver.DutyStatus dutyStatus   = Driver.DutyStatus.INVALID; // driverStatus
        boolean           saveDriverID = false;
        int     _SaveDriverID = Device.GetSaveEventDriverID(); // "always", "nonblank", "never"
        // -- save EventData DriverID
        if (_SaveDriverID == SaveDriverID_NEVER) {
            // -- skip driverID check
            if (evdb.hasOnDutyDriver(true)) { // evdb.hasValidDriverID()
                // -- warn that the event driverID will not be saved per configuration
                Print.logWarn("Ignoring active event driverID (per property configuration)");
            }
        } else
        if (this.hasOnDutyDriver(true) && evdb.hasOnDutyDriver(true) && (evdb.getTimestamp() < this.getDriverTime())) {
            // -- event timestamp predates last driver change timestamp, ignore ...
            Print.logWarn("Ignoring active event driverID (event timestamp predates last driver duty change)");
        } else
        if (evdb.hasValidDriverID() || (_SaveDriverID == SaveDriverID_ALWAYS)) {
            // -- update device driver-id (possibly blank)
            driverID   = evdb.getDriverID();   // may be blank (if SaveDriverID_ALWAYS)
            dutyStatus = evdb.getDriverStatusEnum();
            if (StringTools.isBlank(driverID)) {
                // -- no driver-id, set INVALID duty-status
                dutyStatus = Driver.DutyStatus.INVALID;
                evdb.setDriverStatus(dutyStatus.getIntValue());
            } else
            if (dutyStatus.isInvalid()) {
                // -- valid driver-id has an INVALID duty-status, set to UNKNOWN
                dutyStatus = Driver.DutyStatus.UNKNOWN;
                evdb.setDriverStatus(dutyStatus.getIntValue());
            }
            saveDriverID = true;
        } else
        if (this.hasOnDutyDriver(true)) {
            // -- update event with previous saved DriverID
            evdb.setDriverID(this.getDriverID());
            evdb.setDriverStatus(this.getDriverStatus());
        } else {
            // -- EventData DriverID already cleared 
        }
        ...
        // -- "evdb.getDevice()" instance has not been modified.
        ...
        try {
            ...
            Print.logError("EventData save failed: " + msg);
            // -- check for "com.mysql.jdbc.PacketTooBigException" (==> "max_allowed_packet")
            if (dbe.isPacketTooBigException()) {
                // -- TODO: display SQL that caused the error
            }
            // -- print full stacktrace
            if (Print.isDebugLoggingLevel()) {
                dbe.printException(); // DebugLoggingLevel
            }
        ...
        final boolean deferRuleCheck; // deferredRuleCheck
        if (!this.getDeferRuleCheckToPostInsert()) {
            // -- do not defer: deferred rule check not enabled
            // -  EventData Device instance is NOT a copy
            deferRuleCheck = false;
        } else
        if (extUpdate == EXT_UPDATE_NONE) {
            // -- do not defer: post event processing will not be invoked!
            // -  EventData Device instance IS a copy, switch back to not-a-copy
            deferRuleCheck = false;
            evdb.setDeviceNoCopy(this); // reset to not-a-copy
        } else {
        ...
        if (extUpdate != EXT_UPDATE_NONE) {
            // -- queue for background processing (reverse-geocode, etc)
        ...
        if (!deferRuleCheck) { // this.getDeferRuleCheckToPostInsert()
            ...
            if (this.checkEventRules(evdb,false)) { // non-deferred 
        ...
        /* save driver/duty status */
        if (saveDriverID) {
            // --
            this.setDriverID(driverID);                     // FLD_driverID
            this.setDriverStatus(dutyStatus.getIntValue()); // FLD_driverStatus
            this.setDriverTime(dutyStatus.isOnDuty()? evdb.getTimestamp() : 0L); // FLD_driverTime
            // --
            try {
                Driver.updateDriverStatus(
                    account, driverID, 
                    dutyStatus, evdb.getTimestamp(), 
                    this.getDeviceID(), false);
            } catch (DBException dbe) {
                Print.logError("Driver/Duty status save failed: " + dbe);
            }
        }

        /* device registration */
        // -- set for first event timestamp ever received (or when "registrationTime" has been reset)
        {
            long evTS  = evdb.getTimestamp();
            long regTS = this.getRegistrationTime();
            if (regTS <= 0L) {
                // -- registration not yet set (set to lesser of current
                long regTime = ((evTS > 0L) || (evTS <= nowTS))? evTS : nowTS;
                this.setRegistrationTime(regTime); // FLD_registrationTime
                this.addOtherChangedFieldNames(Device.FLD_registrationTime);
            } else
            if ((evTS > 0L) && (evTS < regTS)) {
                // -- registration was set, but the current event is older/prior
                long regTime = evTS;                
                this.setRegistrationTime(regTime); // FLD_registrationTime
                this.addOtherChangedFieldNames(Device.FLD_registrationTime);
            }
        ...
        /* return success (device not yet saved) */
        return true;
        // try .. catch
    ...
    **/
    private void _postEventInsertionProcessing(EventData evdb, int extUpdate, boolean deferRuleCheck)
    {
        ...
        if (deferRuleCheck) { // this.getDeferRuleCheckToPostInsert()
            ...
            if (this.checkEventRules(evdb,true)) { // deferred
    ...
    **/
    public boolean postCommandHandling(DCServerConfig.Command command, String cmdStr, boolean update)
    {
            ...
            if (this.hasTransport()) {
                ...
                try {
                    xport.update(
                        Transport.FLD_expectAck, 
                        Transport.FLD_expectAckCode, 
                        Transport.FLD_lastAckCommand, 
                      //Transport.FLD_lastAckResponse, 
                        Transport.FLD_lastAckTime);
                    return true;
                } catch (DBException dbe) {
                    Print.logException("Unable to set Transport.lastAck...", dbe);
                ...
                this.addOtherChangedFieldNames(
                    Device.FLD_expectAck, 
                    Device.FLD_expectAckCode, 
                    Device.FLD_lastAckCommand, 
                  //Device.FLD_lastAckResponse, 
                    Device.FLD_lastAckTime);
            }
        }
        ...
        if (command.hasAuditStatusCode()) {
            ...
            if (this.hasTransport()) {
                evd.setTransportID(this.getTransport().getTransportID());
            }
    ...
    **/
    public static OrderedSet<User> getAuthorizedUsers(DBReadWriteMode rwMode, Device device)
        throws DBException
    {
        rwMode = DBReadWriteMode.getDefaultReadWriteMode(rwMode);
        ...
        String userIDs[] = User.getUsersForAccount(rwMode,accountID);
        ...
        for (String userID : userIDs) {
            try {
                ...
                if (user.isAuthorizedDevice(rwMode,deviceID)) {
                    userList.add(user);
                }
            } catch (DBException dbe) {
                // ignore
    ...
    **/
    protected boolean checkEventRules(EventData event, boolean deferRuleCheck)
    {
        ...
        /* explicitly skip rule-checks for this event? */
        if (event.getSkipRuleChecks()) {
            // -- may be set by EventRuleAction "addEvent" pre-defined action
            Print.logInfo("Skipping rule-checks for this event ...");
            return false;
        }

        /* skip synthesized events */
        // -- "synthesized events" are currently those created by the "EventData.copySynthesizedEvent" method
        boolean isSynthesizedEvent = event.getIsSynthesizedEvent();
        // -- the following is commented to allow synthesized events to be checked against rules
        //if (isSynthesizedEvent) {
        //    Print.logInfo("Skipping rule check for synthesized event ...");
        //    return false;
        //}
        ...
        if (!this.getAllowNotify(true/*CheckAccount*/)) {
            Print.logInfo("Notification disallowed for this Account/Device: " + this);
            return false;
        }

        /* notification suspended for first device connection? */
        if (this.suspendFirstConnectNotify(nowTS)) { // first connection within the last X-seconds?
            Print.logInfo("Notification suspended for Device first-connection: " + this);
            return false;
        }
        ...
        if (!StringTools.isBlank(ruleSelector)) {
            // -- only if ENRE is NOT present
            Print.logDebug("Processing Device rule [selector = " + ruleSelector + "] " + this);
            int actionMask = ruleFact.executeSelector(ruleSelector, this.getNotifyAction(), event);
            // -- Fields may have changed: 
            // -   FLD_lastNotifyTime
            // -   FLD_lastNotifyCode
            if (this._setDeviceAction(actionMask,event,null/*Rule*/,deferRuleCheck)) {
                didTrigger = true;
    ...
    **/
    private boolean _setDeviceAction(int actionMask, EventData event, String ruleID, boolean deferRuleAction)
    {
        ...
        if ((actionMask & RuleFactory.ACTION_SAVE_LAST) != 0) {
            try {
                boolean updateNow = deferRuleAction? true : false; // [2.6.6-B19b]
                this.setLastNotifyEvent(event, ruleID, updateNow);
                // -- Fields modified:
                // -    FLD_lastNotifyTime
                // -    FLD_lastNotifyCode
                // -    FLD_lastNotifyRule
            } catch (DBException dbe) {
                // -- will only occur if "deferRuleAction" is true
                Print.logException("Error updating Device instance", dbe);
            }
        } else {
    ...
    **/
    ...
    /**
    *** Returns true if the specific event exists
    **/
    public boolean hasEventData(long timestamp, int statusCode)
        throws DBException
    {
        String aid = this.getAccountID();
        String did = this.getDeviceID();
        long   ts  = timestamp;
        int    sc  = statusCode;
        EventData ev = EventData.getEventData(aid,did,ts,sc); // optimize
    ...
    **/
    ...
    private static AccumulatorBoolean cacheLastFuelRefill_init = new AccumulatorBoolean(false);
    private static volatile EventData cacheLastFuelRefillEvent = null;

    /**
    *** Gets the last Fuel-Refill event
    **/
    public EventData getLastFuelRefillEvent()
    {
        return this.getLastFuelRefillEvent(false);
    }

    /**
    *** Gets the last Fuel-Refill event
    *** @param refresh  True to update the last retrieved fuel-refill event.
    **/
    public EventData getLastFuelRefillEvent(boolean refresh)
    {
        if (refresh || !cacheLastFuelRefill_init.get()) {
            synchronized (cacheLastFuelRefill_init) {
                if (refresh || !cacheLastFuelRefill_init.get()) {
                    try {
                        int sc[] = { StatusCodes.STATUS_FUEL_REFILL };
                        EventData ev = this.getLastEvent(sc);
                        cacheLastFuelRefillEvent = ev; // could be null
                    } catch (DBException dbe) {
                        Print.logError("Unable to read last refuel event: " + dbe);
                        cacheLastFuelRefillEvent = null;
                    }
                }
            }
        }
        return cacheLastFuelRefillEvent;
    }

    // --------------------------------

    /**
    *** Gets the distance (in kilometers) since the last Fuel-Refill.
    *** Returns -1 if unknown.
    **/
    public double getDistanceSinceFuelRefill()
    {
        return this.getDistanceSinceFuelRefill(false);
    }
    ...
    *** Returns -1 if unknown.
    **/
    public double getDistanceSinceFuelRefill(boolean refresh)
    {
        EventData ev = this.getLastFuelRefillEvent(refresh);
        if (ev != null) {
            double odomKM   = this.getLastOdometerKM();
            double refuelKM = ev.getOdometerKM();
            if ((refuelKM > 0.0) && (refuelKM <= odomKM)) {
                double deltaKM = odomKM - refuelKM;
                return deltaKM;
            }
        }
        return -1.0;
    ...
    **/
    ...
    private Transport transport_save = null;

    /**
    *** Returns true if the Transport has been defined for this instance
    **/
    public boolean hasTransport()
    {
        return (this.transport != null)? true : false;
    }

    // --------------------------------

    /**
    *** Clears the Transport instance field
    *** (ignored if already cleared)
    **/
    public boolean clearTransport()
    {
        if (this.transport != null) {
            // -- save copy and clear
            this.transport_save = this.transport;
            this.transport = null;
            return true;
        } else {
            // -- already cleared
    ...
    /**
    *** Restores the Transport instance to the prevously cleared instance
    *** (ignored if no prior Transport is defined)
    **/
    public boolean restoreTransport()
    {
        if (this.transport_save != null) {
            this.transport = this.transport_save;
            this.transport_save = null;
            return true;
        } else {
            // -- nothing to restore
            return false;
    ...
    /**
    *** Gets the Transport for this Device (may be null)
    **/
    ...
    **/
    ...
    {
        return this.hasTransport()? this.getTransport().getTransportID() : "";
    }
    ...
    **/
    ...
    {
        return this.hasTransport()? (DataTransport)this.getTransport() : (DataTransport)this;
    }
    ...
    **/
    ...
    @Override
    public Device _reload(DBReadWriteMode rwMode, String... fldNames)
        throws DBException
    {
        Device devThis = super._reload(rwMode, fldNames);
        /* clear cached items */
    ...
    **/
        ...
        throws DBException // if error occurs while testing existence
    {
        if ((acctID != null) && (devID != null)) {
            ...
            return devKey.exists(DBReadWriteMode.READ_WRITE);
        }
        return false;
    }
    ...
    **/
    ...
    /**
    *** Gets the first Devices matching to the specified DeviceID
    *** When using this method, the DeviceID must be unique among all Devices.
    *** If more than one Device is found matching the specified DeviceID, this method will return null.
    *** It is up to the caller to check whether this Device or Account are inactive.
    *** @param devID  The Device-ID of the device
    *** @return The loaded Device instance, or null if no Devices were found, or null if more than one Device found
    *** @throws DBException if a database error occurs
    **/
    public static Device loadDeviceByDeviceID(String devID)
        throws DBException
    {
        // -- READ_WRITE here because it is possible that a DCS module may receive two events
        // -  from a device in close succession.  Using READ_WRITE ensures that subsequent
        ...
        try {
            ...
            /* select */
            // -- DBSelect: SELECT * FROM Device WHERE (deviceID='dev')
            DBSelect<Device> dsel = new DBSelect<Device>(Device.getFactory());
            DBWhere dwh = dsel.createDBWhere();
            dsel.setWhere(dwh.WHERE_(dwh.EQ(Device.FLD_deviceID,devID)));
            dsel.setLimit(2); // limit 2 to check for multiple matching deviceIDs
            ...
            rs   = stmt.getResultSet();
            while (rs.next()) {
                if (rs.next()) { // found a second matching deviceID
                    java.util.List<String> acctList = Account.getAccountIDsForDeviceID(rwMode, devID);
                    Print.logError("Multiple 'deviceID' found: "+devID+" ["+StringTools.join(acctList,',')+"]");
                    dev = null;
                    break;
                }
                String aid = rs.getString(FLD_accountID);
                String did = rs.getString(FLD_deviceID);  // same as "devID"
                dev = new Device(new Device.Key(aid,did));
                dev.setAllFieldValues(rs);
                break;
            }
            // -- it's possible at this point that we haven't even read 1 device
            ...
            throw new DBException("Getting Device by Device-ID: " + devID, sqe);
        } finally {
            if (rs   != null) { try { rs.close();   } catch (Throwable t) {} }
            if (stmt != null) { try { stmt.close(); } catch (Throwable t) {} }
            DBConnection.release(dbc);
        ...
        /* return device list */
        // -- Note: 'dev' may be null if no devices were found, or if multiple devices were found
        return dev;
    ...
    /**
    *** Loads a Device within a Device Communication Server, based on the Account and Device IDs.
    *** @param account  The Account instance representing the owning account
    *** @param devID    The Device-ID
    *** @param autoAdd  If true will allow auto-add device (if allowed by account)
    *** @return The loaded Device instance, or null if the Device was not found
    *** @throws DBException if a database error occurs
    **/
    public static Device loadDeviceByName(Account account, String devID, boolean autoAdd)
        throws DBException
    {
        // -- READ_WRITE here because it is possible that a DCS module may receive two events
        // -  from a device in close succession.  Using READ_WRITE ensures that subsequent
        ...
        // -- validate
        if (account == null) {
            // -- no account? ... doesn't exist
            return null;
        } else
        if (StringTools.isBlank(devID)) {
            // -- no device? ... doesn't exist
            return null;
        }
        // -- adjust autoAdd/DBReadWriteMode
        if (autoAdd) {
            if (!account.okToAutoAddDevice()) { // account.getAutoAddDevices()
                // -- no auto-add for this account
                autoAdd = false;
            } else
            if (!rwMode.equals(DBReadWriteMode.READ_WRITE)) {
                // -- 'autoAdd' requires READ_WRITE database
                rwMode = DBReadWriteMode.READ_WRITE;
            }
        }
        // -- get device
        Device dev = Device.getDevice(rwMode, account, devID); // null if non-existent
        if (dev != null) {
            // -- found
            return dev;
        }
        ...
        if (autoAdd) {
            // -- Create a 'Device'
            dev = Device.getDevice(rwMode, account, devID, true); // create
            if (dev != null) {
                dev.setDescription(account.getNewDeviceDescription(null,"!"+devID));
                dev.save();
    ...
    //private static Device loadDeviceByName(Account account, String devID)
    //    throws DBException
    //{
    //    return Device.loadDeviceByName(account, devID, true);
    //}
    ...
    **/
    public static Device getDevice(DBReadWriteMode rwMode, Account account, String devID)
        throws DBException
    {
        return Device.getDevice(rwMode, account, devID, (String[])null/*allFields*/);
    }
    // --
    @Deprecated
        ...
        throws DBException
    {
        ...
        return Device.getDevice(rwMode, account, devID);
    }
    ...
    **/
    public static Device getDevice(DBReadWriteMode rwMode, Account account, String devID, String... fieldNames)
        throws DBException
    {
        rwMode = DBReadWriteMode.getDefaultReadWriteMode(rwMode);
        if ((account != null) && (devID != null)) {
            ...
            if (GetDevice_SkipExistsCheck) {
                Device dev = key._getDBRecord();
                if (dev._reload(rwMode,fieldNames) != null) {
                    // -- device exists
            ...
            if (key.exists(rwMode)) {
                // -- device exists
                Device dev = key._getDBRecord(true, rwMode, fieldNames); // throws DBException
                dev.setAccount(account);
                return dev;
            } else {
                // -- device does not exist
    ...
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        return Device.getDevice(rwMode, account, devID, fieldNames);
    }
    ...
    **/
    public static Device _getDevice(DBReadWriteMode rwMode, Account account, String devID)
        throws DBException
    {
        return Device._getDevice(rwMode, account, devID, (String[])null/*fieldNames*/);
    }
    // --
    @Deprecated
    ...
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        return Device._getDevice(rwMode, account, devID);
    }
    ...
    *** @throws DBException If any DB error occurs.
    **/
    public static Device _getDevice(DBReadWriteMode rwMode, Account account, String devID, String... fieldNames)
        throws DBException
    {
        rwMode = DBReadWriteMode.getDefaultReadWriteMode(rwMode);
        if ((account != null) && (devID != null)) {
            ...
            Device.Key key = new Device.Key(acctID, devID);
            // -- get DBRecord
            Device dev = key._getDBRecord(true, rwMode, fieldNames); // may throw DBException
            dev.setAccount(account);
            return dev;
        } else {
    ...
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        return Device._getDevice(rwMode, account, devID, fieldNames);
    }
    ...
    *** @throws DBExeption
    **/
    public static Device getDevice(DBReadWriteMode rwMode, Account account, String devID, boolean create)
        throws DBException // DBNotFoundException, DBAlreadyExistsException
    {

        /* adjust DBReadWriteMode */
        rwMode = DBReadWriteMode.getDefaultReadWriteMode(rwMode);
        if (create && !DBReadWriteMode.READ_WRITE.equals(rwMode)) {
            // -- READ_WRITE database must be used when creating
            rwMode = DBReadWriteMode.READ_WRITE;
        }
        ...
        if (create) {
            ...
            Device dev = devKey._getDBRecord();
            dev.setAccount(account);
        ...
        /* OBSOLETE
        Device.Key devKey = new Device.Key(acctID, devID);
        if (!devKey.exists(DBReadWrieMode.READ_WRITE)) {
            if (create) {
                Device dev = devKey._getDBRecord();
                dev.setAccount(account);
                dev.setCreationDefaultValues();
    ...
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        return Device.getDevice(rwMode, account, devID, create);
    }
    ...
    **/
        ...
        throws DBException
    {
        ...
        if ((account != null) && !StringTools.isBlank(devID)) {
            Device dev = Device.getDevice(rwMode, account, devID, true); // does not return null
    ...
    *** @param acctId    The Account ID
    *** @param selWhere  Additional 'Where' selection (may be null)
    *** @param limit     The maximum number of Device IDs to return
    *** @return A set of Device IDs
    *** @throws DBException
    **/
    public static OrderedSet<String> getDeviceIDsForAccount(DBReadWriteMode rwMode, String acctId, String selWhere, long limit)
        throws DBException
    {
        rwMode = DBReadWriteMode.getDefaultReadWriteMode(rwMode);
    ...
    // --
    @Deprecated
    public static OrderedSet<String> getDeviceIDsForAccount(String acctId, String selWhere, long limit)
        throws DBException
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        return Device.getDeviceIDsForAccount(rwMode, acctId, selWhere, limit);
    }
    ...
    *** @throws DBExeption
    **/
    public static OrderedSet<String> getDeviceIDsForAccount(DBReadWriteMode rwMode, String acctId, User userAuth, boolean inclInactv)
        throws DBException
    {
        return Device.getDeviceIDsForAccount(rwMode, acctId, userAuth, inclInactv, -1L);
    }
    // --
    @Deprecated
    ...
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        return Device.getDeviceIDsForAccount(rwMode, acctId, userAuth, inclInactv);
    }
    ...
    *** @throws DBExeption
    **/
    public static OrderedSet<String> getDeviceIDsForAccount(DBReadWriteMode rwMode, String acctId, User userAuth, boolean inclInactv, long limit)
        throws DBException
    {
        rwMode = DBReadWriteMode.getDefaultReadWriteMode(rwMode);
    ...
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        return Device.getDeviceIDsForAccount(rwMode, acctId, userAuth, inclInactv, limit);
    }
    ...
    private static final String KEY_DEVICE_SHORT[]    = new String[] { "deviceShortName"    , "shortName"      };  // "A12345"
    private static final String KEY_DEVICE_DESC[]     = new String[] { "deviceDescription"  , "devicePrefDesc" };
    ...
    private static final String KEY_FUEL_LEVEL[]      = new String[] { "fuelLevel"                                  };  // "25%"
    private static final String KEY_LAST_FUEL_LEV[]   = new String[] { "lastFuelLevel"                              };  // "25%"
    ...
    private static final String KEY_FUEL_LEVEL2[]     = new String[] { "fuelLevel2"                                 };  // "25%"
    private static final String KEY_LAST_FUEL_LEV2[]  = new String[] { "lastFuelLevel2"                             };  // "25%"
    ...
    private static final String KEY_REFUEL_DATETIME[] = new String[] { "lastRefuelDate"      , "lastRefillDate"     };  // "2007/08/09 03:02:51 GMT"
    private static final String KEY_REFUEL_LEVEL[]    = new String[] { "lastRefuelLevel"     , "lastRefillLevel"    };  // "100%"
    private static final String KEY_REFUEL_VOLUME[]   = new String[] { "lastRefuelVolume"    , "lastRefillVolume"   };  // "20 gal"
    private static final String KEY_REFUEL_DISTANCE[] = new String[] { "lastRefuelDistance"  , "lastRefillDistance" };  // "25 km"
    ...
    private static final String KEY_BATTERY_LEVEL[]   = new String[] { "batteryLevel"        , "battLevel"   , "lastBatteryLevel" };  // "75%"
    private static final String KEY_BATTERY_VOLTS[]   = new String[] { "batteryVolts"        , "battVolts"   , "lastBatteryVolts" };  // "4.5"
    private static final String KEY_VEH_BATTERY_V[]   = new String[] { "vBatteryVolts"       , "vehicleVolts", "lastVehicleVolts" };  // "12.8"

    private static final String KEY_PTO_STATE[]       = new String[] { "ptoState"            , "pto"            };
    ...
    // --
    private static final String KEY_MAINT_DUE_KM[]    = new String[] { "maintDueKM"          , "maintOdomDue"   , "maintLabelKM" }; // maintDueOdom,
    private static final String KEY_MAINT_DIST_NEXT[] = new String[] { "maintNextDueKM"      , "maintOdomNext"  }; // maintNextDelta,maintDistToNext,maintNextDist
  //private static final String KEY_MAINT_ODOMETER[]  = new String[] { "maintKM"             , "maintOdom"      , "maintOdometer" }; // 
  //private static final String KEY_MAINT_INTERV_KM[] = new String[] { "maintIntervalKM"     , "maintOdomInterval"}; // maintInterval
    // --
    private static final String KEY_MAINT_DUE_HR[]    = new String[] { "maintDueHR"          , "maintHoursDue"  , "maintLabelHR" };
    private static final String KEY_MAINT_HOUR_NEXT[] = new String[] { "maintNextDueHR"      , "maintHoursNext" };
    // --
    private static final String KEY_MAINT_DUE_FT[]    = new String[] { "maintDueFT"          , "maintTimeDue"   , "maintLabelFT" };
    private static final String KEY_MAINT_TIME_NEXT[] = new String[] { "maintNextDueFT"      , "maintTimeNext"  };
    ...
    **/
        ...
        Locale locale, BasicPrivateLabel bpl, Device dev)
    {
        ...
        /* Account */
        Account account = (dev != null)? dev.getAccount() : null;
        ...
        if (EventData._keyMatch(key,Device.KEY_DEVICE_ID)) {
            if (getTitle) {
        ...
        if (EventData._keyMatch(key,Device.KEY_DEVICE_SHORT)) {
            if (getTitle) {
                return i18n.getString("Device.key.deviceShortName", "Short Name");
            } else {
                return dev.getShortName(); // same as "this.getDisplayName();"
            }
        } else
        if (EventData._keyMatch(key,Device.KEY_DEVICE_DESC)) {
            if (getTitle) {
                return i18n.getString("Device.key.devicePreferredDesc", "Device Description");
            } else {
                return dev.getPreferredDescription(); // Bean
            }
        } else
        ...
        if (EventData._keyMatch(key,Device.KEY_FUEL_VOLUME)) {
            if (getTitle) {
                ...
                if ((L < 0.0) || (C <= 0.0)) {
                    ...
                    Account.VolumeUnits vu = Account.getVolumeUnits(account);
        ...
        /* FuelRefill/Refuel (tank #1) */
        if (EventData._keyMatch(key,Device.KEY_REFUEL_DATETIME)) {
            if (getTitle) {
                return i18n.getString("Device.key.refuelDateTime", "Last Refuel Date/Time");
            } else {
                EventData ev = dev.getLastFuelRefillEvent();
                if (ev == null) {
                    return i18n.getString("Device.notAvailable", "n/a");
                } else {
                    return ev.getTimestampString();
                }
            }
        } else
        if (EventData._keyMatch(key,Device.KEY_REFUEL_LEVEL)) {
            if (getTitle) {
                return i18n.getString("Device.key.refuelLevel", "Last Refuel Level");
            } else {
                Device.FuelTankIndex tank = Device.FuelTankIndex.TANK_1;
                EventData ev = dev.getLastFuelRefillEvent();
                if (ev == null) {
                    ...
                    double L = ev.getFuelLevel(tank,false/*estimate*/);
                    if (L < 0.0) {
                        return i18n.getString("Device.notAvailable", "n/a");
                    } else {
                        long pct = Math.round(L * 100.0);
        ...
        if (EventData._keyMatch(key,Device.KEY_REFUEL_VOLUME)) {
            if (getTitle) {
                return i18n.getString("Device.key.refuelVolume", "Last Refuel Volume");
            } else {
                Device.FuelTankIndex tank = Device.FuelTankIndex.TANK_1;
                EventData ev = dev.getLastFuelRefillEvent();
                if (ev == null) {
        ...
        if (EventData._keyMatch(key,Device.KEY_REFUEL_DISTANCE)) { // ODOMETER
            if (getTitle) {
                return i18n.getString("Device.key.refuelDistance", "Distance Since Refuel");
            } else {
                double distKM = dev.getDistanceSinceFuelRefill();
                if (distKM < 0.0) {
                    return i18n.getString("Device.notAvailable", "n/a");
                } else {
                    return Account.getDistanceString(account, distKM, true, locale);
                }
            }
        }
        ...
        if (EventData._keyMatch(key,Device.KEY_FAULT_CODES)) {
            if (getTitle) {
                ...
                if (!StringTools.isBlank(fault)) {
                    ...
                    return DTOBDFault.GetFaultString(rtpFault, false/*firstOnly?*/);
                }
                return "";
            }
        } else
        if (EventData._keyMatch(key,Device.KEY_FAULT_CODE)) {
            if (getTitle) {
                return i18n.getString("Device.key.faultCode", "Fault Code");
            } else {
                String fault = dev.getLastFaultCode().toUpperCase();
                if (!StringTools.isBlank(fault)) {
                    RTProperties rtpFault = new RTProperties(fault);
        ...
        if (EventData._keyMatch(key,Device.KEY_CORRIDOR_DESC)) {
            if (getTitle) {
                ...
                if (!StringTools.isBlank(actvCorr)) {
                    RuleFactory rf = Device.getRuleFactory();
                    String corrDesc = (rf != null)? rf.getGeoCorridorDescription(account,actvCorr) : null;
        ...
        if (EventData._keyMatch(key,Device.KEY_LAST_IGN_ON)) {
            if (getTitle) {
                ...
                if ((lastIgnOn > 0L) && (lastIgnOn > lastIgnOff)) {
                    // -- Ignition On
                    TimeZone tmz = (account != null)? account.getTimeZone(null) : DateTime.getGMTTimeZone();
                    return EventData.getTimestampString(lastIgnOn, account, tmz, bpl);
                } else {
        ...
        if (EventData._keyMatch(key,Device.KEY_LAST_IGN_OFF)) {
            if (getTitle) {
                ...
                if ((lastIgnOff > 0L) && (lastIgnOff > lastIgnOn)) {
                    // -- Ignition Off
                    TimeZone tmz = (account != null)? account.getTimeZone(null) : DateTime.getGMTTimeZone();
                    return EventData.getTimestampString(lastIgnOff, account, tmz, bpl);
                } else {
        ...
        /* Battery */
        if (EventData._keyMatch(key,Device.KEY_BATTERY_LEVEL)) {
            if (getTitle) {
                return i18n.getString("Device.key.batteryLevel", "Battery Level");
            } else {
                double BL = dev.getLastBatteryLevel();
                double P  = (BL < 0.0)? 0.0 : (BL <= 1.0)? (BL * 100.0) : BL;
                return Math.round(P) + "%";
            }
        } else
        if (EventData._keyMatch(key,Device.KEY_BATTERY_VOLTS)) {
            if (getTitle) {
                return i18n.getString("Device.key.batteryVolts", "Battery Volts");
            } else {
                double bv = dev.getLastBatteryVolts();
                return StringTools.format(bv, "0.0");
            }
        } else
        if (EventData._keyMatch(key,Device.KEY_VEH_BATTERY_V)) {
            if (getTitle) {
                return i18n.getString("Device.key.vehBatteryVolts", "Veh. Batt Volts");
            } else {
                double bv = dev.getLastVBatteryVolts();
                return StringTools.format(bv, "0.0");
            }
        }

        /* Last PTO state */
        if (EventData._keyMatch(key,Device.KEY_PTO_STATE)) {
            if (getTitle) {
                return i18n.getString("Device.key.ptoState", "PTO\nState");
            } else {
                long lastPtoOn  = dev.getLastPtoOnTime();
                long lastPtoOff = dev.getLastPtoOffTime();
                if ((lastPtoOn > 0L) && (lastPtoOn > lastPtoOff)) {
                    // -- PTO On
                    return i18n.getString("Device.ptoOn", "On");
                } else
                if ((lastPtoOff > 0L) && (lastPtoOff > lastPtoOn)) {
                    // -- PTO Off
                    return i18n.getString("Device.ptoOff", "Off");
                } else {
                    // -- PTO Unknown
                    return i18n.getString("Device.ptoUnknown", "Unkown");
                }
            }
        }
        ...
        if (EventData._keyMatch(key,Device.KEY_SPEED_LIMIT)) {
            if (getTitle) {
                ...
                if (kph <= 0.0) { // still <= 0.0
                    ...
                    return Account.getSpeedString(account,kph,true,locale);
                }
            }
        }
        ...
        if (EventData._keyMatch(key,Device.KEY_COMMAND_TIME)) {
            if (getTitle) {
                ...
                TimeZone tmz = (account != null)? account.getTimeZone(null) : DateTime.getGMTTimeZone();
                return EventData.getTimestampString(T, account, tmz, bpl);
            }
        }
        ...
        /* MaintKM: odometer maintenance fields */
        if (EventData._keyPrefix(key,Device.FLD_maintLabelKM_)) {
            int ndx = StringTools.parseInt(key.substring(Device.FLD_maintLabelKM_.length()),-1);
            if (getTitle) {
                String ndxStr = (ndx >= 0)? String.valueOf(ndx+1) : "";
                return i18n.getString("Device.key.maintOdomLabel", "Maint Odom #{0}", ndxStr);
            } else {
                return (ndx >= 0)? dev.getMaintLabelKM(ndx) : "";
            }
        } else
        if (EventData._keyPrefix(key,Device.FLD_maintIntervalKM_)) {
            int ndx = StringTools.parseInt(key.substring(Device.FLD_maintIntervalKM_.length()),-1);
            if (getTitle) {
                String label = (ndx >= 0)? dev.getMaintLabelKM(ndx) : "?";
                return i18n.getString("Device.key.maintOdomInterval", "Maint Odom Interval '{0}'", label);
            } else {
                double intvKM = (ndx >= 0)? dev.getMaintIntervalKM(ndx) : 0.0;
                if (intvKM <= 0.0) {
                    return i18n.getString("Device.key.maintOdomInterval.none","n/a");
                } else {
                    return Account.getDistanceString(account, intvKM, true, locale);
                }
            }
        } else
        if (EventData._keyPrefix(key,Device.FLD_maintOdometerKM_)) {
            int ndx = StringTools.parseInt(key.substring(Device.FLD_maintOdometerKM_.length()),-1);
            if (getTitle) {
                String label = (ndx >= 0)? dev.getMaintLabelKM(ndx) : "?";
                return i18n.getString("Device.key.maintOdomService", "Maint Odom Service #{0}", label);
            } else {
                double odomKM = (ndx >= 0)? dev.getMaintOdometerKM(ndx) : 0.0;
                if (odomKM <= 0.0) {
                    return i18n.getString("Device.key.maintOdomService.none","n/a");
                } else {
                    return Account.getDistanceString(account, odomKM, true, locale);
                }
            }
        } else
        if (EventData._keyPrefix(key,"maintOdometer")) {
            int ndx = StringTools.parseInt(key.substring("maintOdometer".length()),-1);
            if (getTitle) {
                String label = (ndx >= 0)? dev.getMaintLabelKM(ndx) : "?";
                return i18n.getString("Device.key.maintOdomService", "Maint Odom Service #{0}", label);
            } else {
        ...
        if (EventData._keyMatch(key,Device.KEY_MAINT_DIST_NEXT)) {
            if (getTitle) {
                return i18n.getString("Device.key.maintOdomNext", "Maint Odom Next");
            } else {
                int ndxList[] = dev.getMaintNextDueKM(Account.parseDistance(arg,-1.0));
                if (!ListTools.isEmpty(ndxList)) {
                    StringBuffer sb = new StringBuffer();
                    for (int ndx : ndxList) {
                        if (sb.length() > 0) { sb.append(", "); }
                        // -- eg. "Oil Change [32 miles]"
                        String text = null;
                        double remnKM = dev.getMaintRemainingKM(ndx);
                        if (remnKM < 0.0) {
                            // -- should not occur
                            text = i18n.getString("Device.key.maintOdomNext.none","n/a");
                        } else
                        if (remnKM == 0.0) {
                            // -- already due
                            text = i18n.getString("Device.key.maintOdomNext.pastDue", "Past Due");
                        } else {
                            // -- distance
                            text = Account.getDistanceString(account, remnKM, true, locale);
                        }
                        sb.append(dev.getMaintLabelKM(ndx) + " [" + text + "]");
                    }
                    return sb.toString();
                } else {
                    return i18n.getString("Device.key.maintOdomNext.none","n/a");
                }
            }
        } else
        if (EventData._keyMatch(key,Device.KEY_MAINT_DUE_KM)) { // [2.6.6-B62g]
            if (getTitle) {
                return i18n.getString("Device.key.maintOdomDue", "Maint Odom Due");
            } else {
                return dev.getMaintDueLabelsKM(", ");
            }
        }

        /* MaintHR: EngHours maintenance fields */
        if (EventData._keyPrefix(key,Device.FLD_maintLabelHR_)) {
            int ndx = StringTools.parseInt(key.substring(Device.FLD_maintLabelHR_.length()),-1);
            if (getTitle) {
                String ndxStr = (ndx >= 0)? String.valueOf(ndx+1) : "";
                return i18n.getString("Device.key.maintHoursLabel", "Maint Hours #{0}", ndxStr);
            } else {
                return (ndx >= 0)? StringTools.trim(dev.getMaintLabelHR(ndx)) : "";
            }
        } else
        if (EventData._keyPrefix(key,Device.FLD_maintIntervalHR_)) {
            int ndx = StringTools.parseInt(key.substring(Device.FLD_maintIntervalHR_.length()),-1);
            if (getTitle) {
                String label = (ndx >= 0)? dev.getMaintLabelHR(ndx) : "?";
                return i18n.getString("Device.key.maintHoursInterval", "Maint Hours Interval '{0}'", label);
            } else {
                double intvHR = (ndx >= 0)? dev.getMaintIntervalHR(ndx) : 0.0;
                if (intvHR <= 0.0) {
                    return i18n.getString("Device.key.maintHoursInterval.none","n/a");
                } else
                if (intvHR >= 1.0) {
                    return i18n.getString("Device.key.maintHoursInterval.hours","{0} Hour(s)",StringTools.format(intvHR,"0"));
                } else {
                    return i18n.getString("Device.key.maintHoursInterval.lessThanHour","<1 Hour");
                }
            }
        } else
        if (EventData._keyPrefix(key,Device.FLD_maintEngHoursHR_)) {
            int ndx = StringTools.parseInt(key.substring(Device.FLD_maintEngHoursHR_.length()),-1);
            if (getTitle) {
                String label = (ndx >= 0)? dev.getMaintLabelHR(ndx) : "?";
                return i18n.getString("Device.key.maintHoursService", "Maint Hours Service '{0}'", label);
            } else {
                double hourHR = (ndx >= 0)? dev.getMaintEngHoursHR(ndx) : 0.0;
                if (hourHR <= 0.0) {
                    return i18n.getString("Device.key.maintHoursService.none","n/a");
                } else
                if (hourHR >= 1.0) {
                    return i18n.getString("Device.key.maintHoursService.hours","{0} Hour(s)",StringTools.format(hourHR,"0"));
                } else {
                    return i18n.getString("Device.key.maintHoursService.lessThanHour","<1 Hour");
                }
            }
        } else
        if (EventData._keyPrefix(key,"maintEngineHours")) {
            int ndx = StringTools.parseInt(key.substring("maintEngineHours".length()),-1);
            if (getTitle) {
                String label = (ndx >= 0)? dev.getMaintLabelHR(ndx) : "?";
                return i18n.getString("Device.key.maintHoursService", "Maint Hours Service '{0}'", label);
            } else {
        ...
        if (EventData._keyMatch(key,Device.KEY_MAINT_HOUR_NEXT)) {
            if (getTitle) {
                return i18n.getString("Device.key.maintHoursNext", "Maint Hours NextDue");
            } else {
                int ndxList[] = dev.getMaintNextDueHR(DateTime.parseOffsetHours(arg,-1.0));
                if (!ListTools.isEmpty(ndxList)) {
                    StringBuffer sb = new StringBuffer();
                    for (int ndx : ndxList) {
                        if (sb.length() > 0) { sb.append(", "); }
                        // -- eg. "Oil Change [23.4 hours]"
                        String text = null;
                        double remnHR = dev.getMaintRemainingHR(ndx);
                        if (remnHR < 0.0) {
                            // -- should not occur
                            text = i18n.getString("Device.key.maintHoursNext.none","n/a");
                        } else
                        if (remnHR == 0.0) {
                            // -- already due
                            text = i18n.getString("Device.key.maintHoursNext.pastDue", "Past Due");
                        } else
                        if (remnHR >= 24.0) {
                            // -- days
                            long days = (long)(remnHR / 24.0); // truncate
                            text = i18n.getString("Device.key.maintHoursNext.days", "{0} Day(s)", String.valueOf(days));
                        } else {
                            // -- < 1 day
                            text = i18n.getString("Device.key.maintHoursNext.lessThanDay", "<1 Day");
                        }
                        sb.append(dev.getMaintLabelHR(ndx) + " [" + text + "]");
                    }
                    return sb.toString();
                } else {
                    return i18n.getString("Device.key.maintHoursNext.none","n/a");
                }
            }
        } else
        if (EventData._keyMatch(key,Device.KEY_MAINT_DUE_HR)) { // [2.6.6-B62h]
            if (getTitle) {
                return i18n.getString("Device.key.maintHoursDue", "Maint Hours Due");
            } else {
                return dev.getMaintDueLabelsHR(", ");
            }
        }

        /* MaintFT: FixedTime maintenance fields [2.6.7-B03] */
        if (EventData._keyPrefix(key,Device.FLD_maintLabelFT_)) {
            int ndx = StringTools.parseInt(key.substring(Device.FLD_maintLabelFT_.length()),-1);
            if (getTitle) {
                String ndxStr = (ndx >= 0)? String.valueOf(ndx+1) : "";
                return i18n.getString("Device.key.maintTimeLabel", "Maint Time #{0}", ndxStr);
            } else {
                return (ndx >= 0)? StringTools.trim(dev.getMaintLabelFT(ndx)) : "";
            }
        } else
        if (EventData._keyPrefix(key,Device.FLD_maintIntervalFT_)) {
            int ndx = StringTools.parseInt(key.substring(Device.FLD_maintIntervalFT_.length()),-1);
            if (getTitle) {
                String label = (ndx >= 0)? dev.getMaintLabelFT(ndx) : "?";
                return i18n.getString("Device.key.maintTimeInterval", "Maint Time Interval '{0}'", label);
            } else {
                long intvFT = (ndx >= 0)? dev.getMaintIntervalFT(ndx) : 0L;
                if (intvFT <= 0L) {
                    return i18n.getString("Device.key.maintTimeInterval.none","n/a");
                } else {
                    switch (dev.getMaintUnitsFT(ndx)) {
                        default:
                        case SECONDS: // Seconds
                            return i18n.getString("Device.key.maintTimeInterval.days","{0} Day(s)", StringTools.format((double)intvFT / (double)DateTime.DaySeconds(1),"0.0"));
                        case DAYS:    // Days
                            return i18n.getString("Device.key.maintTimeInterval.days","{0} Day(s)", String.valueOf(intvFT));
                        case MONTHS:  // Months
                            return i18n.getString("Device.key.maintTimeInterval.months","{0} Months(s)", String.valueOf(intvFT));
                    }
                }
            }
        } else
        if (EventData._keyPrefix(key,Device.FLD_maintFixedTimeFT_)) {
            int ndx = StringTools.parseInt(key.substring(Device.FLD_maintFixedTimeFT_.length()),-1);
            if (getTitle) {
                String label = (ndx >= 0)? dev.getMaintLabelFT(ndx) : "?";
                return i18n.getString("Device.key.maintTimeService", "Maint Hours Service '{0}'", label);
            } else {
                long timeFT = (ndx >= 0)? dev.getMaintFixedTimeFT(ndx) : 0L;
                if (timeFT <= 0L) {
                    return i18n.getString("Device.key.maintTimeService.none","n/a");
                } else {
                    TimeZone tmz     = (account != null)? account.getTimeZone(null) : DateTime.getGMTTimeZone();
                    DateTime dateFT  = new DateTime(timeFT, tmz);
                    String   dateFmt = (account != null)? account.getDateFormat() : "yyyy/MM/dd";
                    return dateFT.format(dateFmt);
                }
            }
        } else
        if (EventData._keyPrefix(key,"maintFixedTime")) {
            int ndx = StringTools.parseInt(key.substring("maintFixedTime".length()),-1);
            if (getTitle) {
                String label = (ndx >= 0)? dev.getMaintLabelFT(ndx) : "?";
                return i18n.getString("Device.key.maintTimeService", "Maint Hours Service '{0}'", label);
            } else {
        ...
        if (EventData._keyMatch(key,Device.KEY_MAINT_TIME_NEXT)) {
            if (getTitle) {
                return i18n.getString("Device.key.maintTimeNext", "Maint Time NextDue");
            } else {
                int ndxList[] = dev.getMaintNextDueFT(DateTime.parseOffsetSeconds(arg,-1L));
                if (!ListTools.isEmpty(ndxList)) {
                    StringBuffer sb = new StringBuffer();
                    for (int ndx : ndxList) {
                        if (sb.length() > 0) { sb.append(", "); }
                        // -- eg. "Oil Change [23.4 hours]"
                        String text = null;
                        long remnFT = dev.getMaintRemainingFT(ndx);
                        if (remnFT < 0L) {
                            // -- should not occur
                            text = i18n.getString("Device.key.maintTimeNext.none","n/a");
                        } else
                        if (remnFT == 0L) {
                            // -- already due
                            text = i18n.getString("Device.key.maintTimeNext.pastDue", "Past Due");
                        } else
                        if (remnFT >= DateTime.DaySeconds(1)) {
                            // -- days
                            long days = remnFT / DateTime.DaySeconds(1);
                            text = i18n.getString("Device.key.maintTimeNext.days","{0} Day(s)", String.valueOf(days));
                        } else {
                            // -- < 1 day
                            text = i18n.getString("Device.key.maintTimeNext.lessThanDay", "<1 Day");
                        }
                        sb.append(dev.getMaintLabelFT(ndx) + " [" + text + "]");
                    }
                    return sb.toString();
                } else {
                    return i18n.getString("Device.key.maintTimeNext.none","n/a");
                }
            }
        } else
        if (EventData._keyMatch(key,Device.KEY_MAINT_DUE_FT)) { // [2.6.7-B04]
            if (getTitle) {
                return i18n.getString("Device.key.maintTimeDue", "Maint Time Due");
            } else {
                return dev.getMaintDueLabelsFT(", ");
            }
        }
        ...
        if ((dev != null) && dev.hasTemporaryProperties()) {
            ...
            //Print.logInfo("Checking temporary properties: "+key+" ==> "+text+" ["+StringTools.className(text)+"]");
            ...
            //Print.logWarn("Device does not have any temporary properties: " + key);
        }
  ...
  //private static final String ARG_SETPROP[]           = new String[] { "setprop"                  };
    ...
    private static final String ARG_LIST_RULES[]        = new String[] { "listRules"                }; // not currently used
    private static final String ARG_LIST_DCS[]          = new String[] { "listDCS", "dcsList"       };
    ...
    **/
    private static void usage()
    {
        ...
        Print.sysPrintln("");
        Print.sysPrintln("  -listDCS=<dcsName>          List all Account/Device entries for specified DCS");
        Print.sysPrintln("");
        System.exit(1);
    }
    ...
    **/
    public static void main(String args[])
    {
        ...
        /* "getDevice' test */
        if ((acct != null) && RTConfig.hasProperty("getDevice")) {
            int    mode = RTConfig.getInt("getDevice",0);
            String modeStr = "?";
            Device dev = null;
            if (!StringTools.isBlank(uniqID)) {
                // -- load by UniqueID loadByUnique
                try {
                    modeStr = "Transport.loadDeviceByUniqueID(uniqID)";
                    dev = Transport.loadDeviceByUniqueID(uniqID);  // null if non-existant
                } catch (DBException dbe) {
                    Print.logException(modeStr + " Error ["+uniqID+"]", dbe);
                }
            } else {
                // -- load by AccountID/DeviceID
                try {
                    if (mode > 0) {
                        modeStr = "getDevice(acct, devID, create=false)";
                        dev = Device.getDevice(rwMode, acct, devID, false); // DBNotFoundException if non-existant
                    } else {
                        modeStr = "getDevice(acct, devID, fields[]=null)";
                        dev = Device.getDevice(rwMode, acct, devID, (String[])null);  // null if non-existant
                    }
                } catch (DBException dbe) {
                    Print.logException(modeStr + " Error ["+_fmtDevID(acctID,devID)+"]", dbe);
                }
            }
            Print.sysPrintln(modeStr + " ==> " + dev);
            System.exit(0);
        }
        ...
        if (deviceExists) {
            try {
                deviceRcd = Device.getDevice(rwMode, acct, devID, false); // may throw DBException
            } catch (DBException dbe) {
        ...
        /* list Devices for DCS module */
        if (RTConfig.hasProperty(ARG_LIST_DCS)) {
            opts++;
            String dcsName = StringTools.trim(RTConfig.getString(ARG_LIST_DCS,null));
            try {
                Collection<Device.Key> devList = Device.getDeviceKeysForDeviceCode(dcsName);
                if (!ListTools.isEmpty(devList)) {
                    Print.sysPrintln("Found " + ListTools.size(devList) + " entries for DCS: " + dcsName);
                    for (Device.Key dk : devList) {
                        Print.sysPrintln("  " + dk);
                    }
                } else {
                    Print.logWarn("No devices found with this DCS name: " + dcsName);
                }
            } catch (DBException dbe) {
                Print.logError("Error getting Device list for DCS name: " + dcsName + " ["+dbe.getMessage()+"]");
                System.exit(99);
            }
            System.exit(0);
        }
        ...
        /* delete */
        // -- bin/admin.pl Device -account=ACCOUNT -device=DEV1,DEV2,DEV3 -delete
        if (RTConfig.getBoolean(ARG_DELETE, false) && 
            !StringTools.isBlank(acctID) && !StringTools.isBlank(devID)) {
            deviceExists = false; // not applicable here
            opts++;
            String devIDList[] = StringTools.split(devID,','); // "dev1,dev2,dev3,..."
            for (String D : devIDList) {
                // -- skip blank devices
                if (StringTools.isBlank(D)) {
                    continue; // just in case a blank device was specified
                }
                String adName = _fmtDevID(acctID,D);
                // -- device-id exists?
                boolean devExists = false;
                try {
                    devExists = Device.exists(acctID,D);
                } catch (DBException dbe) {
                    Print.logError("Error determining if Device exists: " + adName);
                    System.exit(99);
                }
                // -- delete
                try {
                    if (devExists) {
                        Print.logInfo("Deleting: " + adName + " ...");
                    } else {
                        Print.logWarn("Device does not exist: " + adName);
                        Print.logInfo(" ... attempting to delete anyway ...");
                    }
                    Device.Key devKey = new Device.Key(acctID,D);
                    devKey.delete(true); // also delete dependencies
                    Print.logInfo(" ... Device deleted (" + adName + ")");
                } catch (DBException dbe) {
                    Print.logError("Error deleting Device: " + adName);
                    dbe.printException(); // CLI
                    System.exit(99);
                }
            }
            ...
            if (!acctExists || !deviceExists) {
                ...
                try {
                    ...
                    evUtil.writeEvents((PrintWriter)null, 
                        acct, devList, 
                        fmt, true/*allTags*/, false/*inclMaint*/,
                        null/*timezone*/,
                        privLabel);
                } catch (IOException ioe) {
                    Print.logError("IO Error");
        ...
        if (RTConfig.hasProperty(ARG_INSERT)) {
            ...
            if (!gp.isValid()) {
                ...
                SmtpProperties.setGlobalThreadModel_DEBUG();
        ...
        if (RTConfig.hasProperty(ARG_MAINTKM)) {
            ...
            if (odomKM >= (maintKM + intervKM)) {
                ...
                String maintLabl = deviceRcd.getMaintLabelKM(ndx);
                Print.logInfo("'"+maintLabl+"' Service Interval due for " + deviceRcd.getDescription());
                ...
                if (!StringTools.isBlank(frEmail) && !StringTools.isBlank(toEmail)) {
                    ...
                    String text = i18n.getString("Device.serviceMaint.dueFor","Periodic Maintenance({1}) due for {0}",devDesc,maintLabl);
        ...
        if (RTConfig.hasProperty(ARG_CHECKRULES)) {
            ...
            // -- check rules
            if (!deviceRcd.checkEventRules(evRcd,false)) { // CLI: command-line
        ...
        if (RTConfig.hasProperty(ARG_RESET_ODOM)) {
            ...
            device.setLastOdometerKM(0.0);  // FLD_lastOdometerKM
        ...
        if (RTConfig.hasProperty(ARG_SIM_PHONE)) {
            ...
            try {
                java.util.List<String> devList = Device.getDeviceIDsForSimPhoneNumber(DBReadWriteMode.READ_ONLY, simPhone);
                if (!ListTools.isEmpty(devList)) {
        ...
        if (RTConfig.hasProperty("maintTemplates")) {
            Print.sysPrintln("Maint Odometer Templates: " + MaintFields_Odom().length + " ("+Device.getMaintCountKM()+")");
            for (DBField dbf : MaintFields_Odom()) {
                Print.sysPrintln("DBField ==> " + dbf + " - " + dbf.getTitle());
            }
            Print.sysPrintln("Maint Hours Templates: " + MaintFields_Hours().length + " ("+Device.getMaintCountHR()+")");
            for (DBField dbf : MaintFields_Hours()) {
                Print.sysPrintln("DBField ==> " + dbf + " - " + dbf.getTitle());
            }
            Print.sysPrintln("Maint Fixed-Time Templates: " + MaintFields_FixedTime().length + " ("+Device.getMaintCountFT()+")");
            for (DBField dbf : MaintFields_FixedTime()) {
                Print.sysPrintln("DBField ==> " + dbf + " - " + dbf.getTitle());
            }
            System.exit(0);
        }
        ...
        /* clear any pending ACK */
        if (RTConfig.hasProperty("maintFT")) {
            opts++;
            if (!acctExists || !deviceExists) {
                Print.logError("Account/Device does not exist: " + _fmtDevID(acctID,devID));
                System.exit(98);
            ...
            // clear ack
            MaintUnitsFT ftUnits = deviceRcd.getMaintUnitsFT(0);
            Print.logInfo("MaintFT Units: " + ftUnits);
            System.exit(0);
        }

        /* "Event:" parse test */
        if (RTConfig.hasProperty("vin")) {
            opts++;
            try {
                String VIN = RTConfig.getString("vin", null);
                Device dev = Device.loadDeviceByVehicleID(VIN);
                if (dev != null) {
                    Print.logInfo("Found Device VIN: " + VIN + " ==> " + dev.getAccountID() + "/" + dev.getDeviceID());
                } else {
                    Print.logInfo("Device VIN not found: " + VIN);
                }
                System.exit(0);
            } catch (DBException dbe) {
                Print.logException("Unable to load Device by VIN", dbe);
                System.exit(1);
            }
        }
