    ...
    implements StatusCodeProvider
{
    ...
    **/
    ...
    /* return StatusCode (READ database assummed) */
        ...
        if (!StringTools.isBlank(accountID)) {
            ...
            if (!StringTools.isBlank(deviceID)) {
                try {
                    ...
                    if (codeKey.exists(rwMode)) { // may throw DBException
                        StatusCode code = codeKey._getDBRecord(true, rwMode);
        ...
        throws DBException // if error occurs while testing existance
    {
        if ((accountID != null) && (deviceID != null)) {
            ...
            return scKey.exists(DBReadWriteMode.READ_WRITE);
        }
        return false;
    }
    ...
    /* Return specified StatusCode, create if specified (READ_WRITE database assumed) */
        ...
        throws DBException
    {
        ...
        /* get/create entity */
        StatusCode.Key scKey = new StatusCode.Key(accountID, deviceID, code);
        if (scKey.exists(rwMode)) { // may throw DBException
            StatusCode sc = scKey._getDBRecord(true, rwMode);
            if (account != null) {
        ...
        if (createOK) {
            StatusCode sc = scKey._getDBRecord();
            if (account != null) {
                sc.setAccount(account);
            }
    ...
    /**
    *** Return all custom status-code values for the specified account/device.
    ...
    **/
        ...
        throws DBException
    {
        ...
        /* select */
        // DBSelect: SELECT statucCode FROM StatusCode WHERE (accountID='acct') AND ((deviceID='*')OR(deviceID='DEVICE')) ORDER BY statucCode
        ...
        if (deviceID.equals(ALL_DEVICES)) {
            ...
            dsel.setWhere(dwh.WHERE_(
                dwh.AND(
                    ...
                    dwh.EQ(StatusCode.FLD_deviceID ,ALL_DEVICES)
                )
            ));
        } else {
            // -- all devices, or those that apply to a specific device
            dsel.setWhere(dwh.WHERE_(
                dwh.AND(
                    dwh.EQ(StatusCode.FLD_accountID,accountID),
                    dwh.OR(
    ...
    /**
    *** Return all custom StatusCode instances for the specified account/device 
    **/
    public static Collection<StatusCode> getStatusCodeList(DBReadWriteMode rwMode, String accountID, String deviceID)
        throws DBException
    {

        /* account-id specified? */
        if (StringTools.isBlank(accountID)) {
            return new Vector<StatusCode>();
        }
        ...
        /* select */
        // DBSelect: SELECT * FROM StatusCode WHERE (accountID='acct') AND ((deviceID='*')OR(deviceID='DEVICE')) ORDER BY statucCode
        DBSelect<StatusCode> dsel = new DBSelect<StatusCode>(StatusCode.getFactory());
        DBWhere dwh = dsel.createDBWhere();
        if (deviceID.equals(ALL_DEVICES)) {
            // -- all devices only
        ...
        /* get list */
        Collection<StatusCode> codeList = new Vector<StatusCode>();
        DBConnection dbc = null;
        Statement   stmt = null;
        ResultSet     rs = null;
        try {
            ...
            rs   = stmt.getResultSet();
            while (rs.next()) {
                String acctID = rs.getString(StatusCode.FLD_accountID);
                int    code   = rs.getInt(StatusCode.FLD_statusCode);
                StatusCode sc = new StatusCode(new StatusCode.Key(acctID,code));
                sc.setAllFieldValues(rs);
                codeList.add(sc);
            }
        } catch (SQLException sqe) {
            throw new DBException("Getting StatusCode List", sqe);
    ...
    // --
    @Deprecated
    public static Collection<StatusCode> getStatusCodeList(String accountID, String deviceID)
        throws DBException
    {
        DBReadWriteMode rwMode = DBReadWriteMode.READ_WRITE;
        return StatusCode.getStatusCodeList(rwMode, accountID, deviceID);
    }
