    ...
    extends UserRecord<UserDevice>
{
        ...
        throws DBException // if error occurs while testing existance
    {
        if ((acctID != null) && (userID != null) && (deviceID != null)) {
            ...
            return usrDevKey.exists(DBReadWriteMode.READ_WRITE);
        }
        return false;
    }
        ...
        throws DBException
    {
        ...
        if (usrDevKey.exists(rwMode)) { // may throw DBException
            // -- already exists
            UserDevice listItem = usrDevKey._getDBRecord(true, rwMode);
            listItem.setUser(user);
            return listItem;
        } else
        if (createOK) {
            UserDevice listItem = usrDevKey._getDBRecord();
    ...
    /* return list of all Devices within the specified User (READ database assumed) */
