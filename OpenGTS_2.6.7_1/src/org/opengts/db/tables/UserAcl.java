    ...
    extends UserRecord<UserAcl>
{
        ...
        throws DBException // if error occurs while testing existance
    {
        if ((acctID != null) && (userID != null) && (aclID != null)) {
            ...
            return aclKey.exists(DBReadWriteMode.READ_WRITE);
        }
        return false;
    }
        ...
        throws DBException
    {
        ...
        UserAcl.Key aclKey = new UserAcl.Key(acctId, userId, aclId);
        if (aclKey.exists(DBReadWriteMode.READ_WRITE)) { // may throw DBException
            userAcl = UserAcl.getUserAcl(user, aclId); // may throw DBException
        } else {
            userAcl = aclKey._getDBRecord();
        ...
        throws DBException
    {
        ...
        if ((user != null) && (aclId != null)) {
            ...
            if (aclKey.exists(rwMode)) {
                UserAcl userAcl = aclKey._getDBRecord(true, rwMode); // reload
                userAcl.setUser(user);
                return userAcl;
            } else
            if (user.isAdminUser()) {
                //UserAcl userAcl = (UserAcl)aclKey._getDBRecord(); // no reload (new record)
                //userAcl.setUser(user);
