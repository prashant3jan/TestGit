...
//     -Added back support for "EventNotificationEMail" (for legacy purposes) [2.6.7-B15e]
// ----------------------------------------------------------------------------
...
public class BasicPrivateLabelLoader
{
    ...
    private static volatile int _ExtendedDebugLogging            = -1;

    /**
    *** Gets the global extended debug logging state
    **/
    protected static boolean ExtendedDebugLogging()
    {
        if (_ExtendedDebugLogging < 0) {
            _ExtendedDebugLogging = RTConfig.isDebugMode()? 1 : 0;
        }
        return (_ExtendedDebugLogging > 0)? true : false;
    }
    ...
    public  static final String CLASS_CelltracGTS               = DBConfig.PACKAGE_OPT_WAR_ + "ctrac.servlet.Data";
    ...
    public  static final String TAG_DefaultLoginAccount         = "DefaultLoginAccount";    // NO LONGER SUPPORTED
  ...
  //public  static final String TAG_EMailAddress                = "EMailAddress";
    ...
    public  static final String TAG_IconImage2                  = "IconImage2";
    public  static final String TAG_MatIcon                     = "MatIcon";
  ...
  //public  static final String TAG_Description                 = "Description";
    ...
    public  static final String ATTR_ftl                        = "ftl"; // Freemarker alias for "jsp" attribute
  ...
  //public  static final String ATTR_jspName                    = "jspName";
    ...
    public  static final String ATTR_iconDir2                   = "iconDir2";
    ...
    public  static final String ATTR_economy                    = "economy"; // "motion","moving","lowcost",...
    ...
    public  static final String ATTR_routeColor                 = "routeColor"; // RouteSegmentColor
        ...
        if (BasicPrivateLabelLoader.privateLabelLoader == null) {
            ...
            if (!BasicPrivateLabelLoader.isTrackServlet()) {
                if (!RTConfig.isWebApp()) {
                    ...
                    String cn[] = new String[] { CLASS_Track, CLASS_Service, CLASS_CelltracGTS };
    ...
    **/
    ...
    {
        //Print.logStackTrace("Loading PrivateLabel file: " + xmlFile);
    ...
    **/
        ...
        boolean ignoreDuplicates)
    {
        ...
        pl.setShowAccountLogin(acctLogin); // see also "DefaultLoginAccount"
        pl.setShowUserLogin(userLogin);
        pl.setShowEmailLogin(emailLogin);
        ...
        for (int c = 0; c < attrList.getLength(); c++) {
            ...
            if (attrName.equalsIgnoreCase(TAG_EventNotificationEMail)) { // EventNotificationEMail [OBSOLETE]
    ...
    **/
    protected void parseTag_ReverseGeocodeProvider(File xmlFile, BasicPrivateLabel pl, Element rgpAttrElem)
    {
        ...
        /* active? */
        String activeStrR = XMLTools.getAttribute(rgpAttrElem, ATTR_active, null, false);
        String activeStr  = RTConfig.insertKeyValues(activeStrR);
        boolean active = BasicPrivateLabelLoader._isAttributeActive(activeStr,rpName);
        //Print.logInfo("["+pl.getName()+":"+pl.getPageTitle()+"] ReverseGeocodeProvider active String ["+rpName+"]: " + activeStrR + " ==> " + activeStr + " ==> " + active);
        ...
        /* economy RGP (used while in-motion) */
        ReverseGeocodeProvider economyRGP = null;
        { // local scope
            String rgpName = XMLTools.getAttribute(rgpAttrElem,ATTR_economy,null,true); // economy RGP
            if (!StringTools.isBlank(rgpName)) {
                ReverseGeocodeProvider rgp = pl.getReverseGeocodeProvider(rgpName);
                if (rgp == null) {
                    printError("Economy ReverseGeocodeProvider not found ["+xmlFile+"]: " + rgpName);
                    this._setHasParsingErrors(xmlFile);
                } else
                if (rgp.hasEconomyReverseGeocodeProvider()) {
                    printError("Economy ReverseGeocodeProvider recursive definition ["+xmlFile+"]: " + rgpName);
                    this._setHasParsingErrors(xmlFile);
                } else {
                    //Print.logInfo("Found Economy RG: " + rgpName);
                    economyRGP = rgp;
                }
            }
        }
        ...
        { // local scope
            String rgpName = XMLTools.getAttribute(rgpAttrElem,ATTR_failover,null,true); // failover RGP
            if (!StringTools.isBlank(rgpName)) {
                ReverseGeocodeProvider rgp = pl.getReverseGeocodeProvider(rgpName);
                if (rgp == null) {
                    printError("Failover ReverseGeocodeProvider not found ["+xmlFile+"]: " + rgpName);
                    this._setHasParsingErrors(xmlFile);
                } else
                if (rgp.hasFailoverReverseGeocodeProvider()) {
                    printError("Failover ReverseGeocodeProvider recursive definition ["+xmlFile+"]: " + rgpName);
                    this._setHasParsingErrors(xmlFile);
                } else {
                    failoverRGP = rgp;
                }
        ...
        try {
            ...
            MethodAction ma = new MethodAction(rgpClass, String.class, String.class, RTProperties.class); // InvocationTargetException
            rgp = (ReverseGeocodeProvider)ma.invoke(rpName, rpKey, rtProps); // ClassCastException
            // -- economy RGP
            if (economyRGP != null) {
                Print.logDebug("["+rgp.getName()+"] Setting economy RGP: " + economyRGP.getName());
                rgp.setEconomyReverseGeocodeProvider(economyRGP);
            }
            // -- failover RGP
            if (failoverRGP != null) {
            ...
            return;
        } catch (Throwable t) { // ClassNotFoundException, ClassCastException, InvocationTargetException, etc.
            if (active) {
        ...
        /* save in PrivateLabel */
        //Print.logInfo("["+pl.getName()+":"+pl.getPageTitle()+"] Adding ReverseGeocodeProvider: " + rgp.getName() + " ("+(active?"active":"inactive")+")");
    ...
    **/
    ...
    /**
    *** [OBSOLETE - do not use] Parse 'EventNotificationEMail' tag
    ...
    **/
    public static BasicPrivateLabel getPrivateLabelForURL(URL url)
    {
        ...
        if (url != null) {
            ...
            /* host as-is (ie. "track.example.com") */
            ...
            int x = hostName.indexOf("."); // found a dot
            if ((x >= 0) && (x < hostName.lastIndexOf("."))) { // but it is not the last dot
