    ...
    extends DeviceRecord<PendingPacket>
{
        ...
        throws DBException
    {
        ...
        try {
            ...
            //pp = (PendingPacket[])DBRecord.select(DBReadWriteMode.READ_ONLY, PendingPacket.getFactory(), dsel.toString(false));
            pp = DBRecord.select(DBReadWriteMode.READ_ONLY, dsel); // select:DBSelect
        } finally {
    ...
    public static void main(String args[])
    {
        ...
        try {
            account = Account.getAccount(DBReadWriteMode.READ_ONLY, acctID); // may return DBException
            if (account == null) {
        ...
        Device device = null;
        try {
            device = Device.getDevice(DBReadWriteMode.READ_ONLY, account, devID, false);
            if (device == null) {
