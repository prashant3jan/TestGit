    ...
    extends DeviceRecord<Property>
{
        ...
        throws DBException
    {
        ...
        try {
            ...
            //p = (Property[])DBRecord.select(DBReadWriteMode.READ_ONLY, Property.getFactory(), dsel.toString(false));
            p = DBRecord.select(DBReadWriteMode.READ_ONLY, dsel); // select:DBSelect
        } finally {
    ...
    public static void main(String args[])
    {
        ...
        try {
            account = Account.getAccount(rwMode, accountID); // may return DBException
            if (account == null) {
        ...
        try {
            if ((deviceID != null) && !deviceID.equals("")) {
                ...
                deviceList = Device.getDeviceIDsForAccount(rwMode, accountID, null, true);
            }
        } catch (DBException dbe) {
        ...
        if (RTConfig.hasProperty(ARG_LIST)) {
            ...
            for (int d = 0; d < deviceList.size(); d++) {
                ...
                try {
                    Device device = Device.getDevice(rwMode, account, devId); // null if non-existent
                    if (device != null) {
        ...
        if (RTConfig.hasProperty(ARG_DEMO)) {
            ...
            try {
                ...
                Property record = key._getDBRecord();
