...
public class DCServerFactory
{
    ...
    **/
    ...
    {
        if (DCServerFactory.IsBlankPrefix(pfx)) {
            // -- no prefix
            return mobileID;
        } else
        if (pfx.equalsIgnoreCase(PREFIX_DEVICE)) {
            // -- this should never occur (it shold be checked before we get to this point)
            return mobileID;
        } else {
            // -- prepend prefix
            return pfx + mobileID;
  ...
  //private static final String ATTR_portOffset         = "portOffset";
            ...
            if (dcsNodeName.equalsIgnoreCase(TAG_DCServer)) {
                ...
                for (int c = 0; c < childNodes.getLength(); c++) {
                    ...
                    if (nodeName.equalsIgnoreCase(TAG_UniqueIDPrefix)) {
                        ...
                        String uniqKey[] = DCServerFactory.CONFIG_uniquePrefix(dcsName);
                        if (RTConfig.hasProperty(uniqKey)) {
                            uidPfx = RTConfig.getString(uniqKey, uidPfx);
                        }
                    ...
                    if (nodeName.equalsIgnoreCase(TAG_ListenPorts)) {
                        ...
                        dcs.setTcpPorts(bindAddr,ParsePorts(dcsName,"TCP",XMLTools.getAttribute(dcsElem,ATTR_tcpPort,null,false)),true);
                        dcs.setUdpPorts(bindAddr,ParsePorts(dcsName,"UDP",XMLTools.getAttribute(dcsElem,ATTR_udpPort,null,false)),true);
                        dcs.setSatPorts(bindAddr,ParsePorts(dcsName,"TCP",XMLTools.getAttribute(dcsElem,ATTR_satPort,null,false)),true);
                        continue;
                    }
    ...
    **/
    private static int[] ParsePorts(String dcsName, String type, String portStr)
    {
            ...
            if (!ListTools.isEmpty(portInt)) {
                for (int i = 0; i < portInt.length; i++) { 
                    if (portInt[i] > 0) { 
                        ...
                        Print.logError("["+dcsName+"] Invalid "+type+" port specification: "+portStr);
                        return null;
                    }
                }
                return portInt;
    ...
    **/
        ...
        String... uniqPfx)
    {
        ...
        DCServerConfig dcs = new DCServerConfig(
            ...
            flags, uniqPfx);
        return DCServerFactory._addDCS(dcs);
    ...
    *** Adds the named server to the list of registered servers
    **/
    public static DCServerConfig _addDCS(DCServerConfig dcs)
    {
    ...
    **/
        ...
        String cmdType, String cmdName, String cmdArgs[])
    {
        ...
        if (!ListTools.isEmpty(cmdArgs)) {
            // -- examine all cmdArgs elements, ignore null [2.6.7-B09e]
            for (int i = 0; (i < cmdArgs.length)/*&&(cmdArgs[i]!=null)*/; i++) { // [2.6.7-B09e]
                if (cmdArgs[i] != null) { // ignore null values [2.6.7-B09e]
                    String argKey = CMDARG_ARG + i; // ie "arg0", "arg1", ...
                    String argVal = cmdArgs[i];     // non-null here, but may be blank
    ...
    /**
    *** Send an internal command request to the command port for the specified server
    **/
    public static RTProperties sendInternalServerCommand(
        String serverName,
        Device device,
        String cmdName, String cmdArgs[])
    {
        String cmdType = DCServerConfig.COMMAND_INTERNAL;
        return DCServerFactory._sendServerCommand(
            serverName,
            device,
    ...
    **/
    public static Device loadDeviceByAccountDeviceID(String accountID, String deviceID)
    {
        ...
        try {
            ...
            /* get Device */
            Device device = Device.getDevice(rwMode, account, deviceID); // null if non-existent
            if (device == null) {
    ...
    *** @return The Device record
    **/
    public static Device _loadDeviceByPrefixedModemID_throw(
        String prefixArray[], String modemID, 
        boolean saveUnassigned, String serverID, String ipAddress, boolean isDuplex, GeoPoint geoPoint)
        throws DBException, Throwable
    {
        // -- READ_WRITE here because it is possible that a DCS module may receive two events
        ...
        /* empty prefix list? */
        String _prefix[] = !ListTools.isEmpty(prefixArray)? prefixArray : DEFAULT_PREFIX_LIST;
        // -- _prefix list is non-empty here

        /* find Device */
        Device  device         = null;
        String  uniqueID       = null;
        boolean loadByUniqueID = Device.GetEnabledLoadByUniqueID();
        boolean loadByDeviceID = Device.GetEnabledLoadByDeviceID();
        String  loadedBy       = null;
        try {

            /* load device record */
            // -- load by UniqueID
            if (loadByUniqueID && (device == null)) {
                for (int u = 0; u < _prefix.length; u++) {
                    if (_prefix[u].equalsIgnoreCase(PREFIX_DEVICE)) {
                        // -- enable DeviceID lookup below
                        loadByDeviceID = true;
                        //break; // skip remaining prefixes?
                    } else {
                        // -- load by UniqueID
                        String pfxID = DCServerFactory.CreateUniqueID(_prefix[u], modemID); // [2.6.2-B19]
                        ...
                        if (uniqueID == null) {
                            // -- default to first valid unique-id
                            uniqueID = pfxID;
                        }
                    }
                }
            ...
            if (loadByDeviceID && (device == null)) {
                for (int u = 0; u < _prefix.length; u++) {
                    if (_prefix[u].equalsIgnoreCase(PREFIX_DEVICE)) {
                        ...
                        // -- load by DeviceID
                        String pfxID = DCServerFactory.CreateUniqueID(_prefix[u], modemID); // [2.6.2-B19]
                        device = Device.loadDeviceByDeviceID(pfxID);
                        if (device != null) {
                            uniqueID = pfxID;
                            loadedBy = "DeviceID";
                            break;
            ...
            /* make sure "uniqueID" is defined */
            if (StringTools.isBlank(uniqueID)) {
                // -- Error condition: 
                // -    1) "loadByUniqueID" and "loadByDeviceID" are both false, or
                // -    2) There are no valid/usable prefixes
                Print.logError("*** Device lookup appears to be disabled! ***");
                uniqueID = modemID;
            }
                ...
                if ((acct != null) && acct.okToAutoAddDevice()) { // getAutoAddDevices()
                    ...
                    try {
                        ...
                        if (Device.GetEnforceUniqueDeviceID()) {
                            // -- check for this "deviceID" owned by any other Account
                            java.util.List<String> acctList = Account.getAccountIDsForDeviceID(rwMode, deviceID);
                            if (!ListTools.isEmpty(acctList)) {
                                throw new DBAlreadyExistsException("DeviceID already exists: "+deviceID+" ["+StringTools.join(acctList,",")+"]");
                            }
                        }
                        String deviceDesc = uniqueID;
                        Device dev = Device.getDevice(rwMode, acct, deviceID, true); // create (does not return null)
                        dev.setUniqueID(uniqueID);
                        dev.setDescription(acct.getNewDeviceDescription(null,deviceDesc));
                        dev.save();
            ...
            /* still not found? */ 
            if (device == null) {
                Print.logWarn("!!!UniqueID not found!: " + uniqueID + " [" + StringTools.join(_prefix,",")+ "]");
                if (saveUnassigned) {
            ...
            String  acctID = device.getAccountID();
            String  devID  = device.getDeviceID();
            String  uniqID = device.getUniqueID(); // same as "uniqueID"?
            if (acct == null) {
                Print.logWarn("Account not found: " + acctID + "/" + devID + " [" + uniqID + "]");
                return null;
            } else
            if (!acct.isActive()) {
                Print.logWarn("Account is inactive: " + acctID + "/" + devID + " [" + uniqID + "]");
                return null;
            } else
            if (acct.isExpired()) {
                Print.logWarn("Account is expired: " + acctID + "/" + devID + " [" + uniqID + "]");
                return null;
            } else
            if (!device.isDCSActive()) {
                Print.logWarn("Account/Device is inactive: " + acctID + "/" + devID + " [" + uniqID + "]");
                return null;
            }
    ...
    **/
    private static void _lookupUniqueID(java.util.List<Device> devList, 
        DCServerConfig dcs,
        String mobileID, Set<String> uidTried)
    {
        ...
        for (String pfx : uidPfx) {
            if (pfx.equalsIgnoreCase(PREFIX_DEVICE)) {
                ...
                String uid = DCServerFactory.CreateUniqueID(pfx, mobileID); // [2.6.6-B58q]
    ...
    **/
    ...
    public static final String  CFG_gpioInputBitCount           = ".gpioInputBitCount";          // long
    public static final String  CFG_simulateDigitalInputs       = ".simulateDigitalInputs";      // long
    ...
    public static final String  CFG_analogDefaultGain           = ".analogDefaultGain";          // EventDataAnalog
    public static final String  CFG_analogDefaultOffset         = ".analogDefaultOffset";        // EventDataAnalog
    ...
    **/
    ...
    {
        // -- DCServer.DCSNAME.uniquePrefix=a_,b_,c_
        // -- DCServer.DCSNAME.uniqueIdPrefix=a_,b_,c_
        return new String[] { 
    ...
    **/
    ...
    {
        // -- DCServer.DCSNAME.minimumMovedMeters=50
        return new String[] { 
    ...
    **/
    ...
    {
        // -- DCServer.DCSNAME.minimumSpeedKPH=3.0
        return new String[] { 
    ...
    **/
    ...
    {
        // -- DCServer.DCSNAME.estimateOdometer=true
        return new String[] { 
    ...
    **/
    ...
    {
        // -- DCServer.DCSNAME.simulateGeozones=true
        return new String[] { 
    ...
    **/
    ...
    {
        // -- DCServer.DCSNAME.maximumHDOP=20.0
        return new String[] { 
    ...
    **/
    ...
    {
        // -- DCServer.DCSNAME.maximumAccuracyMeters=50.0
        return new String[] { 
    ...
    **/
    ...
    /**
    *** Return an array of "Analog Default Gain" property names
    *** @param name  The server name
    *** @return An array of "Analog Default Gain" names
    **/
    public static String[] CONFIG_analogDefaultGain(String name)
    {
        return new String[] { 
            PROP_DCServer_ + name + CFG_analogDefaultGain,
            name + CFG_analogDefaultGain
        };
    }
    /**
    *** Return an array of "Analog Default Offset" property names
    *** @param name  The server name
    *** @return An array of "Analog Default Offset" names
    **/
    public static String[] CONFIG_analogDefaultOffset(String name)
    {
        return new String[] { 
            PROP_DCServer_ + name + CFG_analogDefaultOffset,
            name + CFG_analogDefaultOffset
        };
    }

    /**
    *** Return an array of "GPIO input bit count" property names
    *** @param name  The server name
    *** @return An array of "GPIO input bit count" names
    **/
    public static String[] CONFIG_gpioInputBitCount(String name)
    {
        return new String[] { 
            PROP_DCServer_ + name + CFG_gpioInputBitCount,
            name + CFG_gpioInputBitCount
        };
    }
    ...
    **/
    ...
    {
        // -- DCServer.DCSNAME.statusLocationInMotion=true
        return new String[] { 
    ...
    **/
    ...
    {
        // -- DCServer.DCSNAME.ignoreEventsWithInvalidGPS=true
        return new String[] { 
    ...
    **/
    ...
    // ------------------------------------------------------------------------
    // ------------------------------------------------------------------------
    // Log file timestamp gap analysis
    
    private static double SQ(double X) { return X*X; }
    
    private static class LogFileTimeGap
    {
        // -- this File
        private File            logFile = null;
        private long            logTS   = -1L;
        // -- last LogFileTimeGap
        private LogFileTimeGap  lastLFG = null;
        private long            lastGap = -1L;
        // -- standard deviation
        private double          stdDev  = 0.0;
        // --
        public LogFileTimeGap(File logF, TimeZone tmz) {
            // -- assumptions:
            // -    - 'logF' is not null
            // -    - 'logF.getName()' has the format 'LOGFILE.log.20YYMMDDhhmmss.log'
            this.logFile = logF; // should not be null
            // -- parse timestamp
            String fn = this.logFile.getName();
            int s = fn.indexOf(".20");
            int e = fn.endsWith(".log")? (fn.length() - ".log".length()) : fn.length();
            String dtS = (s >= 0)? fn.substring(s+1,e) : "";
            if ((dtS.length() == 14) && dtS.startsWith("20")) {
                int YYYY = StringTools.parseInt(dtS.substring( 0, 4), -1);
                int MM   = StringTools.parseInt(dtS.substring( 4, 6), -1);
                int DD   = StringTools.parseInt(dtS.substring( 6, 8), -1);
                int hh   = StringTools.parseInt(dtS.substring( 8,10), -1);
                int mm   = StringTools.parseInt(dtS.substring(10,12), -1);
                int ss   = StringTools.parseInt(dtS.substring(12,14), -1);
                if ((YYYY >= 2000) && (MM >= 1) && (DD >= 1) &&
                    (hh   >=    0) && (mm >= 0) && (ss >= 0)   ) {
                    this.logTS = (new DateTime(tmz, YYYY, MM, DD, hh, mm, ss)).getTimeSec();
                } else {
                    Print.logWarn("File name ["+fn+"] contains invalid timestamp: " + dtS);
                }
            } else {
                Print.logWarn("File name ["+fn+"] does not contain timestamp: " + dtS);
            }
        }
        public void setLastLogFileTimeGap(LogFileTimeGap last) {
            if (last != null) {
                this.lastLFG = last;
                // -- gap
                if ((this.logTS > 0L) && (last.logTS > 0L) && (this.logTS > last.logTS)) {
                    this.lastGap = this.logTS - this.lastLFG.logTS;
                }
            } else {
                this.lastLFG = null;
                this.lastGap = -1L;
        ...
        public LogFileTimeGap getLastLogFileTimeGap() {
            return this.lastLFG;
        }
        public long getLastGap() {
            return this.lastGap;
        }
        public void setLastGapDeviation(double stdD) {
            this.stdDev = stdD;
        }
        public double getLastGapDeviation(double stdD) {
            return this.stdDev;
        }
        public String toString() {
            StringBuffer sb = new StringBuffer();
            sb.append("File="+this.logFile);
            sb.append(" ["+this.logTS+"]");
            sb.append(" LastGAP="+this.lastGap);
            return sb.toString();
        }
    }
    ...
    private static final String ARG_LOGFILEGAP[]        = new String[] { "logFileGap"   , "logGap"     };
    private static final String ARG_TIMEZONE[]          = new String[] { "timeZone"     , "tmz"        };
    ...
    **/
    public static void main(String args[])
    {
        ...
        /* log file gap */
        // -- may be used for searching archived log files to find excess gaps in the log file timestamps
        // -  this may indicate an issue dcs within the log files surrounding the gap
        if (RTConfig.hasProperty(ARG_LOGFILEGAP)) {
            double X = RTConfig.getDouble(ARG_LOGFILEGAP,1.0);
            TimeZone tmz = DateTime.getTimeZone(RTConfig.getString(ARG_TIMEZONE,null),null);
            String dcsName = !StringTools.isBlank(server)? server : "";
            File dcsLogFiles[] = DCServerConfig.getArchivedLogFiles(dcsName);
            // -- convert to LogFileTimeGap objects and get average gap
            double gapMean;
            Vector<LogFileTimeGap> lfgList = new Vector<LogFileTimeGap>();
            {
                long gapTotal = 0L;
                int gapCount = 0;
                LogFileTimeGap lastLFG = null;
                for (File F : dcsLogFiles) {
                    LogFileTimeGap lfg = new LogFileTimeGap(F,tmz);
                    if (lfg.getTimestamp() <= 0L) {
                        Print.logInfo("No timestamp, skipping: " + F);
                        continue;
                    }
                    lfg.setLastLogFileTimeGap(lastLFG);
                    lfgList.add(lfg);
                    long lastGap = lfg.getLastGap();
                    if (lastGap >= 0L) {
                        gapTotal += lastGap;
                        gapCount++;
                    }
                ...
                gapMean = (double)gapTotal / (double)gapCount;
            }
            // -- calculate standard-deviation 
            double stdDev;
            {
                double gapMeanDeltaTotal = 0.0;
                int gapMeanDeltaCount = 0;
                for (LogFileTimeGap LFG : lfgList) {
                    long lastGap = LFG.getLastGap();
                    if (lastGap < 0L) {
                        continue; // first entry will not have a gap
                    }
                    gapMeanDeltaTotal += SQ((double)lastGap - gapMean);
                    gapMeanDeltaCount++;
                }
                stdDev = Math.sqrt(gapMeanDeltaTotal / (double)gapMeanDeltaCount);
            }
            // -- list files outside X standard-deviations
            Print.logInfo("Mean="+StringTools.format(gapMean,"0") + " Deviation="+StringTools.format(stdDev,"0"));
            double stdDevX = stdDev * X;
            Print.logInfo("Log file time gaps exceeding " + X + " standard deviations from mean");
            for (LogFileTimeGap LFG : lfgList) {
                long lastGap = LFG.getLastGap();
                if (lastGap < 0L) {
                    continue;
                }
                double gapDelta = (double)lastGap - gapMean;
                if (Math.abs(gapDelta) >= stdDevX) {
                    LogFileTimeGap lastLFG = LFG.getLastLogFileTimeGap();
                    File lastLogFile = lastLFG.getLogFile();
                    File thisLogFile = LFG.getLogFile();
                    Print.logInfo("----------------------------");
                    Print.logInfo("File      : " + lastLogFile);
                    Print.logInfo("File      : " + thisLogFile);
                    if (gapDelta >= 0.0) {
                        // -- greater than mean/average
                        // -  (DCS stopped receiving events, etc)
                        Print.logInfo("Gap(>mean): " + lastGap);
                    } else {
                        // -- greater than mean/average
                        // -  (DCS has encountered excess errors, etc)
                        Print.logInfo("Gap(<mean): " + lastGap);
                    }
                }
            }
        ...
        if (RTConfig.hasProperty(ARG_LOOKUP)) {
            ...
            if (!StringTools.isBlank(uid)) {
                ...
                Print.sysPrintln("");
                Print.sysPrintln("Device UniqueID Lookup: " + uid);
        ...
        try {
            account = Account.getAccount(DBReadWriteMode.READ_ONLY, accountID); // may throw DBException
            if (account == null) {
        ...
        Device device = null;
        try {
            device = Device.getDevice(DBReadWriteMode.READ_ONLY, account, deviceID, false); // may throw DBException
            if (device == null) {
