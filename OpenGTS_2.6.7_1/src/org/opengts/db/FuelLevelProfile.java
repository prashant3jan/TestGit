...
public class FuelLevelProfile
{
    ...
    public static boolean GetUseCurveFit()
    {
        return FuelLevelProfile.USE_CURVEFIT;
    }

    public static void SetUseCurveFit(boolean useCF)
    {
        FuelLevelProfile.USE_CURVEFIT = useCF;
    }

    static {
        USE_CURVEFIT = RTConfig.getBoolean(PROP_FuelLevelProfile_enableCurveFit, USE_CURVEFIT);
    };
    ...
    **/
        ...
        extends XYPair
    {
        ...
        public ProfileItem(double evLvl, double acLvl) {
            super(evLvl, acLvl); // XYPair(X,Y)
            // -- check for valid range
            this._validateRange();
        }
        public ProfileItem(XYPair xy) {
            this(((xy!=null)?xy.getX():0.0), ((xy!=null)?xy.getY():0.0)); // XYPair(X,Y)
        }
    ...
    /**
    *** Converts the XYPair array into a ProfileItem array
    **/
    public static XYPair[] toProfileItemArray(XYPair xy[])
    {
        if (xy == null) {
            return new ProfileItem[0];
        } else
        if (xy instanceof ProfileItem[]) {
            return xy;
        } else {
            ProfileItem pi[] = new ProfileItem[xy.length];
            for (int i = 0; i < xy.length; i++) {
                pi[i] = new ProfileItem(xy[i]);
            }
            return pi;
        }
    }
    ...
    private boolean     useCurveFit = false;
    ...
    **/
    public FuelLevelProfile(String profile, I18N.Text desc)
    {
        ...
        /* profile */
        this.useCurveFit = FuelLevelProfile.GetUseCurveFit();
    ...
    **/
    ...
    /**
    *** Sets the profile scale/offset (performed on event data value X)
    **/
    private void setScale(double divScale, double preOfs)
    {
        this.profScale  = (divScale <= 0.0)? 1.0 : divScale;
        this.profOffset = preOfs;
    }

    /**
    *** Sets the profile scale (divide)
    **/
    private void setScale(double divScale)
    {
        this.setScale(divScale, 0.0); // no offset
    }
    
    /**
    *** Gets the profile scale (divide)
    **/
    ...
    **/
    private void setProfile(ProfileItem flp[])
    {
        ...
        if (this.curveFit != null) {
            //Print.logInfo("Using CurveFit ...");
        } else {
            //Print.logInfo("NOT Using CurveFit ...");
        }
    }
    ...
    **/
    ...
    {
        return this.useCurveFit? this.curveFit : null; // may be null
    }
    ...
    **/
    public String toString(String xFmt, String yFmt)
    {
        ...
        if (!ListTools.isEmpty(pi)) {
            sb.append(":");
            XYPair.ToListString(pi, false, xFmt, yFmt, sb);
        }
        return sb.toString();
    }
