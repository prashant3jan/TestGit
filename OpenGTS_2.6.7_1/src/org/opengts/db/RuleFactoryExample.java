    ...
    **/
    ...
    public int executeSelector(String selector, int selActionMask, EventData event) 
    {
        if (this.isSelectorMatch(selector,event)) {
            ...
            int actionMask = (selActionMask > 0)? selActionMask : RuleFactory.ACTION_DEFAULT;
    ...
    public int executeRules(EventData event) 
    {
        ...
        // IE. loop through other selectors: this.executeSelector(ruleSel[i], -1, event);
    ...
    protected static boolean _sendNotification(EventData evdb, int actionMask)
    {
        ...
        */
        ...
        if ((actionMask & RuleFactory.ACTION_VIA_NOTIFY_QUEUE) != 0) {
        ...
        /* message-queue (MQ) */
        if ((actionMask & RuleFactory.ACTION_VIA_MQ) != 0) {
            // TODO: message-queue (MQTT)
        }
