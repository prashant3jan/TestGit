...
public class EventUtil
{
  ...
  //public  static final String  CSV_SEPARATOR                  = "|";
        ...
        boolean isFleet, Locale locale, char csvSep)
    {
        ...
        js.append("    this.routeC    = (fld.length > 8)? decodeUnicode(fld[ 8]) : '';\n"); // RouteSegmentColor
        js.append("    this.isCellLoc = false;\n");                                         // cell-tower location?
        js.append("    this.latitude  = numParseFloat(((fld.length >  9)? fld[ 9] : '0'), 0);\n");
        js.append("    this.longitude = numParseFloat(((fld.length > 10)? fld[10] : '0'), 0);\n");
        js.append("    this.gpsAge    = numParseFloat(((fld.length > 11)? fld[11] : '0'), 0);\n");
        js.append("    this.createAge = numParseFloat(((fld.length > 12)? fld[12] : '0'), 0);\n");
        js.append("    this.accuracy  = numParseFloat(((fld.length > 13)? fld[13] : '0'), 0);\n");
        js.append("    if (this.accuracy < 0) { this.accuracy = 0; }\n");
        js.append("    this.validGPS  = ((this.latitude != 0) || (this.longitude != 0))? true : false;\n");
        js.append("    this.satCount  = (fld.length > 14)? fld[14] : '0';\n");              // satellite count
        js.append("    if (this.satCount < 0) { this.isCellLoc = true; this.satCount = 0; }\n");
        js.append("    this.speedKPH  = numParseFloat(((fld.length > 15)? fld[15] : '0'), 0);\n");
        js.append("    this.speedMPH  = this.speedKPH * " + GeoPoint.MILES_PER_KILOMETER + ";\n");
        js.append("    this.heading   = numParseFloat(((fld.length > 16)? fld[16] : '0'), 0);\n");
        js.append("    this.compass   = HEADING[Math.round(this.heading / 45.0) % 8];\n");
        js.append("    this.altitude  = numParseFloat(((fld.length > 17)? fld[17] : '0'), 0); // meters\n");
        js.append("    this.odomKM    = numParseFloat(((fld.length > 18)? fld[18] : '0'), 0);\n");  // km
        js.append("    this.stopped   = (fld.length > 19)? parseInt(fld[19]) : 0;\n");      // enum (stoppedState)
        js.append("    this.stopSec   = 0;\n");      // seconds (will be set later for this.stopped==2)
        js.append("    this.gpioInput = numParseInt(((fld.length > 20)? fld[20] : '0'), 0);\n");
        js.append("    this.address   = (fld.length > 21)? decodeUnicode(fld[21].trim()) : '';\n");
      ...
      //js.append("    //if (this.address == '') { this.address = '&nbsp;'; }\n"); // fill with space, so field isn't blank
        js.append("    if (fld.length > 22) {\n");
        js.append("        this.optDesc = new Array();\n");
        js.append("        for (var i = 22; i < fld.length; i++) {\n");
        ...
        if ((optEvFlds != null) && (optFieldCount > 0)) {
            ...
            for (int i = 0; i < optFieldCount; i++) {
                String t = StringTools.trim(optEvFlds.getOptionalEventFieldTitle(i, isFleet, locale));
                t = t.replace('\n',' ');
                t = StringTools.stripChars(t,"\t\r");
    ...
    {
        // ID   |DeviceDesc|Epoch     |Date      |Time    |Timezone|StatusCodeDesc|Icon|RouteC|Latitude |Longitude  |GPSAge|CreateAge|Accuracy|#Sats|SpeedKPH|Heading|Altitude|Odometer|Stopped|GPIO|Address           |extra...
        // demo2|New Device|1268394640|2010/03/12|05:50:40|CST     |InMotion      |19  |green |37.785248|-121.307275|0     |0        |0.0     |0    |87.2    |227.7  |7       |16124.9 |0      |1234|"I-5, Lathrop, CA"
        ...
        /* fld[ 0] VehicleID (VIN,ShortName,DeviceID) (unicode encoded) */
        sb.append(EventUtil._encUnicode(edp.getDeviceVID(),csvSep)); // edp.getLabelPushpinName()
        sb.append(csvSep);
        ...
        /* fld[ 8] RouteSegmentColor ("red"|"green"|"#FFEEEE"|etc) */
        ColorTools.RGB routeC = ColorTools.parseColor(edp.getRouteSegmentColor(isFleet,privLabel),(ColorTools.RGB)null);
        if (routeC != null) {
            sb.append(routeC.toString(true)); // "red"|"green"|"#FFEEEE"|""|etc
        } else {
            sb.append(""); // no-color
        }
        sb.append(csvSep);

        /* fld[ 9] Latitude  (6 decimal places) */
        /* fld[10] Longitude (6 decimal places) */
        ...
        /* B fld[12] Creation Age */
        long createAge = edp.getCreationAge();
        sb.append(createAge);
        sb.append(csvSep);

        /* C fld[13] Accuracy (meters) */
        ...
        throws IOException
    {
        ...
        for (int i = 0; i < poip.length; i++) {
            ...
            EventDataProvider edp = new EventDataProviderAdapter() {
                ...
                public String getRouteSegmentColor(boolean isFleet, BasicPrivateLabel bpl) { return ""; } // RouteSegmentColor
            };
        ...
        char csvSep)
    {
        ...
        if ((edp != null) && (acct != null)) {
            ...
            String nearbyGzRadM = StringTools.trim(privLabel.getStringProperty(BasicPrivateLabel.PROP_TrackMap_showNearbyGeozones,""));
            double MAX_NEARBY_GEOZONE_RADIUS = 12000.0;
            double NEARBY_GEOZONE_RADIUS = 0.0; // was GET_NEARBY_GEOZONES
            if (StringTools.isBlank(nearbyGzRadM) || nearbyGzRadM.equalsIgnoreCase("false")) {
                NEARBY_GEOZONE_RADIUS = 0.0; // disabled
            } else
            if (nearbyGzRadM.equalsIgnoreCase("true")) {
                NEARBY_GEOZONE_RADIUS = 1000.0; // default 1000 meters
            } else {
                ...
                if (NEARBY_GEOZONE_RADIUS > MAX_NEARBY_GEOZONE_RADIUS) { 
                    NEARBY_GEOZONE_RADIUS = MAX_NEARBY_GEOZONE_RADIUS;  // max radius
                }
            }
            boolean GET_ALL_CONTAINED_GEOZONES = privLabel.getBooleanProperty(BasicPrivateLabel.PROP_TrackMap_showAllContainedGeozones, true);
            if (NEARBY_GEOZONE_RADIUS > 0.0) {
  ...
  //public  static final String  ATTR_timezone                  = "timezone";
    ...
    public boolean writeEvents_JSON(PrintWriter pwout, 
        Account account, Collection<Device> devList, 
        boolean allTags, boolean inclMaint,
        TimeZone dispTmz,
        BasicPrivateLabel privLabel)
        throws IOException
    {
        ...
        /* header */
        Print.logInfo("Creating JSON Object output ...");
        JSON._Object jsonObj = new JSON._Object();
        jsonObj.addKeyValue("Account"     , accountID);
        jsonObj.addKeyValue("Account_desc", acctDesc);
        jsonObj.addKeyValue("TimeZone"    , tzStr);

        // -- iterate through devices
        JSON._Array jsonDevList = new JSON._Array();
        jsonObj.addKeyValue("DeviceList"  , jsonDevList);
        // --
        if (!ListTools.isEmpty(devList)) {
            ...
            for (Device dev : devList) {
                ...
                /* Device header */
                JSON._Object jsonDev = new JSON._Object();
                jsonDevList.addValue(jsonDev);
                jsonDev.addKeyValue("Device"     , deviceID);
                jsonDev.addKeyValue("Device_desc", deviceDesc);

                /* event data */
                EventData evList[] = dev.getSavedRangeEvents();
                if (evList != null) {
                    JSON._Array jsonEventList = new JSON._Array();
                    jsonDev.addKeyValue("EventData",jsonEventList);
                    ...
                    for (int e = 0; e <= lastEventNdx; e++) {
                        ...
                        ev.setAccount(account); // redundant
                        // -- EventData contents
                        JSON._Object jsonEvent = this.writeEvents_JSON_Event(pwout, e, ev, isLastEvent, 4, allTags, privLabel);
                        // -- [JSON]
                        jsonEventList.addValue(jsonEvent);
                    }
                }
                ...
                // --           "Index"       : 0,
                // --           "Label"       : "OilChange",
                // --           "LastService" : 100000.0,
                // --           "Interval"    : 1000.0,
                // --           "NextService" : 101000.0,
                // --           "Current"     : 100500.0
                // --       },
                // --       ...
                // --    ],
                ...
                if (inclMaint) {
                    // -- begin "Maintenance"
                    JSON._Object jsonMaint = new JSON._Object();
                    jsonDev.addKeyValue("Maintenance", jsonMaint);
                    // -- Odometer
                    if (Device.supportsMaintenance_KM(0)) {
                        int cnt = Device.getMaintCountKM();
                        JSON._Array jsonMaintOdom = new JSON._Array();
                        jsonMaint.addKeyValue("Odometer",jsonMaintOdom);
                        for (int i = 0; i < cnt; i++) {
                            String label       = dev.getMaintLabelKM(i);
                            double lastService = dev.getMaintOdometerKM(i);
                            double interval    = dev.getMaintIntervalKM(i);
                            double nextService = lastService + interval;
                            double current     = dev.getLastOdometerKM();
                            double remaining   = dev.getMaintRemainingKM(i); // <=0 if past due
                            JSON._Object jsonItem = new JSON._Object();
                            jsonMaintOdom.addValue(jsonItem);
                            jsonItem.addKeyValue("Index"      , i);
                            jsonItem.addKeyValue("Units"      , "KILOMETERS");
                            jsonItem.addKeyValue("LastService", lastService, "0.0");
                            jsonItem.addKeyValue("Interval"   , interval   , "0.0");
                            jsonItem.addKeyValue("NextService", nextService, "0.0");
                            jsonItem.addKeyValue("Current"    , current    , "0.0");
                            jsonItem.addKeyValue("Remaining"  , remaining  , "0.0");
                        }
                        //this.writeKeyValue_JSON(pwout,PFX4,"]", null, false);
                    }
                    // -- Hours
                    if (Device.supportsMaintenance_HR(0)) {
                        int cnt = Device.getMaintCountHR();
                        JSON._Array jsonMaintHour = new JSON._Array();
                        jsonMaint.addKeyValue("Hours",jsonMaintHour);
                        for (int i = 0; i < cnt; i++) {
                            String label       = dev.getMaintLabelHR(i);
                            double lastService = dev.getMaintEngHoursHR(i);
                            double interval    = dev.getMaintIntervalHR(i);
                            double nextService = lastService + interval;
                            double current     = dev.getLastEngineHours();
                            double remaining   = dev.getMaintRemainingHR(i); // <=0 if past due
                            JSON._Object jsonItem = new JSON._Object();
                            jsonMaintHour.addValue(jsonItem);
                            jsonItem.addKeyValue("Index"      , i);
                            jsonItem.addKeyValue("Units"      , "HOURS");
                            jsonItem.addKeyValue("LastService", lastService, "0.0");
                            jsonItem.addKeyValue("Interval"   , interval   , "0.0");
                            jsonItem.addKeyValue("NextService", nextService, "0.0");
                            jsonItem.addKeyValue("Current"    , current    , "0.0");
                    ...
                    // -- FixedTime
                    if (Device.supportsMaintenance_FT(0)) {
                        int cnt = Device.getMaintCountFT();
                        JSON._Array jsonMaintFixT = new JSON._Array();
                        jsonMaint.addKeyValue("FixedTime",jsonMaintFixT);
                        for (int i = 0; i < cnt; i++) {
                            Device.MaintUnitsFT units = dev.getMaintUnitsFT(i);
                            String label       = dev.getMaintLabelFT(i);
                            double lastService = dev.getMaintFixedTimeFT(i); // EpochTime/DayNumber
                            double interval    = dev.getMaintIntervalFT(i);  // Seconds/Days/Months
                            double nextService = dev.getMaintDueFT(i);       // lastService + interval
                            double current     = dev.getMaintCurrentFT(i);
                            double remaining   = dev.getMaintRemainingFT(i); // <=0 if past due
                            JSON._Object jsonItem = new JSON._Object();
                            jsonMaintFixT.addValue(jsonItem);
                            jsonItem.addKeyValue("Index"      , i);
                            jsonItem.addKeyValue("Units"      , units.toString().toUpperCase());
                            jsonItem.addKeyValue("LastService", lastService, units.getFormat());
                            jsonItem.addKeyValue("Interval"   , interval   , units.getFormat());
                            jsonItem.addKeyValue("NextService", nextService, units.getFormat());
                            jsonItem.addKeyValue("Current"    , current    , units.getFormat());
                            jsonItem.addKeyValue("Remaining"  , remaining  , units.getFormat());
                        }
                    }
                    // -- Notes
                    jsonMaint.addKeyValue("Notes", dev.getMaintNotes());
                } // include maintenance
        ...
        /* write */
        this.write(pwout, jsonObj.toString());
        this.flush(pwout); // flush (output may not occur this the PrintWriter is flushed)
        return true;
        
    }

    private JSON._Object writeEvents_JSON_Event(PrintWriter pwout, int index, EventData ev, boolean isLast, 
        ...
        throws IOException
    {
        ...
        /* Event tag start */
        JSON._Object jsonEvent = new JSON._Object();

        // -- Device
        jsonEvent.addKeyValue("Device", ev.getDeviceID());
        ...
        if (allTags || (timestamp > 0L)) {
            ...
            jsonEvent.addKeyValue("Timestamp"     ,timestamp                 );
            jsonEvent.addKeyValue("Timestamp_date",ts.format("yyyy/MM/dd",tz));
            jsonEvent.addKeyValue("Timestamp_time",ts.format("HH:mm:ss",tz)  );
        }
        ...
        String desc    = ev.getStatusCodeDescription(privLabel);
        jsonEvent.addKeyValue("StatusCode"     ,statusCode);
        jsonEvent.addKeyValue("StatusCode_hex" ,hexCode   );
        jsonEvent.addKeyValue("StatusCode_desc",desc      );
        ...
        if (allTags || geoPoint.isValid()) {
            ...
            jsonEvent.addKeyValue("GPSPoint"    ,geoPoint.toString(',') );
            jsonEvent.addKeyValue("GPSPoint_lat",geoPoint.getLatitude() , "0.00000");
            jsonEvent.addKeyValue("GPSPoint_lon",geoPoint.getLongitude(), "0.00000");
            // --
            if (gpsAge > 0) {
                jsonEvent.addKeyValue("GPSPoint_age", gpsAge);
            }
            if (accuracy > 0) {
                jsonEvent.addKeyValue("GPSPoint_accuracy", accuracy);
            }
        }
        ...
        if (allTags || (speedKPH >= 0.0)) {
            ...
            double speedLimKPH = ev.getSpeedLimitKPH();
            double limit = (speedLimKPH > 0.0)? speedUnits.convertFromKPH(speedLimKPH) : 0.0;
            jsonEvent.addKeyValue("Speed_kph"  , speedKPH, "0.0");
            jsonEvent.addKeyValue("Speed"      , speed   , "0.0");
            jsonEvent.addKeyValue("Speed_units", units   );
            if (limit > 0.0) { 
                jsonEvent.addKeyValue("Speed_limit", limit, "0.0");
            }
        }
        ...
        if (allTags || (speedKPH > 0.0)) {
            ...
            jsonEvent.addKeyValue("Heading"     , heading    , "0");
            jsonEvent.addKeyValue("Heading_desc", headingDesc);
        }
        ...
        if (allTags || (altitudeM > 0.0)) {
            Account.AltitudeUnits altUnits = Account.getAltitudeUnits(account);
            double alt   = altUnits.convertFromMeters(altitudeM);
            String units = altUnits.toString(locale);
            jsonEvent.addKeyValue("Altitude_meters", altitudeM, "0");
            jsonEvent.addKeyValue("Altitude"       , alt      , "0");
            jsonEvent.addKeyValue("Altitude_units" , units    );
        }
        ...
        if (allTags || (odomKM > 0.0)) {
            ...
            double odometer = distUnits.convertFromKM(odomKM);
            String units    = distUnits.toString(locale);
            jsonEvent.addKeyValue("Odometer_km"   , odomKM  , "0.0");
            jsonEvent.addKeyValue("Odometer"      , odometer, "0.0");
            jsonEvent.addKeyValue("Odometer_units", units);
        }
        ...
        long geozoneNdx  = ev.getGeozoneIndex();
        if (allTags || !geozoneID.equals("") || (geozoneNdx > 0L)) {
            jsonEvent.addKeyValue("Geozone"      , geozoneID);
            jsonEvent.addKeyValue("Geozone_index", geozoneNdx);
        }
        ...
        if (allTags || !address.equals("")) {
            ...
            jsonEvent.addKeyValue("Address", addrStr);
        }
        ...
        if (allTags || !city.equals("")) {
            ...
            jsonEvent.addKeyValue("City", cityStr);
        }

        // -- PostalCode
        String postalCode = ev.getPostalCode();
        if (allTags || !postalCode.equals("")) {
            jsonEvent.addKeyValue("PostalCode", postalCode);
        }
        ...
        if (allTags || (inputMask != 0L)) {
            ...
            jsonEvent.addKeyValue("DigitalInputMask"    , inputMask);
            jsonEvent.addKeyValue("DigitalInputMask_hex", hexInpStr);
        }

        // -- [allTags] DriverID
        if (allTags && EventData.getFactory().hasField(EventData.FLD_driverID)) {
            String driverID = ev.getDriverID();
            jsonEvent.addKeyValue("DriverID", driverID);
       }

        // -- [allTags] DriverMessage
        if (allTags && EventData.getFactory().hasField(EventData.FLD_driverMessage)) {
            String driverMsg = StringTools.replace(ev.getDriverMessage(),"\"","'");
            jsonEvent.addKeyValue("DriverMessage", driverMsg);
        }

        // -- [allTags] EngineRPM
        if (allTags && EventData.getFactory().hasField(EventData.FLD_engineRpm)) {
            long engineRpm = ev.getEngineRpm();
            jsonEvent.addKeyValue("EngineRPM", engineRpm);
        }

        // -- [allTags] EngineHours
        if (allTags && EventData.getFactory().hasField(EventData.FLD_engineHours)) {
            double engineHours = ev.getEngineHours() + device.getEngineHoursOffset();
            jsonEvent.addKeyValue("EngineHours", engineHours, "0.0");
        }

        // -- [allTags] VehicleBatteryVolts
        if (allTags && EventData.getFactory().hasField(EventData.FLD_vBatteryVolts)) {
            double battVolts = ev.getVBatteryVolts();
            jsonEvent.addKeyValue("VehicleBatteryVolts", battVolts, "0.0");
        }
        ...
        if (allTags && EventData.getFactory().hasField(EventData.FLD_coolantLevel)) {
            ...
            jsonEvent.addKeyValue("EngineCoolantLevel"      , pct100, "0");
            jsonEvent.addKeyValue("EngineCoolantLevel_units", units);
        }
        ...
        if (allTags && EventData.getFactory().hasField(EventData.FLD_coolantTemp)) {
            ...
            jsonEvent.addKeyValue("EngineCoolantTemperature_C"    , tempC, "0.0");
            jsonEvent.addKeyValue("EngineCoolantTemperature"      , temp , "0.0");
            jsonEvent.addKeyValue("EngineCoolantTemperature_units", units);
        }
        ...
        if (allTags && EventData.getFactory().hasField(EventData.FLD_fuelTotal)) {
            ...
            jsonEvent.addKeyValue("EngineFuelUsed_Liter", fuelL, "0.0");
            jsonEvent.addKeyValue("EngineFuelUsed"      , fuel , "0.0");
            jsonEvent.addKeyValue("EngineFuelUsed_units", units);
        }

        /* index */
        jsonEvent.addKeyValue("Index", index);
    ...
    public boolean writeEvents(OutputStream out, 
        Account account, Collection<Device> devList,
        int formatEnum, boolean allTags, boolean inclMaint,
        TimeZone tmz,
        BasicPrivateLabel privLabel)
        throws IOException
    {
    ...
    public static void main(String argv[])
    {
        ...
        Account acct = null;
        try {
            acct = Account.getAccount(rwMode, acctID);
            if (acct == null) {
        ...
        boolean inclMaint = true;

        /* get device(s) */
        Device devList[] = null;
        try {
            if (devID.equals("*") || devID.equals("ALL")) {
                OrderedSet<String> devIdList = Device.getDeviceIDsForAccount(rwMode, acctID, null, true);
                if (devIdList.size() <= 0) {
                ...
                for (int i = 0; i < devIdList.size(); i++) {
                    Device dev = Device._getDevice(rwMode, acct, devIdList.get(i));
                    if (dev == null) {
            ...
            if (devID.indexOf(",") > 0) {
                ...
                for (int i = 0; i < devIdList.length; i++) {
                    Device dev = Device._getDevice(rwMode, acct, devIdList[i]);
                    if (dev == null) {
            ...
            if (devID.startsWith("G:")) {
                ...
                OrderedSet<String> devIdList = DeviceGroup.getDeviceIDsForGroup(rwMode,acctID,groupID,null,false/*inclInactv*/,-1L);
                if (devIdList.size() <= 0) {
                ...
                Device dev = Device.getDevice(rwMode, acct, devID); // null if non-existent
                if (dev == null) {
