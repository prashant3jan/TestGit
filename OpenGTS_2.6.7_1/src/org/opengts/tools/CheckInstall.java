...
//     -Added "(RGCache)"
//     -Added chck for MQTT support jar
// ----------------------------------------------------------------------------
...
public class CheckInstall
{
    ...
    private static       boolean SHOW_NON_OWNED_FILES           = false; // showNonOwnedFiles

    private static final long    RECOMMENDED_MEMORY_MB          = 4096L;
    private static final double  THRESHOLD_DISK_UTIL            = 0.90;

    private static final int     MINIMUM_MAX_CONNECTIONS        = 1000;
    private static final int     MINIMUM_OPEN_FILES_LIMIT       = 5000;
    ...
    private static final String ARG_SHOW_NON_OWNED_FILES[]  = new String[] { "showNonOwnedFiles", "nonOwnedFiled"         };
    ...
    public static void main(String argv[])
    {
        ...
        /* check for show non-owned filed */
        SHOW_NON_OWNED_FILES = RTConfig.getBoolean(ARG_SHOW_NON_OWNED_FILES, SHOW_NON_OWNED_FILES);
        ...
        println("System Information:");
        {
            ...
            OSTools.CpuUsage cpuInfo = OSTools.getCpuUsage(null, false);
            if (cpuInfo != null) {
                int cpuCores = cpuInfo.getAvailableProcessors();
                printVariable("(CPU Cores)", (cpuCores>0)?""+cpuCores:"n/a", "");
                double speedMHz = cpuInfo.getCpuSpeedMHz();
                printVariable("(CPU Speed MHz)", (speedMHz>0.0)?StringTools.format(speedMHz,"0.0"):"n/a", "");
                double cpuLoad  = cpuInfo.getSystemCpuLoad();
                printVariable("(CPU Load)", (cpuLoad>0.0)?StringTools.format(cpuLoad*100.0,"0.0")+"%":"n/a", "");
            } else {
                printVariable("(CPU Usage)", "n/a", "");
            }
        ...
        println("Environment variable paths (canonical):");
        {
            ...
            if (!env_CATALINA_HOME.isDirectory()) {
                        ...
                        FileTools.traverseAllFiles(env_CATALINA_HOME, new FileFilter() {
                            public boolean accept(File file) {
                                ...
                                if (!StringTools.isBlank(owner) && !owner.equalsIgnoreCase(ownerGtsHome)) {
                                    if (SHOW_NON_OWNED_FILES) { println(PFX+"File ["+owner+"]: "+file); }
                                    count.increment();
                                }
                                return false;
                            }
                ...
                if (!isWindows && isJava6plus) {
                    /* (Java 7+) not supported on Java 6/5 */
                    final AccumulatorLong accumCantRead  = new AccumulatorLong(0L);
                    final AccumulatorLong accumSymLinks  = new AccumulatorLong(0L);
                    final long            maxListedFiles = 4L;
                    final File            tomcatDir      = env_CATALINA_HOME;
                    try {
                        FileTools.traverseAllFiles(tomcatDir, 
                            // --  file "accept" filter
                            new FileFilter() {
                                public boolean accept(File f) {
                                    if (FileTools.isSymbolicLink(f)) {
                                        accumSymLinks.increment();
                                        println(PFX+"ERROR: Found symbolic link - " + f);
                                    ...
                                    if (!FileTools.canRead(f)) {
                                        ...
                                        return true; // accept and continue
                                    } else {
                                        // -- OK
                                        return true; // accept and continue
                                    }
                            ...
                            // -- directory descent "accept" filter
                            new FileFilter() {
                                public boolean accept(File d) {
                                    if (FileTools.isSymbolicLink(d)) {
                                        // -- reject all directory symbolic links
                                        accumSymLinks.increment();
                                        println(PFX+"ERROR: Ignoring directory symbolic link - " + d);
                                        return false; // ignore this path
                                    } else {
                                        // -- allow descending into directory
                                        return true;
                                    }
                                }
                            }
                    ...
                    // -- sfound symbolic links?
                    if (accumSymLinks.get() > 0L) {
                        int WC = countWarning("Tomcat '$CATALINA_HOME/' directory contains symbolic links.");
                        println(PFX+"WARNING["+WC+"]: Tomcat '$CATALINA_HOME/' directory contains symbolic links.");
                    }
                    // -- found unreadable files?
        ...
        if ((javaExtDirs == null) || (javaExtDirs.length == 0)) {
            ...
            if (isEnterprise) {
                reqJars = new String[][] { 
                    ...
                    { "*mqtt*"                , OPT_+"MQTT"           }, // "org.eclipse.paho.client.mqttv3_1.1.0.jar"
                    { "aws-iot*"              , OPT_+"AWS MQTT"       }, // "aws-iot-device-sdk-java-1.3.4.jar"
                    { "jackson-core*"         , OPT_+"AWS MQTT"       }, // "jackson-core-2.9.9.jar"
                    { "jackson-databind*"     , OPT_+"AWS MQTT"       }, // "jackson-databind-2.9.9.3.jar"
                };
            } else {
                reqJars = new String[][] { 
                    { "mysql-connector-java*" , REQ_+"MySQL JDBC"     },
            ...
            for (int xd = 0; xd < javaExtDirs.length; xd++) {
                ...
                for (int j = 0; j < reqJars.length; j++) {
                    ...
                    String  libDesc      = reqJars[j][1].substring(REQ_.length()).trim(); // skip "OPT:", "REQ:"
                    String  foundJarName = null;
                    for (int i = 0; i < fileList.length; i++) {
                        if (!StringTools.endsWithIgnoreCase(fileList[i],".jar")) { continue; }
                        if (libJarName.startsWith("*") && libJarName.endsWith("*")) {
                            String pattern = libJarName.substring(1, libJarName.length() - 1); // remove leading/trailing '*'
                            if (StringTools.containsIgnoreCase(fileList[i],pattern)) {
        ...
        if ((webappFile == null) || !webappFile.isFile()) {
            printVariable("(WebApp cfg file)", "", "(ERROR: not found!)"); // "(WebApp cfg URL)"
        ...
        // -- RGCache
        if (isEnterprise) {
            String rgcTitle = "(RGCache)";
            try {
                MethodAction rgcMA = new MethodAction(DBConfig.PACKAGE_EXTRA_TABLES_+"RGCache", "getConfigString");
                String rgcCfg = (String)rgcMA.invoke();
                printVariable(rgcTitle, rgcCfg, "");
            } catch (ClassNotFoundException cnfe) {
                printVariable(rgcTitle, "(not found)", "");
            } catch (Throwable th) {
                // -- InvocationException?
                printVariable(rgcTitle, "(error)", "");
            }
        }
        // -- ELog/HOS
        {
            ...
            if (!StringTools.isBlank(sendTestSMSTo) && (smsIsFunctional == 1)) {
                ...
                try {
                    ...
                    if (gw == null) {
                        ...
                        SmtpProperties.setGlobalThreadModel_CURRENT(); // <-- if SMS is sent via EMail
                ...
                if (!StringTools.isBlank(sendTestEmailTo) && SMTP_port_ok && SendMail_functional) {
                    ...
                    if (StringTools.isBlank(toAddr) || toAddr.endsWith("example.com")) {
                        ...
                        for (int e = 0; e < loadTestCnt; e++) {
                            if (SendMail.send(fromAddr,toAddr,subj,body,null,smtpProps,false).isOK()) {
        ...
        println("DBProvider:");
        // -- DBProperties
        Map<String,String> dbProps = DBProvider.getDBProperties();
        // -- DBProvider
        String dbProvID = "";
        {
            dbProvID = RTConfig.getString(RTKey.DB_PROVIDER,"");
            if (StringTools.isBlank(dbProvID)) {
            ...
            if ((webappProps != null) && !dbProvID.equals(webappProps.getString(RTKey.DB_PROVIDER,""))) {
                printVariable(RTKey.DB_PROVIDER, dbProvID, "(ERROR: does not match 'webapp.conf'!)");
                ...
                printVariable(RTKey.DB_PROVIDER, dbProvID, "");
            }
        }
        // -- DB Host
        {
            String dbHostKeyR[] = DBConnection.AppendRWMode(RTKey.DB_HOST,DBReadWriteMode.READ_ONLY );
            String dbHostKeyW[] = DBConnection.AppendRWMode(RTKey.DB_HOST,DBReadWriteMode.READ_WRITE);
            String dftHostR = (defaultProps != null)? defaultProps.getString(dbHostKeyR,"") : "";
            String dbHostR  = RTConfig.getString(dbHostKeyR,"");
            String dftHostW = (defaultProps != null)? defaultProps.getString(dbHostKeyW,"") : "";
            String dbHostW  = RTConfig.getString(dbHostKeyW,"");
            if (StringTools.isBlank(dftHostW)) {
                // -- no host defined
                printVariable(RTKey.DB_HOST+"[:write]", "", "(ERROR: not specified!)");
                addError("The DB host (write) has not been specified.",
                         "Missing '"+RTKey.DB_HOST+"[:write]' specification in 'default.conf'.",
                         "Please include '"+RTKey.DB_HOST+"[:write]' specification in 'default.conf'.");
            } else
            if (!dftHostW.equals(dbHostW)) {
                // -- host mispatch
                int WC = countWarning("DB host does not match host in 'default.conf'");
                printVariable(RTKey.DB_HOST+"[:write]", dftHostW, "(WARNING["+WC+"]: does not match default host ["+dbHostW+"])");
            } else
            if ((webappProps != null) && !dftHostW.equals(webappProps.getString(dbHostKeyW,""))) {
                // -- host mismatch
                int WC = countWarning("DB host does not match host in 'webapp.conf'");
                printVariable(RTKey.DB_HOST+"[:write]", dftHostW, "(WARNING["+WC+"]: does not match 'webapp.conf')");
            } else
            if (!IPTools.isLocalhost(dftHostW)) {
                // -- not localhost
                String dftHostWIP = IPTools.getIPAddress(dftHostW);
                int WC = countWarning("DB host ["+dftHostWIP+"] does not match 'localhost'");
                printVariable(RTKey.DB_HOST+"[:write]", dftHostW, "(WARNING["+WC+"]: is not 'localhost')");
            } else {
                // -- no warnings if localhost
                printVariable(RTKey.DB_HOST+"[:write]", dftHostW, "");
            }
        }
        // -- DB Name
        {
        ...
        // -- DB User
        {
            String dbUserKeyR[] = DBConnection.AppendRWMode(RTKey.DB_USER,DBReadWriteMode.READ_ONLY );
            String dbUserKeyW[] = DBConnection.AppendRWMode(RTKey.DB_USER,DBReadWriteMode.READ_WRITE);
            String dbUserW = RTConfig.getString(dbUserKeyW,"");
            if (StringTools.isBlank(dbUserW)) {
                printVariable(RTKey.DB_USER+"[:write]", "", "(ERROR: not specified!)");
            ...
            if ((webappProps != null) && !dbUserW.equals(webappProps.getString(dbUserKeyW,""))) {
                String waUserW = webappProps.getString(dbUserKeyW,"");
                printVariable(RTKey.DB_USER+"[:write]", dbUserW, "(ERROR: does not match 'webapp.conf'!)");
                addError("The DB user in 'default.conf' does not match specification in 'webapp.conf'.",
                         ...
                         "Please include proper '"+RTKey.DB_USER+"' specification in 'webapp.conf' ["+waUserW+"].");
            } else
            if (dbUserW.equals("root")) {
                int WC = countWarning("DB user should not be 'root'");
                printVariable(RTKey.DB_USER+"[:write]", dbUserW, "(WARNING["+WC+"]: should not be 'root')");
            } else {
                printVariable(RTKey.DB_USER+"[:write]", dbUserW, "");
            }
        }
        // -- DB utf8
        {
        ...
        // -- DB url
        {
            String dbUriKeyR[]   = DBConnection.AppendRWMode(RTKey.DB_URL   ,DBReadWriteMode.READ_ONLY );
            String dbUriDBKeyR[] = DBConnection.AppendRWMode(RTKey.DB_URL_DB,DBReadWriteMode.READ_ONLY );
            String dbUriKeyW[]   = DBConnection.AppendRWMode(RTKey.DB_URL   ,DBReadWriteMode.READ_WRITE);
            String dbUriDBKeyW[] = DBConnection.AppendRWMode(RTKey.DB_URL_DB,DBReadWriteMode.READ_WRITE);
            printVariable(RTKey.DB_URL   +"[:write]", RTConfig.getString(dbUriKeyW,""), "");
            printVariable(RTKey.DB_URL_DB+"[:write]", RTConfig.getString(dbUriDBKeyW,""), "");
        }
        // -- Connection pooling
        {
        ...
        // -- MySQL
        if (StringTools.containsIgnoreCase(dbProvID,"mysql")) {
                ...
                if (ListTools.size(maxConn) > 0) {
                    ...
                    for (String maxConnLine : maxConn) {
                        ...
                        if (eqSepPos < 0) {
                            ...
                            if (maxInt < MINIMUM_MAX_CONNECTIONS) {
                                // -- less than recommended value
                                printVariable(maxConnTitle, maxStr, "(Recommend at least "+MINIMUM_MAX_CONNECTIONS+")");
                                recommendations.append("- Recommend setting MySQL 'max_connections' to at least "+MINIMUM_MAX_CONNECTIONS+":\n");
                    ...
                    printVariable(maxConnTitle, "default", "(Recommend setting to at least "+MINIMUM_MAX_CONNECTIONS+")");
                    recommendations.append("- Recommend setting MySQL 'max_connections' to at least "+MINIMUM_MAX_CONNECTIONS+".\n");
                    recommendations.append("     see \"http://www.opengts.org/FAQ.html#faq_mysqlConn\"\n");
                }
                */
        ...
        // -- DBProperties
        if (!ListTools.isEmpty(dbProps)) {
            Vector<String> sortedKeys = new Vector<String>(dbProps.keySet());
            ListTools.sort(sortedKeys, new ListTools.StringComparator<String>(true));
            for (String key : sortedKeys) {
                String val = StringTools.trim(dbProps.get(key));
                if (key.equalsIgnoreCase("character_set_database")) {
                    printVariable(""+key+"", val, "");
                } else
                if (key.equalsIgnoreCase("collation_database")) {
                    printVariable(""+key+"", val, "");
                } else
                if (key.equalsIgnoreCase("max_connections")) {
                    int max = StringTools.parseInt(val,0);
                    if (max < MINIMUM_MAX_CONNECTIONS) {
                        // -- less than recommended value
                        printVariable(""+key+"", val, "(Recommend at least "+MINIMUM_MAX_CONNECTIONS+")");
                        recommendations.append("- Recommend setting MySQL 'max_connections' to at least "+MINIMUM_MAX_CONNECTIONS+":\n");
                        recommendations.append("     see \"http://www.opengts.org/FAQ.html#faq_mysqlConn\"\n");
                    } else {
                        printVariable(""+key+"", val, "");
                ...
                if (key.equalsIgnoreCase("max_user_connections")) {
                    ...
                    if (max < MINIMUM_MAX_CONNECTIONS) {
                        // -- less than recommended value
                        printVariable(""+key+"", val, "(Recommend at least "+MINIMUM_MAX_CONNECTIONS+")");
                        recommendations.append("- Recommend setting MySQL 'max_user_connections' to at least "+MINIMUM_MAX_CONNECTIONS+":\n");
                        recommendations.append("     see \"http://www.opengts.org/FAQ.html#faq_mysqlConn\"\n");
                    } else {
                        printVariable(""+key+"", val, "");
                    }
                } else
                if (key.equalsIgnoreCase("open_files_limit")) {
                    int max = StringTools.parseInt(val,0);
                    if (max < MINIMUM_OPEN_FILES_LIMIT) {
                        // -- less than recommended value
                        printVariable(""+key+"", val, "(Recommend at least "+MINIMUM_OPEN_FILES_LIMIT+")");
                        recommendations.append("- Recommend setting MySQL 'open_files_limit' to at least "+MINIMUM_OPEN_FILES_LIMIT+":\n");
                    } else {
                        printVariable(""+key+"", val, "");
                    }
                } else
                if (key.equalsIgnoreCase("secure_file_priv")) {
                    printVariable(""+key+"", val, "");
                } else {
                    // --
                }
        ...
        println("");
        println("Tables");
        printVariable("(read dbURI)" , DBProvider.getDBUri(DBReadWriteMode.READ_ONLY ,true), "");
        printVariable("(write dbURI)", DBProvider.getDBUri(DBReadWriteMode.READ_WRITE,true), "");
        ...
        println("private.xml:");
        {
            ...
            if (BasicPrivateLabelLoader.hasParsingErrors()) {
                ...
                for (BasicPrivateLabel privLabel : privLabelSet) {
                    ...
                    if (privLabel.getShowAccountLogin()) { 
                    ...
                    if (privLabel.getShowEmailLogin()) { 
                    ...
                    try {
                        ...
                        if (pwh != null) {
                            ...
                            if (pwh instanceof GeneralPasswordHandler) {
                                ...
                                pwhCN += ":" + gph.getPasswordEncodingString();
                            }
                    ...
                    */
                    ...
                    // -- DefaultLoginAccount (NO LONGER SUPPORTED)
                    ...
                    if (!privLabel.getShowAccountLogin() && !privLabel.getShowEmailLogin()) {
                    ...
                    if (!privLabel.getShowAccountLogin() && privLabel.getShowEmailLogin() && !StringTools.isBlank(dftAcctID)) {
